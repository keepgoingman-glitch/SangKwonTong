...
function resolveInitialPage() {
  ...
  try {
    ...
    // ?home=1 : 직원 강제진입 (자동연결 해제)
    if (params.get('home') === '1') {
      localStorage.removeItem('sk_home_store')
      return 'home'
    }
    // sk_home_store 자동복원은 "홈 화면 아이콘(standalone)"으로 열었을 때만 적용.
    // 일반 브라우저로 루트 주소(sang-kwon-tong.vercel.app)를 직접 열면
    // 직원 로그인 화면이 떠야 하므로 자동복원하지 않는다.
