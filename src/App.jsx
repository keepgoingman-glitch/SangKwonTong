...
import UturnDashboard from './UturnDashboard'
...
function resolveInitialPage() {
  ...
  try {
    // ?home=1 같은 직원 강제진입 파라미터가 있으면 자동연결 건너뜀
    const params = new URLSearchParams(window.location.search)
    if (params.get('home') === '1') {
      localStorage.removeItem('sk_home_store')
      return 'home'
    }
    // 고객이 QR 랜딩에 들어온 기기에는 sk_home_store 플래그가 있음.
    const homeStore = localStorage.getItem('sk_home_store')
    if (homeStore) {
      window.location.hash = 'qr-store-' + homeStore
      return 'qr-store-' + homeStore
    }
  } catch (e) {}
  return 'home'
}
...
export default function App() {
  ...
  // U-TURN 대시보드 (전 직원 공용)
  if (page === 'uturn') return <ErrorBoundaryWrapper><UturnDashboard onBack={goHome} onGo={goTo} /></ErrorBoundaryWrapper>
  ...
  return (
    <div style={{
      ...
      {/* U-TURN 히어로 이미지 */}
      <img src="/uturn-hero.png" alt="U-TURN" style={{ width: '100%', maxWidth: '360px', objectFit: 'contain', marginBottom: '6px' }} />

      {/* 슬로건 */}
      <p style={{ color: 'rgba(255,255,255,0.75)', fontSize: '14px', margin: '0 0 6px', textAlign: 'center', fontWeight: '700' }}>
        상반기의 부진을 <b style={{color:'#fbbf24'}}>TURN</b>하고 고객을 <b style={{color:'#E31937'}}>KT</b>로 <b style={{color:'#fbbf24'}}>TURN</b>한다!
      </p>
      <p style={{ color: 'rgba(255,255,255,0.4)', fontSize: '12.5px', margin: '0 0 34px', textAlign: 'center' }}>
        모든 활동이 성과로, 모든 성과가 성장으로
      </p>
      ...
      <div style={{ width: '100%', maxWidth: '360px', display: 'flex', flexDirection: 'column', gap: '13px', padding: '0 20px', boxSizing: 'border-box' }}>
        ...
        <button onClick={() => goTo('uturn')} style={{
          ...
          <div style={{ flex: 1 }}>
            ...
            <div style={{ color: 'rgba(255,255,255,0.55)', fontSize: '13px', lineHeight: 1.4 }}>UIT 전투 게이지 · 팀 대항 리더보드</div>
          </div>
