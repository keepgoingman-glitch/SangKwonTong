...
/* ============================================================
   ...
   · sk_home_store 는 고객이 "홈 화면에 저장하기"를 누를 때만 기록
     (진입만으로는 기록하지 않음 — 직원 기기 오염 방지)
   ============================================================ */
...
// KT 정식 로고 (public/kt-logo.png) — 흰 배경 펜던트로 어디서나 잘 보이게
const KTLogo = ({ h=13 }) => (
  <span style={{display:'inline-flex',alignItems:'center',background:'#fff',borderRadius:4,padding:'2px 5px',verticalAlign:'middle'}}>
    <img src="/kt-logo.png" alt="kt" style={{height:h,width:'auto',display:'block'}}/>
  </span>
)
...
export default function QRLanding({ storeCode, onBack }) {
  ...
  async function saveHome() {
    // 고객이 명시적으로 홈 저장을 선택한 경우:
    // 홈 바로가기가 저장할 URL을 ?s=매장코드 로 박아, 아이콘으로 열면 이 매장 랜딩이 뜨게 한다.
    // (삼성 인터넷 등은 해시(#)를 떼지만 쿼리(?)는 유지 → 직원 루트화면으로 새지 않음)
    try {
      if (storeCode) {
        localStorage.setItem('sk_home_store', storeCode)
        const clean = window.location.origin + '/?s=' + encodeURIComponent(storeCode)
        window.history.replaceState(null, '', clean)
      }
    } catch (e) {}
    // 이미 설치된 앱으로 실행 중이면
    const installed = window.matchMedia('(display-mode: standalone)').matches || window.navigator.standalone === true
    if (installed) { showToast('이미 홈 화면에 저장되어 있어요'); return }
    ...
    if (dp) {
      ...
      // 설치 이벤트가 없으면(아이폰 사파리, 또는 조건 미충족) → 수동 안내
      setShowInstallGuide(true)
    }
  }

  useEffect(() => {
    // 고객 랜딩 화면 → 홈 저장 시 고객용 아이콘 사용
    try {
      document.querySelectorAll('link[rel="apple-touch-icon"], link[rel="icon"][data-dyn]').forEach(el => el.remove())
      const a = document.createElement('link'); a.rel = 'apple-touch-icon'; a.href = '/icon-512.png'; document.head.appendChild(a)
  ...
  useEffect(() => {
    (async () => {
      try {
        if (storeCode) {
          // ※ 진입만으로 sk_home_store 를 기록하지 않는다.
          //   (직원이 QR 테스트 스캔만 해도 기기가 고객용으로 오염되던 문제 제거)
          //   브라우저 메뉴(⋮)로 수동 홈저장하는 고객은 바로가기 URL 에
          //   #qr-store-XXX 해시가 담기므로 별도 기록 없이도 매장 랜딩으로 연결됨.
    ...
    <div style={S.wrap}>
      <div style={S.device}>
        ...
        {screen === 'home' && (
          <div style={S.scroll}>
            <div style={S.hero}>
              <span style={S.loc}><KTLogo/> {placeName} 전용</span>
            ...
            <div style={S.body}>
              ...
              <div style={S.ktnet} onClick={() => setScreen('ktnet')}>
                <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:10}}>
                  <span style={S.knTag}><KTLogo/> 기가 인터넷</span><span style={{color:C.blue,fontSize:20}}>›</span>
                </div>
              ...
              <div style={S.foot}>
                ...
                <div style={{marginTop:9,fontSize:12,fontWeight:800}}><KTLogo/> × <span style={{color:C.blue}}>상권통</span></div>
              </div>
            </div>
          </div>
        )}
        ...
        {showInstallGuide && (()=>{
          ...
          return (
            <div style={S.guideWrap} onClick={()=>setShowInstallGuide(false)}>
              <div style={S.guideCard} onClick={e=>e.stopPropagation()}>
                <div style={{fontSize:34,marginBottom:8}}>📲</div>
                <b style={{fontSize:17,fontWeight:800,display:'block',marginBottom:6}}>홈 화면에 추가하는 법</b>
                {isIOS ? (
                  <div style={{fontSize:13,color:C.soft,lineHeight:1.7,textAlign:'left',margin:'10px 0'}}>
                    1. 하단의 <b>공유 버튼</b>(□↑)을 누르세요<br/>
                    2. 메뉴에서 <b>"홈 화면에 추가"</b>를 누르세요<br/>
                    3. 오른쪽 위 <b>"추가"</b>를 누르면 끝!
                  </div>
                ) : (
                  <div style={{fontSize:13,color:C.soft,lineHeight:1.7,textAlign:'left',margin:'10px 0'}}>
                    1. 오른쪽 위 <b>⋮ 메뉴</b>를 누르세요<br/>
                    2. <b>"앱 설치"</b> 또는 <b>"홈 화면에 추가"</b>를 누르세요<br/>
                    3. <b>"설치/추가"</b>를 누르면 끝!
                  </div>
                )}
                <button style={S.guideBtn} onClick={()=>setShowInstallGuide(false)}>확인했어요</button>
              </div>
            </div>
          )
        })()}
...
function Bar({ title, onBack }) {
  return <div style={S.topbar}><button style={S.bk} onClick={onBack}>‹</button><span style={{fontSize:15,fontWeight:800}}>{title}</span>
    <span style={{marginLeft:'auto',fontSize:10.5,color:C.soft,fontWeight:700}}><KTLogo/> 공식 제휴</span></div>
}
function Bars() {
...
const S = {
  ...
  guideWrap:{position:'fixed',inset:0,background:'rgba(8,14,32,.6)',zIndex:200,display:'flex',alignItems:'center',justifyContent:'center',padding:24},
  guideCard:{background:'#fff',borderRadius:20,padding:'26px 24px',maxWidth:320,width:'100%',textAlign:'center',boxShadow:'0 20px 50px rgba(0,0,0,.35)'},
  guideBtn:{marginTop:14,width:'100%',background:'linear-gradient(135deg,#1E7BFF,#0B4FCB)',color:'#fff',border:0,borderRadius:12,padding:14,fontSize:15,fontWeight:800,cursor:'pointer',fontFamily:'inherit'},
}
