    ...
    (async () => {
      try {
        if (aptCode) {
          ...
          if (rows && rows[0]) {
            setCamp(rows[0])
            try { await db.rpc('bump_qr_scan', { p_code: aptCode }) } catch(e) {}
          }
        }
    ...
    <div style={{minHeight:'100vh',background:'#eef4fc',fontFamily:"'Pretendard',-apple-system,sans-serif",color:C.ink,lineHeight:1.45}}>
      <div style={{maxWidth:440,margin:'0 auto',background:'#fff',position:'relative',overflow:'hidden'}}>
        ...
        <div style={{padding:'32px 22px 18px',background:'linear-gradient(180deg,#f6faff,#eaf3ff)'}}>
          ...
          <Field label="고객명"><input value={f.name} onChange={e=>set('name',e.target.value)} placeholder="성함을 입력해주세요" style={S.input}/></Field>
          <Field label="동/호수"><input value={f.dong} onChange={e=>set('dong',e.target.value)} placeholder="예: 101동 1203호" style={S.input}/></Field>
          <Field label="에어컨 제조사">
          ...
          <Field label="연락처"><input value={f.phone} onChange={e=>set('phone',phoneFmt(e.target.value))} placeholder="010-0000-0000" inputMode="numeric" style={S.input}/></Field>
          <Field label="상담 원하는 시간">
            <div style={{display:'flex',gap:8,flexWrap:'wrap'}}>
