...
export default function AirconLanding({ aptCode, onBack }) {
  ...
  // 홈화면 추가 안내
  // ※ PWA 설치(beforeinstallprompt.prompt())를 쓰면 manifest의 start_url(상권통 루트)로 열려
  //    캠페인 랜딩이 아닌 직원 로그인 화면이 떠버림. 그래서 "현재 페이지를 홈 화면에 바로가기로
  //    추가"하도록 브라우저 메뉴 사용을 안내한다 (현재 URL 해시가 그대로 저장됨).
  const addToHome = () => {
    // 홈 바로가기가 저장할 URL을 ?c= 쿼리 형태로 맞춘다.
    // (삼성 인터넷 등은 해시(#)를 떼지만 쿼리(?)는 유지하므로 캠페인이 보존됨)
    try {
      if (aptCode) {
        localStorage.setItem('sk_home_store', aptCode)
        const clean = window.location.origin + '/?c=' + aptCode
  ...
  // 홈 화면에 추가 시 상권통 아이콘·아파트명이 뜨도록 메타 주입
  useEffect(() => {
    try {
      // 아이콘 (기존 public/icon-512.png 재사용)
      document.querySelectorAll('link[rel="apple-touch-icon"], link[rel="icon"][data-dyn]').forEach(el => el.remove())
      const a = document.createElement('link'); a.rel = 'apple-touch-icon'; a.href = '/icon-512.png'; a.setAttribute('data-dyn','1'); document.head.appendChild(a)
      const b = document.createElement('link'); b.rel = 'icon'; b.type = 'image/png'; b.href = '/icon-512.png'; b.setAttribute('data-dyn','1'); document.head.appendChild(b)
    } catch(e) {}
  }, [])

  // 동적 manifest 주입 — 홈추가 시 start_url 에 ?c=CAMP-... 가 박히게 함
  // (삼성 인터넷은 쿼리를 떼고 루트만 저장하지만, manifest 의 start_url 은 그대로 사용 → 캠페인으로 열림)
  useEffect(() => {
    if (!aptCode) return
    let url = null
    try {
      const startUrl = window.location.origin + '/?c=' + aptCode
      const mf = {
        name: (camp && camp.apt_name ? camp.apt_name + ' ' : '') + '에어컨 클린케어',
        short_name: (camp && camp.apt_name) ? camp.apt_name : '에어컨 클린케어',
        start_url: startUrl,
        scope: startUrl,
        display: 'browser',
        background_color: '#1e6fe8',
        theme_color: '#1e6fe8',
        icons: [
      ...
      link.rel = 'manifest'; link.href = url; link.setAttribute('data-dyn-mf','1')
      document.head.appendChild(link)
    } catch(e) {}
    return () => {
      // 랜딩을 떠날 때 원래 manifest 로 복구
      try {
        document.querySelectorAll('link[rel="manifest"][data-dyn-mf]').forEach(el => el.remove())
        if (!document.querySelector('link[rel="manifest"]')) {
          const orig = document.createElement('link')
          orig.rel = 'manifest'; orig.href = '/manifest.json'
  ...
  // 페이지 타이틀을 아파트명으로 (홈 바로가기 이름에 반영)
  useEffect(() => {
    if (camp && camp.apt_name) {
      try { document.title = camp.apt_name + ' 에어컨 클린케어' } catch(e) {}
    }
  }, [camp])
  ...
  async function submit() {
    ...
    try {
      ...
      // 총괄관리자 폰으로 푸시 알림 발송 (실패해도 상담 접수엔 영향 없음)
      try {
        const base = window.location.origin
        const aptNm = camp ? camp.apt_name : '아파트'
        fetch(base + '/api/send-push', {
          method:'POST', headers:{'Content-Type':'application/json'},
          body: JSON.stringify({
            title: '🔔 신규 에어컨 상담',
            body: aptNm + ' · ' + f.name.trim() + '님 (' + f.dong.trim() + ') · ' + f.brand + ' ' + f.count + '대',
            url: base + '/#connector'
          })
        }).catch(()=>{})
      } catch(e) {}
  ...
  if (done) return (
    <div style={{minHeight:'100vh',background:'linear-gradient(160deg,#2c7ef0,#1553b8)',display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',padding:'40px 28px',fontFamily:"'Pretendard',sans-serif",color:'#fff',textAlign:'center'}}>
      ...
      <button onClick={()=>{ setDone(false); setF({ name:'', dong:'', brand:'', type:'', count:'', phone:'', time:'', agree:false }); window.scrollTo(0,0) }}
        style={{marginTop:24,background:'#fff',color:'#1e6fe8',border:'none',borderRadius:14,padding:'15px 40px',fontSize:16,fontWeight:900,cursor:'pointer',fontFamily:'inherit',letterSpacing:'-0.5px',boxShadow:'0 8px 24px rgba(0,0,0,0.15)'}}>
        처음으로 돌아가기
      </button>
    </div>
  )
  ...
  return (
    <div style={{minHeight:'100vh',background:'#eef4fc',fontFamily:"'Pretendard',-apple-system,sans-serif",color:C.ink,lineHeight:1.45}}>
      <div style={{maxWidth:440,margin:'0 auto',background:'#fff',position:'relative',overflow:'hidden'}}>
        ...
        <div style={{background:'linear-gradient(165deg,#2c7ef0,#1e6fe8 55%,#1553b8)',padding:'24px 22px 26px',position:'relative',overflow:'hidden'}}>
          ...
          <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',position:'relative'}}>
            <div style={{display:'inline-flex',alignItems:'center',gap:7,background:'rgba(255,255,255,0.95)',color:C.blueD,fontSize:13,fontWeight:800,padding:'7px 15px',borderRadius:30,marginBottom:15,boxShadow:'0 4px 16px rgba(0,0,0,0.12)',letterSpacing:'-0.3px'}}>
              <span style={{width:7,height:7,borderRadius:'50%',background:C.red}}/>{aptName} 입주민 전용
            </div>
            <button onClick={addToHome} style={{display:'flex',alignItems:'center',gap:4,background:'rgba(255,255,255,0.18)',border:'1px solid rgba(255,255,255,0.4)',color:'#fff',fontSize:12,fontWeight:700,padding:'7px 11px',borderRadius:20,cursor:'pointer',fontFamily:'inherit',whiteSpace:'nowrap',backdropFilter:'blur(6px)',letterSpacing:'-0.3px'}}>
              ⬇ 홈화면 추가
            </button>
          </div>
      ...
      {showIosGuide && (
        <div onClick={()=>setShowIosGuide(false)} style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.6)',zIndex:300,display:'flex',alignItems:'flex-end',justifyContent:'center'}}>
          <div onClick={e=>e.stopPropagation()} style={{background:'#fff',borderRadius:'20px 20px 0 0',padding:'26px 24px 34px',width:'100%',maxWidth:440,textAlign:'left'}}>
            <div style={{fontSize:40,marginBottom:12,textAlign:'center'}}>📲</div>
            <h3 style={{fontSize:19,fontWeight:900,color:C.navy,marginBottom:14,letterSpacing:'-0.5px',textAlign:'center'}}>이 페이지를 홈 화면에 추가</h3>
            <div style={{background:'#f6faff',borderRadius:12,padding:'16px 18px',marginBottom:14}}>
              <div style={{fontSize:13,fontWeight:800,color:C.blue,marginBottom:8}}>📱 삼성 인터넷 / 크롬 (안드로이드)</div>
              <div style={{fontSize:13.5,color:C.ink,lineHeight:1.8,fontWeight:500,letterSpacing:'-0.3px'}}>
                ① 하단(또는 우상단) <b>메뉴( ⋮ / ≡ )</b> 열기<br/>
                ② <b>'현재 페이지 추가'</b> 또는 <b>'홈 화면에 추가'</b> 선택<br/>
                ③ <b>'홈 화면'</b> 선택 후 추가
              </div>
            </div>
            <div style={{background:'#f6faff',borderRadius:12,padding:'16px 18px',marginBottom:16}}>
              <div style={{fontSize:13,fontWeight:800,color:C.blue,marginBottom:8}}>🍎 아이폰 (사파리)</div>
              <div style={{fontSize:13.5,color:C.ink,lineHeight:1.8,fontWeight:500,letterSpacing:'-0.3px'}}>
                ① 하단 <b>공유 버튼</b> 탭<br/>
                ② <b>'홈 화면에 추가'</b> 선택
              </div>
            </div>
            <p style={{fontSize:12,color:C.gray,textAlign:'center',marginBottom:16,fontWeight:500}}>※ 지금 보고 계신 <b style={{color:C.navy}}>이 캠페인 페이지</b>가 그대로 저장됩니다</p>
            <button onClick={()=>setShowIosGuide(false)} style={{width:'100%',padding:15,background:C.blue,border:'none',borderRadius:12,color:'#fff',fontSize:16,fontWeight:900,cursor:'pointer',fontFamily:'inherit'}}>확인</button>
          </div>
        </div>
      )}
    </div>
