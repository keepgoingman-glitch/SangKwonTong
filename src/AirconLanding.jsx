...
export default function AirconLanding({ aptCode, onBack }) {
  ...
  // PWA 홈화면 추가 (Android Chrome: beforeinstallprompt, iOS: 안내)
  useEffect(() => {
    ...
    <div style={{minHeight:'100vh',background:'#eef4fc',fontFamily:"'Pretendard',-apple-system,sans-serif",color:C.ink,lineHeight:1.45}}>
      ...
      {showIosGuide && (
        <div onClick={()=>setShowIosGuide(false)} style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.6)',zIndex:300,display:'flex',alignItems:'flex-end',justifyContent:'center'}}>
          <div onClick={e=>e.stopPropagation()} style={{background:'#fff',borderRadius:'20px 20px 0 0',padding:'26px 24px 34px',width:'100%',maxWidth:440,textAlign:'center'}}>
            <div style={{fontSize:40,marginBottom:12}}>📲</div>
            <h3 style={{fontSize:19,fontWeight:900,color:C.navy,marginBottom:10,letterSpacing:'-0.5px'}}>홈 화면에 추가하기</h3>
            <p style={{fontSize:14,color:C.gray,lineHeight:1.7,fontWeight:500,letterSpacing:'-0.3px'}}>
              브라우저 하단(또는 상단) <b style={{color:C.blue}}>공유 버튼</b>을 누른 뒤<br/>
              <b style={{color:C.blue}}>'홈 화면에 추가'</b>를 선택하시면<br/>앱처럼 바로 접속하실 수 있어요.
            </p>
            <button onClick={()=>setShowIosGuide(false)} style={{marginTop:20,width:'100%',padding:15,background:C.blue,border:'none',borderRadius:12,color:'#fff',fontSize:16,fontWeight:900,cursor:'pointer',fontFamily:'inherit'}}>확인</button>
          </div>
        </div>
      )}
    </div>
