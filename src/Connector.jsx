...
export default function Connector({ onBack }) {
  ...
  const [mDiscStage, setMDiscStage] = useState('전체')  // M발굴 단계 필터  // [] = 전체 선택
...
function buildSlideHTML({ period, today, yearStr, teams, projects, allMM, topMM,
  ...
  *{margin:0;padding:0;box-sizing:border-box}
  ...
  const projRows = projects.map(p=>{
    const wD = (p.mmStats||[]).reduce((a,m)=>a+(wkLookup[m.name]?.actAll||0),0) || p.weekly?.done || 0
    const wS = (p.mmStats||[]).reduce((a,m)=>a+(wkLookup[m.name]?.success||0),0) || p.weekly?.success || 0
  ...
  <div class="body">
    <div class="kpi">
      <div class="kbox"><div class="kv" style="color:#2563eb">${wAct}<span style="font-size:14px">건</span></div><div class="kl">주간 활동</div></div>
      <div class="kbox"><div class="kv" style="color:#16a34a">${wSuc}<span style="font-size:14px">건</span></div><div class="kl">주간 성공</div></div>
      <div class="kbox"><div class="kv" style="color:#2563eb">${mAct}<span style="font-size:14px">건</span></div><div class="kl">월간 활동 (목표 ${totalMonthTarget}건 · ${pct(mAct,totalMonthTarget)}%)</div></div>
      <div class="kbox"><div class="kv" style="color:#16a34a">${mSuc}<span style="font-size:14px">건</span></div><div class="kl">월간 성공 (성공률 ${sucRate(mSuc,mAct)}%)</div></div>
    ...
    <table>
      <thead>
        <tr>
          ...
          <th colspan="2" style="background:#1e3a5f">주간</th>
          <th colspan="3" style="background:#1e3a5f">월간</th>
      ...
      <tbody>
        ...
        <tr style="background:${i%2?'#f8fafc':'#fff'}">
          ...
          <td class="bc">${wD}</td><td class="gc">${wS}</td>
        ...
        <tr style="background:#16213e;color:#fff;font-weight:900">
          ...
          <td style="color:#60a5fa">${sumRow.wD}</td><td style="color:#4ade80">${sumRow.wS}</td>
  ...
  function projDetailSlide(p, sn){
    ...
    const wD=(p.mmStats||[]).reduce((a,m)=>a+(wkLookup[m.name]?.actAll||0),0)
    const wS=(p.mmStats||[]).reduce((a,m)=>a+(wkLookup[m.name]?.success||0),0)
    const mD=p.monthly?.done||0, mS=p.monthly?.success||0
    const yD2=p.annual?.done||0, yS2=p.annual?.success||0
  ...
  <div class="body">
    <div class="kpi">
      ...
      <div class="kbox"><div class="kv" style="color:#2563eb">${wD}<span style="font-size:13px">건</span></div><div class="kl">주간 활동 (성공 ${wS})</div></div>
      <div class="kbox"><div class="kv" style="color:#2563eb">${mD}<span style="font-size:13px">건</span></div><div class="kl">월간 활동 (활동률 ${pct(mD,Math.max(dtot,1))}%)</div></div>
      <div class="kbox"><div class="kv" style="color:#16a34a">${mS}<span style="font-size:13px">건</span></div><div class="kl">월간 성공 (성공률 ${sucRate(mS,mD)}%)</div></div>
    ...
    <div style="display:flex;gap:16px;height:480px">
      <div style="flex:1.3;display:flex;flex-direction:column;gap:10px;overflow:hidden">
        ...
        <table>
          <thead><tr><th style="text-align:left">MM</th><th>팀</th><th>배분/등록</th><th>주간활동</th><th>주간성공</th><th>월간활동</th><th>월간성공</th><th>성공률</th></tr></thead>
          <tbody>
            ${mm.length===0?'<tr><td colspan="8" style="color:#94a3b8;padding:20px">활동 MM 없음</td></tr>':mm.map((m,i)=>{
              const mwk=wkLookup[m.name]||{}
              ...
              return `<tr style="background:${zero?'#fef2f2':i%2?'#f8fafc':'#fff'}">
                ...
                <td class="bc">${mwk.actAll||0}</td><td class="gc">${mwk.success||0}</td>
