...
// ── 팀별 현황 대시보드 컴포넌트 ──────────────────────────────────
function TeamDashboard({ adminInfo, users, leads, directLeads, adminAccounts, onUpdateLabel, isSuper }) {
  ...
  function TeamCard({ acc }) {
    ...
    const muit = teamLeads.length
    const bulk = teamDirect.length
...
// ── 슬라이드 장표 HTML 생성 ──────────────────────────────────────
function buildSlideHTML({ period, teams, projects, topMM }) {
  const total = teams.reduce((a,t)=>a+t.success,0)
  const goal  = teams.reduce((a,t)=>a+t.goal,0)
  const muit  = teams.reduce((a,t)=>a+t.muit,0)
  const bulk  = teams.reduce((a,t)=>a+t.bulk,0)
  const bar = (val,max,color,h=10) =>
    `<div style="background:#e2e8f0;border-radius:4px;height:${h}px;overflow:hidden;margin-top:4px"><div style="width:${Math.min(pct(val,max),100)}%;background:${color};height:100%;border-radius:4px"></div></div>`
  const slide = (n, total_n, title, body) =>
    `<div class="slide" id="s${n}"><div class="snum">${n}/${total_n}</div>${title?`<div class="stitle">${title}</div>`:''}${body}</div>`

  const N = 5 + (teams.length > 0 ? 1 : 0)
  return `<!DOCTYPE html><html lang="ko"><head><meta charset="UTF-8"/>
<title>상권통 보고서 - ${period}</title>
<style>
*{margin:0;padding:0;box-sizing:border-box}
body{font-family:'Noto Sans KR','Apple SD Gothic Neo',sans-serif;background:#1e293b;padding:24px;display:flex;flex-direction:column;align-items:center;gap:20px}
.slide{background:#fff;color:#1e293b;width:100%;max-width:860px;min-height:480px;border-radius:14px;padding:36px 44px;position:relative;box-shadow:0 8px 40px rgba(0,0,0,0.4);page-break-after:always}
.snum{position:absolute;top:16px;right:22px;font-size:12px;color:#94a3b8;font-weight:600}
.stitle{font-size:24px;font-weight:900;color:#1e293b;padding-bottom:14px;border-bottom:3px solid #e67e00;margin-bottom:22px}
...
.kv{font-size:30px;font-weight:900}.kl{font-size:12px;color:#64748b;margin-top:3px}
...
.nav{position:fixed;bottom:16px;left:50%;transform:translateX(-50%);display:flex;gap:6px;background:rgba(30,41,59,0.92);padding:8px 14px;border-radius:50px;z-index:100}
.nb{width:30px;height:30px;border-radius:50%;background:rgba(255,255,255,0.15);border:none;color:#fff;font-weight:700;cursor:pointer;font-size:12px}
.print-btn{padding:0 14px;height:30px;border-radius:20px;background:#e67e00;border:none;color:#fff;font-weight:800;cursor:pointer;font-size:12px}
.cover-center{text-align:center;padding:30px 0}
@media print{body{background:#fff;padding:0}.nav{display:none!important}.slide{box-shadow:none;border-radius:0;margin:0;min-height:100vh}}
</style></head><body>
<div class="nav">
  ${Array.from({length:N},(_,i)=>`<button class="nb" onclick="document.getElementById('s${i+1}').scrollIntoView({behavior:'smooth'})">${i+1}</button>`).join('')}
  <button class="print-btn" onclick="window.print()">🖨 인쇄/PDF</button>
</div>

${slide(1, N, '', `<div class="cover-center">
  <div style="font-size:52px;margin-bottom:14px">📊</div>
  <div style="font-size:38px;font-weight:900;color:#e67e00;margin-bottom:8px">상권통 활동 보고서</div>
  <div style="font-size:16px;color:#64748b;margin-bottom:22px">KT 지역상권 영업관리 플랫폼</div>
  <div style="background:#fff7ed;border:1.5px solid #fed7aa;color:#b45309;font-size:15px;font-weight:700;padding:8px 24px;border-radius:20px;display:inline-block">📅 ${period} 기준</div>
  <div style="display:grid;grid-template-columns:repeat(${teams.length},1fr);gap:14px;max-width:500px;margin:28px auto 0">
    ${teams.map(t=>`<div class="kpi"><div class="kv" style="font-size:18px;color:#e67e00">${t.label}</div><div class="kl">MM ${t.mm}명</div></div>`).join('')}
  </div>
</div>`)}

${slide(2, N, '📌 전체 성과 요약', `
  <div class="grid4">
    <div class="kpi"><div class="kv" style="color:#e67e00">${total}<span style="font-size:16px">건</span></div><div class="kl">총 성공</div></div>
    <div class="kpi"><div class="kv" style="color:#2563eb">${muit}<span style="font-size:16px">건</span></div><div class="kl">MUIT 성과</div></div>
    <div class="kpi"><div class="kv" style="color:#7c3aed">${bulk}<span style="font-size:16px">건</span></div><div class="kl">M벌크 발굴</div></div>
    <div class="kpi"><div class="kv" style="color:#16a34a">${pct(total,Math.max(goal,1))}<span style="font-size:16px">%</span></div><div class="kl">달성률</div></div>
  </div>
  <div style="display:flex;justify-content:space-between;font-size:13px;color:#64748b;margin-bottom:4px"><span>전체 목표 달성률</span><span style="font-weight:800;color:#e67e00">${total}건 / ${goal}건</span></div>
  ${bar(total,goal,'#e67e00',16)}`)}

${slide(3, N, '👥 팀별 활동 현황', `
  <table>
    <thead><tr><th>팀명</th><th>MM수</th><th>성공</th><th>목표</th><th>MUIT</th><th>M벌크</th><th>달성률</th></tr></thead>
    <tbody>
      ${teams.map(t=>`<tr><td><strong>${t.label}</strong></td><td>${t.mm}명</td><td style="color:#16a34a;font-weight:800">${t.success}건</td><td>${t.goal}건</td><td style="color:#2563eb;font-weight:700">${t.muit}건</td><td style="color:#7c3aed;font-weight:700">${t.bulk}건</td><td><strong style="color:${pct(t.success,Math.max(t.goal,1))>=70?'#16a34a':'#e67e00'}">${pct(t.success,Math.max(t.goal,1))}%</strong></td></tr>`).join('')}
    </tbody>
  </table>
  <div style="margin-top:18px">${teams.map(t=>`<div style="margin-bottom:10px"><div style="display:flex;justify-content:space-between;font-size:13px;margin-bottom:3px"><span style="font-weight:700">${t.label}</span><span style="color:#64748b">${t.success}/${t.goal}건 (${pct(t.success,Math.max(t.goal,1))}%)</span></div>${bar(t.success,Math.max(t.goal,1),pct(t.success,Math.max(t.goal,1))>=70?'#16a34a':'#e67e00',12)}</div>`).join('')}</div>`)}

${projects.length > 0 ? slide(4, N, '📁 프로젝트별 진행 현황', `
  <table>
    <thead><tr><th>프로젝트명</th><th>팀</th><th>전체</th><th>활동</th><th>성공</th><th>진행률</th></tr></thead>
    <tbody>
      ${projects.map(p=>`<tr><td><strong>${p.name}</strong></td><td>${p.team}</td><td>${p.total}건</td><td>${p.done}건</td><td style="color:#16a34a;font-weight:800">${p.success}건</td><td><strong style="color:#2563eb">${pct(p.done,Math.max(p.total,1))}%</strong>${bar(p.done,Math.max(p.total,1),'#2563eb',8)}</td></tr>`).join('')}
    </tbody>
  </table>`) : ''}

${slide(projects.length>0?5:4, N, '🏅 우수 직원', `
  <div style="display:grid;grid-template-columns:1fr 1fr;gap:20px">
    <div>${topMM.slice(0,3).map((m,i)=>`<div style="display:flex;align-items:center;gap:14px;padding:12px;background:#f8fafc;border:1px solid #e2e8f0;border-radius:10px;margin-bottom:10px">
      <span style="font-size:28px">${['🥇','🥈','🥉'][i]}</span>
      <div style="flex:1"><div style="font-size:15px;font-weight:800">${m.name}</div><div style="font-size:12px;color:#64748b">${m.team}</div></div>
      <div style="text-align:right"><div style="font-size:20px;font-weight:900;color:#16a34a">${m.success}</div><div style="font-size:10px;color:#64748b">성공건</div></div>
    </div>`).join('')}</div>
    <div>${teams.map(t=>`<div style="margin-bottom:14px"><div style="display:flex;justify-content:space-between;font-size:13px;font-weight:700;margin-bottom:4px"><span>${t.label}</span><span style="color:#16a34a">${t.success}건</span></div>${bar(t.success,Math.max(goal,1),pct(t.success,Math.max(goal,1))>=70?'#16a34a':'#e67e00',12)}</div>`).join('')}</div>
  </div>`)}

${slide(N, N, '📋 종합 요약', `
  <div style="display:grid;grid-template-columns:1fr 1fr;gap:20px">
    <div>
      <div style="font-size:14px;font-weight:800;color:#e67e00;margin-bottom:10px">✅ 이번 달 성과</div>
      ${[`전체 성공 <strong style="color:#16a34a">${total}건</strong> (목표대비 ${pct(total,Math.max(goal,1))}%)`,`MUIT 활동 <strong style="color:#2563eb">${muit}건</strong>`,`M벌크 발굴 <strong style="color:#7c3aed">${bulk}건</strong>`,`진행중 프로젝트 <strong>${projects.filter(p=>p.done<p.total).length}개</strong>`].map(txt=>`<div style="background:#f0fdf4;border:1px solid #bbf7d0;border-radius:8px;padding:10px;font-size:13px;margin-bottom:6px">${txt}</div>`).join('')}
    </div>
    <div>
      <div style="font-size:14px;font-weight:800;color:#2563eb;margin-bottom:10px">🎯 다음 달 목표</div>
      ${[`전체 목표 <strong>${Math.round(goal*1.1)}건</strong> (+10%)`,`MUIT 목표 <strong>${Math.round(muit*1.15)}건</strong>`,`M벌크 목표 <strong>${Math.round(bulk*1.2)}건</strong>`,`신규 프로젝트 발굴`].map(txt=>`<div style="background:#eff6ff;border:1px solid #bfdbfe;border-radius:8px;padding:10px;font-size:13px;margin-bottom:6px">${txt}</div>`).join('')}
    </div>
  </div>
  <div style="position:absolute;bottom:18px;right:36px;font-size:11px;color:#94a3b8">상권통 자동 생성 · ${new Date().toLocaleDateString('ko-KR')}</div>`)}
                ...
                <button onClick={() => {
                  ...
                  const teamData = accs.length > 0
                    ? accs.map(acc => {
                        ...
                        return {
                          label: acc.team_label || acc.name,
                          mm: tm.length,
                          success: tm.reduce((a,u)=>a+(u.success||0),0),
                          goal: tm.reduce((a,u)=>a+(u.goal||10),0),
                          muit: tl.length, bulk: td.length
                        }
                      })
                    : [{ label: adminInfo?.name || '우리팀', mm: myUsers.length, success: myUsers.reduce((a,u)=>a+(u.success||0),0), goal: myUsers.reduce((a,u)=>a+(u.goal||10),0), muit: leads.length, bulk: directLeads.length }]
                  const projData = [...new Set(leads.map(l=>l.project_name).filter(Boolean))].map(pn => {
                    const pl = leads.filter(l=>l.project_name===pn)
                    return { name:pn, team: accs[0]?.team_label||'-', total:pl.length, done:pl.filter(l=>l.activity_status&&l.activity_status!=='미처리').length, success:pl.filter(l=>l.activity_status==='성공').length }
                  })
                  const topData = [...myUsers].sort((a,b)=>(b.success||0)-(a.success||0)).slice(0,3).map(u=>({
                    name: u.name, team: accs.find(a=>a.team_id===u.team_id)?.team_label || adminInfo?.name || '-', success: u.success||0
                  }))
                  const html = buildSlideHTML({ period: new Date().toLocaleDateString('ko-KR',{year:'numeric',month:'long'}), teams:teamData, projects:projData, topMM:topData })
