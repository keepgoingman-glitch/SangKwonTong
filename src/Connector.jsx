            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
              ...
              <div style={{ background: '#fff', border: '1.5px solid #bfdbfe', borderRadius: '14px', padding: '18px', boxShadow: '0 1px 4px rgba(0,0,0,0.04)' }}>
                ...
                <button onClick={() => {
                  ...
                  // 트렌드 차트: 입력된 데이터의 최대 일자까지 표시 (미래 개통일 입력 대응 → KPI와 일치)
                  const _maxDataDay = Object.keys(allDailyMap).length ? Math.max(...Object.keys(allDailyMap).map(Number)) : 0
                  const _monthLastDay = new Date(_y2,_selM+1,0).getDate()
                  const todayDayNumAll = _isPastMonth ? _monthLastDay : Math.min(_monthLastDay, Math.max(new Date().getDate(), _maxDataDay))
