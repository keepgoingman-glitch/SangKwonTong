...
function buildSlideHTML({ period, today, yearStr, teams, projects, allMM, topMM, mDiscoveryCount, globalProdCounts = {}, globalProdAnnual = {}, bizDays = 0, perMMTarget = 0, workDaysAll = {} }) {
  // 영업일수 기반 목표
  const mmCount = allMM.length || 1
  const totalMonthTarget = perMMTarget * mmCount  // 전체 MM 월간 목표
  // 누적 목표: 영업일수 > 0 인 월만 합산
  ...
  function pctTarget(actual, target) { return target > 0 ? Math.round(actual / target * 100) : 0 }
  function sucRate(suc, act) { return act > 0 ? Math.round(suc / act * 100) : 0 }
  ...
  const sf=(n)=>'<div style="padding:5px 32px;background:#f8fafc;border-top:1px solid #e2e8f0;display:flex;justify-content:space-between;font-size:10px;color:#94a3b8;flex-shrink:0">'+
    ...
    '<span style="color:#e67e00;font-weight:600">영업일 '+bizDays+'일 | 1인목표 '+perMMTarget+'건 | 기준일 '+today+'</span>'+
    '<span>'+n+' / '+N+'</span></div>'
...
.np{padding:0 12px;height:24px;border-radius:20px;background:#e67e00;border:none;color:#fff;font-weight:700;cursor:pointer;font-size:10px}`
  ...
  const s1='<div class="slide" id="s1">'+
    ...
    '<div style="position:relative;z-index:10;padding:10px 52px;background:rgba(15,23,42,0.04);border-top:1px solid rgba(0,0,0,0.07);display:flex;justify-content:space-between;align-items:center;flex-shrink:0">'+
      '<div style="font-size:11px;color:#94a3b8">영업일 <strong style="color:#e67e00">'+bizDays+'일</strong> · 1인목표 <strong style="color:#2563eb">'+perMMTarget+'건</strong> · 전체목표 <strong style="color:#2563eb">'+totalMonthTarget+'건</strong></div>'+
      '<div style="font-size:14px;font-weight:700;color:#e67e00;letter-spacing:1px">상권통 SANGKWONTONG</div>'+
      '<div style="font-size:11px;color:#94a3b8">기준일: '+today+'</div>'+
    '</div>'+
  '</div>'
