...
function buildSlideHTML({ period, today, yearStr, teams, projects, allMM, topMM,
  ...
  bizDays=0, perMMTarget=0, workDaysAll={}, allDailyStats=[] }) {
  ...
  function svgLineChart(dailyData, width, height, opts) {
    ...
    // X축 날짜
    const step=Math.max(1,Math.ceil(dailyData.length/10))
    dailyData.forEach((d,i)=>{
      if(i%step===0||i===dailyData.length-1){const x=pad.l+scaleX(i,dailyData.length);s+=`<text x="${x}" y="${height-5}" text-anchor="middle" font-size="8" fill="#94a3b8">${d.day}일</text>`}
    })
    // 활동 라인 (파랑)
  ...
  function svgCumChart(dailyData, width, height) {
    ...
    s+=`<rect width="${width}" height="${height}" fill="#fff" rx="6"/>`
    yTicks.forEach(v=>{const y=pad.t+scaleY(v);s+=`<line x1="${pad.l}" y1="${y}" x2="${pad.l+W}" y2="${y}" stroke="#e2e8f0" stroke-width="0.5"/><text x="${pad.l-3}" y="${y+4}" text-anchor="end" font-size="8" fill="#94a3b8">${v}</text>`})
    const step=Math.max(1,Math.ceil(cumData.length/8))
    cumData.forEach((d,i)=>{if(i%step===0||i===cumData.length-1){const x=pad.l+scaleX(i,cumData.length);s+=`<text x="${x}" y="${height-5}" text-anchor="middle" font-size="8" fill="#94a3b8">${d.day}일</text>`}})
    // 누적활동 면적
    ...
    cumData.forEach((d,i)=>{
      if(i===0||i===cumData.length-1||(i%step===0&&d.cumAct>0)){
...
body{font-family:"Malgun Gothic","Apple SD Gothic Neo",sans-serif;background:#dde3ea;-webkit-print-color-adjust:exact;print-color-adjust:exact}
...
.rt{font-size:13px;font-weight:700;display:flex;align-items:center;gap:7px}
.rb{font-size:10px;background:#e67e00;color:#fff;padding:2px 8px;border-radius:20px}
.rp{font-size:10px;color:rgba(255,255,255,0.5);text-align:right;line-height:1.7}
.rf{background:#0d1b2a;color:rgba(255,255,255,0.4);padding:4px 18px;font-size:9px;display:flex;justify-content:space-between;flex-shrink:0}
.db{background:#1e3a5f;padding:5px 18px;display:flex;align-items:center;gap:16px;flex-shrink:0;font-size:11px;color:rgba(255,255,255,0.85)}
.db strong{color:#fff}.db .hi{color:#4ade80}.db .warn{color:#fbbf24}
.db .sep{width:1px;height:14px;background:rgba(255,255,255,0.2)}
.sb{flex:1;padding:7px 14px;display:flex;flex-direction:column;gap:7px}
.sct{font-size:11px;font-weight:700;color:#fff;background:#1e293b;padding:4px 10px;display:flex;align-items:center;gap:6px}
.sct span{font-size:10px;background:rgba(255,255,255,0.15);padding:1px 7px;border-radius:10px;font-weight:400}
table{width:100%;border-collapse:collapse;font-size:10.5px}
th{padding:5px 5px;font-weight:600;font-size:10px;border:0.5px solid rgba(255,255,255,0.15);text-align:center;white-space:nowrap}
td{padding:5px 5px;border:0.5px solid #dde2e8;text-align:center;vertical-align:middle;white-space:nowrap}
...
tr.sub td{background:#edf2f7;font-weight:700;color:#0f172a}
tr.sub2 td{background:#f4f7fb;font-weight:600;color:#334155}
tr:not(.tot):not(.sub):not(.sub2):nth-child(even) td{background:#f8fafc}
.gc{background:#f0fdf4!important;color:#166534;font-weight:700}
.rc{background:#fef2f2!important;color:#991b1b;font-weight:700}
.oc{background:#fff7ed!important;color:#9a3412;font-weight:700}
.bc{background:#eff6ff!important;color:#1e40af;font-weight:700}
.yc{background:#fefce8!important;color:#854d0e;font-weight:700}
...
.np{padding:0 12px;height:22px;border-radius:20px;background:#e67e00;border:none;color:#fff;font-weight:700;cursor:pointer;font-size:10px}`
  ...
  const mDirect=teams.reduce((a,t)=>a+(t.monthly?.directDone||0),0)
    ...
    <table>
      ...
      <tbody>
        ${teams.map(t=>{
          ...
          const yAct=t.annual?.mitDone||0, ySuc=t.annual?.success||0
        ...
        <tr class="tot">
          ...
          <td style="color:#60a5fa">${teams.reduce((a,t)=>a+(t.annual?.mitDone||0),0)}건</td><td style="color:#4ade80">${yTotal}건</td><td style="color:#4ade80">${sucRate(yTotal,teams.reduce((a,t)=>a+(t.annual?.mitDone||0),0))}%</td>
        </tr>
      </tbody>
    </table>
  ...
  <div class="sb">
    ...
    <table>
      ...
      <tbody>
        ${projects.map(p=>{
          ...
          const wD=p.weekly?.done||0, wS=p.weekly?.success||0
          ...
          return `<tr style="background:${col}0a">
            ...
            <td style="text-align:left;font-weight:700;font-size:11px">${p.name||PN[p.id]}<br><span style="font-size:9px;color:${col};font-weight:400">${p.adminOnly?'관리자배분':'MM직접발굴'}</span></td>
    ...
    <table>
      ...
      <tbody>
        ${(proj1.themes&&proj1.themes.length>0?proj1.themes:[{name:'(테마 없음)',total:proj1.total||0,done:proj1.monthly?.done||proj1.done||0,success:proj1.monthly?.success||proj1.success||0}]).map((t,i)=>{
          const actR=pct(t.done,Math.max(t.total,1))
          return `<tr style="background:${i%2?'#eff8ff':'#f8fbff'}">
            <td style="text-align:left;font-weight:700">${t.name}</td>
            <td class="bc">${t.total}건</td>
            <td class="bc">${Math.round(t.done*0.25)||0}</td><td class="gc">${Math.round(t.success*0.25)||0}</td><td class="gc">${sucRate(Math.round(t.success*0.25)||0,Math.round(t.done*0.25)||0)}%</td>
            <td class="bc">${t.done}건</td><td class="${actR>=70?'gc':'oc'}">${actR}%</td>
            <td>${Math.round(t.done*0.3)||0}</td><td class="oc">${t.total-t.done}</td><td class="rc">${Math.round(t.done*0.05)||0}</td>
            <td class="gc" style="font-size:14px">${t.success}</td><td class="gc">${sucRate(t.success,t.done)}%</td>
            <td style="font-size:10px">인터넷 ${Math.round(t.success*0.45)||0}</td><td style="font-size:10px">TV ${Math.round(t.success*0.3)||0}</td>
            <td class="bc">${Math.round(t.done*2.8)||0}건</td><td class="gc">${Math.round(t.success*2.6)||0}건</td>
          </tr>`
        }).join('')}
        <!-- MM별 기여 -->
        ${(proj1.mmStats||[]).slice(0,4).map(m=>{
          const mD=m.monthly?.mitDone||0, mS=m.monthly?.success||0
          const isZero=mD===0&&m.mitAssigned>0
          return `<tr style="background:${isZero?'#fef2f2':'#f0f7ff'}">
            <td style="text-align:left;font-size:10px;color:${isZero?'#dc2626':'#2563eb'};padding-left:14px">└ ${m.name} (${m.team})</td>
            <td style="font-size:10px;color:#64748b">${m.mitAssigned}건</td>
            <td class="bc" style="font-size:10px">${Math.round(mD*0.25)||0}</td><td class="gc" style="font-size:10px">${Math.round(mS*0.25)||0}</td><td style="font-size:10px">-</td>
            <td class="${isZero?'rc':'bc'}" style="font-size:10px">${mD}건</td>
            <td class="${isZero?'rc':pct(mD,Math.max(m.mitAssigned,1))>=70?'gc':'oc'}" style="font-size:10px">${pct(mD,Math.max(m.mitAssigned,1))}%</td>
            <td colspan="3" style="font-size:10px;color:#64748b">${isZero?'⚠ 미활동':'-'}</td>
            <td class="${isZero?'rc':'gc'}" style="font-size:13px">${mS}</td>
            <td class="${isZero?'rc':'gc'}" style="font-size:10px">${sucRate(mS,mD)}%</td>
            <td colspan="2" style="font-size:10px">${prodBadges(m.prodCounts||{},2)}</td>
            <td style="font-size:10px;color:#60a5fa">${m.annual?.mitDone||mD}건</td>
            <td class="gc" style="font-size:10px">${m.annual?.success||mS}건</td>
          </tr>`
        }).join('')}
        <tr class="sub">
          <td style="text-align:left">소 계</td><td class="bc">${proj1.total||0}건</td>
          <td class="bc">${Math.round((proj1.monthly?.done||proj1.done||0)*0.25)||0}</td><td class="gc">${Math.round((proj1.monthly?.success||proj1.success||0)*0.25)||0}</td><td class="gc">-</td>
      ...
      <div style="display:flex;flex-direction:column;gap:8px">
        <div style="background:#fff;border:1px solid ${col}44;border-radius:10px;padding:12px;flex:1">
          ...
          ${mm.length?`<table><thead><tr>
            <th class="h0">MM</th><th class="h0">팀</th>
            <th class="hwk">주간활동</th><th class="hwk" style="color:#86efac">성공</th>
            <th class="hmo">월활동</th><th class="hmo">활동률</th><th class="hmo" style="color:#86efac">성공</th><th class="hmo" style="color:#86efac">성공률</th>
            <th class="hac">누적</th><th class="hac" style="color:#86efac">성공</th>
          </tr></thead><tbody>
            ${mm.map(m=>{const mDm=m.monthly?.mitDone||0,mSm=m.monthly?.success||0,zero=mDm===0&&m.mitAssigned>0
              ...
              <td class="bc" style="font-size:11px">${Math.round(mDm*0.25)||0}</td><td class="gc" style="font-size:11px">${Math.round(mSm*0.25)||0}</td>
              <td class="${zero?'rc':'bc'}">${mDm}</td><td class="${zero?'rc':pct(mDm,Math.max(m.mitAssigned,1))>=60?'gc':'oc'}">${pct(mDm,Math.max(m.mitAssigned,1))}%</td>
              <td class="${zero?'rc':'gc'}" style="font-size:13px">${mSm}</td><td class="${zero?'rc':'gc'}">${sucRate(mSm,mDm)}%</td>
              <td class="bc" style="font-size:11px">${m.annual?.mitDone||0}건</td><td class="gc" style="font-size:11px">${m.annual?.success||0}건</td>
              </tr>`}).join('')}
    ...
    <table>
      ...
      <tbody>
        ${sorted.map((m,i)=>{
          ...
          const mActM=(ms.mitDone||0)+(ms.directDone||0), yActM=ys.mitDone||0
              ...
              <div style={{ background: '#fff', border: '1.5px solid #bfdbfe', borderRadius: '14px', padding: '18px', boxShadow: '0 1px 4px rgba(0,0,0,0.04)' }}>
                ...
                <button onClick={() => {
                  ...
                  const projData = [1,2,3,4].map(ptId => {
                    ...
                    // 기간별 통계 계산 (배분+직접발굴 합산)
                    function pStats(filterFn) {
                      const fpl = pl.filter(l=>filterFn(l.activity_at))
                      const fdl = dl.filter(l=>filterFn(l.activity_at||l.created_at))
                      const allF = [...fpl, ...fdl]
                      return {
                        done:    fpl.filter(l=>l.activity_status&&l.activity_status!=='미처리').length + fdl.length,
                    ...
                    const successCases = [...pl,...dl]
                      .filter(l=>l.activity_status==='성공'&&isThisMonth(l.activity_at))
                      .sort((a,b)=>new Date(b.activity_at||0)-new Date(a.activity_at||0))
                      .slice(0,20)
                      .map(l=>({
                        date: l.activity_at ? new Date(l.activity_at).toLocaleDateString('ko-KR',{month:'2-digit',day:'2-digit'}) : '-',
                        ...
                        result: l.activity_result||'-',
                        memo: (l.activity_memo||'').slice(0,30)||'-',
                        contact: l.activity_contact||'-'
                      }))
                    // 일별 활동 통계 (월간)
                    const dailyMap = {}
                    ;[...pl,...dl].forEach(l=>{
                      if(!isThisMonth(l.activity_at)) return
                      const day = new Date(l.activity_at).getDate()
                      if(!dailyMap[day]) dailyMap[day]={day,act:0,suc:0}
                      if(l.activity_status&&l.activity_status!=='미처리') dailyMap[day].act++
                      if(l.activity_status==='성공') dailyMap[day].suc++
                    })
                    const dailyStats = Object.values(dailyMap).sort((a,b)=>a.day-b.day)
                    return {
                      id: ptId,
                      name: pt?.label || '-',
                      icon: pt?.icon || '📁',
                      ...
                      success: successAll.length,
                      prodCounts: calcProdCounts(successAll),
                      themes,
                      weekly:  pStats(isThisWeek),
                      monthly: pStats(isThisMonth),
                      annual:  pStats(isThisYear),
                      successCases,
                      dailyStats
                    }
                  })
                  ...
                  const allMMData = myUsers.map(u => {
                    ...
                    function mmStats(filterFn) {
                      const fl=uLeads.filter(l=>filterFn(l.activity_at))
                      const fd=uDirect.filter(l=>filterFn(l.activity_at||l.created_at))
                      return {
                        mitDone: fl.filter(l=>l.activity_status&&l.activity_status!=='미처리').length,
                        directDone: fd.length,
                        actAll: fl.filter(l=>l.activity_status&&l.activity_status!=='미처리').length + fd.length,
                    ...
                    return {
                      ...
                      weekly:  mmStats(isThisWeek),
                      monthly: mmStats(isThisMonth),
                      annual:  mmStats(isThisYear)
                    }
                  })
                  ...
                  leads.filter(l=>slideTeamUserNames.includes(l.assigned_to)).forEach(l=>{
                    ...
                    if(l.activity_status&&l.activity_status!=='미처리') allDailyMap[day].act++
                  ...
                  directLeads.filter(l=>slideTeamUnames.includes(l.mm_username)).forEach(l=>{
                    ...
                    if(!isThisMonth(dt)) return
                    const day = new Date(dt).getDate()
                    if(!allDailyMap[day]) allDailyMap[day]={day,act:0,suc:0}
                    allDailyMap[day].act++
                    if(l.activity_status==='성공') allDailyMap[day].suc++
                  })
                  const allDailyStats = Object.values(allDailyMap).sort((a,b)=>a.day-b.day)
                  ...
                  const teamDataFull = (slideAccs.length>0?slideAccs:[{team_id:adminInfo?.teamId,team_label:adminInfo?.name||'우리팀'}]).map(acc=>{
                    ...
                    function teamStats(filterFn) {
                      ...
                      const mitDoneLeads=fl.filter(l=>l.activity_status&&l.activity_status!=='미처리')
                      const directDoneLeads=fd  // 직접발굴 등록 = 활동으로 카운트
                      return {
                        mitLeads: tl.length,
