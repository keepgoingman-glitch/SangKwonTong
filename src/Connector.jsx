...
import AirconCampaign from './AirconCampaign'
...
// ── M플러스 기간 필터: daily_records 를 전체/월간/주간/일간으로 집계 ──
// period: '전체' | '월간' | '주간' | '일간'
function mpInPeriod(dateStr, period) {
  ...
  if (period === '주간') {
    // 이번 주 월요일 00:00 ~ 현재
    const day = now.getDay() === 0 ? 7 : now.getDay()   // 일=7
    const mon = new Date(now.getFullYear(), now.getMonth(), now.getDate() - (day - 1))
    return d >= mon && d <= new Date(now.getFullYear(), now.getMonth(), now.getDate(), 23, 59, 59)
  }
  return true
}
// 현장 1곳의 daily_records → 기간별 상품 집계
function mpAgg(m, period) {
  let net = 0, tv = 0, renew = 0, chg = 0, mnp = 0, sec = 0, days = 0
  try {
    const recs = JSON.parse(m.daily_records || '[]')
    recs.forEach(r => {
      if (!mpInPeriod(r.date, period)) return
      days++
      net += Number(r.net || 0); tv += Number(r.tv || 0); renew += Number(r.renew || 0)
      chg += Number(r.wireChange || 0); mnp += Number(r.mnp || 0); sec += Number(r.second || 0)
    })
  } catch {}
  const wire = net + tv + renew, mob = chg + mnp + sec
  return { net, tv, renew, chg, mnp, sec, wire, mob, days, total: wire + mob }
}
// ── 주소 핵심 추출 (법정동+번지) — MM.jsx 중복검증과 동일 로직 ──
function coreAddr(a) {
  if (!a) return ''
  const s = String(a).trim()
  // 날짜 형식(2026-05-21, 05.20, 2026.5.21 등)은 주소 아님 → 제외
  if (/^\s*\d{2,4}[-.\s]\d{1,2}[-.\s]\d{1,2}/.test(s)) return ''
  let m = s.match(/([가-힣]+\d*(?:동|리|읍|면|가))\s*(?:산\s*)?(\d+(?:-\d+)?)/)
  if (m) return (m[1] + m[2]).replace(/\s+/g, '')
  let m2 = s.match(/([가-힣]+\d*(?:로|길))\s*(\d+(?:-\d+)?)/)
  if (m2) return (m2[1] + m2[2]).replace(/\s+/g, '')
  // 동/번지·도로명 패턴을 못 찾으면 중복 판정에서 제외 (날짜·순수숫자·모호한 텍스트 방지)
  return ''
}
// ── 수주 상품 파싱/표시 ──
function parseWonProds(raw) {
  try {
    const p = JSON.parse(raw || 'null')
    if (!p) return null
    if (Array.isArray(p)) return Object.fromEntries(p.map(n => [n, 1]))
    return p
  } catch { return null }
}
function WonBadge({ wonRaw }) {
  const prods = parseWonProds(wonRaw)
  if (!prods || !Object.keys(prods).length) return null
  const total = Object.values(prods).reduce((a, v) => a + Number(v), 0)
  return (
    <div style={{ background: '#f0fdf4', border: '1px solid #bbf7d0', borderRadius: '10px', padding: '8px 10px', marginTop: '6px', marginBottom: '4px' }}>
      <div style={{ fontSize: '12px', fontWeight: '800', color: '#15803d', marginBottom: '4px' }}>✅ 수주 상품 (총 {total}회선)</div>
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: '5px' }}>
        {Object.entries(prods).map(([name, lines]) => (
          <span key={name} style={{ background: '#dcfce7', border: '1px solid #86efac', borderRadius: '20px', padding: '3px 10px', fontSize: '12px', fontWeight: '700', color: '#15803d' }}>{name} {lines}회선</span>
        ))}
      </div>
    </div>
  )
}
// ── 전체 영업기회 중복 검사 모달 (배분+직접발굴 교차 / 완료건 포함) ──
function DupCheckModal({ leads, directLeads, users, onClose, onDeleted }) {
  const [onlyCross, setOnlyCross] = useState(false)
  const [deleting, setDeleting] = useState(null)
  const [confirmTarget, setConfirmTarget] = useState(null)  // 삭제 확인 모달 대상
  const items = [
    ...leads.map(l => {
      const disp = l.address || l.col2 || ''
      // 주소가 여러 칸에 흩어질 수 있어 후보를 모두 시도 → 첫 유효 키 사용
      const k = coreAddr(l.address) || coreAddr(l.col2)
      return {
        id: l.id, table: 'connector_leads',
        key: k, addr: disp || '-', customer: l.col1 || '',
        mm: l.assigned_to || '미배분', src: '배분', status: l.activity_status || '미처리',
        proj: l.project_name || '', date: l.activity_at || ''
      }
    }),
    ...directLeads.map(l => ({
      id: l.id, table: 'mm_direct_leads',
      key: coreAddr(l.address), addr: l.address || '-', customer: l.customer || '',
      mm: (users.find(u => u.username === l.mm_username)?.name) || l.mm_username || '-', src: '직접발굴',
      status: l.activity_status || '미처리', proj: '', date: l.activity_at || l.created_at || ''
    }))
  ].filter(x => x.key)

  // 삭제 버튼 클릭 → 확인 모달 열기
  const delItem = (x) => {
    if (!x.id) { alert('삭제할 수 없는 항목입니다 (ID 없음)'); return }
    setConfirmTarget(x)
  }
  // 모달에서 최종 확정 → 실제 테이블 반영
  const confirmDelete = async () => {
    const x = confirmTarget
    if (!x) return
    setDeleting(x.table + ':' + x.id)
    setConfirmTarget(null)
    try {
      await db.del(x.table, 'id=eq.' + x.id)
      if (onDeleted) await onDeleted()
    } catch (e) {
      alert('삭제 실패: ' + e.message); setDeleting(null)
    }
  }
  const groups = {}
  items.forEach(x => { (groups[x.key] = groups[x.key] || []).push(x) })
  const dupGroups = Object.entries(groups)
    .filter(([, arr]) => arr.length > 1)
    .map(([key, arr]) => ({ key, arr, cross: new Set(arr.map(a => a.mm)).size > 1 }))
    .sort((a, b) => (b.cross ? 1 : 0) - (a.cross ? 1 : 0) || b.arr.length - a.arr.length)
  const crossCount = dupGroups.filter(g => g.cross).length
  const shown = onlyCross ? dupGroups.filter(g => g.cross) : dupGroups
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.45)', zIndex: 1001, display: 'flex', alignItems: 'flex-end', justifyContent: 'center', backdropFilter: 'blur(3px)' }}>
      <div style={{ background: '#fff', borderRadius: '20px 20px 0 0', padding: '20px 18px 32px', width: '100%', maxWidth: '480px', maxHeight: '92vh', overflowY: 'auto', position: 'relative', boxShadow: '0 -4px 24px rgba(0,0,0,0.15)' }}>
        {confirmTarget && (
          <div onClick={() => setConfirmTarget(null)} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.55)', zIndex: 1100, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '24px' }}>
            <div onClick={e => e.stopPropagation()} style={{ background: '#fff', borderRadius: '18px', padding: '24px 22px', width: '100%', maxWidth: '360px', textAlign: 'center', boxShadow: '0 12px 40px rgba(0,0,0,0.25)' }}>
              <div style={{ fontSize: '38px', marginBottom: '10px' }}>⚠️</div>
              <div style={{ fontSize: '18px', fontWeight: '900', color: '#dc2626', marginBottom: '10px' }}>정말 삭제하시겠습니까?</div>
              <div style={{ background: '#f8fafc', border: '1px solid #e2e8f0', borderRadius: '10px', padding: '11px 13px', marginBottom: '12px', textAlign: 'left' }}>
                <div style={{ fontSize: '11px', fontWeight: '800', color: confirmTarget.src === '배분' ? '#e67e00' : '#16a34a', marginBottom: '3px' }}>{confirmTarget.src}</div>
                <div style={{ fontSize: '13px', fontWeight: '700', color: '#1e293b', lineHeight: 1.4 }}>{confirmTarget.addr}</div>
                <div style={{ fontSize: '12px', color: '#64748b', marginTop: '2px' }}>{[confirmTarget.customer, confirmTarget.mm].filter(Boolean).join(' · ')}</div>
              </div>
              <div style={{ fontSize: '13px', color: '#dc2626', fontWeight: '700', marginBottom: '18px', lineHeight: 1.5 }}>삭제되면 복구가 불가능합니다.<br /><span style={{ fontSize: '12px', color: '#94a3b8', fontWeight: '500' }}>실제 데이터에서 영구 삭제되며 성과 집계에서 제외됩니다.</span></div>
              <div style={{ display: 'flex', gap: '10px' }}>
                <button onClick={() => setConfirmTarget(null)} style={{ flex: 1, padding: '13px', background: '#f1f5f9', border: 'none', color: '#64748b', borderRadius: '11px', fontSize: '15px', fontWeight: '800', cursor: 'pointer', fontFamily: 'inherit' }}>취소</button>
                <button onClick={confirmDelete} style={{ flex: 1, padding: '13px', background: '#dc2626', border: 'none', color: '#fff', borderRadius: '11px', fontSize: '15px', fontWeight: '900', cursor: 'pointer', fontFamily: 'inherit' }}>삭제</button>
              </div>
            </div>
          </div>
        )}
        ...
        <div style={{ fontSize: '17px', fontWeight: '900', color: '#1e293b', marginBottom: '4px' }}>🔎 전체 영업기회 중복 검사</div>
        <div style={{ fontSize: '12px', color: '#64748b', marginBottom: '12px' }}>배분 + 직접발굴 전체 · 법정동+번지 기준 · 완료건 포함 (검사 대상 {items.length}건)</div>
        <div style={{ display: 'flex', gap: '8px', marginBottom: '12px' }}>
          <div style={{ flex: 1, background: '#fff7ed', border: '1.5px solid #fed7aa', borderRadius: '12px', padding: '10px', textAlign: 'center' }}>
            <div style={{ fontSize: '22px', fontWeight: '900', color: '#e67e00' }}>{dupGroups.length}</div>
            <div style={{ fontSize: '11px', color: '#64748b' }}>중복 주소 그룹</div>
          </div>
          <div style={{ flex: 1, background: '#fef2f2', border: '1.5px solid #fecaca', borderRadius: '12px', padding: '10px', textAlign: 'center' }}>
            <div style={{ fontSize: '22px', fontWeight: '900', color: '#dc2626' }}>{crossCount}</div>
            <div style={{ fontSize: '11px', color: '#64748b' }}>담당자 상이 그룹</div>
          </div>
        </div>
        <div style={{ display: 'flex', gap: '6px', marginBottom: '12px' }}>
          {[[false, '전체 중복'], [true, '⚠ 담당자 상이만']].map(([v, label]) => (
            <button key={String(v)} onClick={() => setOnlyCross(v)}
              style={{ flex: 1, padding: '8px', background: onlyCross === v ? (v ? '#dc2626' : '#1e293b') : '#fff', border: '1.5px solid ' + (onlyCross === v ? (v ? '#dc2626' : '#1e293b') : '#e2e8f0'), color: onlyCross === v ? '#fff' : '#64748b', borderRadius: '10px', fontSize: '13px', fontWeight: '700', cursor: 'pointer', fontFamily: 'inherit' }}>{label}</button>
          ))}
        </div>
        {shown.length === 0
          ? <div style={{ textAlign: 'center', padding: '40px 0', color: '#16a34a', fontSize: '15px', fontWeight: '800' }}>✅ {onlyCross ? '담당자가 다른 중복이 없습니다' : '중복된 주소가 없습니다'}</div>
          : shown.map(g => (
            <div key={g.key} style={{ background: g.cross ? '#fef2f2' : '#fff', border: '1.5px solid ' + (g.cross ? '#fecaca' : '#e2e8f0'), borderRadius: '12px', padding: '12px', marginBottom: '10px' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '8px' }}>
                <span style={{ fontSize: '13px', fontWeight: '900', color: g.cross ? '#dc2626' : '#1e293b' }}>📍 {g.key} <span style={{ color: '#94a3b8', fontWeight: '400' }}>({g.arr.length}건)</span></span>
                <span style={{ fontSize: '10px', fontWeight: '800', padding: '2px 8px', borderRadius: '10px', background: g.cross ? '#dc2626' : '#f1f5f9', color: g.cross ? '#fff' : '#64748b' }}>{g.cross ? '⚠ 담당자 다름' : '동일 담당'}</span>
              </div>
              {g.arr.map((x, i) => {
                const stc = SC[x.status] || SC['미처리']
                const isDel = deleting === (x.table + ':' + x.id)
                return (
                  <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '6px', padding: '7px 0', borderTop: i > 0 ? '1px solid #f1f5f9' : 'none', fontSize: '12px', opacity: isDel ? 0.4 : 1 }}>
                    <span style={{ fontSize: '10px', fontWeight: '800', padding: '2px 7px', borderRadius: '6px', background: x.src === '배분' ? '#fff7ed' : '#f0fdf4', color: x.src === '배분' ? '#e67e00' : '#16a34a', flexShrink: 0 }}>{x.src}</span>
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ fontWeight: '700', color: '#1e293b', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{x.addr}</div>
                      <div style={{ color: '#94a3b8', fontSize: '11px' }}>{[x.customer, x.proj, x.date ? fmtAt(x.date) : ''].filter(Boolean).join(' · ')}</div>
                    </div>
                    <span style={{ fontWeight: '800', color: '#2563eb', flexShrink: 0 }}>{x.mm}</span>
                    <span style={{ fontSize: '10px', fontWeight: '700', padding: '2px 7px', borderRadius: '10px', background: stc.bg, color: stc.text, border: '1px solid ' + stc.border, flexShrink: 0 }}>{x.status}</span>
                    <button onClick={() => delItem(x)} disabled={isDel} title="이 건 삭제"
                      style={{ flexShrink: 0, background: '#fee2e2', border: 'none', color: '#dc2626', width: '24px', height: '24px', borderRadius: '7px', fontSize: '13px', cursor: isDel ? 'wait' : 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 0 }}>🗑</button>
                  </div>
                )
              })}
            </div>
        ...
        <div style={{ fontSize: '11px', color: '#94a3b8', marginTop: '6px', lineHeight: 1.6 }}>※ 중복 기준은 신규등록 검증과 동일한 법정동/도로명+번지입니다. 담당자가 서로 다른 그룹부터 표시됩니다. 처리(성공/실패 포함)된 건도 재영업 방지를 위해 포함됩니다.</div>
      </div>
    </div>
  )
}
// ── 오더사항 전송 모달 (관리자 → 상권마스터) ──
function OrderSendModal({ target, senderName, senderRole, onClose, onSent }) {
  const [title, setTitle] = useState('')
  const [body, setBody] = useState('')
  const [sending, setSending] = useState(false)
  const [err, setErr] = useState('')
  const quick = ['현장 재방문 요청', '수주 상품 확인 바랍니다', '개통일/회선번호 입력 요망', '고객 재컨택 필요', '실적 입력 확인']
  const handleSend = async () => {
    if (!body.trim()) { setErr('오더 내용을 입력하세요'); return }
    setSending(true); setErr('')
    try {
      await db.post('mm_orders', {
        mm_username: target.username, mm_name: target.name,
        sender_name: senderName || '관리자', sender_role: senderRole || '',
        title: title.trim(), body: body.trim(), is_read: false
      })
      onSent(target.name)
    } catch (e) { setErr('전송 실패: ' + e.message); setSending(false) }
  }
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.4)', zIndex: 1001, display: 'flex', alignItems: 'flex-end', justifyContent: 'center', backdropFilter: 'blur(3px)' }}>
      <div style={{ background: '#fff', borderRadius: '20px 20px 0 0', padding: '22px 18px 32px', width: '100%', maxWidth: '480px', maxHeight: '90vh', overflowY: 'auto', position: 'relative', boxShadow: '0 -4px 24px rgba(0,0,0,0.15)' }}>
        <button onClick={onClose} style={{ position: 'absolute', top: '14px', right: '14px', background: '#f1f5f9', border: 'none', color: '#64748b', width: '28px', height: '28px', borderRadius: '50%', fontSize: '16px', cursor: 'pointer' }}>✕</button>
        <div style={{ fontSize: '17px', fontWeight: '900', color: '#1e293b', marginBottom: '4px' }}>📢 오더사항 전송</div>
        <div style={{ fontSize: '13px', color: '#64748b', marginBottom: '14px' }}>받는 사람: <b style={{ color: '#e67e00' }}>{target.name} MM</b> {target.region ? '· ' + target.region : ''}</div>
        {err && <div style={{ color: '#dc2626', fontSize: '13px', background: '#fef2f2', border: '1px solid #fecaca', padding: '9px 11px', borderRadius: '8px', marginBottom: '10px' }}>{err}</div>}
        <div style={{ fontSize: '12px', color: '#64748b', fontWeight: '700', marginBottom: '5px' }}>빠른 입력</div>
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '5px', marginBottom: '12px' }}>
          {quick.map(q => (
            <button key={q} onClick={() => setBody(b => b ? b + '\n' + q : q)} style={{ background: '#f8fafc', border: '1px solid #e2e8f0', color: '#475569', fontSize: '12px', fontWeight: '600', padding: '5px 10px', borderRadius: '16px', cursor: 'pointer', fontFamily: 'inherit' }}>+ {q}</button>
          ))}
        </div>
        <div style={{ fontSize: '12px', color: '#64748b', fontWeight: '700', marginBottom: '4px' }}>제목 (선택)</div>
        <input value={title} onChange={e => setTitle(e.target.value)} placeholder="예: 명지동 현장 관련" style={{ ...IS, marginBottom: '10px' }} />
        <div style={{ fontSize: '12px', color: '#64748b', fontWeight: '700', marginBottom: '4px' }}>오더 내용</div>
        <textarea value={body} onChange={e => setBody(e.target.value)} placeholder="상권마스터에게 전달할 현장 활동 피드백·지시사항을 입력하세요" rows={5}
          style={{ ...IS, resize: 'vertical', minHeight: '110px', lineHeight: 1.6, marginBottom: '14px' }} />
        <button onClick={handleSend} disabled={sending} style={{ width: '100%', padding: '15px', background: sending ? '#e2e8f0' : '#e67e00', border: 'none', color: sending ? '#94a3b8' : '#fff', borderRadius: '12px', fontSize: '16px', fontWeight: '900', cursor: sending ? 'wait' : 'pointer', fontFamily: 'inherit' }}>
          {sending ? '전송 중...' : '📢 오더 전송'}
        </button>
        <p style={{ fontSize: '11.5px', color: '#94a3b8', textAlign: 'center', marginTop: '8px', marginBottom: 0 }}>전송하면 해당 상권마스터 앱의 '오더사항'에 알림과 함께 표시됩니다</p>
      </div>
    </div>
  )
}
// ── 보낸 오더 조회 모달 (읽음 여부 + MM별 필터) ──
function SentOrdersModal({ orders, users, onClose, onRefresh, onDelete }) {
  const [filterMM, setFilterMM] = useState('전체')
  const [onlyUnread, setOnlyUnread] = useState(false)
  const mmList = ['전체', ...Array.from(new Set(orders.map(o => o.mm_name || o.mm_username)))]
  const shown = orders.filter(o => {
    if (filterMM !== '전체' && (o.mm_name || o.mm_username) !== filterMM) return false
    if (onlyUnread && o.is_read) return false
    return true
  })
  const unreadTotal = orders.filter(o => !o.is_read).length
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.45)', zIndex: 1001, display: 'flex', alignItems: 'flex-end', justifyContent: 'center', backdropFilter: 'blur(3px)' }}>
      <div style={{ background: '#fff', borderRadius: '20px 20px 0 0', padding: '20px 18px 32px', width: '100%', maxWidth: '480px', maxHeight: '92vh', overflowY: 'auto', position: 'relative', boxShadow: '0 -4px 24px rgba(0,0,0,0.15)' }}>
        <button onClick={onClose} style={{ position: 'absolute', top: '14px', right: '14px', background: '#f1f5f9', border: 'none', color: '#64748b', width: '28px', height: '28px', borderRadius: '50%', fontSize: '16px', cursor: 'pointer' }}>✕</button>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '4px', paddingRight: '32px' }}>
          <div style={{ fontSize: '17px', fontWeight: '900', color: '#1e293b' }}>📢 보낸 오더사항</div>
          <button onClick={onRefresh} style={{ background: '#f8fafc', border: '1px solid #e2e8f0', color: '#64748b', padding: '5px 10px', borderRadius: '8px', fontSize: '12px', cursor: 'pointer', fontFamily: 'inherit' }}>🔄 새로고침</button>
        </div>
        <div style={{ fontSize: '12px', color: '#64748b', marginBottom: '12px' }}>총 {orders.length}건 · 미확인 <b style={{ color: '#dc2626' }}>{unreadTotal}건</b></div>
        <div style={{ display: 'flex', gap: '6px', marginBottom: '12px', flexWrap: 'wrap', alignItems: 'center' }}>
          <select value={filterMM} onChange={e => setFilterMM(e.target.value)} style={{ flex: 1, minWidth: '120px', padding: '8px 10px', border: '1.5px solid #e2e8f0', borderRadius: '9px', fontSize: '13px', fontFamily: 'inherit', background: '#fff', color: '#1e293b' }}>
            {mmList.map(m => <option key={m} value={m}>{m === '전체' ? '👥 전체 MM' : m}</option>)}
          </select>
          <button onClick={() => setOnlyUnread(v => !v)} style={{ padding: '8px 13px', background: onlyUnread ? '#dc2626' : '#fff', border: '1.5px solid ' + (onlyUnread ? '#dc2626' : '#e2e8f0'), color: onlyUnread ? '#fff' : '#64748b', borderRadius: '9px', fontSize: '13px', fontWeight: '700', cursor: 'pointer', fontFamily: 'inherit' }}>미확인만</button>
        </div>
        {shown.length === 0
          ? <div style={{ textAlign: 'center', padding: '40px 0', color: '#94a3b8', fontSize: '14px' }}>해당 조건의 오더가 없습니다</div>
          : shown.map(o => {
            const read = o.is_read
            return (
              <div key={o.id} style={{ background: '#fff', border: '1.5px solid #e2e8f0', borderLeft: '4px solid ' + (read ? '#16a34a' : '#e67e00'), borderRadius: '12px', padding: '12px 14px', marginBottom: '9px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '5px' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '6px', flexWrap: 'wrap' }}>
                    <span style={{ fontSize: '13px', fontWeight: '900', color: '#2563eb' }}>{o.mm_name || o.mm_username} MM</span>
                    {read
                      ? <span style={{ fontSize: '10px', fontWeight: '800', padding: '2px 8px', borderRadius: '10px', background: '#f0fdf4', color: '#15803d', border: '1px solid #bbf7d0' }}>✓ 확인함</span>
                      : <span style={{ fontSize: '10px', fontWeight: '800', padding: '2px 8px', borderRadius: '10px', background: '#fef2f2', color: '#dc2626', border: '1px solid #fecaca' }}>● 미확인</span>}
                  </div>
                  <span style={{ fontSize: '11px', color: '#94a3b8', flexShrink: 0 }}>{fmtAt(o.created_at)}</span>
                </div>
                {o.title && <div style={{ fontSize: '13px', fontWeight: '800', color: '#e67e00', marginBottom: '3px' }}>{o.title}</div>}
                <div style={{ fontSize: '13px', color: '#1e293b', lineHeight: 1.6, whiteSpace: 'pre-wrap' }}>{o.body}</div>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: '7px' }}>
                  <span style={{ fontSize: '11px', color: '#94a3b8' }}>{read && o.read_at ? '✓ ' + fmtAt(o.read_at) + ' 확인' : '보낸이: ' + (o.sender_name || '관리자')}</span>
                  <button onClick={() => onDelete(o)} style={{ background: 'none', border: 'none', color: '#cbd5e1', fontSize: '12px', cursor: 'pointer', fontFamily: 'inherit', padding: '2px 6px' }}>🗑 삭제</button>
                </div>
              </div>
            )
          })
...
function EditMMModal({ user, onClose, onSave, isSuper }) {
  const [form, setForm] = useState({ name: user.name, password: '', region: user.region, team_id: user.team_id || '' })
  const set = useCallback((k, v) => setForm(prev => ({ ...prev, [k]: v })), [])
  const handleSave = async () => {
    if (!form.name) { alert('이름을 입력하세요'); return }
    try {
      const patch = { name: form.name, region: form.region }
      if (form.password.trim()) patch.password = form.password.trim()  // 입력한 경우에만 비밀번호 변경
  ...
  return (
    <Modal onClose={onClose} title={`계정 수정 — ${user.name}`}>
      {[{ k: 'name', l: '이름' }, { k: 'password', l: '비밀번호 (변경 시에만 입력)' }, { k: 'region', l: '담당 지역' }].map(f => (
...
// ── 메인 컴포넌트 ─────────────────────────────────────────────────
export default function Connector({ onBack, initialTab }) {
  ...
  // U-TURN UIT 점수 입력
  const UTURN_TEAMS = [
    { id: 'team_mkt',  label: '마케팅팀' },
    { id: 'team1',     label: '시너지영업1팀' },
    { id: 'team2',     label: '시너지영업2팀' },
    { id: 'team_cs',   label: 'CS운영팀' },
    { id: 'team_plan', label: '고객기획팀' },
  ]
  const [uturnMonth, setUturnMonth] = useState(new Date().getMonth() + 1)
  const [uturnScores, setUturnScores] = useState({})  // { team_id: {u_goal,u_actual,...} }
  const [uturnSaving, setUturnSaving] = useState(false)
  ...
  const [mDiscPeriod, setMDiscPeriod] = useState('전체')  // 전체 | 월간 | 주간 | 일간
  const [mEdit, setMEdit] = useState(null)   // 관리자 수정 대상 현장
  const [mEditForm, setMEditForm] = useState({ stage:'', agency_name:'', agency_manager:'', sales_schedule:'' })
  ...
  const annualRef = useRef(false)   // 연간 누계 전용 보고서 여부 (즉시 반영 위해 ref)
  ...
  const [orderTarget, setOrderTarget] = useState(null)  // 오더 전송 대상 MM
  const [sentOrders, setSentOrders] = useState([])  // 보낸 오더 목록
  const [showSentOrders, setShowSentOrders] = useState(false)  // 보낸 오더 조회 모달
  ...
  const [showDupCheck, setShowDupCheck] = useState(false)  // 전체 영업기회 중복 검사 모달
  ...
  // ── 보낸 오더사항 로드 (읽음 여부 포함) ──
  // ── U-TURN UIT 점수 로드/저장 ──
  const loadUturnScores = async (mon) => {
    const m = mon || uturnMonth
    try {
      const rows = await db.get('uturn_scores', 'select=*&year=eq.' + new Date().getFullYear() + '&month=eq.' + m)
      const map = {}
      ;(rows || []).forEach(r => { map[r.team_id] = r })
      setUturnScores(map)
    } catch { setUturnScores({}) }
  }
  const setUturnField = (teamId, field, val) => {
    setUturnScores(prev => ({ ...prev, [teamId]: { ...(prev[teamId] || {}), [field]: val } }))
  }
  const saveUturnScores = async () => {
    setUturnSaving(true)
    const y = new Date().getFullYear()
    try {
      for (const t of UTURN_TEAMS) {
        const s = uturnScores[t.id] || {}
        const row = {
          team_id: t.id, team_label: t.label, year: y, month: uturnMonth,
          u_goal: parseInt(s.u_goal) || 0, i_goal: parseInt(s.i_goal) || 0, t_goal: parseInt(s.t_goal) || 0,
          u_actual: parseInt(s.u_actual) || 0, i_actual: parseInt(s.i_actual) || 0, t_actual: parseInt(s.t_actual) || 0,
          note: s.note || '', updated_by: adminInfo?.name || '관리자', updated_at: new Date().toISOString()
        }
        const existing = await db.get('uturn_scores', 'select=id&team_id=eq.' + t.id + '&year=eq.' + y + '&month=eq.' + uturnMonth)
        if (existing && existing.length) await db.patch('uturn_scores', 'id=eq.' + existing[0].id, row)
        else await db.post('uturn_scores', row)
      }
      t2('U-TURN 목표·실적 저장 완료!')
      loadUturnScores()
    } catch (e) { t2('저장 실패: ' + e.message) }
    setUturnSaving(false)
  }

  const loadSentOrders = async () => {
    try {
      const rows = await db.get('mm_orders', 'select=*&order=created_at.desc&limit=300')
      // 총괄=전체, 팀관리자=본인 팀 MM에게 보낸 것만
      if (adminInfo?.role === 'super') { setSentOrders(rows || []) }
      else {
        const teamUnames = new Set(users.filter(u => u.team_id === adminInfo?.teamId).map(u => u.username))
        setSentOrders((rows || []).filter(o => teamUnames.has(o.mm_username)))
      }
    } catch (e) { setSentOrders([]) }
  }
  ...
  const load = async (info, scrollPos) => {
    ...
    setLoading(true)
    try {
      const _uCols = 'select=id,username,name,region,goal,success,team_id&'; const uq = (ai?.role === 'team' && ai?.teamId) ? _uCols+'team_id=eq.' + ai.teamId + '&order=created_at.asc&limit=2000' : _uCols+'order=created_at.asc&limit=2000'
      // photos 컬럼 제외 (base64 대용량 → 타임아웃 원인)
      const lSel = 'select=id,col1,col2,col3,col4,col5,address,products,assigned_to,assign_status,activity_status,activity_result,activity_memo,activity_contact,activity_at,team_id,project_name,theme_name,project_type,won_products,won_lines,circuit_number,activation_date,agency_type,agency_name'
      const dSel = 'select=id,mm_username,customer,address,contact,products,note,project_type,activity_status,activity_result,activity_memo,activity_contact,activity_at,created_at,won_products,won_lines,circuit_number,activation_date,agency_type,agency_name'
      ...
      if (ai?.role === 'super') {
        db.get('admin_accounts', 'select=id,username,name,role,team_id,team_label&role=eq.team&order=username.asc').then(r => setAdminAccounts(r || [])).catch(()=>{})
      }
      ...
      const [u, l, dlRaw, md, qrP, qrS, qrL, agcy] = await Promise.all([
        ...
        db.get('qr_partners', 'select=id,username,name,team_id,qr_code,created_at&order=created_at.desc').catch(() => []),
  ...
  const handleLogin = async () => {
    setLerr('')
    try {
      // 서버 함수(RPC)로 검증 — 비밀번호는 클라이언트로 내려오지 않음
      const rows = await db.rpc('verify_admin_login', { p_username: String(lid).trim(), p_password: String(lpw).trim() })
      if (!rows || !rows.length) { setLerr('아이디 또는 비밀번호가 올바르지 않습니다'); return }
      const a = rows[0]
      const info = { role: a.role, name: a.name, teamId: a.team_id || null }
      setAdminInfo(info); setScreen('main'); load(info); if (initialTab === 'uturn') loadUturnScores()
  ...
  const handleSaveTeamAdmin = async (teamId, form) => {
    try {
      ...
      const rows = await db.get('admin_accounts', 'select=id,username,name,role,team_id,team_label&role=eq.team&order=username.asc')
...
function buildSlideHTML({ period, today, yearStr, teams, projects, allMM, topMM,
  ...
  bizDays=0, perMMTarget=0, workDaysAll={}, allDailyStats=[], _debug={}, reportMode='current', momData=null, mDiscoveryList=[], agenciesList=[], allLeadsForPS=null, psMonthRange=null,
  monthlySeries=[], annualTargetCalc=0, startMonth=4, annualTeams=null, annualMMs=null }) {
  const isPast = reportMode==='past'
  const isAnnual = reportMode==='annual'   // 연간 누계 전용 보고서
  ...
  function prodGroup(pc){
    ...
    const mob=(pc['모바일']||0)                        // 모바일만 (USIM은 기타로 분류 → 상세표와 일치)
    const known=['인터넷','TV-M','TV','TV-B','다량회선','모바일']
  ...
  // 상세표 11개 항목 (구버전 명칭은 신버전에 합산하여 5분류표와 합계 일치)
  const DETAIL_PRODS=['인터넷','TV-M','TV-B','모바일','USIM','전화','약정갱신','기타','하이오더','로봇','CCTV']
  function prodVal(pc,prod){
    pc=pc||{}
    if(prod==='TV-M') return (pc['TV-M']||0)+(pc['TV']||0)        // 구 'TV' 합산
    if(prod==='TV-B') return (pc['TV-B']||0)+(pc['다량회선']||0)  // 구 '다량회선' 합산
    if(prod==='전화') return (pc['전화']||0)+(pc['일반전화']||0)  // 구 '일반전화' 합산
    return pc[prod]||0
  }
  ...
  *{margin:0;padding:0;box-sizing:border-box;-webkit-print-color-adjust:exact!important;print-color-adjust:exact!important}
  ...
  .ps-slide th{padding:4px 6px;font-size:11px}
  .ps-slide td{padding:3px 6px;font-size:11.5px}
  ...
  // M플러스 구간별 현장 수에 따라 페이지가 자동 분할되므로 총 페이지 수를 먼저 산출
  const _MP_PER_PAGE = 15
  const _mpSites = mDiscoveryList || []
  const _mpCnt = (arr) => _mpSites.filter(m => arr.includes(m.stage || '접촉중')).length
  const _pgOf = (n) => Math.max(1, Math.ceil(n / _MP_PER_PAGE))
  const MP_PAGES = {
    run:  _pgOf(_mpCnt(['영업중'])),
    done: _pgOf(_mpCnt(['영업종료'])),
    init: _pgOf(_mpCnt(['접촉중','발굴성공']))
  }
  // 고정 9P(1~8 + M플러스 종합) + 구간별 페이지 + 마지막 대리점 1P
  const TOTAL_PAGES = reportMode==='annual' ? 8 : (9 + MP_PAGES.run + MP_PAGES.done + MP_PAGES.init + 1)
  ...
  // ════ S1: 표지 ════
  // 연간 누계 집계 (표지에서도 사용)
  const AN_MS = monthlySeries || []
  const AN_SUM = AN_MS.reduce((a,m)=>({act:a.act+m.act, suc:a.suc+m.success}),{act:0,suc:0})
  const AN_MOLABEL = m => m+'월'
  // 상품 5분류 누계
  function anProdSum(){
    const t={inet:0,tv:0,tvbiz:0,mob:0,etc:0}
    AN_MS.forEach(m=>{ const g=prodGroup(m.prodCounts||{})
      t.inet+=g.inet; t.tv+=g.tv; t.tvbiz+=g.tvbiz; t.mob+=g.mob; t.etc+=g.etc })
    return t
  }
  ...
  <div style="text-align:center">
    ...
    <h1 style="font-size:54px;font-weight:900;margin-bottom:10px">상권통 ${isAnnual?'연간 누계 보고서':'영업활동 보고서'}</h1>
    <div style="font-size:20px;color:rgba(255,255,255,0.7);margin-bottom:14px">${period} 실적 현황${isPast?' (월마감)':''}</div>
    <div style="display:inline-block;background:rgba(230,126,0,0.15);border:1px solid rgba(230,126,0,0.5);border-radius:30px;padding:8px 26px;font-size:14px;color:#fbbf24;font-weight:700;margin-bottom:32px">${isAnnual
      ? `${startMonth}월 ~ ${AN_MS.length?AN_MS[AN_MS.length-1].month:startMonth}월 (${AN_MS.length}개월) | 연간목표 ${annualTargetCalc}건 | 달성 ${annualTargetCalc>0?Math.round(AN_SUM.suc/annualTargetCalc*100):0}%`
      : `영업일 ${bizDays}일 | 1인목표 ${perMMTarget}건 | ${mmCount}명 합계목표 ${totalMonthTarget}건`}</div>
    ${isAnnual ? `<div style="display:flex;gap:14px;justify-content:center;margin-bottom:22px">
      <div style="background:rgba(124,58,237,0.18);border:1px solid rgba(167,139,250,0.4);border-radius:14px;padding:16px 26px;min-width:132px">
        <div style="font-size:30px;font-weight:900;color:#c4b5fd">${AN_SUM.act}</div><div style="font-size:12px;color:rgba(255,255,255,0.7);margin-top:2px">연간 누적 활동</div></div>
      <div style="background:rgba(22,163,74,0.18);border:1px solid rgba(134,239,172,0.4);border-radius:14px;padding:16px 26px;min-width:132px">
        <div style="font-size:30px;font-weight:900;color:#86efac">${AN_SUM.suc}</div><div style="font-size:12px;color:rgba(255,255,255,0.7);margin-top:2px">연간 누적 성공</div></div>
      <div style="background:rgba(37,99,235,0.18);border:1px solid rgba(147,197,253,0.4);border-radius:14px;padding:16px 26px;min-width:132px">
        <div style="font-size:30px;font-weight:900;color:#93c5fd">${AN_SUM.act>0?Math.round(AN_SUM.suc/AN_SUM.act*100):0}%</div><div style="font-size:12px;color:rgba(255,255,255,0.7);margin-top:2px">누적 성공률</div></div>
      <div style="background:rgba(217,119,6,0.18);border:1px solid rgba(253,186,116,0.4);border-radius:14px;padding:16px 26px;min-width:132px">
        <div style="font-size:30px;font-weight:900;color:#fdba74">${(()=>{const p=anProdSum();return p.inet+p.tv+p.tvbiz+p.mob+p.etc})()}</div><div style="font-size:12px;color:rgba(255,255,255,0.7);margin-top:2px">누적 수주 회선</div></div>
    </div>` : ''}
    <div style="display:flex;gap:16px;justify-content:center">
      ${((isAnnual&&annualTeams&&annualTeams.length)?annualTeams:teams).map(t=>`<div style="background:rgba(255,255,255,0.08);border:1px solid rgba(255,255,255,0.15);border-radius:14px;padding:18px 30px;min-width:200px">
        <div style="font-size:17px;font-weight:800;color:#e67e00;margin-bottom:8px">${t.label}</div>
        <div style="font-size:13px;color:rgba(255,255,255,0.85);line-height:1.9">MM ${t.mm}명${isAnnual?'':' · 목표 '+(t.goal||t.mm*perMMTarget)+'건'}<br/>${isAnnual
          ? `연간 활동 <b style="color:#60a5fa">${t.annual?.actAll||0}건</b> · 성공 <b style="color:#4ade80">${t.annual?.success||0}건</b>`
          : `${isPast?'월마감':'월간'} 활동 <b style="color:#60a5fa">${t.monthly?.actAll||0}건</b> · 성공 <b style="color:#4ade80">${t.monthly?.success||0}건</b>`}</div>
      </div>`).join('')}
    </div>
  </div>
  ...
  <div class="body">
    ...
    <table>
      <thead><tr><th style="width:80px">구분</th>${DETAIL_PRODS.map(p=>`<th>${p}</th>`).join('')}<th style="background:#166534">합계</th></tr></thead>
      <tbody>
        ${teams.map((t,i)=>{
          ...
          return `<tr style="background:${i%2?'#f8fafc':'#fff'}">
            ...
            ${DETAIL_PRODS.map(p=>`<td class="${p==='인터넷'?'bc':''}">${prodVal(pc2,p)||'-'}</td>`).join('')}
        ...
        <tr style="background:#16213e;color:#fff;font-weight:900">
          ...
          ${DETAIL_PRODS.map(p=>{
            const v=teams.reduce((a,t)=>a+prodVal(t.monthly?.prodCounts||{},p),0)
  ...
  function svgLineChart(data, opts){
    ...
    const xLabels=data.map((d,i)=>`<text x="${xs(i).toFixed(1)}" y="${h-5}" font-size="8.5" fill="#94a3b8" text-anchor="middle">${d.day}</text>`).join('')
  ...
  function svgCumChart(data, opts){
    ...
    const xLabels=cum.map((d,i)=>`<text x="${xs(i).toFixed(1)}" y="${h-5}" font-size="8.5" fill="#94a3b8" text-anchor="middle">${d.day}</text>`).join('')
    return `<svg viewBox="0 0 ${w} ${h}" style="width:100%;height:auto">
      <line x1="${padL}" y1="${padT+ch}" x2="${w-padR}" y2="${padT+ch}" stroke="#e2e8f0"/>
      <text x="${padL-5}" y="${padT+4}" font-size="9.5" fill="#94a3b8" text-anchor="end">${maxV}</text>
      <text x="${padL-5}" y="${padT+ch+4}" font-size="9.5" fill="#94a3b8" text-anchor="end">0</text>
  ...
  // ════ S4: 프로젝트 종합 표 ════
  // M플러스(id=5) 실적을 m_discovery(9페이지) 기반으로 집계 — 4페이지와 9페이지 일치
  function mPlusAggForP4(){
    const sites=mDiscoveryList||[]
    // 당월 판정 (psMonthRange 기준, 없으면 현재 월)
    const _mrP4 = psMonthRange
    const _inMoP4 = (ds)=>{
      if(!ds) return false
      const t=new Date(ds).getTime(); if(isNaN(t)) return false
      if(_mrP4) return t>=_mrP4.start && t<=_mrP4.end
      const d=new Date(ds), n=new Date()
      return d.getFullYear()===n.getFullYear() && d.getMonth()===n.getMonth()
    }
    let net=0,tv=0,renew=0,mob=0            // 당월
    let aNet=0,aTv=0,aRenew=0,aMob=0        // 누적
    sites.forEach(m=>{try{const recs=JSON.parse(m.daily_records||'[]')
      recs.forEach(r=>{
        const _n=Number(r.net||0), _t=Number(r.tv||0), _rw=Number(r.renew||0)
        const _mb=Number(r.wireChange||0)+Number(r.mnp||0)+Number(r.second||0)
        aNet+=_n; aTv+=_t; aRenew+=_rw; aMob+=_mb
        if(_inMoP4(r.date)){ net+=_n; tv+=_t; renew+=_rw; mob+=_mb }
      })}catch{}})
    // 활동/성공 = 현장 단계 기준 (영업중·영업종료=활동, 영업종료=성공)
    const active=sites.filter(m=>['영업중','영업종료'].includes(m.stage||'')).length
    const success=sites.filter(m=>(m.stage||'')==='영업종료').length
    // 상품 5분류: 인터넷=net, TV-M=tv, 약정갱신은 기타로, 모바일=mob
    const pg={inet:net, tv:tv, tvbiz:0, mob:mob, etc:renew, total:net+tv+mob+renew}          // 월간(표의 상품 칸)
    const pgAll={inet:aNet, tv:aTv, tvbiz:0, mob:aMob, etc:aRenew, total:aNet+aTv+aMob+aRenew} // 누적(참고용)
    return { siteCount:sites.length, active, success, pg, pgAll }
  }
  const _mp = mPlusAggForP4()
  const projRows = projects.map(p=>{
    const wD=(p.mmStats||[]).reduce((a,m)=>a+(m.weekly?.actAll||0),0)
    const wS=(p.mmStats||[]).reduce((a,m)=>a+(m.weekly?.success||0),0)
    let mD=p.monthly?.done||0, mS=p.monthly?.success||0
    let yD2=p.annual?.done||0, yS2=p.annual?.success||0
    let pg=prodGroup(p.monthly?.prodCounts||{})
    let targetOverride=null
    // M플러스(id=5)는 m_discovery 기반으로 교체
    if(p.id===5){
      mD=_mp.active; mS=_mp.success
      yD2=_mp.active; yS2=_mp.success
      pg=_mp.pg
      targetOverride=_mp.siteCount
    }
    return {p,wD,wS,mD,mS,yD:yD2,yS:yS2,pg,targetOverride}
  })
  ...
  <div class="body">
    <table>
      ...
      <tbody>
        ${projRows.map(({p,wD,wS,mD,mS,yD,yS,pg,targetOverride})=>`
        <tr>
          <td style="text-align:left;font-weight:800;color:${PC[p.id]||'#1e293b'};padding-left:12px">${p.icon} ${p.name}</td>
          <td>${targetOverride!=null?targetOverride:(p.total||0)+(p.directTotal||0)}건</td>
        ...
        <tr style="font-weight:900">
          ...
          <td style="background:#16213e;color:#fff">${projRows.reduce((a,r)=>a+(r.targetOverride!=null?r.targetOverride:(r.p.total||0)+(r.p.directTotal||0)),0)}건</td>
    ...
    <div style="margin-top:8px;font-size:11px;color:#94a3b8">※ 상품별 실적 = <b style="color:#15803d">월간</b> 성공건의 수주 상품·회선수 · TV-M=일반TV, TV-B=舊 다량회선 · 회선수 미입력 건은 상품당 1회선 · <b style="color:#7c3aed">M플러스는 영업구간(현장) 기준 집계(9P 참조)</b></div>
  ...
  // ════ S9: M플러스 영업구간(현장)별 실적 ════
  // ════ M플러스: 공통 집계 + 단계별 슬라이드 ════
  function mpBuild(){
    const sites = mDiscoveryList||[]
    const _msT=(dt)=>{if(!dt)return -1;try{const t=new Date(dt).getTime();return isNaN(t)?-1:t}catch{return -1}}
    const _mrange = psMonthRange
    const aggOf=(m,onlyMonth)=>{
      let net=0,tv=0,renew=0,chg=0,mnp=0,sec=0,days=0,last=''
      try{const recs=JSON.parse(m.daily_records||'[]')
        recs.forEach(r=>{
          if(onlyMonth&&_mrange){const t=_msT(r.date); if(!(t>=_mrange.start&&t<=_mrange.end)) return}
          days++
          if(r.date&&(!last||String(r.date)>String(last))) last=r.date
          net+=Number(r.net||0);tv+=Number(r.tv||0);renew+=Number(r.renew||0)
          chg+=Number(r.wireChange||0);mnp+=Number(r.mnp||0);sec+=Number(r.second||0)
        })}catch{}
      const wire=net+tv+renew, mob=chg+mnp+sec
      return {net,tv,renew,chg,mnp,sec,wire,mob,days,last,total:wire+mob}
    }
    const rows = sites.map(m=>({
      name:m.customer_name||m.site_name||'-', stage:m.stage||'접촉중',
      emp:(String(m.emp_count||'-').length>10?String(m.emp_count).slice(0,10)+'…':(m.emp_count||'-')),
      agency:m.agency_name||'', agencyMgr:m.agency_manager||'', schedule:m.sales_schedule||'',
      industry:m.industry||'', mm:m.registered_by||'', addr:m.address||'', contact:m.contact||'',
      review:m.site_review||'', result:m.result||'',
      all:aggOf(m,false), mon:aggOf(m,true)
    }))
    return { rows, sites }
  }
  const MP = mpBuild()
  const mpStageColor=s=>({'접촉중':'#0891b2','발굴성공':'#7c3aed','영업중':'#d97706','영업종료':'#16a34a','발굴실패':'#dc2626'}[s]||'#64748b')
  const mpSum=(arr,k)=>arr.reduce((a,r)=>({net:a.net+r[k].net,tv:a.tv+r[k].tv,renew:a.renew+r[k].renew,chg:a.chg+r[k].chg,mnp:a.mnp+r[k].mnp,sec:a.sec+r[k].sec,wire:a.wire+r[k].wire,mob:a.mob+r[k].mob,total:a.total+r[k].total}),{net:0,tv:0,renew:0,chg:0,mnp:0,sec:0,wire:0,mob:0,total:0})

  // ── M플러스 종합 현황 (진행단계 퍼널 + 핵심지표) ──
  function mPlusOverviewSlide(sn){
    const {rows,sites}=MP
    const sumA=mpSum(rows,'all'), sumM=mpSum(rows,'mon')
    const stages=['접촉중','발굴성공','영업중','영업종료','발굴실패']
    const stageCount=s=>sites.filter(m=>(m.stage||'접촉중')===s).length
    const totalSites=sites.length
    const funnel=stages.map(st=>{
      const c=stageCount(st), pct=totalSites?Math.round(c/totalSites*100):0
      return `<div style="flex:1;text-align:center">
        <div style="font-size:26px;font-weight:900;color:${mpStageColor(st)}">${c}</div>
        <div style="font-size:11.5px;color:#334155;font-weight:700;margin-bottom:4px">${st}</div>
        <div style="height:7px;background:#eef2f7;border-radius:4px;overflow:hidden"><div style="width:${pct}%;height:100%;background:${mpStageColor(st)}"></div></div>
    ...
    const byStage=stages.map(st=>{
      ...
      const sA=mpSum(rs,'all'), sM=mpSum(rs,'mon')
      return {st, cnt:rs.length, mon:sM.total, all:sA.total, wire:sA.wire, mob:sA.mob}
    })
    return `<div class="slide" id="s${sn}">
  ${rh('🔍 M플러스 종합 현황','산단·협단체 임직원 모바일 벌크 — 진행단계별 파이프라인',sn)}
  ${dbar}
  <div class="body">
    <div style="background:#f8fafc;border:1.5px solid #e2e8f0;border-radius:14px;padding:14px 16px;margin-bottom:12px">
      <div style="font-size:13px;font-weight:800;color:#16213e;margin-bottom:10px">▶ 진행단계 파이프라인 <span style="color:#94a3b8;font-weight:400">(총 ${totalSites}개 현장)</span></div>
      <div style="display:flex;gap:10px">${funnel}</div>
    </div>
    <div style="display:flex;gap:9px;margin-bottom:13px">
      <div style="flex:1;background:#eef2ff;border:1.5px solid #c7d2fe;border-radius:12px;padding:12px;text-align:center"><div style="font-size:26px;font-weight:900;color:#4338ca">${sumM.total}</div><div style="font-size:11px;color:#64748b">당월 수주 회선</div></div>
      <div style="flex:1;background:#f5f3ff;border:1.5px solid #ddd6fe;border-radius:12px;padding:12px;text-align:center"><div style="font-size:26px;font-weight:900;color:#7c3aed">${sumA.total}</div><div style="font-size:11px;color:#64748b">누적 수주 회선</div></div>
      <div style="flex:1;background:#eff6ff;border:1.5px solid #bfdbfe;border-radius:12px;padding:12px;text-align:center"><div style="font-size:26px;font-weight:900;color:#2563eb">${sumA.wire}</div><div style="font-size:11px;color:#64748b">누적 유선</div></div>
      <div style="flex:1;background:#faf5ff;border:1.5px solid #e9d5ff;border-radius:12px;padding:12px;text-align:center"><div style="font-size:26px;font-weight:900;color:#9333ea">${sumA.mob}</div><div style="font-size:11px;color:#64748b">누적 무선</div></div>
      <div style="flex:1;background:#f0fdf4;border:1.5px solid #bbf7d0;border-radius:12px;padding:12px;text-align:center"><div style="font-size:26px;font-weight:900;color:#16a34a">${rows.filter(r=>r.all.total>0).length}</div><div style="font-size:11px;color:#64748b">실적 발생 현장</div></div>
    </div>
    <div style="font-size:13px;font-weight:800;color:#16213e;margin-bottom:6px">단계별 실적 요약</div>
    <table>
      <thead><tr>
        <th style="text-align:left;padding-left:12px;width:130px">진행단계</th><th style="width:90px">현장 수</th>
        <th style="background:#4338ca">당월 회선</th><th style="background:#7c3aed">누적 회선</th>
        <th style="background:#2563eb">누적 유선</th><th style="background:#9333ea">누적 무선</th>
        <th style="text-align:left">비고</th>
      </tr></thead>
      <tbody>
        ${byStage.map((b,i)=>`<tr style="background:${i%2?'#f8fafc':'#fff'}">
          <td style="text-align:left;padding-left:12px"><span style="font-size:11.5px;padding:3px 10px;border-radius:9px;background:${mpStageColor(b.st)}22;color:${mpStageColor(b.st)};font-weight:800">${b.st}</span></td>
          <td style="font-weight:800">${b.cnt}</td>
          <td style="font-weight:800;color:#4338ca">${b.mon||'-'}</td>
          <td style="font-weight:900;color:#7c3aed">${b.all||'-'}</td>
          <td>${b.wire||'-'}</td><td>${b.mob||'-'}</td>
          <td style="text-align:left;font-size:11px;color:#64748b">${b.st==='영업중'?`▶ 집중 관리 대상 (${10}P~)`:b.st==='영업종료'?`▶ 성과 확정 (${10+MP_PAGES.run}P~)`:b.st==='접촉중'||b.st==='발굴성공'?`▶ 파이프라인 (${10+MP_PAGES.run+MP_PAGES.done}P~)`:'-'}</td>
        </tr>`).join('')}
        <tr style="background:#16213e;color:#fff;font-weight:900">
          <td style="color:#fff;text-align:left;padding-left:12px">전 체</td><td style="color:#fff">${totalSites}</td>
          <td style="background:#312e81;color:#fff">${sumM.total||'-'}</td><td style="background:#5b21b6;color:#fff">${sumA.total||'-'}</td>
          <td style="color:#fff">${sumA.wire||'-'}</td><td style="color:#fff">${sumA.mob||'-'}</td><td style="color:#fff"></td>
        </tr>
      </tbody>
    </table>
    <div style="margin-top:10px;font-size:11px;color:#94a3b8">※ 각 현장의 일별 실적(daily_records) 집계 · 당월 = ${period.replace(' (월마감)','')} · 유선=인터넷+TV+약정갱신, 무선=기변+MNP+2nd</div>
  </div>
  ${ftr(sn)}
</div>`
  }
  ...
  // ── M플러스 단계별 상세 (구간별 페이지) ──
  // ── M플러스 단계별 상세 (압축 1행 + 넘치면 자동 페이지 분할) ──
  // 반환: 슬라이드 HTML 배열 (페이지 수만큼)
  const MP_ROWS_PER_PAGE = 15   // 압축 1행 기준 한 페이지 수용량
  function mPlusStageSlides(startNo, stageList, title, subtitle, accent){
    const rows = MP.rows.filter(r=>stageList.includes(r.stage))
      .sort((a,b)=>b.all.total-a.all.total)
    const sumA=mpSum(rows,'all'), sumM=mpSum(rows,'mon')
    const pages = Math.max(1, Math.ceil(rows.length / MP_ROWS_PER_PAGE))
    const out=[]
    for(let p=0;p<pages;p++){
      const sn = startNo + p
      const chunk = rows.slice(p*MP_ROWS_PER_PAGE, (p+1)*MP_ROWS_PER_PAGE)
      // 당월/누적을 한 셀에 병기 (당월 위 · 누적 아래) → 행 높이 절반
      const cell=(mv,av,color)=>`<td style="padding:3px 2px"><div style="font-size:9.5px;color:#a5b4fc;line-height:1.15">${mv||'·'}</div><div style="font-size:12px;font-weight:800;color:${av?color:'#cbd5e1'};line-height:1.2">${av||'-'}</div></td>`
      out.push(`<div class="slide" id="s${sn}">
  ${rh(title + (pages>1?` (${p+1}/${pages})`:''), subtitle, sn)}
  ${dbar}
  <div class="body">
    ${p===0?`<div style="display:flex;gap:8px;margin-bottom:9px">
      <div style="flex:1;background:${accent.bg};border:1.5px solid ${accent.br};border-radius:11px;padding:9px;text-align:center"><div style="font-size:21px;font-weight:900;color:${accent.c}">${rows.length}</div><div style="font-size:10.5px;color:#64748b">해당 현장 수</div></div>
      <div style="flex:1;background:#eef2ff;border:1.5px solid #c7d2fe;border-radius:11px;padding:9px;text-align:center"><div style="font-size:21px;font-weight:900;color:#4338ca">${sumM.total}</div><div style="font-size:10.5px;color:#64748b">당월 회선</div></div>
      <div style="flex:1;background:#f5f3ff;border:1.5px solid #ddd6fe;border-radius:11px;padding:9px;text-align:center"><div style="font-size:21px;font-weight:900;color:#7c3aed">${sumA.total}</div><div style="font-size:10.5px;color:#64748b">누적 회선</div></div>
      <div style="flex:1;background:#eff6ff;border:1.5px solid #bfdbfe;border-radius:11px;padding:9px;text-align:center"><div style="font-size:21px;font-weight:900;color:#2563eb">${sumA.wire}</div><div style="font-size:10.5px;color:#64748b">누적 유선</div></div>
      <div style="flex:1;background:#faf5ff;border:1.5px solid #e9d5ff;border-radius:11px;padding:9px;text-align:center"><div style="font-size:21px;font-weight:900;color:#9333ea">${sumA.mob}</div><div style="font-size:10.5px;color:#64748b">누적 무선</div></div>
    </div>`:''}
    <div style="font-size:10.5px;color:#94a3b8;margin-bottom:4px">각 칸 <span style="color:#a5b4fc;font-weight:700">위=당월</span> · <span style="color:#334155;font-weight:800">아래=누적</span></div>
    <table style="font-size:11px">
      <thead><tr>
        <th style="text-align:left;padding-left:8px;width:150px">영업구간(현장)</th>
        <th style="width:52px">단계</th><th style="width:56px">임직원</th>
        <th style="background:#1d4ed8;width:52px">인터넷</th><th style="background:#0891b2;width:46px">TV</th><th style="background:#0d9488;width:46px">약정</th>
        <th style="background:#7c3aed;width:46px">기변</th><th style="background:#9333ea;width:46px">MNP</th><th style="background:#c026d3;width:44px">2nd</th>
        <th style="background:#15803d;width:52px">합계</th><th style="text-align:left;width:118px">연계 대리점 / 일정</th>
      </tr></thead>
      <tbody>
        ${chunk.length===0?`<tr><td colspan="11" style="color:#94a3b8;padding:26px">해당 구간의 현장이 없습니다</td></tr>`:chunk.map((r,i)=>`
          <tr style="background:${i%2?'#f8fafc':'#fff'}">
            <td style="text-align:left;padding:4px 8px;max-width:150px">
              <div style="font-weight:700;font-size:11.5px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${r.name}</div>
              <div style="font-size:9px;color:#94a3b8;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${[r.industry,r.mm?'담당 '+r.mm:''].filter(Boolean).join(' · ')||'&nbsp;'}</div>
            </td>
            <td><span style="font-size:9.5px;padding:2px 6px;border-radius:8px;background:${mpStageColor(r.stage)}22;color:${mpStageColor(r.stage)};font-weight:700;white-space:nowrap">${r.stage}</span></td>
            <td style="font-size:9.5px;max-width:56px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${r.emp}</td>
            ${cell(r.mon.net,r.all.net,'#1d4ed8')}${cell(r.mon.tv,r.all.tv,'#0891b2')}${cell(r.mon.renew,r.all.renew,'#0d9488')}
            ${cell(r.mon.chg,r.all.chg,'#7c3aed')}${cell(r.mon.mnp,r.all.mnp,'#9333ea')}${cell(r.mon.sec,r.all.sec,'#c026d3')}
            <td style="background:#f0fdf4;padding:3px 2px"><div style="font-size:9.5px;color:#86efac;line-height:1.15">${r.mon.total||'·'}</div><div style="font-size:13px;font-weight:900;color:${r.all.total?'#15803d':'#cbd5e1'};line-height:1.2">${r.all.total||'-'}</div></td>
            <td style="text-align:left;font-size:9.5px;color:#7c3aed;line-height:1.35;padding:3px 6px;max-width:118px">
              <div style="overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${r.agency||'<span style="color:#cbd5e1">미지정</span>'}${r.agencyMgr?` <span style="color:#94a3b8">${r.agencyMgr}</span>`:''}</div>
              ${r.schedule?`<div style="font-size:9px;color:#d97706;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">📅 ${r.schedule}</div>`:''}
            </td>
          </tr>`).join('')}
        ${p===pages-1&&rows.length>0?`<tr style="background:#16213e;color:#fff;font-weight:900">
          <td colspan="3" style="color:#fff;text-align:left;padding-left:8px">구간 합계 (${rows.length}개 현장)</td>
          <td style="color:#60a5fa"><div style="font-size:9.5px;color:#a5b4fc">${sumM.net||'·'}</div>${sumA.net||'-'}</td>
          <td style="color:#fff"><div style="font-size:9.5px;color:#a5b4fc">${sumM.tv||'·'}</div>${sumA.tv||'-'}</td>
          <td style="color:#fff"><div style="font-size:9.5px;color:#a5b4fc">${sumM.renew||'·'}</div>${sumA.renew||'-'}</td>
          <td style="color:#fff"><div style="font-size:9.5px;color:#a5b4fc">${sumM.chg||'·'}</div>${sumA.chg||'-'}</td>
          <td style="color:#fff"><div style="font-size:9.5px;color:#a5b4fc">${sumM.mnp||'·'}</div>${sumA.mnp||'-'}</td>
          <td style="color:#fff"><div style="font-size:9.5px;color:#a5b4fc">${sumM.sec||'·'}</div>${sumA.sec||'-'}</td>
          <td style="background:#166534;color:#fff"><div style="font-size:9.5px;color:#86efac">${sumM.total||'·'}</div>${sumA.total||'-'}</td>
  ...
  function psAgencySlide(sn){
    const D = allLeadsForPS||{leads:[],directLeads:[],users:[]}
    const myUN2 = (D.myUsers||[]).map(u=>u.username)
    const myNM2 = (D.myUsers||[]).map(u=>u.name)
    function _ms2(dt){if(!dt)return -1;try{const t=new Date(dt).getTime();return isNaN(t)?-1:t}catch{return -1}}
    const mr = psMonthRange
    const inMonth = l => { if(!mr) return true; const t=_ms2(l.activity_at||l.activation_date||l.created_at); return t>=mr.start&&t<=mr.end }
    // 한 건의 won_products → 5분류 상품별 회선수
    function prodOf(l){
      const pc={}
      try{const wp=JSON.parse(l.won_products||'null')
        if(wp&&!Array.isArray(wp)){Object.entries(wp).forEach(([p,v])=>{pc[p]=(pc[p]||0)+Number(v||0)})}
        else if(Array.isArray(wp)){wp.forEach(p=>{pc[p]=(pc[p]||0)+1})}
      }catch{}
      return prodGroup(pc)  // {inet,tv,tvbiz,mob,etc,total}
    }
    // M플러스 현장 1곳의 daily_records → 상품 집계 (월간이면 당월 레코드만)
    function mPlusProdOf(m, onlyMonth){
      let net=0,tv=0,renew=0,mob=0
      try{const recs=JSON.parse(m.daily_records||'[]')
        recs.forEach(r=>{
          if(onlyMonth && mr){ const t=_ms2(r.date); if(!(t>=mr.start&&t<=mr.end)) return }
          net+=Number(r.net||0); tv+=Number(r.tv||0); renew+=Number(r.renew||0)
          mob+=Number(r.wireChange||0)+Number(r.mnp||0)+Number(r.second||0)
        })
      }catch{}
      return {inet:net, tv:tv, tvbiz:0, mob:mob, etc:renew, total:net+tv+mob+renew}
    }
    // 성공+대리점 건 수집 → 유형별 상품 집계 (배분 + 직접발굴 + M플러스 현장)
    const collect = (onlyMonth) => {
      const arr=[]
      ;(D.leads||[]).filter(l=>l.activity_status==='성공'&&myNM2.includes(l.assigned_to)&&l.agency_name&&(!onlyMonth||inMonth(l))).forEach(l=>arr.push({type:l.agency_type||'미분류',name:l.agency_name,pg:prodOf(l)}))
      ;(D.directLeads||[]).filter(l=>l.activity_status==='성공'&&myUN2.includes(l.mm_username)&&l.agency_name&&(!onlyMonth||inMonth(l))).forEach(l=>arr.push({type:l.agency_type||'미분류',name:l.agency_name,pg:prodOf(l)}))
      // M플러스 현장: 연계 대리점이 있는 현장의 실적
      ;(mDiscoveryList||[]).filter(m=>m.agency_name).forEach(m=>{
        const pg=mPlusProdOf(m, onlyMonth)
        if(pg.total>0) arr.push({type:m.agency_type||'미분류', name:m.agency_name, pg})
      })
      return arr
    }
    const aggregate = (arr) => {
      const byType={}
      arr.forEach(s=>{
        if(!byType[s.type])byType[s.type]={inet:0,tv:0,tvbiz:0,mob:0,etc:0,total:0,count:0,agencies:{}}
        const b=byType[s.type]
        b.inet+=s.pg.inet; b.tv+=s.pg.tv; b.tvbiz+=s.pg.tvbiz; b.mob+=s.pg.mob; b.etc+=s.pg.etc; b.total+=s.pg.total; b.count++
        b.agencies[s.name]=(b.agencies[s.name]||0)+1
      })
      const tot={inet:0,tv:0,tvbiz:0,mob:0,etc:0,total:0,count:arr.length}
      Object.values(byType).forEach(b=>{tot.inet+=b.inet;tot.tv+=b.tv;tot.tvbiz+=b.tvbiz;tot.mob+=b.mob;tot.etc+=b.etc;tot.total+=b.total})
      return { byType, tot }
    }
    const monthData = aggregate(collect(true))
    const cumData = aggregate(collect(false))
    const typeColor=['#2563eb','#16a34a','#d97706','#7c3aed','#0891b2','#dc2626','#475569']
    const allTypes=[...new Set((agenciesList||[]).map(a=>a.type))]
    // 상품별 표 렌더
    const renderTable = (data, label, accent) => {
      const types=Object.keys(data.byType).sort((a,b)=>data.byType[b].total-data.byType[a].total)
      const dispTypes=[...new Set([...types,...allTypes])]
      return `
      <div style="font-size:13px;font-weight:800;color:${accent};margin-bottom:6px">${label} <span style="font-size:11px;color:#94a3b8;font-weight:400">— 성공 ${data.tot.count}건 · 총 ${data.tot.total}회선</span></div>
      <table>
        <thead><tr><th style="text-align:left;padding-left:10px">대리점 유형</th><th style="background:#1d4ed8">인터넷</th><th style="background:#d97706">TV-M</th><th style="background:#ea580c">TV-B</th><th style="background:#7c3aed">모바일</th><th style="background:#64748b">기타</th><th style="background:#15803d">합계</th></tr></thead>
        <tbody>
          ${dispTypes.length===0?`<tr><td colspan="7" style="color:#94a3b8;padding:14px">실적 없음</td></tr>`:dispTypes.map((t,i)=>{
            const d=data.byType[t]||{inet:0,tv:0,tvbiz:0,mob:0,etc:0,total:0}
            return `<tr style="background:${i%2?'#f8fafc':'#fff'}">
              <td style="text-align:left;font-weight:700;padding-left:10px"><span style="display:inline-block;width:8px;height:8px;border-radius:50%;background:${typeColor[i%7]};margin-right:6px"></span>${t}</td>
              <td class="${d.inet?'bc':''}">${d.inet||'-'}</td><td>${d.tv||'-'}</td><td>${d.tvbiz||'-'}</td><td>${d.mob||'-'}</td><td>${d.etc||'-'}</td>
              <td style="background:#f0fdf4;font-weight:900;color:#15803d">${d.total||'-'}</td>
            </tr>`}).join('')}
          <tr style="background:#16213e;color:#fff;font-weight:900">
            <td style="color:#fff;text-align:left;padding-left:10px">전 체</td>
            <td style="color:#60a5fa">${data.tot.inet||'-'}</td><td style="color:#fff">${data.tot.tv||'-'}</td><td style="color:#fff">${data.tot.tvbiz||'-'}</td><td style="color:#fff">${data.tot.mob||'-'}</td><td style="color:#fff">${data.tot.etc||'-'}</td>
            <td style="background:#166534;color:#fff">${data.tot.total||'-'}</td>
          </tr>
        </tbody>
      </table>`
    }
    return `<div class="slide ps-slide" id="s${sn}">
  ${rh('🤝 P&S 대리점 유형별 상품 성과','패스앤슈팅 대리점 상품별 실적 (월간 / 누적)',sn)}
  ${dbar}
  <div class="body">
    <div style="display:flex;gap:10px;margin-bottom:9px">
      <div style="flex:1;background:#eff6ff;border:1.5px solid #bfdbfe;border-radius:12px;padding:8px;text-align:center"><div style="font-size:21px;font-weight:900;color:#2563eb">${monthData.tot.count}<span style="font-size:12px">건</span></div><div style="font-size:10.5px;color:#64748b">월간 P&S 성공</div></div>
      <div style="flex:1;background:#f0fdf4;border:1.5px solid #bbf7d0;border-radius:12px;padding:8px;text-align:center"><div style="font-size:21px;font-weight:900;color:#16a34a">${monthData.tot.total}<span style="font-size:12px">회선</span></div><div style="font-size:10.5px;color:#64748b">월간 수주 회선</div></div>
      <div style="flex:1;background:#faf5ff;border:1.5px solid #e9d5ff;border-radius:12px;padding:8px;text-align:center"><div style="font-size:21px;font-weight:900;color:#7c3aed">${cumData.tot.count}<span style="font-size:12px">건</span></div><div style="font-size:10.5px;color:#64748b">누적 P&S 성공</div></div>
      <div style="flex:1;background:#fff7ed;border:1.5px solid #fed7aa;border-radius:12px;padding:8px;text-align:center"><div style="font-size:21px;font-weight:900;color:#e67e00">${cumData.tot.total}<span style="font-size:12px">회선</span></div><div style="font-size:10.5px;color:#64748b">누적 수주 회선</div></div>
      <div style="flex:1;background:#f1f5f9;border:1.5px solid #cbd5e1;border-radius:12px;padding:8px;text-align:center"><div style="font-size:21px;font-weight:900;color:#475569">${(agenciesList||[]).length}</div><div style="font-size:10.5px;color:#64748b">등록 대리점</div></div>
    </div>
    <div style="margin-bottom:7px">${renderTable(monthData,'▶ 월간 유형별 상품 성과','#15803d')}</div>
    <div>${renderTable(cumData,'▶ 누적 유형별 상품 성과','#7c3aed')}</div>
    <div style="margin-top:7px;font-size:10.5px;color:#94a3b8">※ 월간 = 당월 성공건 · 누적 = 전체 기간 성공건 · 상품별 회선수는 수주 상품 기준 · 모바일은 USIM 제외</div>
  </div>
  ${ftr(sn)}
</div>`
  }
  ...
  // ════ 연간 누계 전용 슬라이드 (reportMode='annual' 일 때만 추가) ════
  // ════════ 연간 누계 전용 슬라이드 (reportMode='annual') ════════
  // 미니 막대 차트 (SVG)
  function anBarChart(data, key, color, w=980, h=150){
    ...
    const bars=data.map((d,i)=>{
      const bh=(d[key]/max)*ch, x=padL+i*bw+bw*0.18, y=padT+ch-bh
      return `<rect x="${x.toFixed(1)}" y="${y.toFixed(1)}" width="${(bw*0.64).toFixed(1)}" height="${Math.max(1,bh).toFixed(1)}" fill="${color}" rx="3"/>
        <text x="${(x+bw*0.32).toFixed(1)}" y="${(y-3).toFixed(1)}" font-size="11" font-weight="800" fill="${color}" text-anchor="middle">${d[key]}</text>
        <text x="${(x+bw*0.32).toFixed(1)}" y="${(h-6).toFixed(1)}" font-size="10.5" fill="#64748b" text-anchor="middle">${AN_MOLABEL(d.month)}</text>`
    }).join('')
    return `<svg viewBox="0 0 ${w} ${h}" style="width:100%;height:auto">
      <line x1="${padL}" y1="${padT+ch}" x2="${w-padR}" y2="${padT+ch}" stroke="#e2e8f0"/>${bars}</svg>`
  }

  // ── A1: 연간 핵심 성과 요약 ──
  function anSummarySlide(sn){
    const p=anProdSum()
    const lines=p.inet+p.tv+p.tvbiz+p.mob+p.etc
    const rate=AN_SUM.act>0?Math.round(AN_SUM.suc/AN_SUM.act*100):0
    const tgt=annualTargetCalc||0
    const achv=tgt>0?Math.round(AN_SUM.suc/tgt*100):0
    const best=AN_MS.length?AN_MS.reduce((a,b)=>b.success>a.success?b:a):null
    const worst=AN_MS.length?AN_MS.reduce((a,b)=>b.success<a.success?b:a):null
    const avg=AN_MS.length?Math.round(AN_SUM.suc/AN_MS.length):0
    // 최근 3개월 추세
    const last3=AN_MS.slice(-3)
    const trend=last3.length>=2 ? (last3[last3.length-1].success - last3[0].success) : 0
    const trendIcon=trend>0?'▲':trend<0?'▼':'—'
    const trendColor=trend>0?'#16a34a':trend<0?'#dc2626':'#64748b'
    return `<div class="slide" id="s${sn}">
  ${rh('연간 핵심 성과 요약', yearStr+' '+startMonth+'월 ~ '+today+' 누계', sn)}
  ${dbar}
  <div class="body">
    <div style="display:flex;gap:10px;margin-bottom:13px">
      <div style="flex:1;background:linear-gradient(135deg,#f5f3ff,#ede9fe);border:1.5px solid #ddd6fe;border-radius:14px;padding:15px;text-align:center">
        <div style="font-size:34px;font-weight:900;color:#7c3aed;line-height:1">${AN_SUM.act}</div><div style="font-size:12px;color:#64748b;margin-top:4px">연간 누적 활동</div></div>
      <div style="flex:1;background:linear-gradient(135deg,#f0fdf4,#dcfce7);border:1.5px solid #bbf7d0;border-radius:14px;padding:15px;text-align:center">
        <div style="font-size:34px;font-weight:900;color:#16a34a;line-height:1">${AN_SUM.suc}</div><div style="font-size:12px;color:#64748b;margin-top:4px">연간 누적 성공</div></div>
      <div style="flex:1;background:linear-gradient(135deg,#eff6ff,#dbeafe);border:1.5px solid #bfdbfe;border-radius:14px;padding:15px;text-align:center">
        <div style="font-size:34px;font-weight:900;color:#2563eb;line-height:1">${rate}%</div><div style="font-size:12px;color:#64748b;margin-top:4px">누적 성공률</div></div>
      <div style="flex:1;background:linear-gradient(135deg,#fff7ed,#ffedd5);border:1.5px solid #fed7aa;border-radius:14px;padding:15px;text-align:center">
        <div style="font-size:34px;font-weight:900;color:#d97706;line-height:1">${lines}</div><div style="font-size:12px;color:#64748b;margin-top:4px">누적 수주 회선</div></div>
      <div style="flex:1;background:linear-gradient(135deg,#eef2ff,#e0e7ff);border:1.5px solid #c7d2fe;border-radius:14px;padding:15px;text-align:center">
        <div style="font-size:34px;font-weight:900;color:#4338ca;line-height:1">${achv}%</div><div style="font-size:12px;color:#64748b;margin-top:4px">목표 달성률<br><span style="font-size:10px">(목표 ${tgt}건)</span></div></div>
    </div>
    <div style="font-size:13px;font-weight:800;color:#16213e;margin-bottom:5px">월별 성공 추이</div>
    <div style="background:#fff;border:1.5px solid #e2e8f0;border-radius:12px;padding:8px 10px 4px;margin-bottom:12px">
      ${anBarChart(AN_MS,'success','#16a34a',980,152)}
    </div>
    <div style="display:flex;gap:10px">
      <div style="flex:1;background:#f8fafc;border:1px solid #e2e8f0;border-radius:11px;padding:11px 14px">
        <div style="font-size:11px;color:#64748b;margin-bottom:3px">최고 실적 월</div>
        <div style="font-size:17px;font-weight:900;color:#16a34a">${best?AN_MOLABEL(best.month)+' · '+best.success+'건':'-'}</div></div>
      <div style="flex:1;background:#f8fafc;border:1px solid #e2e8f0;border-radius:11px;padding:11px 14px">
        <div style="font-size:11px;color:#64748b;margin-bottom:3px">최저 실적 월</div>
        <div style="font-size:17px;font-weight:900;color:#dc2626">${worst?AN_MOLABEL(worst.month)+' · '+worst.success+'건':'-'}</div></div>
      <div style="flex:1;background:#f8fafc;border:1px solid #e2e8f0;border-radius:11px;padding:11px 14px">
        <div style="font-size:11px;color:#64748b;margin-bottom:3px">월평균 성공</div>
        <div style="font-size:17px;font-weight:900;color:#2563eb">${avg}건</div></div>
      <div style="flex:1;background:#f8fafc;border:1px solid #e2e8f0;border-radius:11px;padding:11px 14px">
        <div style="font-size:11px;color:#64748b;margin-bottom:3px">최근 3개월 추세</div>
        <div style="font-size:17px;font-weight:900;color:${trendColor}">${trendIcon} ${Math.abs(trend)}건</div></div>
    </div>
    <div style="margin-top:9px;font-size:11px;color:#94a3b8">※ 연간 목표 = 1인 월목표 ${perMMTarget}건 × 인원 × 경과 개월(${AN_MS.length}개월) 환산</div>
  </div>
  ${ftr(sn)}
</div>`
  }
  ...
  // ── A2: 월별 성과 추이 상세 ──
  function anMonthlySlide(sn){
    const p=anProdSum()
    const totalLines=p.inet+p.tv+p.tvbiz+p.mob+p.etc
    return `<div class="slide" id="s${sn}">
  ${rh('월별 성과 추이', startMonth+'월 ~ '+(AN_MS.length?AN_MS[AN_MS.length-1].month:startMonth)+'월 · 활동 · 성공 · 상품별 회선', sn)}
  ${dbar}
  <div class="body">
    <table style="font-size:12px">
      <thead><tr>
        <th style="text-align:left;padding-left:12px;width:70px">구분</th>
        ${AN_MS.map(m=>`<th style="background:#1e293b">${AN_MOLABEL(m.month)}</th>`).join('')}
        <th style="background:#7c3aed;width:78px">누계</th>
      </tr></thead>
      <tbody>
        <tr>
          <td style="text-align:left;font-weight:800;padding-left:12px;color:#7c3aed">활동</td>
          ${AN_MS.map(m=>`<td style="color:#7c3aed;font-weight:700">${m.act||'-'}</td>`).join('')}
          <td style="background:#f5f3ff;font-weight:900;color:#7c3aed">${AN_SUM.act}</td>
        </tr>
        <tr style="background:#f8fafc">
          <td style="text-align:left;font-weight:800;padding-left:12px;color:#16a34a">성공</td>
          ${AN_MS.map(m=>`<td style="color:#16a34a;font-weight:900;font-size:13px">${m.success||'-'}</td>`).join('')}
          <td style="background:#f0fdf4;font-weight:900;color:#16a34a;font-size:14px">${AN_SUM.suc}</td>
        </tr>
        <tr>
          <td style="text-align:left;font-weight:800;padding-left:12px;color:#2563eb">성공률</td>
          ${AN_MS.map(m=>`<td style="color:#2563eb">${m.rate}%</td>`).join('')}
          <td style="background:#eff6ff;font-weight:900;color:#2563eb">${AN_SUM.act>0?Math.round(AN_SUM.suc/AN_SUM.act*100):0}%</td>
        </tr>
        <tr style="background:#fffbeb"><td colspan="${AN_MS.length+2}" style="text-align:left;padding-left:12px;font-size:11px;font-weight:800;color:#92400e">상품별 회선</td></tr>
        ${[['인터넷','inet','#1d4ed8'],['TV-M','tv','#0891b2'],['TV-B','tvbiz','#7c3aed'],['모바일','mob','#9333ea'],['기타','etc','#64748b']].map(([lbl,k,c])=>`
        <tr>
          <td style="text-align:left;padding-left:20px;font-size:11.5px;color:${c};font-weight:700">${lbl}</td>
          ${AN_MS.map(m=>{const g=prodGroup(m.prodCounts||{});return `<td style="font-size:11.5px">${g[k]||'-'}</td>`}).join('')}
          <td style="background:#fff7ed;font-weight:800;color:${c}">${p[k]||'-'}</td>
        </tr>`).join('')}
        <tr style="background:#16213e;color:#fff;font-weight:900">
          <td style="color:#fff;text-align:left;padding-left:12px">회선 계</td>
          ${AN_MS.map(m=>{const g=prodGroup(m.prodCounts||{});return `<td style="color:#fff">${g.inet+g.tv+g.tvbiz+g.mob+g.etc||'-'}</td>`}).join('')}
          <td style="background:#92400e;color:#fff">${totalLines}</td>
        </tr>
      </tbody>
    </table>
    <div style="font-size:10.5px;color:#94a3b8;margin-top:5px">※ '기타' 항목의 상세 내역(하이오더 · 전화 · USIM · 로봇 · CCTV 등)은 <b style="color:#7c3aed">4P 상품별 상세 성과</b> 참조</div>
    <div style="font-size:13px;font-weight:800;color:#16213e;margin:11px 0 4px">월별 활동량 추이</div>
    <div style="background:#fff;border:1.5px solid #e2e8f0;border-radius:12px;padding:8px 10px 4px">
      ${anBarChart(AN_MS,'act','#7c3aed',980,132)}
    </div>
  </div>
  ${ftr(sn)}
</div>`
  ...
  // ── A3: 테마(프로젝트)별 연간 성과 + 월별 히트맵 ──
  function anThemeSlide(sn){
    const PTS=[{id:1,name:'기업체 통신환경 개선공사',icon:'🏭'},{id:2,name:'세움터 신축건물',icon:'🏗️'},
      {id:3,name:'빌딩/상가 공략',icon:'🏢'},{id:4,name:'직접 판매',icon:'🤝'},{id:5,name:'M플러스',icon:'🔍'}]
    const rows=PTS.map(pt=>{
      const monthly=AN_MS.map(m=>m.byProj?.[pt.id]||0)
      const total=monthly.reduce((a,b)=>a+b,0)
      const pj=(projects||[]).find(p=>p.id===pt.id)
      const act=pj?.annual?.done||pj?.annual?.actAll||0
      return {...pt, monthly, total, act}
    })
    const grand=rows.reduce((a,r)=>a+r.total,0)
    const maxCell=Math.max(1,...rows.flatMap(r=>r.monthly))
    const heat=v=>{ if(!v) return '#f8fafc'
      const i=v/maxCell
      return i>0.75?'#15803d':i>0.5?'#22c55e':i>0.25?'#86efac':'#dcfce7' }
    const heatTx=v=>{ if(!v) return '#cbd5e1'
      return (v/maxCell)>0.5?'#fff':'#166534' }
    return `<div class="slide" id="s${sn}">
  ${rh('테마별 연간 성과', '프로젝트 유형별 누적 실적 및 월별 기여도', sn)}
  ${dbar}
  <div class="body">
    <table style="font-size:12px">
      <thead><tr>
        <th style="text-align:left;padding-left:12px;width:190px">테마(프로젝트)</th>
        ${AN_MS.map(m=>`<th style="background:#15803d;width:52px">${AN_MOLABEL(m.month)}</th>`).join('')}
        <th style="background:#7c3aed;width:70px">누계</th><th style="background:#b45309;width:66px">기여도</th>
      </tr></thead>
      <tbody>
        ${rows.map((r,i)=>`<tr style="background:${i%2?'#f8fafc':'#fff'}">
          <td style="text-align:left;font-weight:800;padding-left:12px;color:${PC[r.id]||'#1e293b'}">${r.icon} ${r.name}</td>
          ${r.monthly.map(v=>`<td style="background:${heat(v)};color:${heatTx(v)};font-weight:${v?'800':'400'}">${v||'-'}</td>`).join('')}
          <td style="background:#f5f3ff;font-weight:900;color:#7c3aed;font-size:13px">${r.total||'-'}</td>
          <td style="background:#fff7ed;font-weight:800;color:#b45309">${grand>0?Math.round(r.total/grand*100):0}%</td>
        </tr>`).join('')}
        <tr style="background:#16213e;color:#fff;font-weight:900">
          <td style="color:#fff;text-align:left;padding-left:12px">전 체</td>
          ${AN_MS.map((m,i)=>`<td style="color:#fff">${rows.reduce((a,r)=>a+r.monthly[i],0)||'-'}</td>`).join('')}
          <td style="background:#5b21b6;color:#fff">${grand}</td><td style="background:#92400e;color:#fff">100%</td>
        </tr>
      </tbody>
    </table>
    <div style="display:flex;gap:9px;margin-top:12px">
      ${rows.sort((a,b)=>b.total-a.total).slice(0,3).map((r,i)=>`
      <div style="flex:1;background:${i===0?'#f0fdf4':'#f8fafc'};border:1.5px solid ${i===0?'#bbf7d0':'#e2e8f0'};border-radius:12px;padding:12px 14px">
        <div style="font-size:11px;color:#64748b">${i===0?'🥇 최대 기여':i===1?'🥈 2위':'🥉 3위'}</div>
        <div style="font-size:14px;font-weight:900;color:${PC[r.id]||'#1e293b'};margin-top:2px">${r.icon} ${r.name}</div>
        <div style="font-size:12px;color:#64748b;margin-top:2px">${r.total}건 · 전체의 ${grand>0?Math.round(r.total/grand*100):0}%</div>
      </div>`).join('')}
    </div>
    <div style="margin-top:9px;font-size:11px;color:#94a3b8">※ 색이 진할수록 해당 월 실적이 높음 · 성공 건수 기준 · M플러스는 영업기회 등록 기준</div>
  </div>
  ${ftr(sn)}
</div>`
  }
  ...
  // ── A4: 팀별 연간 성과 ──
  function annualTeamSlide(sn){
    const _src = (annualTeams && annualTeams.length) ? annualTeams : (teams||[])
    const rows = _src.map(t=>({
      name: t.label||t.name||'팀', mm: (t.mm||0),
      act: t.annual?.actAll ?? t.annual?.done ?? 0, suc: t.annual?.success||0,
      pg: prodGroup(t.annual?.prodCounts||{})
    }))
    const sum = rows.reduce((a,r)=>({act:a.act+r.act,suc:a.suc+r.suc,mm:a.mm+r.mm,
      inet:a.inet+r.pg.inet,tv:a.tv+r.pg.tv,tvbiz:a.tvbiz+r.pg.tvbiz,mob:a.mob+r.pg.mob,etc:a.etc+r.pg.etc}),
      {act:0,suc:0,mm:0,inet:0,tv:0,tvbiz:0,mob:0,etc:0})
    const sumLines = sum.inet+sum.tv+sum.tvbiz+sum.mob+sum.etc
    return `<div class="slide" id="s${sn}">
  ${rh('팀별 연간 성과', yearStr+' '+startMonth+'월 ~ '+today+' 누계', sn)}
  ${dbar}
  <div class="body">
    <table>
      <thead><tr>
        <th style="text-align:left;padding-left:12px;width:120px">팀</th><th style="width:56px">MM</th>
        <th style="background:#7c3aed">누적 활동</th><th style="background:#16a34a">누적 성공</th><th style="background:#2563eb">성공률</th><th style="background:#0891b2">1인당 성공</th>
        <th style="background:#d97706">인터넷</th><th style="background:#d97706">TV-M</th><th style="background:#d97706">TV-B</th><th style="background:#d97706">모바일</th><th style="background:#d97706">기타</th>
        <th style="background:#92400e">회선 계</th>
      </tr></thead>
      <tbody>
        ${rows.map((r,i)=>`<tr style="background:${i%2?'#f8fafc':'#fff'}">
          <td style="text-align:left;font-weight:800;padding-left:12px">${r.name}</td><td>${r.mm}명</td>
          <td style="font-weight:700;color:#7c3aed">${r.act}</td><td style="font-weight:900;color:#16a34a;font-size:14px">${r.suc}</td>
          <td>${pct(r.suc,r.act)}%</td><td style="color:#0891b2;font-weight:800">${r.mm>0?Math.round(r.suc/r.mm):0}</td>
          <td>${r.pg.inet||'-'}</td><td>${r.pg.tv||'-'}</td><td>${r.pg.tvbiz||'-'}</td><td>${r.pg.mob||'-'}</td><td>${r.pg.etc||'-'}</td>
          <td style="font-weight:900;color:#92400e">${r.pg.inet+r.pg.tv+r.pg.tvbiz+r.pg.mob+r.pg.etc||'-'}</td>
        </tr>`).join('')}
        <tr style="background:#16213e;color:#fff;font-weight:900">
          <td style="color:#fff;text-align:left;padding-left:12px">전 체</td><td style="color:#fff">${sum.mm}명</td>
          <td style="background:#5b21b6;color:#fff">${sum.act}</td><td style="background:#166534;color:#fff">${sum.suc}</td>
          <td style="color:#fff">${pct(sum.suc,sum.act)}%</td><td style="color:#fff">${sum.mm>0?Math.round(sum.suc/sum.mm):0}</td>
          <td style="color:#fff">${sum.inet||'-'}</td><td style="color:#fff">${sum.tv||'-'}</td><td style="color:#fff">${sum.tvbiz||'-'}</td><td style="color:#fff">${sum.mob||'-'}</td><td style="color:#fff">${sum.etc||'-'}</td>
          <td style="background:#92400e;color:#fff">${sumLines}</td>
        </tr>
      </tbody>
    </table>
    <div style="margin-top:9px;font-size:11px;color:#94a3b8">※ 팀 실적에는 <b style="color:#7c3aed">M플러스 현장 실적 포함</b> (MM 개인 귀속 불가 → 다음 장 MM별 표와 차이 발생)</div>
  </div>
  ${ftr(sn)}
</div>`
  }
  ...
  // ── A5: MM별 연간 성과 ──
  function annualMMSlide(sn){
    const nMon=Math.max(1,AN_MS.length)
    const _srcM = (annualMMs && annualMMs.length) ? annualMMs : (allMM||[])
    const rows=_srcM.map(m=>({
      name:m.name, team:m.teamLabel||m.team||'-',
      act:m.annual?.actAll ?? m.annual?.mitDone ?? m.annual?.done ?? 0, suc:m.annual?.success||0,
      pg:prodGroup(m.annual?.prodCounts||{})
    })).sort((a,b)=>b.suc-a.suc)
    const sum=rows.reduce((a,r)=>({act:a.act+r.act,suc:a.suc+r.suc,
      inet:a.inet+r.pg.inet,tv:a.tv+r.pg.tv,tvbiz:a.tvbiz+r.pg.tvbiz,mob:a.mob+r.pg.mob,etc:a.etc+r.pg.etc}),
      {act:0,suc:0,inet:0,tv:0,tvbiz:0,mob:0,etc:0})
    return `<div class="slide" id="s${sn}">
  ${rh('MM별 연간 성과', yearStr+' '+startMonth+'월 ~ '+today+' 누계 · 성공 순', sn)}
  ${dbar}
  <div class="body">
    <table style="font-size:11.5px">
      <thead><tr>
        <th style="width:36px">순위</th><th style="text-align:left;padding-left:10px;width:92px">MM</th><th style="width:78px">팀</th>
        <th style="background:#7c3aed">누적 활동</th><th style="background:#16a34a">누적 성공</th><th style="background:#2563eb">성공률</th><th style="background:#0891b2">월평균</th>
        <th style="background:#d97706">인터넷</th><th style="background:#d97706">TV-M</th><th style="background:#d97706">TV-B</th><th style="background:#d97706">모바일</th><th style="background:#d97706">기타</th>
        <th style="background:#92400e">회선</th>
      </tr></thead>
      <tbody>
        ${rows.slice(0,15).map((r,i)=>{
          const line=r.pg.inet+r.pg.tv+r.pg.tvbiz+r.pg.mob+r.pg.etc
          const medal=i===0?'🥇':i===1?'🥈':i===2?'🥉':(i+1)
          return `<tr style="background:${i%2?'#f8fafc':'#fff'}">
          <td style="font-weight:800">${medal}</td>
          <td style="text-align:left;font-weight:800;padding-left:10px">${r.name}</td>
          <td style="font-size:10.5px;color:#64748b">${r.team}</td>
          <td style="color:#7c3aed;font-weight:700">${r.act}</td><td style="color:#16a34a;font-weight:900;font-size:13px">${r.suc}</td>
          <td>${pct(r.suc,r.act)}%</td><td style="color:#0891b2;font-weight:700">${Math.round(r.suc/nMon*10)/10}</td>
          <td>${r.pg.inet||'-'}</td><td>${r.pg.tv||'-'}</td><td>${r.pg.tvbiz||'-'}</td><td>${r.pg.mob||'-'}</td><td>${r.pg.etc||'-'}</td>
          <td style="font-weight:800;color:#92400e">${line||'-'}</td></tr>`}).join('')}
        <tr style="background:#16213e;color:#fff;font-weight:900">
          <td colspan="3" style="color:#fff;text-align:left;padding-left:10px">전 체 (${rows.length}명)</td>
          <td style="background:#5b21b6;color:#fff">${sum.act}</td><td style="background:#166534;color:#fff">${sum.suc}</td>
          <td style="color:#fff">${pct(sum.suc,sum.act)}%</td><td style="color:#fff">${Math.round(sum.suc/nMon*10)/10}</td>
          <td style="color:#fff">${sum.inet||'-'}</td><td style="color:#fff">${sum.tv||'-'}</td><td style="color:#fff">${sum.tvbiz||'-'}</td><td style="color:#fff">${sum.mob||'-'}</td><td style="color:#fff">${sum.etc||'-'}</td>
          <td style="background:#92400e;color:#fff">${sum.inet+sum.tv+sum.tvbiz+sum.mob+sum.etc}</td>
        </tr>
      </tbody>
    </table>
    ${rows.length>15?`<div style="font-size:11px;color:#94a3b8;text-align:center;margin-top:4px">외 ${rows.length-15}명</div>`:''}
    <div style="margin-top:9px;font-size:11px;color:#94a3b8">※ MM 개인 귀속 실적만 집계 · <b style="color:#7c3aed">M플러스 현장 실적 제외</b>(팀 단위 귀속) · 월평균 = 누적 성공 ÷ ${nMon}개월</div>
  </div>
  ${ftr(sn)}
</div>`
  }
  ...
  // ── A6: M플러스 연간 성과 ──
  // ── A: 상품별 상세 성과 (11개 분류, 기타 세분화) ──
  function anProductDetailSlide(sn){
    // 월별 × 상품 상세
    const monthCols = AN_MS.map(m=>({ month:m.month, pc:m.prodCounts||{} }))
    const totalPc = {}
    monthCols.forEach(c=>{ Object.entries(c.pc).forEach(([k,v])=>{ totalPc[k]=(totalPc[k]||0)+Number(v||0) }) })
    const detailRows = DETAIL_PRODS.map(p=>({
      name:p,
      months: monthCols.map(c=>prodVal(c.pc,p)),
      total: prodVal(totalPc,p)
    }))   // 실적 0인 상품도 표시 (미판매 품목 파악)
    const grand = detailRows.reduce((a,r)=>a+r.total,0)
    // 팀별 상품 상세
    const teamSrc = (annualTeams && annualTeams.length) ? annualTeams : (teams||[])
    const teamRows = teamSrc.map(t=>({ name:t.label||t.name||'팀', pc:t.annual?.prodCounts||{} }))
    const PCOLOR={'인터넷':'#1d4ed8','TV-M':'#0891b2','TV-B':'#7c3aed','모바일':'#9333ea','USIM':'#c026d3',
      '전화':'#0d9488','약정갱신':'#16a34a','기타':'#64748b','하이오더':'#d97706','로봇':'#dc2626','CCTV':'#4338ca'}
    return `<div class="slide" id="s${sn}">
  ${rh('상품별 상세 성과', '11개 상품 분류 · 월별 추이 및 팀별 구성 (연간 누계)', sn)}
  ${dbar}
  <div class="body">
    <div style="font-size:12.5px;font-weight:800;color:#16213e;margin-bottom:5px">월별 × 상품별 수주 회선</div>
    <table style="font-size:11.5px;margin-bottom:11px">
      <thead><tr>
        <th style="text-align:left;padding-left:12px;width:96px">상품</th>
        ${AN_MS.map(m=>`<th style="background:#1e293b;width:52px">${AN_MOLABEL(m.month)}</th>`).join('')}
        <th style="background:#7c3aed;width:64px">누계</th><th style="background:#b45309;width:58px">비중</th>
      </tr></thead>
      <tbody>
        ${detailRows.map((r,i)=>`<tr style="background:${i%2?'#f8fafc':'#fff'}">
          <td style="text-align:left;padding-left:12px;font-weight:800;color:${PCOLOR[r.name]||'#1e293b'}">${r.name}</td>
          ${r.months.map(v=>`<td style="${v?'':'color:#cbd5e1'}">${v||'-'}</td>`).join('')}
          <td style="background:#f5f3ff;font-weight:900;color:${PCOLOR[r.name]||'#7c3aed'};font-size:12.5px">${r.total}</td>
          <td style="background:#fff7ed;font-weight:700;color:#b45309">${grand>0?Math.round(r.total/grand*100):0}%</td>
        </tr>`).join('')}
        <tr style="background:#16213e;color:#fff;font-weight:900">
          <td style="color:#fff;text-align:left;padding-left:12px">합 계</td>
          ${monthCols.map(c=>{const s=DETAIL_PRODS.reduce((a,p)=>a+prodVal(c.pc,p),0);return `<td style="color:#fff">${s||'-'}</td>`}).join('')}
          <td style="background:#5b21b6;color:#fff">${grand}</td><td style="background:#92400e;color:#fff">100%</td>
        </tr>
      </tbody>
    </table>
    <div style="font-size:12.5px;font-weight:800;color:#16213e;margin-bottom:5px">팀별 × 상품별 수주 회선</div>
    <table style="font-size:11.5px">
      <thead><tr>
        <th style="text-align:left;padding-left:12px;width:96px">팀</th>
        ${detailRows.map(r=>`<th style="background:${PCOLOR[r.name]||'#475569'};width:52px">${r.name}</th>`).join('')}
        <th style="background:#92400e;width:64px">계</th>
      </tr></thead>
      <tbody>
        ${teamRows.map((t,i)=>{
          const vals=detailRows.map(r=>prodVal(t.pc,r.name))
          return `<tr style="background:${i%2?'#f8fafc':'#fff'}">
          <td style="text-align:left;padding-left:12px;font-weight:800">${t.name}</td>
          ${vals.map(v=>`<td style="${v?'font-weight:700':'color:#cbd5e1'}">${v||'-'}</td>`).join('')}
          <td style="background:#fff7ed;font-weight:900;color:#92400e">${vals.reduce((a,b)=>a+b,0)}</td></tr>`
        }).join('')}
        <tr style="background:#16213e;color:#fff;font-weight:900">
          <td style="color:#fff;text-align:left;padding-left:12px">전 체</td>
          ${detailRows.map(r=>`<td style="color:#fff">${r.total}</td>`).join('')}
          <td style="background:#92400e;color:#fff">${grand}</td>
        </tr>
      </tbody>
    </table>
    <div style="display:flex;gap:9px;margin-top:11px">
      ${detailRows.slice().sort((a,b)=>b.total-a.total).slice(0,4).map((r,i)=>`
      <div style="flex:1;background:${i===0?'#f0fdf4':'#f8fafc'};border:1.5px solid ${i===0?'#bbf7d0':'#e2e8f0'};border-radius:11px;padding:10px 13px">
        <div style="font-size:10.5px;color:#64748b">${i===0?'🥇 최다 수주':(i+1)+'위'}</div>
        <div style="font-size:14.5px;font-weight:900;color:${PCOLOR[r.name]||'#1e293b'};margin-top:2px">${r.name}</div>
        <div style="font-size:11.5px;color:#64748b;margin-top:1px">${r.total}회선 · ${grand>0?Math.round(r.total/grand*100):0}%</div>
      </div>`).join('')}
    </div>
    <div style="margin-top:8px;font-size:10.5px;color:#94a3b8">※ 5분류표의 '기타'는 위 표에서 USIM·전화·약정갱신·하이오더·로봇·CCTV 등으로 세분화됨 · 회선수 미입력 건은 상품당 1회선</div>
  </div>
  ${ftr(sn)}
</div>`
  }
  function anMPlusSlide(sn){
    const mpTot=AN_MS.reduce((a,m)=>({net:a.net+(m.mp?.net||0),tv:a.tv+(m.mp?.tv||0),renew:a.renew+(m.mp?.renew||0),mob:a.mob+(m.mp?.mob||0)}),{net:0,tv:0,renew:0,mob:0})
    const mpAll=mpTot.net+mpTot.tv+mpTot.renew+mpTot.mob
    const stages=['접촉중','발굴성공','영업중','영업종료','발굴실패']
    const sites=MP.sites||[]
    const stageCount=s=>sites.filter(m=>(m.stage||'접촉중')===s).length
    const maxM=Math.max(1,...AN_MS.map(m=>m.mp?.total||0))
    return `<div class="slide" id="s${sn}">
  ${rh('🔍 M플러스 연간 성과', '산단·협단체 모바일 벌크 — 연간 누계 및 월별 추이', sn)}
  ${dbar}
  <div class="body">
    <div style="display:flex;gap:9px;margin-bottom:12px">
      <div style="flex:1;background:#f5f3ff;border:1.5px solid #ddd6fe;border-radius:12px;padding:12px;text-align:center"><div style="font-size:26px;font-weight:900;color:#7c3aed">${mpAll}</div><div style="font-size:11px;color:#64748b">연간 총 회선</div></div>
      <div style="flex:1;background:#faf5ff;border:1.5px solid #e9d5ff;border-radius:12px;padding:12px;text-align:center"><div style="font-size:26px;font-weight:900;color:#9333ea">${mpTot.mob}</div><div style="font-size:11px;color:#64748b">무선 (기변·MNP·2nd)</div></div>
      <div style="flex:1;background:#eff6ff;border:1.5px solid #bfdbfe;border-radius:12px;padding:12px;text-align:center"><div style="font-size:26px;font-weight:900;color:#2563eb">${mpTot.net+mpTot.tv+mpTot.renew}</div><div style="font-size:11px;color:#64748b">유선 (인터넷·TV·약정)</div></div>
      <div style="flex:1;background:#f0fdf4;border:1.5px solid #bbf7d0;border-radius:12px;padding:12px;text-align:center"><div style="font-size:26px;font-weight:900;color:#16a34a">${sites.length}</div><div style="font-size:11px;color:#64748b">전체 현장</div></div>
    </div>
    <div style="font-size:13px;font-weight:800;color:#16213e;margin-bottom:5px">월별 수주 회선 추이</div>
    <table style="font-size:12px;margin-bottom:12px">
      <thead><tr>
        <th style="text-align:left;padding-left:12px;width:78px">구분</th>
        ${AN_MS.map(m=>`<th style="background:#7c3aed">${AN_MOLABEL(m.month)}</th>`).join('')}
        <th style="background:#5b21b6;width:70px">누계</th>
      </tr></thead>
      <tbody>
        ${[['인터넷','net','#1d4ed8'],['TV','tv','#0891b2'],['약정갱신','renew','#0d9488'],['무선','mob','#9333ea']].map(([l,k,c])=>`
        <tr><td style="text-align:left;padding-left:12px;color:${c};font-weight:700">${l}</td>
        ${AN_MS.map(m=>`<td>${m.mp?.[k]||'-'}</td>`).join('')}
        <td style="background:#f5f3ff;font-weight:800;color:${c}">${mpTot[k]||'-'}</td></tr>`).join('')}
        <tr style="background:#16213e;color:#fff;font-weight:900">
          <td style="color:#fff;text-align:left;padding-left:12px">계</td>
          ${AN_MS.map(m=>`<td style="color:#fff">${m.mp?.total||'-'}</td>`).join('')}
          <td style="background:#5b21b6;color:#fff">${mpAll}</td>
        </tr>
      </tbody>
    </table>
    <div style="font-size:13px;font-weight:800;color:#16213e;margin-bottom:6px">진행단계 현황</div>
    <div style="display:flex;gap:9px">
      ${stages.map(st=>`<div style="flex:1;background:#f8fafc;border:1.5px solid #e2e8f0;border-radius:11px;padding:11px;text-align:center">
        <div style="font-size:24px;font-weight:900;color:${mpStageColor(st)}">${stageCount(st)}</div>
        <div style="font-size:11px;color:#64748b">${st}</div></div>`).join('')}
    </div>
    <div style="margin-top:9px;font-size:11px;color:#94a3b8">※ 월별 수주 회선은 현장 일별 실적(daily_records) 기준 집계</div>
  </div>
  ${ftr(sn)}
</div>`
  }
  const s8  = mPlusOverviewSlide(9)
  let _pn = 10
  const s8bArr = mPlusStageSlides(_pn, ['영업중'], '🔍 M플러스 · 진행중 구간', '영업중 현장 — 집중 관리·전략 협의 대상', {bg:'#fffbeb',br:'#fde68a',c:'#d97706'})
  _pn += s8bArr.length
  const s8cArr = mPlusStageSlides(_pn, ['영업종료'], '🔍 M플러스 · 완료 구간', '영업종료 현장 — 확정 성과 및 BP 도출', {bg:'#f0fdf4',br:'#bbf7d0',c:'#16a34a'})
  _pn += s8cArr.length
  const s8dArr = mPlusStageSlides(_pn, ['접촉중','발굴성공'], '🔍 M플러스 · 접촉중 구간', '접촉중·발굴성공 현장 — 진입 파이프라인', {bg:'#f0f9ff',br:'#bae6fd',c:'#0891b2'})
  _pn += s8dArr.length
  const s8b = s8bArr.join(''), s8c = s8cArr.join(''), s8d = s8dArr.join('')
  const s9  = psAgencySlide(_pn)
  _pn += 1
  // 연간 누계 모드: 팀별·MM별 연간 세부 슬라이드 추가
  // 연간 모드: 표지 + 연간 전용 7장으로 구성 (월간 페이지 미포함)
  const sAnnual = isAnnual
    ? (anSummarySlide(2) + anMonthlySlide(3) + anProductDetailSlide(4) + anThemeSlide(5) + annualTeamSlide(6) + annualMMSlide(7) + anMPlusSlide(8))
    : ''

  return `<!DOCTYPE html><html lang="ko"><head><meta charset="UTF-8"><title>상권통 보고서 ${today}</title><style>${css}</style></head><body>
<button class="print-btn" onclick="doPrint()">🖨 PDF 저장 / 인쇄</button>
...
${isAnnual ? (s1 + sAnnual) : (s1 + s2 + s3t + s3 + s4 + s5 + s6 + s7 + s8 + s8b + s8c + s8d + s9)}
<script>
function doPrint(){
  try{
    if(!window._pdfTipShown){
      window._pdfTipShown=true;
      alert('PDF 저장 시 색상이 유지되려면 인쇄 창에서 [배경 그래픽] 옵션을 체크하세요.');
    }
    window.print();
  ...
  const downloadActivity = async () => {
    ...
    try {
      ...
      // 사진(photos)은 전체 로드 시 제외했으므로(타임아웃 방지), 다운로드 시점에 별도 조회
      // id 목록으로 photos만 가져와 매핑
      const photoMap = {}
      try {
        const leadIds = filteredLeads.map(l=>l.id).filter(Boolean)
        const directIds = filteredDirect.map(l=>l.id).filter(Boolean)
        // 200건씩 나눠 조회 (URL 길이·타임아웃 방지)
        const chunk = (arr,n)=>{ const o=[]; for(let i=0;i<arr.length;i+=n) o.push(arr.slice(i,i+n)); return o }
        for (const ids of chunk(leadIds,150)) {
          if (!ids.length) continue
          const r = await db.get('connector_leads', 'select=id,photos&id=in.(' + ids.join(',') + ')').catch(()=>[])
          ;(r||[]).forEach(x=>{ if(x.photos) photoMap['L'+x.id]=x.photos })
        }
        for (const ids of chunk(directIds,150)) {
          if (!ids.length) continue
          const r = await db.get('mm_direct_leads', 'select=id,photos&id=in.(' + ids.join(',') + ')').catch(()=>[])
          ;(r||[]).forEach(x=>{ if(x.photos) photoMap['D'+x.id]=x.photos })
        }
      } catch(e) { console.warn('photo fetch fail', e) }

      const pickAddr = (l) => {
        const cands = [l.address, l.col2, l.col3, l.col4].filter(Boolean)
        for (const c of cands) { if (coreAddr(c)) return c }
        return cands[0] || ''
      }
      const allItems = [...filteredLeads.map(l=>({...l,_type:'관리자배분',_customer:l.col1||'',_addr:pickAddr(l),_photos:photoMap['L'+l.id]||l.photos||'[]'})),...filteredDirect.map(l=>({...l,_type:'직접발굴',_customer:l.customer||'',_addr:l.address||'',_photos:photoMap['D'+l.id]||l.photos||'[]'}))]
      const headers = ['번호','구분','고객명','주소','배분MM','활동상태','결과요약','메모','연락처','활동일시','사진파일명']
      const rows = [headers]
      allItems.forEach((l, idx) => {
        let photos = []; try { photos = JSON.parse(l._photos || '[]') } catch {}
        ...
        rows.push([idx+1,l._type,l._customer||'-',l._addr||'-',l.assigned_to||l.mm_username||'-',l.activity_status||'미처리',l.activity_result||'',l.activity_memo||'',l.activity_contact||'',l.activity_at?new Date(l.activity_at).toLocaleString('ko-KR'):'',photoNames.join(' / ')||'없음'])
      })
  ...
  // 관리자: M플러스 현장 진행단계·연계대리점 수정 (MM 미갱신분 보완)
  const saveMEdit = async () => {
    if (!mEdit) return
    try {
      await db.patch('m_discovery', 'id=eq.' + mEdit.id, {
        stage: mEditForm.stage,
        agency_name: mEditForm.agency_name.trim(),
        agency_manager: mEditForm.agency_manager.trim(),
        sales_schedule: mEditForm.sales_schedule.trim()
      })
      t2('수정 완료'); setMEdit(null); load()
    } catch (e) { t2('수정 실패: ' + e.message) }
  }
  ...
  // 유효 팀(팀명이 설정된 팀)에 속한 직원만 포함 — 팀 미배정(이상천 등) 제외
  const _validTeamIds = new Set(adminAccounts.filter(a => a.team_label).map(a => a.team_id))
  const myUsers = isSuper
    ? users.filter(u => _validTeamIds.has(u.team_id))
    : users.filter(u => u.team_id === adminInfo?.teamId)
  ...
  const navTabs = [
    ...
    ...(isSuper ? [{ id: 'uturn', label: 'U-TURN', icon: '⤴' }] : []),
    ...(isSuper ? [{ id: 'aircon', label: '캠페인', icon: '❄️' }] : []),
  ...
  return (
    <div style={{ minHeight: '100vh', background: C.bg, color: C.text, fontFamily: "'Noto Sans KR',sans-serif", maxWidth: '480px', margin: '0 auto', display: 'flex', flexDirection: 'column' }}>
      ...
      {orderTarget && <OrderSendModal target={orderTarget} senderName={adminInfo?.name} senderRole={adminInfo?.role} onClose={() => setOrderTarget(null)} onSent={(name) => { setOrderTarget(null); t2('📢 ' + name + ' MM에게 오더 전송 완료!'); if (showSentOrders) loadSentOrders() }} />}
      {showSentOrders && <SentOrdersModal orders={sentOrders} users={users} onClose={() => setShowSentOrders(false)} onRefresh={loadSentOrders} onDelete={async (o) => { if (!window.confirm('이 오더를 삭제하시겠습니까?')) return; try { await db.del('mm_orders', 'id=eq.' + o.id); setSentOrders(prev => prev.filter(x => x.id !== o.id)); t2('삭제 완료') } catch { t2('삭제 실패') } }} />}
      ...
      {showDupCheck && (() => {
        const _dupTeamIds = new Set(adminAccounts.filter(a => a.team_label).map(a => a.team_id))
        const _dupUsers = isSuper ? users.filter(u => _dupTeamIds.has(u.team_id)) : users.filter(u => u.team_id === adminInfo?.teamId)
        const _dupUN = new Set(_dupUsers.map(u => u.name))
        const _dupUname = new Set(_dupUsers.map(u => u.username))
        const _dupLeads = isSuper ? leads : leads.filter(l => _dupUN.has(l.assigned_to))
        const _dupDirect = isSuper ? directLeads : directLeads.filter(l => _dupUname.has(l.mm_username))
        return <DupCheckModal leads={_dupLeads} directLeads={_dupDirect} users={_dupUsers} onClose={() => setShowDupCheck(false)} onDeleted={() => load()} />
      })()}
      ...
      {/* 데이터 로딩 오버레이 — 로딩 중 옛 데이터/0건으로 오인하는 것 방지 */}
      {loading && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(255,255,255,0.82)', zIndex: 900, display: 'flex', alignItems: 'center', justifyContent: 'center', backdropFilter: 'blur(2px)' }}>
          <div style={{ background: '#fff', borderRadius: '16px', padding: '26px 34px', boxShadow: '0 10px 34px rgba(0,0,0,0.14)', textAlign: 'center', border: '1px solid ' + C.border }}>
            <div style={{ width: '34px', height: '34px', margin: '0 auto 12px', border: '3.5px solid #e2e8f0', borderTopColor: C.acc, borderRadius: '50%', animation: 'ldspin 0.8s linear infinite' }} />
            <div style={{ fontSize: '14.5px', fontWeight: '800', color: C.text }}>최신 데이터를 불러오는 중…</div>
            <div style={{ fontSize: '12px', color: C.sub, marginTop: '3px' }}>잠시만 기다려 주세요</div>
          </div>
      ...
      <div style={{ padding: '10px 16px 8px', borderBottom: 'none' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '10px' }}>
          ...
          <div style={{ display: 'flex', gap: '8px' }}>
            ...
            <button onClick={() => {
              // 로그아웃 시 이전 계정/이전 시점 데이터가 화면에 남지 않도록 초기화
              setScreen('login'); setAdminInfo(null)
              setLeads([]); setDirectLeads([]); setUsers([]); setMDiscovery([])
              setQrPartners([]); setQrStores([]); setQrLeads([]); setAgencies([])
              setAdminAccounts([])
            }} style={{ background: '#f8fafc', border: '1px solid #e2e8f0', color: C.sub, padding: '7px 12px', borderRadius: '8px', fontSize: '12px', cursor: 'pointer', fontFamily: 'inherit' }}>로그아웃</button>
          </div>
        </div>
      ...
      <div style={{ background: '#fff', borderBottom: '1px solid #e2e8f0', display: 'flex', overflowX: 'auto', padding: '0' }}>
        {navTabs.map(t => (
          <button key={t.id} onClick={() => { setTab(t.id); if (t.id === 'uturn') loadUturnScores() }}
      ...
      <div ref={scrollRef} style={{ flex: 1, overflowY: 'auto', padding: '14px' }}>
          ...
          <div>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '10px' }}>
              ...
              <div style={{ display: 'flex', gap: '6px' }}>
                <button onClick={() => setShowDupCheck(true)} style={{ background: '#fff', border: '1.5px solid #fecaca', color: C.red, padding: '7px 12px', borderRadius: '8px', fontSize: '13px', fontWeight: '800', cursor: 'pointer', fontFamily: 'inherit' }}>🔎 중복검사</button>
              ...
              return (
                <>
                 ...
                 {l.project_name && <span style={{ fontSize:'10px', background:'#eff6ff', color:'#2563eb', border:'1px solid #bfdbfe', padding:'1px 6px', borderRadius:'8px' }}>🏷️ {l.project_name}</span>}
                            ...
                            {/* 주소 (이름과 다를 때만) */}
                            {(l.address || l.col2) && (l.address || l.col2) !== l._name && <div style={{ fontSize:'12.5px', color:C.sub, marginBottom:'4px' }}>📍 {l.address || l.col2}</div>}
                            {/* 대리점 */}
                            {l.agency_name && <div style={{ fontSize:'12.5px', color:'#7c3aed', fontWeight:'700', marginBottom:'4px' }}>🤝 {l.agency_type ? l.agency_type + ' · ' : ''}{l.agency_name}</div>}
                            ...
                            {/* 수주 상품 (성공건) */}
                            {l.activity_status === '성공' && <WonBadge wonRaw={l.won_products} />}
                            {/* 개통일 · 회선번호 (성공건) */}
                            {l.activity_status === '성공' && (l.activation_date || l.circuit_number) && (
                              <div style={{ display:'flex', gap:'12px', flexWrap:'wrap', fontSize:'12.5px', fontWeight:'700', color:C.text, marginBottom:'8px', marginTop:'2px' }}>
                                {l.activation_date && <span>📅 개통일: <b style={{ color:C.blue }}>{l.activation_date}</b></span>}
                                {l.circuit_number && <span>🔢 회선번호: <b style={{ color:C.blue }}>{l.circuit_number}</b></span>}
                              </div>
                            )}
        ...
        {tab === 'staff' && (
          <div>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '14px' }}>
              ...
              <div style={{ display: 'flex', gap: '6px' }}>
                <button onClick={() => { setShowSentOrders(true); loadSentOrders() }} style={{ background: C.accBg, border: '1px solid ' + C.accBorder, color: C.acc, padding: '10px 14px', borderRadius: '10px', fontSize: '14px', fontWeight: '800', cursor: 'pointer', fontFamily: 'inherit' }}>📢 보낸 오더</button>
                ...
                <div key={u.id} style={{ background: '#fff', border: '1px solid #e2e8f0', borderRadius: '12px', padding: '14px', marginBottom: '10px', boxShadow: '0 1px 4px rgba(0,0,0,0.04)' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '10px' }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                      ...
                      <button onClick={() => setOrderTarget(u)} style={{ background: C.accBg, border: '1px solid ' + C.accBorder, color: C.acc, padding: '6px 11px', borderRadius: '8px', fontSize: '13px', fontWeight: '700', cursor: 'pointer', fontFamily: 'inherit' }}>📢 오더</button>
        ...
        {tab === 'mdiscovery' && (
          <div>
            {mEdit && (
              <div onClick={()=>setMEdit(null)} style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.55)',zIndex:1200,display:'flex',alignItems:'flex-end',justifyContent:'center'}}>
                <div onClick={e=>e.stopPropagation()} style={{background:'#fff',borderRadius:'20px 20px 0 0',padding:'24px 20px 32px',width:'100%',maxWidth:'460px',maxHeight:'90vh',overflowY:'auto'}}>
                  <div style={{fontSize:'17px',fontWeight:'900',color:C.text,marginBottom:'3px'}}>✏️ M플러스 현장 수정</div>
                  <div style={{fontSize:'13px',color:C.sub,marginBottom:'16px'}}>{mEdit.customer_name||mEdit.site_name}</div>

                  <label style={{display:'block',fontSize:'13px',fontWeight:'700',color:C.text,marginBottom:'7px'}}>진행 단계</label>
                  <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:'6px',marginBottom:'16px'}}>
                    {['접촉중','발굴성공','영업중','영업종료','발굴실패'].map(s=>{
                      const sc={'접촉중':['#e0f2fe','#0369a1'],'발굴성공':['#ede9fe','#6d28d9'],'영업중':['#fef3c7','#92400e'],'영업종료':['#dcfce7','#15803d'],'발굴실패':['#fee2e2','#b91c1c']}[s]
                      const on=mEditForm.stage===s
                      return <button key={s} onClick={()=>setMEditForm(p=>({...p,stage:s}))} style={{padding:'11px 6px',background:on?sc[1]:sc[0],border:'1.5px solid '+(on?sc[1]:'transparent'),color:on?'#fff':sc[1],borderRadius:'10px',fontSize:'12.5px',fontWeight:'800',cursor:'pointer',fontFamily:'inherit'}}>{s}</button>
                    })}
                  </div>

                  <label style={{display:'block',fontSize:'13px',fontWeight:'700',color:C.text,marginBottom:'6px'}}>연계 대리점</label>
                  <input value={mEditForm.agency_name} onChange={e=>setMEditForm(p=>({...p,agency_name:e.target.value}))} placeholder="예: 연우컴퍼니" style={{width:'100%',padding:'12px 14px',border:'1.5px solid '+C.border,borderRadius:'10px',fontSize:'15px',fontFamily:'inherit',outline:'none',boxSizing:'border-box',marginBottom:'12px'}}/>

                  <label style={{display:'block',fontSize:'13px',fontWeight:'700',color:C.text,marginBottom:'6px'}}>대리점 담당자</label>
                  <input value={mEditForm.agency_manager} onChange={e=>setMEditForm(p=>({...p,agency_manager:e.target.value}))} placeholder="예: 이팀장" style={{width:'100%',padding:'12px 14px',border:'1.5px solid '+C.border,borderRadius:'10px',fontSize:'15px',fontFamily:'inherit',outline:'none',boxSizing:'border-box',marginBottom:'12px'}}/>

                  <label style={{display:'block',fontSize:'13px',fontWeight:'700',color:C.text,marginBottom:'6px'}}>영업 일정</label>
                  <input value={mEditForm.sales_schedule} onChange={e=>setMEditForm(p=>({...p,sales_schedule:e.target.value}))} placeholder="예: 8/18~8/22 또는 조율중" style={{width:'100%',padding:'12px 14px',border:'1.5px solid '+C.border,borderRadius:'10px',fontSize:'15px',fontFamily:'inherit',outline:'none',boxSizing:'border-box',marginBottom:'20px'}}/>

                  <div style={{display:'flex',gap:'10px'}}>
                    <button onClick={()=>setMEdit(null)} style={{flex:1,padding:'14px',background:'#f1f5f9',border:'none',color:C.sub,borderRadius:'11px',fontSize:'15px',fontWeight:'800',cursor:'pointer',fontFamily:'inherit'}}>취소</button>
                    <button onClick={saveMEdit} style={{flex:2,padding:'14px',background:'#4338ca',border:'none',color:'#fff',borderRadius:'11px',fontSize:'15px',fontWeight:'900',cursor:'pointer',fontFamily:'inherit'}}>수정 저장</button>
                  </div>
                </div>
              </div>
            )}
            ...
            <div style={{display:'flex',gap:'6px',marginBottom:'12px'}}>
              {['전체','월간','주간','일간'].map(p=>(
                <button key={p} onClick={()=>setMDiscPeriod(p)} style={{flex:1,padding:'9px 6px',background:mDiscPeriod===p?'#7c3aed':'#fff',border:'1.5px solid '+(mDiscPeriod===p?'#7c3aed':'#e2e8f0'),borderRadius:'10px',fontSize:'13px',fontWeight:mDiscPeriod===p?'900':'600',color:mDiscPeriod===p?'#fff':'#64748b',cursor:'pointer',fontFamily:'inherit'}}>{p}</button>
              ))}
            </div>
            ...
            {(()=>{
              let A={net:0,tv:0,renew:0,chg:0,mnp:0,sec:0,wire:0,mob:0,total:0}, sites=0
              mDiscovery.forEach(m=>{
                const g=mpAgg(m,mDiscPeriod)
                if(g.days>0) sites++
                A.net+=g.net;A.tv+=g.tv;A.renew+=g.renew;A.chg+=g.chg;A.mnp+=g.mnp;A.sec+=g.sec
                A.wire+=g.wire;A.mob+=g.mob;A.total+=g.total
              })
              const pLbl = mDiscPeriod==='전체'?'누적':mDiscPeriod
              return(
                <div style={{marginBottom:'14px'}}>
                  <div style={{display:'flex',gap:'10px',marginBottom:'8px'}}>
                    <div style={{flex:1,background:'#eff6ff',border:'1px solid #bfdbfe',borderRadius:'12px',padding:'12px',textAlign:'center'}}><div style={{fontSize:'22px',fontWeight:'900',color:'#2563eb'}}>{A.wire}</div><div style={{fontSize:'11px',color:C.sub}}>{pLbl} 유선</div></div>
                    <div style={{flex:1,background:'#faf5ff',border:'1px solid #e9d5ff',borderRadius:'12px',padding:'12px',textAlign:'center'}}><div style={{fontSize:'22px',fontWeight:'900',color:'#7c3aed'}}>{A.mob}</div><div style={{fontSize:'11px',color:C.sub}}>{pLbl} 무선</div></div>
                    <div style={{flex:1,background:'#f0fdf4',border:'1px solid #bbf7d0',borderRadius:'12px',padding:'12px',textAlign:'center'}}><div style={{fontSize:'22px',fontWeight:'900',color:'#16a34a'}}>{A.total}</div><div style={{fontSize:'11px',color:C.sub}}>{pLbl} 합계</div></div>
                    <div style={{flex:1,background:'#fff7ed',border:'1px solid #fed7aa',borderRadius:'12px',padding:'12px',textAlign:'center'}}><div style={{fontSize:'22px',fontWeight:'900',color:'#e67e00'}}>{sites}</div><div style={{fontSize:'11px',color:C.sub}}>실적 현장</div></div>
                  </div>
                  {A.total>0&&(
                    <div style={{display:'flex',gap:'6px',flexWrap:'wrap',background:'#f8fafc',border:'1px solid #e2e8f0',borderRadius:'10px',padding:'9px 11px'}}>
                      {[['인터넷',A.net,'#2563eb'],['TV',A.tv,'#0891b2'],['약정갱신',A.renew,'#0d9488'],['기변',A.chg,'#7c3aed'],['MNP',A.mnp,'#9333ea'],['2nd',A.sec,'#c026d3']].map(([l,v,c])=>(
                        <span key={l} style={{fontSize:'11.5px',fontWeight:'700',color:v>0?c:'#cbd5e1'}}>{l} <b style={{fontSize:'13px'}}>{v}</b></span>
                      ))}
                    </div>
                  )}
                </div>
              ...
              :mDiscovery.filter(m=>mDiscStage==='전체'||(m.stage||'접촉중')===mDiscStage).map(m=>{
                ...
                return(
                  <div key={m.id} style={{background:'#fff',border:'1px solid #e2e8f0',borderRadius:'12px',padding:'14px',marginBottom:'10px',boxShadow:'0 1px 3px rgba(0,0,0,0.04)'}}>
                    ...
                    <div style={{fontSize:'12px',color:m.agency_name?'#7c3aed':'#cbd5e1',marginBottom:'3px'}}>🤝 연계 대리점: {m.agency_name||'미지정'}{m.agency_manager?' ('+m.agency_manager+')':''}{m.sales_schedule?' · 일정 '+m.sales_schedule:''}</div>
                    <button onClick={()=>{setMEdit(m);setMEditForm({stage:m.stage||'접촉중',agency_name:m.agency_name||'',agency_manager:m.agency_manager||'',sales_schedule:m.sales_schedule||''})}}
                      style={{marginTop:'4px',padding:'5px 12px',background:'#eef2ff',border:'1px solid #c7d2fe',color:'#4338ca',borderRadius:'8px',fontSize:'11.5px',fontWeight:'800',cursor:'pointer',fontFamily:'inherit'}}>✏️ 단계·대리점 수정</button>
                    {(wireT>0||mobT>0)&&<div style={{marginTop:'6px'}}>
                      <div style={{display:'flex',gap:'8px',alignItems:'center',flexWrap:'wrap'}}>
                        ...
                        <span style={{fontSize:'11px',color:'#94a3b8'}}>· {g.days}일 실적{mDiscPeriod!=='전체'?' ('+mDiscPeriod+')':''}</span>
                      </div>
                      <div style={{display:'flex',gap:'7px',flexWrap:'wrap',marginTop:'5px',fontSize:'11px',color:'#64748b'}}>
                        {g.net>0&&<span>인터넷 <b style={{color:'#2563eb'}}>{g.net}</b></span>}
                        {g.tv>0&&<span>TV <b style={{color:'#0891b2'}}>{g.tv}</b></span>}
                        {g.renew>0&&<span>약정갱신 <b style={{color:'#0d9488'}}>{g.renew}</b></span>}
                        {g.chg>0&&<span>기변 <b style={{color:'#7c3aed'}}>{g.chg}</b></span>}
                        {g.mnp>0&&<span>MNP <b style={{color:'#9333ea'}}>{g.mnp}</b></span>}
                        {g.sec>0&&<span>2nd <b style={{color:'#c026d3'}}>{g.sec}</b></span>}
                      </div>
                    </div>}
                  ...
                  :qrPartners.map(p=>(
                    <div key={p.id} style={{background:'#fff',border:'1px solid #e2e8f0',borderRadius:'12px',padding:'13px',marginBottom:'8px',display:'flex',justifyContent:'space-between',alignItems:'center'}}>
                      <div>
                        ...
                        <div style={{fontSize:'12px',color:C.sub,marginTop:'2px'}}>아이디: <b>{p.username}</b> · {adminAccounts.find(a=>a.team_id===p.team_id)?.team_label||p.team_id||'-'}</div>
                      </div>
                      <div style={{display:'flex',gap:'6px'}}>
                      <button onClick={async()=>{
                        const np = prompt('【'+p.name+'】 새 비밀번호를 입력하세요 (4자 이상)')
                        if(np===null) return
                        if(!np || np.trim().length<4){ alert('비밀번호는 4자 이상이어야 합니다'); return }
                        try{ await db.patch('qr_partners','id=eq.'+p.id,{password:np.trim()}); t2(p.name+' 비밀번호 재설정 완료') }
                        catch(e){ alert('재설정 실패: '+e.message) }
                      }} style={{background:'#eff6ff',border:'1px solid #bfdbfe',color:'#1d4ed8',borderRadius:'8px',padding:'6px 12px',fontSize:'12px',fontWeight:'700',cursor:'pointer',fontFamily:'inherit'}}>🔑 비번 재설정</button>
                      <button onClick={async()=>{
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
              ...
              <div style={{ background: '#fff', border: '1.5px solid #bfdbfe', borderRadius: '14px', padding: '18px', boxShadow: '0 1px 4px rgba(0,0,0,0.04)' }}>
                ...
                {(() => { window.__genSlideReport = () => {
                  ...
                  // M플러스(m_discovery) 일별실적 → 상품별 회선수 변환 (팀별)
                  // scope: 'all'(누적) | 'month'(당월) | 'year'(당해년도)
                  function mPlusProdCountsByTeam(teamId, scope){
                    const pc={}
                    const _n=new Date(), _cy=_n.getFullYear(), _cm=_n.getMonth()
                    const inScope=(ds)=>{
                      if(!scope||scope==='all') return true
                      if(!ds) return false
                      const d=new Date(ds); if(isNaN(d)) return false
                      if(scope==='month') return d.getFullYear()===_cy && d.getMonth()===_cm
                      if(scope==='year')  return d.getFullYear()===_cy
                      return true
                    }
                    const sites=mDiscovery.filter(m=>{
                    ...
                    sites.forEach(m=>{
                      try{const recs=JSON.parse(m.daily_records||'[]')
                        recs.forEach(r=>{
                          ...
                          pc['인터넷']=(pc['인터넷']||0)+Number(r.net||0)
                          pc['TV-M']=(pc['TV-M']||0)+Number(r.tv||0)
                          pc['약정갱신']=(pc['약정갱신']||0)+Number(r.renew||0)
                          pc['모바일']=(pc['모바일']||0)+Number(r.wireChange||0)+Number(r.mnp||0)+Number(r.second||0)
                        })
                      }catch{}
                    })
                    return pc
                  }
                  function mergeProdCounts(a,b){
                    const r={...a}
                  ...
                  const teamData = (slideAccs.length > 0 ? slideAccs : [{
                    ...
                    return {
                      ...
                      prodCounts: mergeProdCounts(calcProdCounts([...successLeads, ...successDirect]), mPlusProdCountsByTeam(acc.team_id))  // 팀 상품별 성공건수 (배분+직접발굴+M플러스현장)
                    }
                  }).filter(Boolean)
                  ...
                  const globalProdCounts = mergeProdCounts(calcProdCounts([...allSuccessLeads, ...allSuccessDirect]), mPlusProdCountsByTeam(isSuper?null:adminInfo?.teamId))
                  ...
                  // 트렌드 차트: 과거월은 말일까지, 당월은 '오늘'까지만 표시
                  // (미래 개통일이 입력돼도 오늘 이후는 의미 없으므로 제외)
                  const _monthLastDay = new Date(_y2,_selM+1,0).getDate()
                  const todayDayNumAll = _isPastMonth ? _monthLastDay : Math.min(_monthLastDay, new Date().getDate())
                  ...
                  const teamDataFull = (slideAccs.length>0?slideAccs:[{team_id:adminInfo?.teamId,team_label:adminInfo?.name||'우리팀'}]).map(acc=>{
                    ...
                    const _monthlyStats = teamStats(_inMo)
                    // M플러스 현장 실적(daily_records)을 월간 상품집계에 병합
                    _monthlyStats.prodCounts = mergeProdCounts(_monthlyStats.prodCounts, mPlusProdCountsByTeam(acc.team_id,'month'))
                    const _annualStats = teamStats(_inYr)
                    _annualStats.prodCounts = mergeProdCounts(_annualStats.prodCounts, mPlusProdCountsByTeam(acc.team_id,'year'))
                    return {
                  ...
                  const globalProdMonthly = mergeProdCounts(calcProdCounts(allSuccM), mPlusProdCountsByTeam(isSuper?null:adminInfo?.teamId,'month'))
                  const globalProdAnnual  = mergeProdCounts(calcProdCounts(allSuccY), mPlusProdCountsByTeam(isSuper?null:adminInfo?.teamId,'year'))
                  ...
                  // ── 연간 보고서용: 월별 시리즈 (4월 ~ 현재월) ──
                  const _startMonth = 4   // 데이터 시작 월
                  // 종료월 = 사용자가 선택한 보고서 대상 월 (예: 8월 선택 → 4~8월 마감 기준)
                  const _nowM = Math.max(_startMonth, Math.min(12, slideMonth || (new Date().getMonth()+1)))
                  // 선택된 팀 소속 MM만 집계 (팀 선택 시 타 팀 제외)
                  const _selTeamIds = new Set(slideAccs.map(a=>a.team_id))
                  const _selUsers = myUsers.filter(u=>_selTeamIds.has(u.team_id))
                  const _selNames = _selUsers.map(u=>u.name)
                  const _selUnames = _selUsers.map(u=>u.username)
                  const monthlySeries = []
                  for (let mo = _startMonth; mo <= _nowM; mo++) {
                    const ms = new Date(curYear, mo-1, 1).getTime()
                    const me = new Date(curYear, mo, 1).getTime()-1
                    const inThis = dt => { const t=_ms(dt); return t>=ms && t<=me }
                    const cl = leads.filter(l=>_selNames.includes(l.assigned_to) && l.activity_status && l.activity_status!=='미처리' && inThis(l.activity_at||l.created_at))
                    const dl = directLeads.filter(l=>_selUnames.includes(l.mm_username) && inThis(l.activity_at||l.created_at))
                    const all = [...cl, ...dl]
                    const suc = all.filter(l=>l.activity_status==='성공')
                    // 상품별 회선 (M플러스 월별 실적 병합 → 팀별 표와 수치 일치)
                    const pcBase = calcProdCounts(suc)
                    // M플러스 월별 실적
                    let mpNet=0,mpTv=0,mpRw=0,mpMob=0
                    ;mDiscovery.filter(m=>_selTeamIds.has(m.team_id)).forEach(m=>{
                      try{ JSON.parse(m.daily_records||'[]').forEach(r=>{
                        const t=_ms(r.date); if(!(t>=ms&&t<=me)) return
                        mpNet+=Number(r.net||0); mpTv+=Number(r.tv||0); mpRw+=Number(r.renew||0)
                        mpMob+=Number(r.wireChange||0)+Number(r.mnp||0)+Number(r.second||0)
                      })}catch{}
                    })
                    const pcM = mergeProdCounts(pcBase, { '인터넷':mpNet, 'TV-M':mpTv, '약정갱신':mpRw, '모바일':mpMob })
                    // 프로젝트(테마)별 성공
                    const byProj = {}
                    ;[1,2,3,4,5].forEach(pid=>{ byProj[pid] = suc.filter(l=>Number(l.project_type||0)===pid).length })
                    byProj[5] = (byProj[5]||0)   // M플러스 영업기회 기준
                    monthlySeries.push({
                      month: mo, act: all.length, success: suc.length,
                      rate: all.length>0 ? Math.round(suc.length/all.length*100) : 0,
                      prodCounts: pcM, mp: { net:mpNet, tv:mpTv, renew:mpRw, mob:mpMob, total:mpNet+mpTv+mpRw+mpMob },
                      byProj
                    })
                  }
                  const annualTargetCalc = perMMTarget * _selUsers.length * Math.max(1, _nowM - _startMonth + 1)

                  // ── 연간 보고서 수치 통일: 팀별·MM별도 4월~현재 기준으로 재계산 ──
                  const _anStart = new Date(curYear, _startMonth-1, 1).getTime()
                  const _anEnd = new Date(curYear, _nowM, 1).getTime()-1
                  const _inAnnual = dt => { const t=_ms(dt); return t>=_anStart && t<=_anEnd }
                  // 팀별 (M플러스 포함)
                  const annualTeams = slideAccs.map(acc => {
                    const tm = myUsers.filter(u=>u.team_id===acc.team_id)
                    const tmN = tm.map(u=>u.name), tmU = tm.map(u=>u.username)
                    const tl = leads.filter(l=>tmN.includes(l.assigned_to) && l.activity_status && l.activity_status!=='미처리' && _inAnnual(l.activity_at||l.created_at))
                    const td = directLeads.filter(l=>tmU.includes(l.mm_username) && _inAnnual(l.activity_at||l.created_at))
                    const all=[...tl,...td], suc=all.filter(l=>l.activity_status==='성공')
                    let pc = calcProdCounts(suc)
                    // M플러스(팀 귀속) 4월~현재 실적 병합
                    const mpPc = {}
                    mDiscovery.filter(m=>m.team_id===acc.team_id).forEach(m=>{
                      try{ JSON.parse(m.daily_records||'[]').forEach(r=>{
                        const t=_ms(r.date); if(!(t>=_anStart&&t<=_anEnd)) return
                        mpPc['인터넷']=(mpPc['인터넷']||0)+Number(r.net||0)
                        mpPc['TV-M']=(mpPc['TV-M']||0)+Number(r.tv||0)
                        mpPc['약정갱신']=(mpPc['약정갱신']||0)+Number(r.renew||0)
                        mpPc['모바일']=(mpPc['모바일']||0)+Number(r.wireChange||0)+Number(r.mnp||0)+Number(r.second||0)
                      })}catch{}
                    })
                    pc = mergeProdCounts(pc, mpPc)
                    return { label: acc.team_label||acc.name||'팀', mm: tm.length,
                      annual: { actAll: all.length, success: suc.length, prodCounts: pc } }
                  })
                  // MM별 (M플러스 제외 — 개인 귀속 불가)
                  // 선택된 팀(slideAccs) 소속 MM만 대상 — 팀별 표와 동일 범위
                  const annualMMs = _selUsers.map(u => {
                    const acc2 = adminAccounts.find(a=>a.team_id===u.team_id)
                    const ul = leads.filter(l=>l.assigned_to===u.name && l.activity_status && l.activity_status!=='미처리' && _inAnnual(l.activity_at||l.created_at))
                    const ud = directLeads.filter(l=>l.mm_username===u.username && _inAnnual(l.activity_at||l.created_at))
                    const all=[...ul,...ud], suc=all.filter(l=>l.activity_status==='성공')
                    return { name:u.name, team:acc2?.team_label||'-',
                      annual: { actAll: all.length, success: suc.length, prodCounts: calcProdCounts(suc) } }
                  })

                  const html = buildSlideHTML({ _debug: dbgInfo, reportMode: annualRef.current?'annual':(_isPastMonth?'past':'current'),
                    bizDays: curBizDays, perMMTarget, workDaysAll: workDays,
                    allDailyStats,
                    period: annualRef.current ? (curYear+'년 연간 누계') : periodStr, today: todayStr, yearStr: curYear+'년',
                    teams: teamDataFull, projects: projDataFull,
                    allMM: allMMData, topMM: [...allMMData].sort((a,b)=>(annualRef.current?((b.annual?.success||0)-(a.annual?.success||0)):((b.monthly?.success||0)-(a.monthly?.success||0)))).slice(0,5),
                    mDiscoveryCount: mDiscoveryTotal,
                    globalProdCounts: annualRef.current ? globalProdAnnual : globalProdMonthly,
                    globalProdAnnual,
                    momData,
                    mDiscoveryList: (isSuper ? mDiscovery : mDiscovery.filter(m=>m.team_id===adminInfo?.teamId)),
                    agenciesList: agencies, allLeadsForPS: { leads, directLeads, myUsers, users },
                    monthlySeries, annualTargetCalc, startMonth: 4,
                    annualTeams, annualMMs,
                    psMonthRange: { start: _moStart, end: _moEnd }
                  })
                  const _fname = annualRef.current
                    ? ('상권통_연간누계보고서_' + curYear + '_' + String(slideMonth||curMonth).padStart(2,'0') + '월기준.html')
                    : ('상권통_보고서_' + curYear + '-' + String(slideMonth||curMonth).padStart(2,'0') + (_isPastMonth?'_월마감':'') + '.html')
                  const _blob = new Blob([html],{type:'text/html;charset=utf-8'})
                  const _url = URL.createObjectURL(_blob)
                  // 모바일 브라우저 호환: a 태그를 DOM에 부착 후 클릭, 실패 시 새 탭으로 열기
                  try {
                    const a = document.createElement('a')
                    a.href = _url; a.download = _fname; a.rel = 'noopener'
                  ...
                  // 일부 모바일(삼성 인터넷 등)은 download 속성을 무시 → 새 탭 폴백 안내
                  setTimeout(()=>{ URL.revokeObjectURL(_url) }, 60000)
                  t2(annualRef.current ? '연간 누계 보고서 다운로드 완료!' : '슬라이드 장표 다운로드 완료!')
                  } catch(err) { t2('슬라이드 오류: ' + err.message); console.error('Slide err:', err) }
                  finally { annualRef.current = false }
                }; return null })()}
                <button onClick={() => { annualRef.current = false; window.__genSlideReport() }}
                ...
                {/* 연간 누계 전용 보고서 */}
                <button onClick={() => { annualRef.current = true; window.__genSlideReport() }}
                  style={{ width: '100%', marginTop: '9px', background: '#7c3aed', border: 'none', color: '#fff', padding: '13px', borderRadius: '10px', fontSize: '15px', fontWeight: '900', cursor: 'pointer', fontFamily: 'inherit' }}>
                  📊 연간 누계 보고서 다운로드 ({new Date().getFullYear()}년 · 4월~{slideMonth||new Date().getMonth()+1}월)
                </button>
                <div style={{ fontSize: '11.5px', color: C.sub, marginTop: '6px', lineHeight: 1.5 }}>
                  ※ 연간 누계 보고서: 위에서 선택한 <b>대상 월까지의 누계</b>(4월~선택월) 기준으로 팀·MM·테마·상품별 세부 실적을 정리합니다.
                </div>
              </div>
        ...
        {/* ── U-TURN UIT 목표·실적 입력 (총괄 전용) ─────────────────── */}
        {tab === 'aircon' && isSuper && (
          <div style={{ margin: '-14px -14px 0' }}>
            <AirconCampaign adminInfo={adminInfo} onBack={null} />
          </div>
        )}
        {tab === 'uturn' && isSuper && (
          <div>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '6px' }}>
              <h2 style={{ fontSize: '19px', fontWeight: '900', color: C.text, margin: 0 }}>⤴ U-TURN 관리</h2>
              <select value={uturnMonth} onChange={e => { const m = parseInt(e.target.value); setUturnMonth(m); loadUturnScores(m) }}
                style={{ padding: '8px 12px', border: '1.5px solid ' + C.border, borderRadius: '9px', fontSize: '14px', fontFamily: 'inherit', background: '#fff', color: C.text, fontWeight: '700' }}>
                {Array.from({ length: 12 }, (_, i) => i + 1).map(m => <option key={m} value={m}>{m}월</option>)}
              </select>
            </div>
            <p style={{ fontSize: '12.5px', color: C.sub, margin: '0 0 14px' }}>팀별 UIT 목표·실적을 입력하면 U-TURN 대시보드 성과현황에 반영됩니다 · <b style={{ color: '#7c3aed' }}>U=신규·MNP(기변 제외)</b></p>
            <div style={{ display: 'flex', justifyContent: 'flex-end', marginBottom: '10px' }}>
              <button onClick={() => loadUturnScores()} style={{ background: '#f8fafc', border: '1px solid ' + C.border, color: C.sub, padding: '7px 13px', borderRadius: '9px', fontSize: '13px', cursor: 'pointer', fontFamily: 'inherit' }}>🔄 불러오기</button>
            </div>

            {UTURN_TEAMS.map(t => {
              const s = uturnScores[t.id] || {}
              const F = (field, ph, color) => (
                <input type="number" inputMode="numeric" value={s[field] ?? ''} onChange={e => setUturnField(t.id, field, e.target.value)} placeholder={ph}
                  style={{ width: '100%', padding: '9px 8px', border: '1.5px solid ' + C.border, borderRadius: '8px', fontSize: '15px', fontFamily: 'inherit', textAlign: 'center', color: color, fontWeight: '800', outline: 'none', boxSizing: 'border-box', background: '#fff' }} />
              )
              const uit = (parseInt(s.u_actual) || 0) * 1.5 + (parseInt(s.i_actual) || 0) + (parseInt(s.t_actual) || 0)
              return (
                <div key={t.id} style={{ background: '#fff', border: '1.5px solid ' + C.border, borderRadius: '14px', padding: '14px', marginBottom: '11px', boxShadow: '0 1px 4px rgba(0,0,0,0.04)' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '10px' }}>
                    <span style={{ fontSize: '15px', fontWeight: '900', color: C.text }}>{t.label}</span>
                    <span style={{ fontSize: '12px', fontWeight: '800', color: '#E31937', background: '#fef2f2', border: '1px solid #fecaca', padding: '3px 10px', borderRadius: '12px' }}>UIT 스코어 {Math.round(uit)}</span>
                  </div>
                  <div style={{ display: 'grid', gridTemplateColumns: '46px 1fr 1fr', gap: '7px', alignItems: 'center' }}>
                    <div></div>
                    <div style={{ fontSize: '11px', fontWeight: '800', color: C.sub, textAlign: 'center' }}>목표</div>
                    <div style={{ fontSize: '11px', fontWeight: '800', color: C.sub, textAlign: 'center' }}>실적</div>
                    <div style={{ fontSize: '13px', fontWeight: '900', color: '#7c3aed' }}>U</div>
                    {F('u_goal', '목표', '#7c3aed')}{F('u_actual', '실적', '#7c3aed')}
                    <div style={{ fontSize: '13px', fontWeight: '900', color: '#2563eb' }}>I</div>
                    {F('i_goal', '목표', '#2563eb')}{F('i_actual', '실적', '#2563eb')}
                    <div style={{ fontSize: '13px', fontWeight: '900', color: '#d97706' }}>T</div>
                    {F('t_goal', '목표', '#d97706')}{F('t_actual', '실적', '#d97706')}
                  </div>
                </div>
              )
            })}

            <button onClick={saveUturnScores} disabled={uturnSaving}
              style={{ width: '100%', padding: '15px', background: uturnSaving ? '#e2e8f0' : '#E31937', border: 'none', color: uturnSaving ? '#94a3b8' : '#fff', borderRadius: '12px', fontSize: '16px', fontWeight: '900', cursor: uturnSaving ? 'wait' : 'pointer', fontFamily: 'inherit', marginTop: '4px' }}>
              {uturnSaving ? '저장 중...' : '⤴ ' + uturnMonth + '월 UIT 목표·실적 저장'}
            </button>
            <p style={{ fontSize: '11.5px', color: C.sub, textAlign: 'center', marginTop: '8px' }}>저장 즉시 U-TURN 대시보드에 반영됩니다 · UIT 스코어 = U×1.5 + I + T</p>
          </div>
        )}
