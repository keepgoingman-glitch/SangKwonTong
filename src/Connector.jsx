...
function AssignOneModal({ lead, users, onClose, onSave }) {
  ...
  const [selProj, setSelProj] = useState(lead.project_name || '')
  const [selTheme, setSelTheme] = useState(lead.theme_name || '')
  ...
  return (
    <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.35)', display:'flex', alignItems:'flex-end', justifyContent:'center', zIndex:998, backdropFilter:'blur(3px)' }}>
      <div style={{ background:'#fff', borderRadius:'20px 20px 0 0', padding:'22px 18px 36px', width:'100%', maxWidth:'480px', boxShadow:'0 -4px 24px rgba(0,0,0,0.12)', maxHeight:'88vh', overflowY:'auto', position:'relative' }}>
        ...
        <div style={{ fontSize:'13px', fontWeight:'700', color:'#64748b', marginBottom:'5px' }}>📁 프로젝트명</div>
        <input value={selProj} onChange={e=>setSelProj(e.target.value)} placeholder="프로젝트명"
          style={{ width:'100%', padding:'9px 12px', border:'1.5px solid #e2e8f0', borderRadius:'8px', fontSize:'13px', outline:'none', fontFamily:'inherit', background:'#f8fafc', marginBottom:'10px', boxSizing:'border-box' }}/>
        <div style={{ fontSize:'13px', fontWeight:'700', color:'#64748b', marginBottom:'5px' }}>🏷️ 테마 (선택)</div>
        <input value={selTheme} onChange={e=>setSelTheme(e.target.value)} placeholder="예: 녹산공단 1차"
        ...
        <button onClick={() => selMM && onSave(selMM, selProj, selTheme)} disabled={!selMM}
...
function buildSlideHTML({ period, teams, projects, allMM, topMM, mDiscoveryCount, globalProdCounts = {} }) {
  ...
  // ── SVG 도넛 차트 생성 함수 ─────────────────────────────────
  function donut(segments, size=120, strokeW=18) {
    const r = (size-strokeW)/2, cx=size/2, cy=size/2
    const circ = 2*Math.PI*r
    let offset=0, svg='<svg width="'+size+'" height="'+size+'" viewBox="0 0 '+size+' '+size+'">'
    svg+='<circle cx="'+cx+'" cy="'+cy+'" r="'+r+'" fill="none" stroke="#e8edf2" stroke-width="'+strokeW+'"/>'
    segments.filter(s=>s.v>0).forEach(s=>{
      const total2=segments.reduce((a,x)=>a+x.v,0)
      const dash=(s.v/Math.max(total2,1))*circ
      svg+='<circle cx="'+cx+'" cy="'+cy+'" r="'+r+'" fill="none" stroke="'+s.c+'" stroke-width="'+strokeW+'" stroke-dasharray="'+dash+' '+(circ-dash)+'" stroke-dashoffset="'+(-(offset))+'" transform="rotate(-90 '+cx+' '+cy+')" style="transition:stroke-dasharray 0.4s"/>'
      offset+=dash
    })
    svg+='</svg>'
    return svg
  ...
  // ── CSS ─────────────────────────────────────────────────────
  const css=`
:root{--bg:#0d1b2a;--card:#ffffff;--accent:#e67e00;--blue:#2563eb;--green:#16a34a;--red:#dc2626;--gray:#64748b;--border:#e2e8f0;--light:#f8fafc}
*{margin:0;padding:0;box-sizing:border-box}
html{scroll-behavior:smooth}
body{font-family:"Noto Sans KR","Apple SD Gothic Neo",Malgun Gothic,sans-serif;background:var(--bg);padding:28px 16px;display:flex;flex-direction:column;align-items:center;gap:24px}
.slide{background:#fff;width:100%;max-width:940px;border-radius:16px;overflow:hidden;box-shadow:0 24px 80px rgba(0,0,0,0.5);page-break-after:always;display:flex;flex-direction:column}
/* 헤더 */
.sh{padding:22px 40px 16px;display:flex;justify-content:space-between;align-items:flex-end;border-bottom:3px solid var(--accent)}
.sh-title{font-size:22px;font-weight:900;color:#0f172a}
.sh-meta{font-size:12px;color:var(--gray);text-align:right;line-height:1.6}
/* 바디 */
.sb{padding:22px 40px 26px;flex:1}
/* 푸터 */
.sf{padding:8px 40px;background:#f8fafc;border-top:1px solid #e2e8f0;display:flex;justify-content:space-between;font-size:11px;color:#94a3b8;font-weight:600}
/* 표지 */
.cover{background:linear-gradient(150deg,#0d1b2a 0%,#1a3a5c 45%,#0d1b2a 100%);color:#fff;display:flex;flex-direction:column;align-items:center;justify-content:center;text-align:center;padding:50px 40px;min-height:540px;position:relative;overflow:hidden}
.cover::before{content:'';position:absolute;inset:0;background:radial-gradient(ellipse at 60% 40%,rgba(230,126,0,0.08),transparent 60%)}
/* KPI 그리드 */
.kpi-row{display:grid;gap:14px;margin-bottom:18px}
.kpi-4{grid-template-columns:repeat(4,1fr)}
.kpi-3{grid-template-columns:repeat(3,1fr)}
.kpi-2{grid-template-columns:repeat(2,1fr)}
.kpi{background:#f8fafc;border:1px solid #e2e8f0;border-radius:12px;padding:16px 14px;text-align:center;position:relative;overflow:hidden}
.kpi-top{position:absolute;top:0;left:0;right:0;height:3px}
.kpi-v{font-size:30px;font-weight:900;line-height:1.1}
.kpi-u{font-size:15px;font-weight:600}
.kpi-l{font-size:12px;color:var(--gray);margin-top:5px}
...
.bg{background:#f0fdf4;color:#15803d;border:1px solid #bbf7d0}
.bo{background:#fff7ed;color:#b45309;border:1px solid #fed7aa}
.br{background:#fef2f2;color:#b91c1c;border:1px solid #fecaca}
.bb{background:#eff6ff;color:#1d4ed8;border:1px solid #bfdbfe}
/* 프로그레스 바 */
.bar-w{background:#e8edf2;border-radius:4px;overflow:hidden}
.bar-f{height:100%;border-radius:4px}
/* 차트 섹션 */
.chart-row{display:flex;gap:20px;align-items:center}
.chart-center{position:relative;flex-shrink:0}
.chart-label{position:absolute;inset:0;display:flex;flex-direction:column;align-items:center;justify-content:center}
/* 상품 바 */
.prod-row{display:flex;align-items:center;gap:8px;margin-bottom:6px}
.prod-name{font-size:12px;font-weight:600;color:#1e293b;min-width:55px}
.prod-bar{flex:1;background:#e8edf2;border-radius:3px;height:8px;overflow:hidden}
.prod-cnt{font-size:12px;font-weight:800;color:#16a34a;min-width:22px;text-align:right}
/* 팀 카드 */
.team-card{background:#f8fafc;border:1px solid #e2e8f0;border-radius:12px;padding:16px;position:relative}
.team-bar-top{position:absolute;top:0;left:0;right:0;height:4px;border-radius:12px 12px 0 0}
/* 네비 */
.nav{position:fixed;bottom:16px;left:50%;transform:translateX(-50%);display:flex;gap:5px;background:rgba(13,27,42,0.95);padding:8px 14px;border-radius:50px;z-index:100;backdrop-filter:blur(8px)}
.nb{width:28px;height:28px;border-radius:50%;background:rgba(255,255,255,0.12);border:none;color:#fff;font-weight:700;cursor:pointer;font-size:11px;transition:background 0.2s}
.nb:hover{background:rgba(255,255,255,0.3)}
.np{padding:0 16px;height:28px;border-radius:20px;background:var(--accent);border:none;color:#fff;font-weight:800;cursor:pointer;font-size:11px}
@media print{body{background:#fff;padding:0;gap:0}.slide{border-radius:0;box-shadow:none;page-break-after:always}.nav{display:none!important}}
`

  const slideHead=(n,title,sub)=>
    '<div class="sh"><div><div class="sh-title">'+title+'</div>'+(sub?'<div style="font-size:12px;color:#64748b;margin-top:3px">'+sub+'</div>':'')+'</div><div class="sh-meta">'+period+' 기준<br/>'+n+' / '+N+'</div></div>'
  const sf='<div class="sf"><span>상권통 영업 보고서 · '+period+'</span><span>KT 지역상권 영업관리</span></div>'

  // ── 상품별 집계 헬퍼 ─────────────────────────────────────────
  const PRODS=['인터넷','TV','다량회선','모바일','일반전화','하이오더','로봇','CCTV']
  function getProdCounts(mmList, leadsList) {
    const pc={}
    // allMM의 각 이름으로 성공 leads 필터
    const successLeads = leadsList.filter(l=>l.activity_status==='성공')
  ...
  const PRODS_ORDER=['인터넷','TV','다량회선','모바일','일반전화','하이오더','로봇','CCTV']
  function prodBarsHTML(pc, total, compact=false) {
    const entries=PRODS_ORDER.filter(p=>pc[p]).map(p=>[p,pc[p]])
    if(!entries.length) return '<div style="color:#94a3b8;font-size:12px;padding:4px 0">성공 데이터 없음</div>'
    const mx=Math.max(...entries.map(([,v])=>v))
    return '<div style="display:flex;flex-wrap:wrap;gap:6px;margin-bottom:6px">'+
      entries.map(([p,cnt])=>'<div style="background:#f0fdf4;border:1px solid #bbf7d0;border-radius:8px;padding:'+(compact?'4px 8px':'6px 10px')+';text-align:center;min-width:'+(compact?'50':'60')+'px"><div style="font-size:'+(compact?'14':'16')+'px;font-weight:900;color:#16a34a">'+cnt+'</div><div style="font-size:9px;color:#64748b;margin-top:1px">'+p+'</div></div>').join('')+
    '</div>'+
    entries.map(([p,cnt])=>
      '<div class="prod-row">'+
      '<span class="prod-name" style="font-size:11px;min-width:50px">'+p+'</span>'+
      '<div class="prod-bar"><div class="bar-f" style="width:'+pct(cnt,Math.max(mx,1))+'%;background:#16a34a"></div></div>'+
      '<span class="prod-cnt" style="font-size:11px">'+cnt+'</span>'+
      '</div>'
    ).join('')
  }
  ...
  const s1='<div class="slide" id="s1">'+
    '<div class="cover">'+
    '<div style="position:absolute;top:0;left:0;right:0;height:4px;background:linear-gradient(90deg,#e67e00,#f59e0b,#e67e00)"></div>'+
    '<div style="font-size:13px;font-weight:700;color:rgba(230,126,0,0.9);letter-spacing:3px;margin-bottom:16px;text-transform:uppercase">KT 상권통 플랫폼</div>'+
    '<div style="font-size:46px;font-weight:900;line-height:1.1;background:linear-gradient(135deg,#fff 40%,#f59e0b);-webkit-background-clip:text;-webkit-text-fill-color:transparent;margin-bottom:12px">영업 성과 보고서</div>'+
    '<div style="font-size:15px;color:rgba(255,255,255,0.5);margin-bottom:30px">지역 상권 영업관리 현황 분석</div>'+
    '<div style="background:rgba(230,126,0,0.12);border:1.5px solid rgba(230,126,0,0.3);color:#f59e0b;font-size:15px;font-weight:700;padding:9px 28px;border-radius:30px;display:inline-block;margin-bottom:36px">📅 '+period+' 보고 기준</div>'+
    '<div style="display:flex;gap:16px;justify-content:center;flex-wrap:wrap">'+
    teams.map(t=>'<div style="background:rgba(255,255,255,0.06);border:1px solid rgba(255,255,255,0.12);border-radius:12px;padding:14px 24px;text-align:center;min-width:120px"><div style="font-size:17px;font-weight:900;color:#f59e0b">'+t.label+'</div><div style="font-size:12px;color:rgba(255,255,255,0.45);margin-top:4px">MM '+t.mm+'명 · 목표 '+t.goal+'건</div></div>').join('')+
    '</div>'+
    '<div style="margin-top:28px;font-size:12px;color:rgba(255,255,255,0.25)">전체 성공 '+total+'건 / 목표 '+goal+'건 · 달성률 '+rateAll+'%</div>'+
    '</div>'+sf+'</div>'

  // ── 슬라이드 2: 핵심 성과 요약 ───────────────────────────────
  const ST_COLORS={성공:'#16a34a',접촉완료:'#2563eb',미처리:'#f59e0b',실패:'#dc2626'}
  // 전체 활동 상태 집계 (allMM 기반)
  const totalMM=allMM.length
  const totalAct=allMM.reduce((a,m)=>a+m.mitDone+m.directDone,0)
  const totalSuccess=total

  const donut1=donut([
    {v:total,c:'#16a34a'},
    {v:Math.max(goal-total,0),c:'#e8edf2'}
  ],140,20)
  const donut2=donut([
    {v:mitT,c:'#2563eb'},
    {v:mDiscoveryCount,c:'#7c3aed'},
    {v:Math.max(totalAct-mitT,0),c:'#e8edf2'}
  ],140,20)

  const s2='<div class="slide" id="s2">'+
    slideHead(2,'핵심 성과 요약','전체 팀 통합 기준 · '+period)+
    '<div class="sb">'+
    '<div class="kpi-row kpi-4" style="margin-bottom:20px">'+
    '<div class="kpi"><div class="kpi-top" style="background:#16a34a"></div><div class="kpi-v" style="color:#16a34a">'+total+'<span class="kpi-u">건</span></div><div class="kpi-l">총 성공</div><div class="kpi-s">목표 대비 '+rateAll+'%</div></div>'+
    '<div class="kpi"><div class="kpi-top" style="background:#2563eb"></div><div class="kpi-v" style="color:#2563eb">'+mitT+'<span class="kpi-u">건</span></div><div class="kpi-l">MIT 활동</div><div class="kpi-s">배분 영업기회</div></div>'+
    '<div class="kpi"><div class="kpi-top" style="background:#7c3aed"></div><div class="kpi-v" style="color:#7c3aed">'+mDiscoveryCount+'<span class="kpi-u">건</span></div><div class="kpi-l">M발굴 등록</div><div class="kpi-s">현장 발굴</div></div>'+
    '<div class="kpi"><div class="kpi-top" style="background:#e67e00"></div><div class="kpi-v" style="color:#e67e00">'+rateAll+'<span class="kpi-u">%</span></div><div class="kpi-l">목표 달성률</div><div class="kpi-s">'+total+' / '+goal+'건</div></div>'+
    '</div>'+
    '<div style="display:grid;grid-template-columns:1fr 1fr;gap:20px">'+
    '<div style="background:#f8fafc;border:1px solid #e2e8f0;border-radius:12px;padding:16px">'+
    '<div style="font-size:13px;font-weight:800;color:#0f172a;margin-bottom:12px">목표 달성 현황</div>'+
    '<div class="chart-row">'+
    '<div class="chart-center"><div>'+donut1+'</div><div class="chart-label"><div style="font-size:22px;font-weight:900;color:#16a34a">'+rateAll+'%</div><div style="font-size:10px;color:#64748b">달성률</div></div></div>'+
    '<div style="flex:1"><div style="margin-bottom:10px"><div style="display:flex;justify-content:space-between;font-size:12px;margin-bottom:3px"><span style="font-weight:700">성공</span><span style="color:#16a34a;font-weight:800">'+total+'건</span></div><div class="bar-w" style="height:10px"><div class="bar-f" style="width:'+rateAll+'%;background:#16a34a"></div></div></div><div><div style="display:flex;justify-content:space-between;font-size:12px;margin-bottom:3px"><span style="font-weight:700">잔여</span><span style="color:#94a3b8;font-weight:700">'+Math.max(goal-total,0)+'건</span></div><div class="bar-w" style="height:10px"><div class="bar-f" style="width:'+pct(Math.max(goal-total,0),goal)+'%;background:#e8edf2"></div></div></div></div>'+
    '</div></div>'+
    '<div style="background:#f8fafc;border:1px solid #e2e8f0;border-radius:12px;padding:16px">'+
    '<div style="font-size:13px;font-weight:800;color:#0f172a;margin-bottom:12px">활동 구성 비율</div>'+
    '<div class="chart-row">'+
    '<div class="chart-center"><div>'+donut2+'</div><div class="chart-label"><div style="font-size:18px;font-weight:900;color:#0f172a">'+totalMM+'</div><div style="font-size:10px;color:#64748b">전체 MM</div></div></div>'+
    '<div style="flex:1">'+
    [['MIT 활동',mitT,'#2563eb'],['M발굴',mDiscoveryCount,'#7c3aed']].map(([l,v,c])=>
      '<div style="margin-bottom:8px"><div style="display:flex;justify-content:space-between;font-size:12px;margin-bottom:3px"><span style="display:flex;align-items:center;gap:5px"><span style="width:8px;height:8px;border-radius:50%;background:'+c+';display:inline-block"></span><span style="font-weight:700">'+l+'</span></span><span style="color:'+c+';font-weight:800">'+v+'건</span></div><div class="bar-w" style="height:8px"><div class="bar-f" style="width:'+pct(v,Math.max(mitT+mDiscoveryCount,1))+'%;background:'+c+'"></div></div></div>'
    ).join('')+
    '</div></div></div>'+
    '</div>'+
    // 전체 상품별 성공건수
    (Object.keys(globalProdCounts).length>0?
    '<div style="background:#f0fdf4;border:1px solid #bbf7d0;border-radius:12px;padding:16px;margin-top:14px">'+
    '<div style="font-size:13px;font-weight:800;color:#15803d;margin-bottom:10px">📦 상품별 성공 실적 (전체 합산)</div>'+
    '<div style="display:flex;flex-wrap:wrap;gap:8px;margin-bottom:10px">'+
    PRODS_ORDER.filter(p=>globalProdCounts[p]).map(p=>'<div style="background:#fff;border:1.5px solid #bbf7d0;border-radius:10px;padding:8px 14px;text-align:center;min-width:65px"><div style="font-size:20px;font-weight:900;color:#16a34a">'+globalProdCounts[p]+'</div><div style="font-size:10px;color:#64748b;margin-top:2px">'+p+'</div></div>').join('')+
    '</div>'+
    prodBarsHTML(globalProdCounts, total)+
    '</div>':'')+
    '</div>'+sf+'</div>'

  // ── 슬라이드 3: 팀별 성과 ────────────────────────────────────
  const teamDonutsHTML = teams.map(t=>{
    const r=pct(t.success,t.goal)
    const color=r>=70?'#16a34a':r>=50?'#e67e00':'#dc2626'
    const d=donut([{v:t.success,c:color},{v:Math.max(t.goal-t.success,0),c:'#e8edf2'}],90,14)
    const pc=t.prodCounts||{}
    const topProds=PRODS_ORDER.filter(p=>pc[p]).slice(0,4)
    return '<div class="team-card">'+
      '<div class="team-bar-top" style="background:'+color+'"></div>'+
      '<div style="font-size:15px;font-weight:900;margin:8px 0 3px">'+t.label+'</div>'+
      '<div style="font-size:11px;color:#64748b;margin-bottom:10px">MM '+t.mm+'명</div>'+
      '<div style="display:flex;align-items:center;gap:10px;margin-bottom:10px">'+
      '<div style="position:relative;flex-shrink:0"><div>'+d+'</div><div style="position:absolute;inset:0;display:flex;flex-direction:column;align-items:center;justify-content:center"><div style="font-size:14px;font-weight:900;color:'+color+'">'+r+'%</div></div></div>'+
      '<div style="flex:1;font-size:11px">'+
      '<div style="display:flex;justify-content:space-between;margin-bottom:4px"><span>성공</span><span style="font-weight:800;color:#16a34a">'+t.success+'건</span></div>'+
      '<div style="display:flex;justify-content:space-between;margin-bottom:4px"><span>목표</span><span style="font-weight:700">'+t.goal+'건</span></div>'+
      '<div style="display:flex;justify-content:space-between;margin-bottom:4px"><span>MIT</span><span style="color:#2563eb;font-weight:700">'+t.mitDone+'건</span></div>'+
      '<div style="display:flex;justify-content:space-between"><span>M발굴</span><span style="color:#7c3aed;font-weight:700">'+t.bulkRegistered+'건</span></div>'+
      '</div></div>'+
      (topProds.length>0?'<div style="border-top:1px solid #e2e8f0;padding-top:7px;margin-top:2px"><div style="font-size:9px;color:#64748b;font-weight:700;margin-bottom:4px">📦 성공 상품</div>'+
      '<div style="display:flex;flex-wrap:wrap;gap:3px">'+topProds.map(p=>'<span style="background:#f0fdf4;border:1px solid #bbf7d0;border-radius:4px;padding:2px 6px;font-size:9px;font-weight:800;color:#15803d">'+p+' '+pc[p]+'</span>').join('')+'</div></div>':'')+
      '</div>'
  }).join('')

  const s3='<div class="slide" id="s3">'+
    slideHead(3,'팀별 성과 현황',teams.length+'개 팀 비교 분석')+
    '<div class="sb">'+
    '<div style="display:grid;grid-template-columns:repeat('+Math.min(teams.length,3)+',1fr);gap:14px;margin-bottom:18px">'+teamDonutsHTML+'</div>'+
    '<table><thead><tr><th>팀명</th><th>MM</th><th>성공</th><th>목표</th><th>MIT활동</th><th>MIT활동률</th><th>M발굴</th><th>달성률</th></tr></thead><tbody>'+
    teams.map(t=>{
      const r=pct(t.success,t.goal), mr=pct(t.mitDone,Math.max(t.mitLeads||t.mitDone+1,1))
      return '<tr><td><strong>'+t.label+'</strong></td><td>'+t.mm+'명</td>'+
        '<td style="color:#16a34a;font-weight:800">'+t.success+'건</td>'+
        '<td>'+t.goal+'건</td>'+
        '<td style="color:#2563eb;font-weight:700">'+t.mitDone+'건</td>'+
        '<td><div style="display:flex;align-items:center;gap:6px"><div class="bar-w" style="width:60px;height:7px"><div class="bar-f" style="width:'+mr+'%;background:#2563eb"></div></div><span style="font-size:11px;color:#2563eb;font-weight:700">'+mr+'%</span></div></td>'+
        '<td style="color:#7c3aed;font-weight:700">'+t.bulkRegistered+'건</td>'+
        '<td><span class="b '+(r>=70?'bg':r>=50?'bo':'br')+'">'+r+'%</span></td></tr>'
    }).join('')+'</tbody></table>'+
    '</div>'+sf+'</div>'

  // ── 슬라이드 4: 프로젝트 현황 ────────────────────────────────
  const projSlide = projects.length>0?'<div class="slide" id="s4">'+
    slideHead(4,'프로젝트별 활동 현황','진행중 프로젝트 '+projects.length+'개')+
    '<div class="sb">'+
    '<table><thead><tr><th>프로젝트명</th><th>팀</th><th>전체</th><th>활동</th><th>성공</th><th>진행률</th><th>성공률</th><th>주요 성공 상품</th></tr></thead><tbody>'+
    projects.map(p=>{
      const ar=pct(p.done,p.total), sr=pct(p.success,Math.max(p.done,1))
      const pc=p.prodCounts||{}
      const topP=PRODS_ORDER.filter(x=>pc[x]).slice(0,3).map(x=>'<span style="background:#f0fdf4;border:1px solid #bbf7d0;border-radius:4px;padding:2px 6px;font-size:10px;font-weight:700;color:#15803d;margin-right:2px">'+x+' '+pc[x]+'</span>').join('')
      return '<tr><td><strong>'+p.name+'</strong></td><td>'+p.team+'</td>'+
        '<td>'+p.total+'건</td><td>'+p.done+'건</td>'+
        '<td style="color:#16a34a;font-weight:800">'+p.success+'건</td>'+
        '<td><div style="display:flex;align-items:center;gap:6px"><div class="bar-w" style="width:70px;height:8px"><div class="bar-f" style="width:'+ar+'%;background:#2563eb"></div></div><span style="font-size:11px;color:#2563eb;font-weight:800">'+ar+'%</span></div></td>'+
        '<td><span class="b '+(sr>=50?'bg':'bo')+'">'+sr+'%</span></td>'+
        '<td>'+(topP||'<span style="color:#94a3b8;font-size:10px">없음</span>')+'</td></tr>'
    }).join('')+'</tbody></table>'+
    '</div>'+sf+'</div>':''

  // ── 슬라이드 5: 전직원 세부 활동 ─────────────────────────────
  const slideNum_detail = 4 + (projects.length>0?1:0)
  const detailSlide = allMM.length>0?'<div class="slide" id="s'+slideNum_detail+'">'+
    slideHead(slideNum_detail,'전직원 세부 활동 실적',allMM.length+'명 전원 기준 · 성공 내림차순')+
    '<div class="sb">'+
    '<table><thead><tr><th>#</th><th>이름</th><th>팀</th><th>MIT배분</th><th>MIT활동</th><th>직접발굴</th><th>성공</th><th>목표</th><th>달성률</th><th>MIT활동률</th></tr></thead><tbody>'+
    [...allMM].sort((a,b)=>b.success-a.success).map((m,i)=>{
      const r=pct(m.success,Math.max(m.goal,1))
      const mr=pct(m.mitDone,Math.max(m.mitAssigned,1))
      return '<tr>'+
        ...
        '<td style="color:#2563eb;font-weight:700">'+m.mitDone+'건</td>'+
        '<td style="color:#7c3aed;font-weight:700">'+m.directDone+'건</td>'+
        '<td style="color:#16a34a;font-weight:800">'+m.success+'건</td>'+
        '<td>'+m.goal+'건</td>'+
        '<td><span class="b '+(r>=80?'bg':r>=50?'bo':'br')+'">'+r+'%</span></td>'+
        '<td><div style="display:flex;align-items:center;gap:5px"><div class="bar-w" style="width:50px;height:6px"><div class="bar-f" style="width:'+Math.min(mr,100)+'%;background:#2563eb"></div></div><span style="font-size:10px;color:#2563eb;font-weight:700">'+mr+'%</span></div></td>'+
        '</tr>'
    }).join('')+'</tbody></table>'+
    '</div>'+sf+'</div>':''

  // ── 슬라이드 6: 성과 우수자 ──────────────────────────────────
  const topN=topMM.slice(0,5)
  const top3sum=topN.slice(0,3).reduce((a,m)=>a+m.success,0)
  const topSlide='<div class="slide" id="s'+(N-1)+'">'+
    slideHead(N-1,'성과 우수자 현황','기간 내 성공 실적 기준')+
    '<div class="sb" style="display:grid;grid-template-columns:1.2fr 0.8fr;gap:22px">'+
    '<div>'+
    topN.map((m,i)=>{
      const medals=['🥇','🥈','🥉','4','5']
      const bgs=['#fef9c3','#f1f5f9','#fdf4e7','#f8fafc','#f8fafc']
      const r=pct(m.success,Math.max(m.goal,1))
      const barW=pct(m.success,Math.max(topN[0].success,1))
      return '<div style="display:flex;align-items:center;gap:12px;padding:11px;background:'+bgs[i]+';border:1px solid '+(i===0?'#fde68a':'#e2e8f0')+';border-radius:10px;margin-bottom:8px">'+
        '<div style="width:34px;height:34px;border-radius:50%;background:'+(i===0?'#f59e0b':i===1?'#94a3b8':i===2?'#b45309':'#e2e8f0')+';display:flex;align-items:center;justify-content:center;font-size:'+(i<3?'18':'13')+'px;font-weight:900;color:#fff;flex-shrink:0">'+medals[i]+'</div>'+
        '<div style="flex:1"><div style="font-size:14px;font-weight:800">'+m.name+'</div>'+
        '<div style="font-size:11px;color:#64748b;margin-bottom:5px">'+m.team+' · MIT '+( m.mitDone||0)+'건 · 직접발굴 '+(m.directDone||0)+'건</div>'+
        '<div style="display:flex;align-items:center;gap:6px"><div class="bar-w" style="flex:1;height:6px"><div class="bar-f" style="width:'+barW+'%;background:'+(i===0?'#f59e0b':'#16a34a')+'"></div></div><span style="font-size:10px;font-weight:700;color:#64748b">'+r+'%</span></div>'+
        '</div>'+
        '<div style="text-align:right;flex-shrink:0"><div style="font-size:22px;font-weight:900;color:#16a34a">'+m.success+'</div><div style="font-size:9px;color:#64748b">성공건</div></div>'+
        '</div>'
    }).join('')+
    '</div>'+
    '<div>'+
    '<div style="font-size:13px;font-weight:800;color:#0f172a;margin-bottom:12px">팀별 달성 현황</div>'+
    teams.map(t=>{
      const r=pct(t.success,t.goal)
      const d=donut([{v:t.success,c:r>=70?'#16a34a':'#e67e00'},{v:Math.max(t.goal-t.success,0),c:'#e8edf2'}],70,12)
      return '<div style="display:flex;align-items:center;gap:10px;margin-bottom:12px">'+
        '<div style="position:relative;flex-shrink:0"><div>'+d+'</div><div style="position:absolute;inset:0;display:flex;align-items:center;justify-content:center;font-size:12px;font-weight:900;color:'+(r>=70?'#16a34a':'#e67e00')+'">'+r+'%</div></div>'+
        '<div style="flex:1"><div style="font-size:13px;font-weight:800">'+t.label+'</div>'+
        '<div style="font-size:11px;color:#64748b">'+t.success+'/'+t.goal+'건 · MIT '+t.mitDone+'건</div></div></div>'
    }).join('')+
    '<div style="background:#f8fafc;border:1px solid #e2e8f0;border-radius:10px;padding:12px;margin-top:8px">'+
    '<div style="font-size:12px;font-weight:800;color:#0f172a;margin-bottom:8px">성과 집중도</div>'+
    '<div style="font-size:12px;color:#64748b">상위 3인 기여율 <strong style="color:#16a34a">'+pct(top3sum,Math.max(total,1))+'%</strong></div>'+
    '<div style="font-size:12px;color:#64748b;margin-top:4px">전체 MM 달성률 <strong style="color:#e67e00">'+rateAll+'%</strong></div>'+
    '</div>'+
    '</div></div>'+
    sf+'</div>'

  // ── 슬라이드 마지막: 종합 ──────────────────────────────────
  const sLast='<div class="slide" id="s'+N+'">'+
    slideHead(N,'종합 요약','')+
    '<div class="sb" style="display:grid;grid-template-columns:1fr 1fr;gap:24px">'+
    '<div>'+
    '<div style="font-size:13px;font-weight:800;color:#e67e00;margin-bottom:10px;display:flex;align-items:center;gap:6px"><span style="width:16px;height:16px;background:#e67e00;border-radius:50%;display:inline-block"></span> 기간 내 주요 성과</div>'+
    [
      '전체 성공 <strong style="color:#16a34a">'+total+'건</strong> (목표 '+goal+'건 · '+rateAll+'%)',
      'MIT 활동 <strong style="color:#2563eb">'+mitT+'건</strong> 완료',
      'M발굴 등록 <strong style="color:#7c3aed">'+mDiscoveryCount+'건</strong>',
      '최고 성과 '+( topMM[0]?.name||'-')+' <strong style="color:#f59e0b">'+(topMM[0]?.success||0)+'건</strong>',
      '운영 프로젝트 <strong>'+projects.length+'개</strong>',
    ].map(t=>'<div style="background:#f0fdf4;border:1px solid #bbf7d0;border-radius:8px;padding:10px 14px;font-size:12.5px;margin-bottom:7px;color:#1e293b">'+t+'</div>').join('')+
    '</div>'+
    '<div>'+
    '<div style="font-size:13px;font-weight:800;color:#2563eb;margin-bottom:10px;display:flex;align-items:center;gap:6px"><span style="width:16px;height:16px;background:#2563eb;border-radius:50%;display:inline-block"></span> 다음 기간 목표</div>'+
    [
      '전체 목표 <strong>'+Math.round(goal*1.1)+'건</strong> (전기 대비 +10%)',
      'MIT 활동 목표 <strong>'+Math.round(mitT*1.15)+'건</strong>',
      'M발굴 목표 <strong>'+Math.max(mDiscoveryCount+5,10)+'건</strong>',
      '달성률 미달 팀원 <strong>'+allMM.filter(m=>pct(m.success,Math.max(m.goal,1))<50).length+'명</strong> 집중 지원',
      '프로젝트 활동률 <strong>70% 이상</strong> 달성',
    ].map(t=>'<div style="background:#eff6ff;border:1px solid #bfdbfe;border-radius:8px;padding:10px 14px;font-size:12.5px;margin-bottom:7px;color:#1e293b">'+t+'</div>').join('')+
    '</div></div>'+
    '<div style="position:absolute;bottom:42px;right:40px;font-size:10px;color:#94a3b8">KT 상권통 자동 생성 · '+new Date().toLocaleDateString('ko-KR')+'</div>'+
    sf+'</div>'
    ...
    <div style={{ minHeight: '100vh', background: C.bg, color: C.text, fontFamily: "'Noto Sans KR',sans-serif", maxWidth: '480px', margin: '0 auto', display: 'flex', flexDirection: 'column' }}>
      ...
      {assignTarget && (
        <AssignOneModal lead={assignTarget} users={myUsers} onClose={() => setAssignTarget(null)}
          onSave={async (mmName, proj, theme) => {
            const pos = saveScroll()
            try {
              const patch = { assigned_to: mmName, assign_status: '배분완료' }
              if (proj) patch.project_name = proj
              if (theme !== undefined) patch.theme_name = theme
      ...
      <div ref={scrollRef} style={{ flex: 1, overflowY: 'auto', padding: '14px' }}>
        ...
        {tab === 'dashboard' && (
          <div>
            ...
            {/* 프로젝트별 뷰 - 직접발굴 분리 */}
            {dashView === '프로젝트별' && (() => {
              const projects = [...new Set(leads.map(l=>l.project_name).filter(Boolean))]
              const now = new Date()
              ...
              const myDirect = directLeads.filter(l=>myUnames.includes(l.mm_username)).filter(l=>inP(l.activity_at))
              const directSuccess = myDirect.filter(l=>l.activity_status==='성공')
              // 직접발굴 상품 집계
              const PRODS = ['인터넷','TV','다량회선','모바일','일반전화','하이오더','로봇','CCTV']
              function getProdCounts(successList) {
                const pc = {}
              ...
              function ProdBar({pc, total}) {
                const entries=Object.entries(pc).sort((a,b)=>b[1]-a[1])
                if(!entries.length) return null
                return <div style={{marginTop:'8px'}}>
                  {PRODS.filter(p=>pc[p]).map(p=>(
                    <div key={p} style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'4px'}}>
                      <span style={{fontSize:'11px',color:C.text,fontWeight:'600',minWidth:'60px'}}>{p}</span>
                      <div style={{display:'flex',alignItems:'center',gap:'5px',flex:1}}>
                        <div style={{flex:1,background:'#e2e8f0',borderRadius:'3px',height:'5px'}}>
                          <div style={{width:pct(pc[p],Math.max(total,1))+'%',background:C.green,height:'100%',borderRadius:'3px'}}/>
                        </div>
                        <span style={{fontSize:'11px',fontWeight:'800',color:C.green,minWidth:'20px',textAlign:'right'}}>{pc[p]}</span>
                      </div>
                    </div>
                  ))}
                </div>
              ...
              return (
                <div>
                  {/* ── 직접발굴 카드 (별도 표시) ── */}
                  <div style={{ background:'#fff',border:'1px solid #e2e8f0',borderLeft:'4px solid '+C.green,borderRadius:'14px',padding:'14px',marginBottom:'14px',boxShadow:'0 1px 4px rgba(0,0,0,0.04)' }}>
                    <div style={{ fontSize:'15px',fontWeight:'900',color:C.text,marginBottom:'4px' }}>🔍 직접발굴</div>
                    <div style={{ fontSize:'12px',color:C.sub,marginBottom:'10px' }}>{period} 활동 {myDirect.length}건 · 성공 {directSuccess.length}건</div>
                    <div style={{ display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:'6px',marginBottom:'8px' }}>
                      {[{l:'전체',v:myDirect.length,c:C.text},{l:'활동',v:myDirect.filter(l=>l.activity_status&&l.activity_status!=='미처리').length,c:C.green},{l:'성공',v:directSuccess.length,c:C.green},{l:'미처리',v:myDirect.filter(l=>!l.activity_status||l.activity_status==='미처리').length,c:C.acc}].map((s,i)=>(
                        <div key={i} style={{background:'#f0fdf4',border:'1px solid #bbf7d0',borderRadius:'8px',padding:'8px',textAlign:'center'}}>
                          <div style={{fontSize:'18px',fontWeight:'900',color:s.c}}>{s.v}</div>
                          <div style={{fontSize:'10px',color:C.sub,marginTop:'2px'}}>{s.l}</div>
                        </div>
                      ))}
                    </div>
                    {directSuccess.length>0 && <><div style={{fontSize:'11px',color:C.sub,fontWeight:'700',marginBottom:'4px'}}>📦 성공 상품별</div><ProdBar pc={getProdCounts(directSuccess)} total={directSuccess.length}/></>}
                  </div>

                  {/* ── 프로젝트별 카드 ── */}
                  {projects.length===0
                    ? <div style={{textAlign:'center',padding:'30px',color:C.sub,background:'#fff',borderRadius:'12px',border:'1px solid #e2e8f0'}}>업로드된 프로젝트가 없습니다</div>
                    : projects.map(pn => {
                        const pl = leads.filter(l=>l.project_name===pn&&myUsers.map(u=>u.name).includes(l.assigned_to))
                        const pFiltered = pl.filter(l=>inP(l.activity_at))
                        const done = pFiltered.filter(l=>l.activity_status&&l.activity_status!=='미처리').length
                        const success = pFiltered.filter(l=>l.activity_status==='성공')
                        const themes = [...new Set(pl.map(l=>l.theme_name).filter(Boolean))]
                        const pc = getProdCounts(success)
                        return (
                          <div key={pn} style={{background:'#fff',border:'1px solid #e2e8f0',borderLeft:'4px solid '+C.blue,borderRadius:'14px',padding:'16px',marginBottom:'12px',boxShadow:'0 1px 4px rgba(0,0,0,0.04)'}}>
                            <div style={{fontSize:'16px',fontWeight:'900',color:C.text,marginBottom:'4px'}}>📁 {pn}</div>
                            <div style={{fontSize:'12px',color:C.sub,marginBottom:'10px'}}>전체 {pl.length}건 · {period} 활동 {pFiltered.length}건</div>
                            <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:'6px',marginBottom:'10px'}}>
                              {[{l:'전체',v:pl.length,c:C.text},{l:'활동',v:done,c:C.blue},{l:'성공',v:success.length,c:C.green},{l:'미처리',v:pl.length-done,c:C.acc}].map((s,i)=>(
                                <div key={i} style={{background:'#f8fafc',border:'1px solid #e2e8f0',borderRadius:'8px',padding:'8px',textAlign:'center'}}>
                                  <div style={{fontSize:'18px',fontWeight:'900',color:s.c}}>{s.v}</div>
                                  <div style={{fontSize:'10px',color:C.sub,marginTop:'2px'}}>{s.l}</div>
                                </div>
                            ...
                            <div style={{background:'#e2e8f0',borderRadius:'4px',height:'8px',overflow:'hidden',marginBottom:'4px'}}>
                              <div style={{width:Math.min(pct(done,Math.max(pl.length,1)),100)+'%',background:C.blue,height:'100%',borderRadius:'4px'}}/>
                            </div>
                            <div style={{fontSize:'11px',color:C.sub,marginBottom:'8px',display:'flex',justifyContent:'space-between'}}>
                              <span>활동률 {pct(done,Math.max(pl.length,1))}%</span>
                              <span>성공률 {pct(success.length,Math.max(done,1))}%</span>
                            </div>
                            {success.length>0 && <><div style={{fontSize:'11px',color:C.sub,fontWeight:'700',marginBottom:'4px'}}>📦 성공 상품별</div><ProdBar pc={pc} total={success.length}/></>}
                            {themes.length>0 && (
                              <div style={{marginTop:'10px',background:'#f8fafc',borderRadius:'8px',padding:'8px 10px'}}>
                                <div style={{fontSize:'11px',fontWeight:'700',color:C.sub,marginBottom:'6px'}}>🏷️ 테마별</div>
                                {themes.map(tn=>{
                                  const tl=pl.filter(l=>l.theme_name===tn)
                                  const td=tl.filter(l=>l.activity_status&&l.activity_status!=='미처리').length
                                  return <div key={tn} style={{display:'flex',justifyContent:'space-between',fontSize:'12px',marginBottom:'4px'}}>
                                    <span style={{color:C.text,fontWeight:'600'}}>{tn}</span>
                                    <span style={{color:C.blue,fontWeight:'700'}}>{td}/{tl.length}건 ({pct(td,tl.length)}%)</span>
                                  </div>
                                })}
                              </div>
                            )}
                            ...
                            <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:'8px' }}>
                              <div style={{ flex:1, marginRight:'10px' }}>
                                <div style={{ display:'flex', gap:'5px', alignItems:'center', marginBottom:'4px', flexWrap:'wrap' }}>
                                  ...
                                  {l.project_name && <span style={{ fontSize:'10px', background:'#f1f5f9', color:'#475569', border:'1px solid #e2e8f0', padding:'1px 6px', borderRadius:'8px' }}>📁 {l.project_name}</span>}
              ...
              <div style={{ background: '#fff', border: '1.5px solid #bfdbfe', borderRadius: '14px', padding: '18px', boxShadow: '0 1px 4px rgba(0,0,0,0.04)' }}>
                ...
                <button onClick={() => {
                  ...
                  const projData = [...new Set(leads.map(l=>l.project_name).filter(Boolean))].map(pn => {
                    ...
                    const successPl = pl.filter(l=>l.activity_status==='성공')
                    return {
                      name: pn, team: teamAcc?.team_label||'-',
                      total: pl.length,
                      done: pl.filter(l=>l.activity_status&&l.activity_status!=='미처리').length,
                      success: successPl.length,
                      prodCounts: calcProdCounts(successPl)
                    }
                  })
