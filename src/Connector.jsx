...
function buildSlideHTML({ period, today, yearStr, teams, projects, allMM, topMM,
  ...
  *{margin:0;padding:0;box-sizing:border-box;-webkit-print-color-adjust:exact!important;print-color-adjust:exact!important}
  ...
  function mPlusSiteSlide(sn){
    const sites = mDiscoveryList||[]
    // 각 현장의 daily_records 집계 → 인터넷/TV/약정갱신/모바일
    const rows = sites.map(m=>{
      let net=0,tv=0,renew=0,mob=0
      try{const recs=JSON.parse(m.daily_records||'[]');recs.forEach(r=>{net+=Number(r.net||0);tv+=Number(r.tv||0);renew+=Number(r.renew||0);mob+=Number(r.wireChange||0)+Number(r.mnp||0)+Number(r.second||0)})}catch{}
      const wire=net+tv+renew
      return { name:m.customer_name||m.site_name||'-', stage:m.stage||'접촉중', emp:m.emp_count||'-', agency:m.agency_name||'', net,tv,renew,mob, wire, total:wire+mob }
    }).filter(r=>r.total>0 || ['영업중','영업종료'].includes(r.stage))
      .sort((a,b)=>b.total-a.total)
    const sum=rows.reduce((a,r)=>({net:a.net+r.net,tv:a.tv+r.tv,renew:a.renew+r.renew,mob:a.mob+r.mob,wire:a.wire+r.wire,total:a.total+r.total}),{net:0,tv:0,renew:0,mob:0,wire:0,total:0})
    ...
    return `<div class="slide" id="s${sn}">
  ${rh('🔍 M플러스 영업구간별 실적','산단·협단체 임직원 모바일 벌크 현장 현황',sn)}
  ${dbar}
  <div class="body">
    <div style="display:flex;gap:10px;margin-bottom:12px">
      <div style="flex:1;background:#f0f9ff;border:1.5px solid #bae6fd;border-radius:12px;padding:10px;text-align:center"><div style="font-size:22px;font-weight:900;color:#0891b2">${stageCount('접촉중')}</div><div style="font-size:10.5px;color:#64748b">접촉중</div></div>
      <div style="flex:1;background:#faf5ff;border:1.5px solid #e9d5ff;border-radius:12px;padding:10px;text-align:center"><div style="font-size:22px;font-weight:900;color:#7c3aed">${stageCount('발굴성공')}</div><div style="font-size:10.5px;color:#64748b">발굴성공</div></div>
      <div style="flex:1;background:#fffbeb;border:1.5px solid #fde68a;border-radius:12px;padding:10px;text-align:center"><div style="font-size:22px;font-weight:900;color:#d97706">${stageCount('영업중')}</div><div style="font-size:10.5px;color:#64748b">영업중</div></div>
      <div style="flex:1;background:#f0fdf4;border:1.5px solid #bbf7d0;border-radius:12px;padding:10px;text-align:center"><div style="font-size:22px;font-weight:900;color:#16a34a">${stageCount('영업종료')}</div><div style="font-size:10.5px;color:#64748b">영업종료</div></div>
      <div style="flex:1;background:#eef2ff;border:1.5px solid #c7d2fe;border-radius:12px;padding:10px;text-align:center"><div style="font-size:22px;font-weight:900;color:#4338ca">${sum.total}</div><div style="font-size:10.5px;color:#64748b">총 수주 회선</div></div>
    </div>
    <div style="font-size:13.5px;font-weight:800;color:#16213e;margin-bottom:6px">영업구간(현장)별 수주 실적 <span style="font-size:11px;color:#94a3b8;font-weight:400">— 유선(인터넷·TV·약정갱신) + 모바일 회선</span></div>
    <table>
      <thead><tr>
        <th style="text-align:left;padding-left:10px;width:150px">영업구간(현장)</th>
        <th>단계</th><th>임직원수</th>
        <th style="background:#1d4ed8">인터넷</th><th style="background:#d97706">TV</th><th style="background:#0891b2">약정갱신</th><th style="background:#7c3aed">모바일</th>
        <th style="background:#15803d">합계회선</th><th style="text-align:left">연계 대리점</th>
      </tr></thead>
      <tbody>
        ${rows.length===0?`<tr><td colspan="9" style="color:#94a3b8;padding:24px">실적이 입력된 영업구간이 없습니다</td></tr>`:rows.slice(0,13).map((r,i)=>`
          <tr style="background:${i%2?'#f8fafc':'#fff'}">
            <td style="text-align:left;font-weight:700;padding-left:10px;max-width:150px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${r.name}</td>
            <td><span style="font-size:10px;padding:2px 7px;border-radius:8px;background:${stageColor(r.stage)}22;color:${stageColor(r.stage)};font-weight:700">${r.stage}</span></td>
            <td style="font-size:11px">${r.emp}</td>
            <td class="bc">${r.net||'-'}</td><td>${r.tv||'-'}</td><td>${r.renew||'-'}</td><td>${r.mob||'-'}</td>
            <td style="background:#f0fdf4;font-weight:900;color:#15803d">${r.total||'-'}</td>
            <td style="text-align:left;font-size:10.5px;color:#7c3aed">${r.agency||'-'}</td>
          </tr>`).join('')}
        <tr style="background:#16213e;color:#fff;font-weight:900">
          <td style="color:#fff;text-align:left;padding-left:10px">전 체 (${rows.length}개 현장)</td>
          <td style="color:#fff">-</td><td style="color:#fff">-</td>
          <td style="color:#60a5fa">${sum.net||'-'}</td><td style="color:#fff">${sum.tv||'-'}</td><td style="color:#fff">${sum.renew||'-'}</td><td style="color:#fff">${sum.mob||'-'}</td>
          <td style="background:#166534;color:#fff">${sum.total||'-'}</td><td style="color:#fff"></td>
        </tr>
      </tbody>
    </table>
    ${rows.length>13?`<div style="font-size:11px;color:#94a3b8;text-align:center;margin-top:4px">외 ${rows.length-13}개 현장</div>`:''}
    <div style="margin-top:10px;font-size:11px;color:#94a3b8">※ 영업구간별 실적 = 각 현장의 일별 실적(daily_records) 누적 합산 · 유선=인터넷+TV+약정갱신, 모바일=기변+MNP+2nd</div>
  </div>
  ${ftr(sn)}
</div>`
  }

  // ════ S10: P&S 대리점 유형별 상품 성과 (월간 + 누적) ════
