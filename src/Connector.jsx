        ...
        {tab === 'activity' && (
          <div>
            ...
            {(() => {
              ...
              const baseLeads = leads.filter(l => myUN.includes(l.assigned_to)).filter(l => inActP(l.activity_at))
              const baseDirect = directLeads.filter(l => myUnames.includes(l.mm_username)).filter(l => inActP(l.activity_at))
              const allBase = [
                ...baseLeads.map(l=>({...l,_type:'배분',_name:l.address})),
              ...
              return (
                <>
                  ...
                  {filtered.length === 0
                    ? <div style={{ textAlign: 'center', padding: '40px 20px', color: C.sub }}>해당 기간 활동 결과가 없습니다</div>
                    : filtered.map((l, idx2) => {
