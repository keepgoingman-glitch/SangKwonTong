...
export default function Connector({ onBack }) {
  ...
  const [qrView, setQrView] = useState('overview')  // M발굴 단계 필터
  ...
  const load = async (info, scrollPos) => {
    ...
    try {
      ...
      const [u, l, dlRaw, md, qrP, qrS, qrL] = await Promise.all([
        db.get('mm_users', uq),
      ...
      setQrPartners(qrP||[]); setQrStores(qrS||[]); setQrLeads(qrL||[])
      // 드랍된 테마 로드
      ...
      <div ref={scrollRef} style={{ flex: 1, overflowY: 'auto', padding: '14px' }}>
        ...
        {tab === 'mdiscovery' && (
          <div>
            ...
            <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:'8px',marginBottom:'14px'}}>
              {[['접촉중','#0891b2','#e0f2fe'],['발굴성공','#7c3aed','#ede9fe'],['영업중','#d97706','#fef3c7'],['영업종료','#16a34a','#dcfce7']].map(([s,c,bg])=>(
                <div key={s} onClick={()=>setMDiscStage(mDiscStage===s?'전체':s)} style={{background:mDiscStage===s?c:bg,borderRadius:'12px',padding:'14px 8px',textAlign:'center',cursor:'pointer'}}>
                  <div style={{fontSize:'24px',fontWeight:'900',color:mDiscStage===s?'#fff':c}}>{mDiscovery.filter(m=>(m.stage||'접촉중')===s).length}</div>
            ...
            {(mDiscovery.filter(m=>mDiscStage==='전체'||(m.stage||'접촉중')===mDiscStage)).length===0
              ?<div style={{textAlign:'center',padding:'40px',color:C.sub}}>등록된 M발굴 현장이 없습니다</div>
        ...
        {tab === 'qrpartners' && (
          <div>
            ...
            {/* 뷰 전환 */}
            <div style={{display:'flex',gap:'6px',marginBottom:'14px'}}>
              {[['overview','직원별 성과'],['stores','매장 목록'],['leads','상담 내역']].map(([v,label])=>(
