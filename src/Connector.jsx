...
export default function Connector({ onBack }) {
  ...
  const [dashView, setDashView] = useState('전체')  // '전체' | 프로젝트명
  ...
  return (
    <div style={{ minHeight: '100vh', background: C.bg, color: C.text, fontFamily: "'Noto Sans KR',sans-serif", maxWidth: '480px', margin: '0 auto', display: 'flex', flexDirection: 'column' }}>
      ...
      <div ref={scrollRef} style={{ flex: 1, overflowY: 'auto', padding: '14px' }}>
        ...
        {tab === 'dashboard' && (
          <div>
            {/* 뷰 선택: 전체 활동 / 프로젝트별 / 팀별(총괄) */}
            <div style={{ display: 'flex', background: '#f1f5f9', borderRadius: '12px', padding: '4px', marginBottom: '12px', gap: '4px' }}>
              {['전체 활동', '프로젝트별', ...(isSuper?['팀별 현황']:[])].map(v => (
                <button key={v} onClick={() => setDashView(v)}
                  style={{ flex: 1, padding: '9px 6px', background: dashView === v ? '#fff' : 'transparent', border: 'none', color: dashView === v ? C.text : C.sub, borderRadius: '9px', fontSize: '13px', fontWeight: dashView === v ? '800' : '500', cursor: 'pointer', fontFamily: 'inherit', boxShadow: dashView === v ? '0 1px 4px rgba(0,0,0,0.08)' : 'none' }}>{v}</button>
              ))}
            </div>
            ...
            {dashView === '프로젝트별' && (() => {
              ...
              return (
                <div>
                  {projects.length===0
                    ? <div style={{ textAlign:'center',padding:'40px',color:C.sub }}>업로드된 프로젝트가 없습니다</div>
                    : projects.map(pn => {
                        ...
                        const success = pFiltered.filter(l=>l.activity_status==='성공').length
                        ...
                        return (
                          <div key={pn} style={{ background:'#fff',border:'1px solid #e2e8f0',borderLeft:'4px solid '+C.blue,borderRadius:'14px',padding:'16px',marginBottom:'12px',boxShadow:'0 1px 4px rgba(0,0,0,0.04)' }}>
                            ...
                            <div style={{ fontSize:'12px',color:C.sub,marginBottom:'12px' }}>전체 {pl.length}건 · {period} 활동 {pFiltered.length}건</div>
                            <div style={{ display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:'6px',marginBottom:'12px' }}>
                              {[{l:'전체',v:pl.length,c:C.text},{l:'활동',v:done,c:C.blue},{l:'성공',v:success,c:C.green},{l:'미처리',v:pl.length-done,c:C.acc}].map((s,i)=>(
                                <div key={i} style={{ background:'#f8fafc',border:'1px solid #e2e8f0',borderRadius:'8px',padding:'8px',textAlign:'center' }}>
                                  <div style={{ fontSize:'18px',fontWeight:'900',color:s.c }}>{s.v}</div>
                                  <div style={{ fontSize:'10px',color:C.sub,marginTop:'2px' }}>{s.l}</div>
                                </div>
                            ...
                            <div style={{ fontSize:'11px',color:C.sub,marginTop:'4px',display:'flex',justifyContent:'space-between' }}>
                              <span>활동률 {pct(done,Math.max(pl.length,1))}%</span>
                              <span>성공률 {pct(success,Math.max(done,1))}%</span>
                            </div>
                            {themes.length>0 && (
        ...
        {tab === 'activity' && (
          <div>
            ...
            {/* 뷰 선택 */}
            <div style={{ display: 'flex', gap: '4px', background: '#f1f5f9', borderRadius: '12px', padding: '4px', marginBottom: '10px' }}>
              {['전체','팀별','프로젝트별','테마별'].map(v => (
                <button key={v} onClick={() => { setActView(v); setActTeam('전체'); setActProj('전체'); setActTheme('전체') }}
                  style={{ flex: 1, padding: '8px 4px', background: actView === v ? '#fff' : 'transparent', border: 'none', color: actView === v ? C.text : C.sub, borderRadius: '9px', fontSize: '12px', fontWeight: actView === v ? '800' : '500', cursor: 'pointer', fontFamily: 'inherit', boxShadow: actView === v ? '0 1px 4px rgba(0,0,0,0.08)' : 'none' }}>{v}</button>
              ))}
            </div>
            {/* 기간 필터 */}
