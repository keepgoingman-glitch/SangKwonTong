...
.np{padding:0 12px;height:22px;border-radius:20px;background:#e67e00;border:none;color:#fff;font-weight:700;cursor:pointer;font-size:10px}`
  ...
  const mAllD=projects.reduce((a,p)=>a+(p.monthly?.done||0),0)
  const mAllS=projects.reduce((a,p)=>a+(p.monthly?.success||0),0)
  const yAllS=projects.reduce((a,p)=>a+(p.annual?.success||0),0)
  ...
  <div class="sb">
    ...
    <table>
      ...
      <tbody>
        ${projects.map(p=>{
          // 월간만 사용 (전체기간 fallback 제거 - 데이터 혼재 방지)
          const mD=p.monthly?.done||0
          const mS=p.monthly?.success||0
          // allMM weekly로 대체
        ...
        <tr class="tot">
          ...
          <td>${projects.reduce((a,p)=>a+(p.annual?.done||0),0)}건</td><td style="color:#4ade80">${yAllS}건</td>
        </tr>
      </tbody>
    </table>
    ...
    <table>
      ...
      <tbody>
        ...
        <tr class="sub">
          ...
          <td class="bc">${proj1.monthly?.done||0}건</td><td class="gc">${pct(proj1.monthly?.done||0,Math.max(proj1.total||1,1))}%</td>
          <td colspan="3"></td>
          <td class="gc" style="font-size:14px">${proj1.monthly?.success||0}</td><td class="gc">${sucRate(proj1.monthly?.success||0,proj1.monthly?.done||0)}%</td>
