...
function buildSlideHTML({ period, today, yearStr, teams, projects, allMM, topMM,
  ...
  *{margin:0;padding:0;box-sizing:border-box}
  ...
  function momSection(){
    ...
    const bh=(v)=>Math.max(Math.round(v/mx*95),2)
    const grp=(x,label,pv,cv,colPrev,colCur)=>{
      const ph=bh(pv),ch=bh(cv)
      return `
        <rect x="${x}" y="${135-ph}" width="30" height="${ph}" fill="${colPrev}" rx="3"/>
        <rect x="${x+34}" y="${135-ch}" width="30" height="${ch}" fill="${colCur}" rx="3"/>
        <text x="${x+15}" y="${130-ph}" font-size="9.5" fill="#64748b" text-anchor="middle" font-weight="700">${pv}</text>
        <text x="${x+49}" y="${130-ch}" font-size="9.5" fill="${colCur}" text-anchor="middle" font-weight="800">${cv}</text>
        <text x="${x+32}" y="151" font-size="10.5" fill="#334155" text-anchor="middle" font-weight="700">${label}</text>`
    }
    ...
    <div style="border-top:2px dashed #cbd5e1;padding-top:14px;margin-top:14px">
      ...
      <div style="display:flex;gap:14px">
        ...
        <div style="flex:1;background:#f8fafc;border:1.5px solid #e2e8f0;border-radius:12px;padding:12px">
          ...
          <svg width="100%" height="165" viewBox="0 0 320 165">
            <line x1="20" y1="135" x2="305" y2="135" stroke="#cbd5e1" stroke-width="1.5"/>
      ...
      <div style="margin-top:10px;background:${sumBg};border:1px solid ${sumBd};border-radius:8px;padding:8px 14px;font-size:11.5px;color:${sumCol};font-weight:600">
  ...
  <div class="body">
    ...
    <div style="margin-top:10px;font-size:11px;color:#94a3b8">※ 일자별 집계 기준: 배분건은 활동일(activity_at), 직접발굴은 결과입력일 우선 · 성공건은 개통일자 기준으로 반영</div>
    ${momSection()}
  </div>
  ${ftr(3)}
</div>`
