...
function buildSlideHTML({ period, today, yearStr, teams, projects, allMM, topMM,
  ...
  *{margin:0;padding:0;box-sizing:border-box;-webkit-print-color-adjust:exact!important;print-color-adjust:exact!important}
  ...
  <div class="body">
    ...
    <table>
      <thead><tr><th style="width:80px">구분</th><th>인터넷</th><th>TV-M</th><th>TV-B</th><th>모바일</th><th>기타</th><th style="background:#166534">합계</th></tr></thead>
      <tbody>
        ${teams.map((t,i)=>{
          const g=prodGroup(t.monthly?.prodCounts||{})
          return `<tr style="background:${i%2?'#f8fafc':'#fff'}">
            <td style="font-weight:800">${t.label}</td>
            <td class="bc">${g.inet||'-'}</td><td>${g.tv||'-'}</td><td>${g.tvbiz||'-'}</td><td>${g.mob||'-'}</td><td>${g.etc||'-'}</td>
            <td style="background:#f0fdf4;font-weight:900;color:#15803d">${g.total||'-'}</td>
          </tr>`}).join('')}
        <tr style="background:#16213e;color:#fff;font-weight:900">
          <td style="color:#fff">전 체</td>
          <td style="color:#60a5fa">${allProdMonthly.inet||'-'}</td><td style="color:#fff">${allProdMonthly.tv||'-'}</td><td style="color:#fff">${allProdMonthly.tvbiz||'-'}</td><td style="color:#fff">${allProdMonthly.mob||'-'}</td><td style="color:#fff">${allProdMonthly.etc||'-'}</td>
          <td style="background:#166534;color:#fff">${allProdMonthly.total||'-'}</td>
        </tr>
      </tbody>
    </table>
  </div>
