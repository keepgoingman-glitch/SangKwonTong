...
function buildSlideHTML({ period, today, yearStr, teams, projects, allMM, topMM,
  ...
  *{margin:0;padding:0;box-sizing:border-box;-webkit-print-color-adjust:exact!important;print-color-adjust:exact!important}
  ...
  @media print{body{background:#fff;-webkit-print-color-adjust:exact!important;print-color-adjust:exact!important}.slide{margin:0;box-shadow:none}.print-btn{display:none!important};@page{size:1280px 720px;margin:0}}
  `
  ...
  function svgLineChart(data, opts){
    const {w=560,h=138,padL=34,padR=14,padT=20,padB=20} = opts||{}
  ...
  function svgCumChart(data, opts){
    const {w=560,h=138,padL=40,padR=14,padT=20,padB=20} = opts||{}
    if(!data.length) return '<div style="color:#94a3b8;font-size:12px;padding:30px;text-align:center">데이터 없음</div>'
    let ca=0, cs=0
  ...
  <div class="body">
    <div style="display:flex;gap:10px;margin-bottom:10px">
      <div style="flex:1;background:#eff6ff;border:1.5px solid #bfdbfe;border-radius:12px;padding:9px;text-align:center"><div style="font-size:24px;font-weight:900;color:#2563eb">${dailyAct}<span style="font-size:13px">건</span></div><div style="font-size:11px;color:#64748b;margin-top:2px">${isPast?'월마감':'월간'} 총 활동</div></div>
      <div style="flex:1;background:#f0fdf4;border:1.5px solid #bbf7d0;border-radius:12px;padding:9px;text-align:center"><div style="font-size:24px;font-weight:900;color:#16a34a">${dailySuc}<span style="font-size:13px">건</span></div><div style="font-size:11px;color:#64748b;margin-top:2px">${isPast?'월마감':'월간'} 총 성공</div></div>
      <div style="flex:1;background:#fff7ed;border:1.5px solid #fed7aa;border-radius:12px;padding:9px;text-align:center"><div style="font-size:24px;font-weight:900;color:#e67e00">${bestDay?bestDay.day+'일':'-'}</div><div style="font-size:11px;color:#64748b;margin-top:2px">최다 활동일 (${bestDay?.act||0}건)</div></div>
      <div style="flex:1;background:#faf5ff;border:1.5px solid #e9d5ff;border-radius:12px;padding:9px;text-align:center"><div style="font-size:24px;font-weight:900;color:#7c3aed">${allDailyStats.length?Math.round(dailyAct/allDailyStats.length*10)/10:0}<span style="font-size:13px">건</span></div><div style="font-size:11px;color:#64748b;margin-top:2px">일평균 활동</div></div>
    </div>
    <div style="display:flex;gap:14px">
