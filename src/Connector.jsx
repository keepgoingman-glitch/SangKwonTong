...
function buildSlideHTML({ period, today, yearStr, teams, projects, allMM, topMM, mDiscoveryCount, globalProdCounts = {}, globalProdAnnual = {} }) {
  ...
  const PRODS = ['인터넷','TV','다량회선','모바일','일반전화','하이오더','로봇','CCTV']
  const PCOLS  = {인터넷:'#2563eb',TV:'#3b82f6',다량회선:'#22c55e',모바일:'#16a34a',일반전화:'#64748b',하이오더:'#e67e00',로봇:'#7c3aed',CCTV:'#0891b2'}
  const PT_COL  = {1:'#2563eb',2:'#7c3aed',3:'#0891b2',4:'#059669'}
  const PT_LITE = {1:'#eff6ff',2:'#f5f3ff',3:'#ecfeff',4:'#ecfdf5'}
  const PT_BRD  = {1:'#bfdbfe',2:'#c4b5fd',3:'#a5f3fc',4:'#6ee7b7'}
  const PT_TXT  = {1:'#1e40af',2:'#5b21b6',3:'#0e7490',4:'#047857'}
  const PT_ICO  = {1:'🏭',2:'🏗️',3:'🏢',4:'🤝'}
  const PT_LBL  = {1:'기업체 통신환경 개선공사',2:'세움터 신축건물',3:'빌딩/상가 공략',4:'직접 판매'}

  // ── 헬퍼 ──────────────────────────────────────────────
  function donut(act, suc, tot, col, size, sw) {
    size = size||120; sw = sw||20
    const r=(size-sw)/2, c=size/2, ci=2*Math.PI*r
    const aD=pct(act,Math.max(tot,1))/100*ci
    const sD=pct(suc,Math.max(tot,1))/100*ci
    return '<svg width="'+size+'" height="'+size+'" viewBox="0 0 '+size+' '+size+'">'+
      '<circle cx="'+c+'" cy="'+c+'" r="'+r+'" fill="none" stroke="#e8edf2" stroke-width="'+sw+'"/>'+
      '<circle cx="'+c+'" cy="'+c+'" r="'+r+'" fill="none" stroke="'+col+'" stroke-width="'+sw+'" stroke-dasharray="'+aD+' '+(ci-aD)+'" stroke-dashoffset="0" transform="rotate(-90 '+c+' '+c+')" />'+
      '<circle cx="'+c+'" cy="'+c+'" r="'+r+'" fill="none" stroke="#16a34a" stroke-width="'+sw+'" stroke-dasharray="'+sD+' '+(ci-sD)+'" stroke-dashoffset="'+(-aD)+'" transform="rotate(-90 '+c+' '+c+')" />'+
      '</svg>'
  }
  function prodDonut(pc, total2) {
    const r=54, c=70, sw=22, ci=2*Math.PI*r
    const cols=['#2563eb','#3b82f6','#22c55e','#16a34a','#64748b','#e67e00','#7c3aed','#0891b2']
    let s='<svg width="140" height="140" viewBox="0 0 140 140"><circle cx="70" cy="70" r="54" fill="none" stroke="#e8edf2" stroke-width="22"/>', off=0
    PRODS.forEach((p,i)=>{
      if(!pc[p]) return
      const d=pct(pc[p],Math.max(total2,1))/100*ci
      s+='<circle cx="70" cy="70" r="54" fill="none" stroke="'+cols[i%cols.length]+'" stroke-width="22" stroke-dasharray="'+d+' '+(ci-d)+'" stroke-dashoffset="'+(-off)+'" transform="rotate(-90 70 70)"/>'
      off+=d
    })
    return s+'</svg>'
  }
  function bar(w, col, h) { h=h||9; return '<div style="flex:1;background:#e8edf2;border-radius:3px;overflow:hidden"><div style="width:'+Math.min(w,100)+'%;height:'+h+'px;background:'+col+';border-radius:3px"></div></div>' }
  function pd(txt, bg, tc) { return '<span style="background:'+bg+';color:'+tc+';padding:2px 8px;border-radius:5px;font-size:11px;font-weight:700;display:inline-block;margin:2px">'+txt+'</span>' }
  function prodBadges(pc, lim) {
    lim=lim||5
    return PRODS.filter(p=>pc[p]).slice(0,lim).map(p=>pd(p+' '+pc[p], PCOLS[p]+'22', PCOLS[p]||'#475569')).join('')
  }

  // ── 월간/연간 구분 배지 ────────────────────────────────
  function dualBadge(mVal, yVal, unit, mCol, yCol) {
    unit=unit||'건'; mCol=mCol||'#2563eb'; yCol=yCol||'#64748b'
    return '<div style="display:flex;gap:6px;align-items:center;flex-wrap:wrap">'+
      '<div style="background:'+mCol+'18;border:1px solid '+mCol+'44;border-radius:6px;padding:3px 10px;text-align:center">'+
        '<div style="font-size:9px;color:'+mCol+';font-weight:600;margin-bottom:1px">해당월</div>'+
        '<div style="font-size:17px;font-weight:700;color:'+mCol+'">'+mVal+'<span style="font-size:10px">'+unit+'</span></div>'+
      '</div>'+
      '<div style="background:#f1f5f9;border:1px solid #e2e8f0;border-radius:6px;padding:3px 10px;text-align:center">'+
        '<div style="font-size:9px;color:#64748b;font-weight:600;margin-bottom:1px">연간누적</div>'+
        '<div style="font-size:17px;font-weight:700;color:#475569">'+yVal+'<span style="font-size:10px">'+unit+'</span></div>'+
      '</div>'+
    '</div>'
  }
  ...
  // ── 슬라이드 공통 UI ───────────────────────────────────
  const N=10
  const sf = (n) => '<div style="padding:6px 34px;background:#f8fafc;border-top:1px solid #e2e8f0;display:flex;justify-content:space-between;font-size:10px;color:#94a3b8;flex-shrink:0">'+
    '<span>상권통 영업 보고서 · '+period+'</span>'+
    '<span style="color:#e67e00;font-weight:600">기준일: '+today+'</span>'+
    '<span>'+n+' / '+N+'</span></div>'
  const sh = (title, sub, n, accentCol) => '<div style="display:flex;justify-content:space-between;align-items:center;padding:11px 34px 10px;border-bottom:3px solid '+(accentCol||'#e67e00')+';flex-shrink:0">'+
    '<div><div style="font-size:18px;font-weight:700;color:#0f172a">'+title+'</div>'+(sub?'<div style="font-size:11px;color:#64748b;margin-top:1px">'+sub+'</div>':'')+' </div>'+
    '<div style="font-size:10px;color:#94a3b8;text-align:right;line-height:1.8">'+period+' <span style="color:#e67e00">/ '+yearStr+' 누적</span><br/>'+n+' / '+N+'</div></div>'
  const stb = (title, sub, n, col) => '<div style="display:flex;justify-content:space-between;align-items:center;padding:11px 34px 10px;border-bottom:3px solid '+col+';flex-shrink:0">'+
    ...
    '<div style="font-size:10px;color:#94a3b8;text-align:right;line-height:1.8">'+period+' <span style="color:#e67e00">/ '+yearStr+' 누적</span><br/>'+n+' / '+N+'</div></div>'
  const sb = '<div style="padding:14px 34px 16px;flex:1;display:flex;flex-direction:column;gap:11px">'

  // ── 집계 값 ───────────────────────────────────────────
  const mTeams = teams.map(t=>t.monthly||{})
  const yTeams = teams.map(t=>t.annual||{})
  const mTotal = mTeams.reduce((a,t)=>a+(t.success||0),0)
  const yTotal = yTeams.reduce((a,t)=>a+(t.success||0),0)
  const mMitAct= mTeams.reduce((a,t)=>a+(t.mitDone||0),0)
  const yMitAct= yTeams.reduce((a,t)=>a+(t.mitDone||0),0)
  const mMitLd = teams.reduce((a,t)=>a+(t.mitLeads||0),0)

  const css=`*{margin:0;padding:0;box-sizing:border-box}
body{font-family:"Apple SD Gothic Neo","Noto Sans KR",Malgun Gothic,sans-serif;background:#dde3ea;-webkit-print-color-adjust:exact;print-color-adjust:exact}
...
@media screen{body{padding:24px;display:flex;flex-direction:column;align-items:center;gap:24px}.slide{box-shadow:0 12px 48px rgba(0,0,0,0.18)}}
...
.kpi{background:#f8fafc;border:1px solid #e2e8f0;border-radius:10px;padding:11px 10px;text-align:center;position:relative}
...
thead tr{background:#0f172a;color:#fff}th{padding:6px 9px;text-align:left;font-weight:600;font-size:10px;letter-spacing:0.3px}
...
.nav{position:fixed;bottom:14px;left:50%;transform:translateX(-50%);display:flex;gap:4px;background:rgba(10,15,30,0.92);padding:8px 14px;border-radius:50px;z-index:100;backdrop-filter:blur(10px)}
...
.np{padding:0 12px;height:24px;border-radius:20px;background:#e67e00;border:none;color:#fff;font-weight:700;cursor:pointer;font-size:10px}`
  ...
  const s1='<div class="slide" id="s1">'+
    '<div style="position:absolute;inset:0;background:linear-gradient(145deg,#ffffff 0%,#f0f6ff 40%,#e8f0fe 70%,#fef3e8 100%)"></div>'+
    '<div style="position:absolute;top:0;left:0;right:0;height:6px;background:linear-gradient(90deg,#e67e00,#f59e0b,#e67e00)"></div>'+
    '<div style="position:absolute;width:500px;height:500px;border-radius:50%;background:radial-gradient(circle,rgba(230,126,0,0.07) 0%,transparent 70%);top:-130px;right:-100px"></div>'+
    '<div style="position:absolute;width:400px;height:400px;border-radius:50%;background:radial-gradient(circle,rgba(37,99,235,0.06) 0%,transparent 70%);bottom:-100px;left:-60px"></div>'+
    '<div style="position:absolute;inset:0;background-image:linear-gradient(rgba(37,99,235,0.035) 1px,transparent 1px),linear-gradient(90deg,rgba(37,99,235,0.035) 1px,transparent 1px);background-size:30px 30px"></div>'+
    '<div style="position:relative;z-index:10;flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;text-align:center;padding:44px 60px">'+
      '<div style="display:inline-flex;align-items:center;gap:10px;background:rgba(230,126,0,0.1);border:1.5px solid rgba(230,126,0,0.3);padding:7px 20px;border-radius:30px;margin-bottom:24px">'+
        '<div style="width:8px;height:8px;border-radius:50%;background:#e67e00"></div>'+
        '<span style="font-size:12px;font-weight:700;color:#b45309;letter-spacing:3px;text-transform:uppercase">KT 상권통 플랫폼</span>'+
      '</div>'+
      '<div style="font-size:58px;font-weight:700;color:#0f172a;line-height:1.05;margin-bottom:14px">영업 성과 보고서</div>'+
      '<div style="font-size:16px;color:#64748b;margin-bottom:36px;line-height:1.7">4대 프로젝트 기반 영업활동 현황 분석</div>'+
      '<div style="display:inline-flex;align-items:center;gap:8px;background:#fff;border:1.5px solid #e67e00;padding:10px 26px;border-radius:10px;margin-bottom:44px;box-shadow:0 2px 12px rgba(230,126,0,0.12)">'+
        '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#e67e00" stroke-width="2.5" stroke-linecap="round"><rect x="3" y="4" width="18" height="18" rx="2"/><path d="M16 2v4M8 2v4M3 10h18"/></svg>'+
        '<span style="font-size:14px;font-weight:700;color:#e67e00">'+period+' 기준</span>'+
      '</div>'+
      '<div style="display:flex;gap:20px;justify-content:center">'+
      teams.map(t=>'<div style="background:#fff;border:1px solid #e2e8f0;border-radius:14px;padding:18px 32px;text-align:center;box-shadow:0 4px 16px rgba(0,0,0,0.06);min-width:130px">'+
        '<div style="font-size:15px;font-weight:700;color:#0f172a;margin-bottom:4px">'+t.label+'</div>'+
        '<div style="font-size:11px;color:#94a3b8;margin-bottom:10px">MM '+t.mm+'명</div>'+
        '<div style="font-size:36px;font-weight:700;color:#16a34a;line-height:1">'+(t.monthly?.success||0)+'<span style="font-size:14px">건</span></div>'+
        '<div style="font-size:11px;color:#e67e00;font-weight:600;margin-top:3px">해당월</div>'+
        '<div style="font-size:20px;font-weight:700;color:#64748b;margin-top:4px;line-height:1">'+(t.annual?.success||0)+'<span style="font-size:11px">건</span></div>'+
    ...
    '<div style="position:relative;z-index:10;padding:10px 52px;background:rgba(15,23,42,0.04);border-top:1px solid rgba(0,0,0,0.07);display:flex;justify-content:space-between;align-items:center;flex-shrink:0">'+
      ...
      '<div style="font-size:13px;font-weight:700;color:#e67e00;letter-spacing:1px">상권통 SANGKWONTONG</div>'+
      '<div style="font-size:12px;color:#94a3b8">기준일: '+today+'</div>'+
    '</div>'+
  '</div>'

  // ══════════════════════════════════════════════════════
  // 슬라이드 2: 핵심 성과 요약 (월간/연간)
  // ══════════════════════════════════════════════════════
  const mProdE=PRODS.filter(p=>globalProdCounts[p]).map(p=>[p,globalProdCounts[p]])
  const yProdE=PRODS.filter(p=>globalProdAnnual[p]).map(p=>[p,globalProdAnnual[p]])
  const maxPM=mProdE.length>0?Math.max(...mProdE.map(([,v])=>v)):1
  const s2='<div class="slide" id="s2">'+
    sh('핵심 성과 요약','전체 팀 통합 · 해당월 / 연간누적 구분',2)+
    sb+
    // 월간 KPI
    '<div style="background:#fff7ed;border:1px solid #fed7aa;border-radius:10px;padding:10px 14px">'+
      '<div style="font-size:11px;font-weight:700;color:#e67e00;margin-bottom:8px">📅 해당월 실적 ('+period+')</div>'+
      '<div class="g4">'+
      [['#16a34a','총 성공',mTotal+'건',pct(mTotal,Math.max(mMitAct,1))+'% 전환'],
       ['#2563eb','MIT 활동',mMitAct+'건','배분 '+mMitLd+'건 중'],
       ['#e67e00','MIT 활동률',pct(mMitAct,Math.max(mMitLd,1))+'%',mMitAct+' / '+mMitLd+'건'],
       ['#7c3aed','성공 전환율',pct(mTotal,Math.max(mMitAct,1))+'%',mTotal+' / '+mMitAct+'건']
      ].map(([col,lbl,val,sub])=>
        '<div class="kpi"><div class="kt" style="background:'+col+'"></div>'+
        '<div style="font-size:30px;font-weight:700;color:'+col+';line-height:1">'+val+'</div>'+
        '<div style="font-size:11px;color:#64748b;margin-top:3px">'+lbl+'</div>'+
        '<div style="font-size:10px;color:#94a3b8;margin-top:2px">'+sub+'</div></div>'
      ).join('')+
      '</div>'+
    '</div>'+
    // 연간 KPI
    '<div style="background:#f8fafc;border:1px solid #e2e8f0;border-radius:10px;padding:10px 14px">'+
      '<div style="font-size:11px;font-weight:700;color:#64748b;margin-bottom:8px">📊 연간누적 실적 ('+yearStr+')</div>'+
      '<div class="g4">'+
      [['#16a34a','총 성공',yTotal+'건'],
       ['#2563eb','MIT 활동',yMitAct+'건'],
       ['#e67e00','MIT 활동률',pct(yMitAct,Math.max(mMitLd,1))+'%'],
       ['#7c3aed','성공 전환율',pct(yTotal,Math.max(yMitAct,1))+'%']
      ].map(([col,lbl,val])=>
        '<div style="background:#fff;border:1px solid #e2e8f0;border-radius:8px;padding:9px;text-align:center">'+
        '<div style="font-size:24px;font-weight:700;color:'+col+';line-height:1">'+val+'</div>'+
        '<div style="font-size:11px;color:#64748b;margin-top:3px">'+lbl+'</div></div>'
      ).join('')+
      '</div>'+
    '</div>'+
    // 상품별
    '<div style="background:#f8fafc;border:1px solid #e2e8f0;border-radius:10px;padding:12px 16px;flex:1">'+
      '<div style="font-size:13px;font-weight:700;color:#0f172a;margin-bottom:10px">📦 상품별 성공 실적 (해당월 기준)</div>'+
      '<div style="display:grid;grid-template-columns:1fr 1fr;gap:20px;align-items:center">'+
      '<div>'+
      (mProdE.length>0
        ? mProdE.slice(0,6).map(([p,cnt])=>
            '<div style="display:flex;align-items:center;gap:8px;margin-bottom:6px">'+
            '<span style="font-size:12px;font-weight:600;min-width:60px">'+p+'</span>'+
            bar(pct(cnt,maxPM), PCOLS[p]||'#64748b', 16)+
            '<span style="font-size:14px;font-weight:700;color:'+(PCOLS[p]||'#64748b')+';min-width:26px;text-align:right">'+cnt+'</span>'+
            '</div>'
          ).join('')
        : '<div style="text-align:center;padding:16px;color:#94a3b8;font-size:12px">상품 데이터 없음<br/><span style="font-size:10px">활동 결과 입력 시 자동 집계됩니다</span></div>'
      )+
      '</div>'+
      '<div style="display:flex;align-items:center;gap:16px;justify-content:center">'+
        '<div class="dw">'+prodDonut(globalProdCounts,mTotal)+'<div class="dc"><div style="font-size:26px;font-weight:700;color:#0f172a">'+mTotal+'</div><div style="font-size:10px;color:#64748b">해당월</div></div></div>'+
        '<div>'+
        (mProdE.length>0
          ? mProdE.slice(0,6).map(([p,cnt])=>
              '<div style="display:flex;align-items:center;gap:6px;margin-bottom:7px">'+
              '<span style="width:9px;height:9px;border-radius:2px;background:'+(PCOLS[p]||'#64748b')+';display:inline-block"></span>'+
              '<span style="font-size:12px;font-weight:600">'+p+'</span>'+
              '<span style="margin-left:auto;font-size:13px;font-weight:700;color:'+(PCOLS[p]||'#64748b')+';padding-left:12px">'+cnt+'건 ('+pct(cnt,mTotal)+'%)</span>'+
              '</div>'
            ).join('')
  ...
  // ══════════════════════════════════════════════════════
  // 슬라이드 3: 4대 프로젝트 개요 (월간/연간)
  // ══════════════════════════════════════════════════════
  const s3='<div class="slide" id="s3">'+
    sh('4대 프로젝트 성과 개요','프로젝트별 활동 현황 · 해당월 / 연간누적',3)+
    sb+
    '<div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;flex:1">'+
    projects.map(p=>{
      const col=PT_COL[p.id]||'#2563eb', lite=PT_LITE[p.id]||'#eff6ff', brd=PT_BRD[p.id]||'#bfdbfe', txt=PT_TXT[p.id]||'#1e40af'
      const dtot=p.adminOnly?p.total:p.directTotal
      const mDone=p.monthly?.done||p.done||0, mSuc=p.monthly?.success||p.success||0
      const yDone=p.annual?.done||p.done||0, ySuc=p.annual?.success||p.success||0
      return '<div style="background:'+lite+';border:2px solid '+brd+';border-radius:12px;padding:14px">'+
        '<div style="font-size:11px;font-weight:700;color:'+txt+';margin-bottom:8px">'+PT_ICO[p.id]+' '+p.id+'. '+PT_LBL[p.id]+(p.adminOnly?' — 관리자 배분':' — MM 직접발굴')+'</div>'+
        '<div style="display:flex;align-items:center;gap:12px;margin-bottom:10px">'+
          '<div class="dw">'+donut(mDone,mSuc,Math.max(dtot,1),col,96,17)+
            '<div class="dc"><div style="font-size:18px;font-weight:700;color:'+col+'">'+pct(mDone,Math.max(dtot,1))+'%</div><div style="font-size:9px;color:#64748b">활동률</div></div>'+
          '</div>'+
          '<div style="flex:1">'+dualBadge(mSuc, ySuc, '건', '#16a34a')+'<div style="font-size:10px;color:#64748b;margin-top:5px">'+
            '활동: <strong style="color:'+col+'">'+mDone+'</strong> / 연간 <strong style="color:#475569">'+yDone+'</strong>건'+
          '</div></div>'+
        '</div>'+
        '<div style="display:flex;gap:3px;flex-wrap:wrap">'+prodBadges(p.prodCounts||{},4)+'</div>'+
      '</div>'
    }).join('')+
    '</div>'+
    // 집계 바
    (()=>{
      ...
      return '<div style="background:#f8fafc;border:1px solid #e2e8f0;border-radius:8px;padding:9px 16px;display:flex;justify-content:space-around;align-items:center">'+
        '<div style="text-align:center"><div style="font-size:11px;color:#94a3b8">총 배분+직접</div><div style="font-size:20px;font-weight:700;color:#0f172a">'+allTot+'</div></div>'+
        '<div style="width:1px;background:#e2e8f0;height:28px"></div>'+
        '<div style="text-align:center"><div style="font-size:11px;color:#94a3b8">해당월 활동</div><div style="font-size:20px;font-weight:700;color:#2563eb">'+mAllD+'</div></div>'+
        '<div style="width:1px;background:#e2e8f0;height:28px"></div>'+
        '<div style="text-align:center"><div style="font-size:11px;color:#94a3b8">해당월 성공</div><div style="font-size:20px;font-weight:700;color:#16a34a">'+mAllS+'</div></div>'+
        '<div style="width:1px;background:#e2e8f0;height:28px"></div>'+
        '<div style="text-align:center"><div style="font-size:11px;color:#94a3b8">연간누적 성공</div><div style="font-size:20px;font-weight:700;color:#64748b">'+yAllS+'</div></div>'+
        '<div style="width:1px;background:#e2e8f0;height:28px"></div>'+
        '<div style="text-align:center"><div style="font-size:11px;color:#94a3b8">전체 활동률</div><div style="font-size:20px;font-weight:700;color:#e67e00">'+pct(mAllD,Math.max(allTot,1))+'%</div></div>'+
      '</div>'
    })()+
    '</div>'+sf(3)+'</div>'

  // ══════════════════════════════════════════════════════
  // 슬라이드 4~7: 프로젝트 상세 (월간/연간 + 담당 MM 정확히)
  // ══════════════════════════════════════════════════════
  function projDetailSlide(p, sn) {
    ...
    // 담당 MM — project.mmStats 사용 (프로젝트별 정확한 데이터)
    const mmList=p.mmStats||[]

    return '<div class="slide" id="s'+sn+'">'+
      stb(PT_ICO[p.id]+' '+PT_LBL[p.id], (p.adminOnly?'관리자 배분 전용':'MM 직접발굴')+(p.themes&&p.themes.length>0?' · 테마 '+p.themes.length+'개':''), sn, col)+
      sb+
      // KPI 4개 (월간/연간 듀얼)
      '<div class="g4">'+
      [
        [col, p.adminOnly?'MIT 배분':'직접 등록', dtot, '전체 건수', dtot],
        [col, '해당월 활동', mDone, '활동률 '+pct(mDone,Math.max(dtot,1))+'%', mDone],
        ['#16a34a', '해당월 성공', mSuc, '전환율 '+pct(mSuc,Math.max(mDone,1))+'%', mSuc],
        ['#475569', '연간누적 성공', ySuc, '연간 활동 '+yDone+'건', ySuc],
      ].map(([c,lbl,val,sub])=>
        '<div class="kpi" style="background:'+c+'11;border-color:'+c+'33"><div class="kt" style="background:'+c+'"></div>'+
        '<div style="font-size:32px;font-weight:700;color:'+c+';line-height:1">'+val+'<span style="font-size:13px">건</span></div>'+
        '<div style="font-size:11px;color:#374151;margin-top:3px;font-weight:600">'+lbl+'</div>'+
        '<div style="font-size:10px;color:#64748b;margin-top:2px">'+sub+'</div></div>'
      ).join('')+
      '</div>'+
      '<div class="g2" style="flex:1">'+
      // 좌: 도넛 + 상품
      '<div style="background:'+lite+';border:1px solid '+brd+';border-radius:12px;padding:14px">'+
        '<div style="font-size:12px;font-weight:700;color:'+txt+';margin-bottom:10px">활동 현황 · 해당월 기준</div>'+
        '<div style="display:flex;align-items:center;gap:12px;margin-bottom:12px">'+
          '<div class="dw">'+donut(mDone,mSuc,Math.max(dtot,1),col,116,20)+
            '<div class="dc"><div style="font-size:20px;font-weight:700;color:'+col+'">'+pct(mDone,Math.max(dtot,1))+'%</div><div style="font-size:9px;color:#64748b">활동률</div></div>'+
          '</div>'+
          '<div style="flex:1">'+
            dualBadge(mSuc,ySuc,'건 성공','#16a34a')+
            '<div style="font-size:11px;color:#64748b;margin-top:6px">'+
              '활동: 해당월 <strong style="color:'+col+'">'+mDone+'</strong> / 연간 <strong style="color:#475569">'+yDone+'</strong>건<br/>'+
              '미활동 잔여 <strong style="color:#e67e00">'+(dtot-mDone)+'</strong>건'+
            '</div>'+
          '</div>'+
        '</div>'+
        '<div style="font-size:11px;font-weight:700;color:'+txt+';margin-bottom:6px">상품별 성공 실적</div>'+
        (prodE.length>0
          ? prodE.slice(0,5).map(([pp,cnt])=>
              '<div style="display:flex;align-items:center;gap:7px;margin-bottom:5px">'+
              '<span style="font-size:11px;font-weight:600;min-width:56px">'+pp+'</span>'+
              bar(pct(cnt,maxP), PCOLS[pp]||col, 13)+
              '<span style="font-weight:700;color:'+(PCOLS[pp]||col)+';font-size:13px;min-width:20px;text-align:right">'+cnt+'</span>'+
              '</div>'
            ).join('')
          : '<div style="text-align:center;padding:12px;color:#94a3b8;font-size:11px">상품 데이터 없음<br/>활동 결과 입력 시 집계됩니다</div>'
        )+
      '</div>'+
      // 우: 테마 + 담당 MM
      '<div style="display:flex;flex-direction:column;gap:10px">'+
      (p.themes&&p.themes.length>0
        ? '<div style="background:#fff;border:1px solid '+brd+';border-radius:10px;padding:12px">'+
          '<div style="font-size:11px;font-weight:700;color:'+txt+';margin-bottom:8px">🏷️ 테마별 활동 현황</div>'+
          '<div style="border:1px solid '+brd+';border-radius:7px;overflow:hidden">'+
          '<div style="display:grid;grid-template-columns:1fr 44px 44px 44px;background:'+col+';color:#fff;padding:5px 10px;font-size:10px;font-weight:600;gap:6px"><span>테마</span><span style="text-align:center">배분</span><span style="text-align:center">활동</span><span style="text-align:center">성공</span></div>'+
          p.themes.map((t,i)=>'<div style="display:grid;grid-template-columns:1fr 44px 44px 44px;padding:7px 10px;gap:6px;font-size:11px;align-items:center;'+(i%2?'background:#f8fafc':'')+' "><span style="font-weight:600">'+t.name+'</span><span style="text-align:center;color:#64748b">'+t.total+'</span><span style="text-align:center;color:'+col+';font-weight:700">'+t.done+'</span><span style="text-align:center;color:#16a34a;font-weight:700;font-size:13px">'+t.success+'</span></div>').join('')+
          '</div></div>'
        : ''
      )+
      '<div style="background:#fff;border:1px solid '+brd+';border-radius:10px;padding:12px;flex:1">'+
        '<div style="font-size:11px;font-weight:700;color:'+txt+';margin-bottom:8px">👤 담당 MM별 활동 현황</div>'+
        '<div style="font-size:9px;color:#94a3b8;margin-bottom:6px;font-weight:600">해당월 / 연간누적 기준</div>'+
        (mmList.length>0
          ? '<table><thead><tr>'+
              '<th>이름</th><th>팀</th><th>배분</th>'+
              '<th style="color:#60a5fa">해당월<br/>활동</th><th style="color:#4ade80">해당월<br/>성공</th>'+
              '<th style="color:#94a3b8">연간<br/>성공</th>'+
            '</tr></thead><tbody>'+
            mmList.map(m=>{
              const mDoneM=m.monthly?.mitDone||0, mSucM=m.monthly?.success||0, ySucM=m.annual?.success||0
              return '<tr>'+
                '<td><strong style="color:'+(mDoneM===0&&m.mitAssigned>0?'#dc2626':'#0f172a')+'">'+m.name+'</strong></td>'+
                '<td style="font-size:10px;color:#64748b">'+m.team+'</td>'+
                '<td style="color:#64748b">'+m.mitAssigned+'</td>'+
                '<td style="color:#2563eb;font-weight:700">'+mDoneM+'</td>'+
                '<td style="color:#16a34a;font-weight:700">'+mSucM+'</td>'+
                '<td style="color:#64748b;font-weight:600">'+ySucM+'</td>'+
              '</tr>'
            }).join('')+
            '</tbody></table>'+
            (mmList.some(m=>m.mitAssigned>0&&(m.monthly?.mitDone||0)===0)
              ? '<div style="margin-top:6px;background:#fef2f2;border:1px solid #fecaca;border-radius:6px;padding:6px 10px;font-size:10px;color:#991b1b;font-weight:600">⚠ '+mmList.filter(m=>m.mitAssigned>0&&(m.monthly?.mitDone||0)===0).map(m=>m.name).join(', ')+' 미활동</div>'
              : '')
          : '<div style="text-align:center;padding:12px;color:#94a3b8;font-size:11px">담당 MM 없음</div>'
        )+
      '</div>'+
      '</div>'+
      '</div>'+
  ...
  const s4=projDetailSlide(projects[0]||{id:1,total:0,directTotal:0,done:0,success:0,prodCounts:{},themes:[],mmStats:[],monthly:{},annual:{}},4)
  const s5=projDetailSlide(projects[1]||{id:2,total:0,directTotal:0,done:0,success:0,prodCounts:{},themes:[],mmStats:[],monthly:{},annual:{}},5)
  const s6=projDetailSlide(projects[2]||{id:3,total:0,directTotal:0,done:0,success:0,prodCounts:{},themes:[],mmStats:[],monthly:{},annual:{}},6)
  const s7=projDetailSlide(projects[3]||{id:4,total:0,directTotal:0,done:0,success:0,prodCounts:{},themes:[],mmStats:[],monthly:{},annual:{}},7)
  ...
  const s8='<div class="slide" id="s8">'+
    ...
    '<div class="g2">'+
    teams.map((t,ti)=>{
      const bg=ti===0?'#0f172a':'#1e3a5f', pc=t.prodCounts||{}
      const ms=t.monthly||{}, ys=t.annual||{}
      return '<div style="background:#fff;border:1px solid #e2e8f0;border-radius:12px;overflow:hidden">'+
        '<div style="background:'+bg+';color:#fff;padding:10px 16px;display:flex;justify-content:space-between;align-items:center">'+
          '<div><span style="font-size:17px;font-weight:700">'+t.label+'</span><span style="font-size:11px;opacity:0.4;margin-left:8px">MM '+t.mm+'명</span></div>'+
          dualBadge(ms.success||0, ys.success||0, '건 성공').replace('style="display','style="display').replace(/color:#2563eb/g,'color:#4ade80').replace(/border:1px solid #2563eb44/,'border:1px solid rgba(74,222,128,0.3)').replace(/background:#2563eb18/,'background:rgba(74,222,128,0.15)')+
        '</div>'+
        '<div style="padding:12px 16px">'+
        '<div class="g3" style="margin-bottom:10px">'+
          '<div style="text-align:center;background:#f0fdf4;border-radius:8px;padding:8px"><div style="font-size:9px;color:#94a3b8">해당월</div><div style="font-size:20px;font-weight:700;color:#16a34a">'+(ms.success||0)+'</div><div style="font-size:9px;color:#64748b">성공</div></div>'+
          '<div style="text-align:center;background:#eff6ff;border-radius:8px;padding:8px"><div style="font-size:9px;color:#94a3b8">해당월</div><div style="font-size:20px;font-weight:700;color:#2563eb">'+(ms.mitDone||0)+'</div><div style="font-size:9px;color:#64748b">MIT활동</div></div>'+
          '<div style="text-align:center;background:#f1f5f9;border-radius:8px;padding:8px"><div style="font-size:9px;color:#94a3b8">연간누적</div><div style="font-size:20px;font-weight:700;color:#475569">'+(ys.success||0)+'</div><div style="font-size:9px;color:#64748b">성공</div></div>'+
        '</div>'+
        '<div style="font-size:10px;font-weight:700;color:#64748b;margin-bottom:5px">MIT 활동률</div>'+
        '<div style="display:flex;align-items:center;gap:6px;margin-bottom:8px;font-size:11px">'+
          '<span style="min-width:48px;color:#64748b">해당월</span>'+
          bar(pct(ms.mitDone||0, Math.max(t.mitLeads||1,1)), '#2563eb', 8)+
          '<strong style="color:#2563eb;min-width:36px">'+pct(ms.mitDone||0,Math.max(t.mitLeads||1,1))+'%</strong>'+
        '</div>'+
        '<div style="font-size:10px;font-weight:700;color:#64748b;margin-bottom:5px">상품별 성공</div>'+
        '<div style="display:flex;gap:3px;flex-wrap:wrap">'+prodBadges(pc)+'</div>'+
        '</div></div>'
    }).join('')+
    '</div>'+
    // 비교 바
    '<div style="background:#f8fafc;border:1px solid #e2e8f0;border-radius:8px;padding:9px 16px;display:grid;grid-template-columns:1fr 1fr;gap:16px">'+
      '<div><div style="font-size:10px;font-weight:700;color:#64748b;margin-bottom:6px">팀별 MIT 활동률 (해당월)</div>'+
      teams.map(t=>'<div style="display:flex;align-items:center;gap:8px;margin-bottom:5px;font-size:11px"><span style="min-width:50px;font-weight:700">'+t.label+'</span>'+bar(pct(t.monthly?.mitDone||0,Math.max(t.mitLeads||1,1)),'#2563eb',9)+'<strong style="color:#2563eb;padding-left:6px">'+pct(t.monthly?.mitDone||0,Math.max(t.mitLeads||1,1))+'%</strong></div>').join('')+
      '</div>'+
      '<div><div style="font-size:10px;font-weight:700;color:#64748b;margin-bottom:6px">팀별 성공 전환율 (해당월)</div>'+
      teams.map(t=>'<div style="display:flex;align-items:center;gap:8px;margin-bottom:5px;font-size:11px"><span style="min-width:50px;font-weight:700">'+t.label+'</span>'+bar(pct(t.monthly?.success||0,Math.max(t.monthly?.mitDone||1,1)),'#7c3aed',9)+'<strong style="color:#7c3aed;padding-left:6px">'+pct(t.monthly?.success||0,Math.max(t.monthly?.mitDone||1,1))+'%</strong></div>').join('')+
      '</div>'+
    '</div>'+
    '</div>'+sf(8)+'</div>'
  ...
  const s9='<div class="slide" id="s9">'+
    sh('전직원 세부 활동 실적','전원 · 해당월 / 연간누적 구분 · 성공 실적 내림차순',9)+
    sb+
    '<table>'+
    '<thead><tr>'+
      '<th>#</th><th>이름</th><th>팀</th><th>MIT배분</th>'+
      '<th style="background:#1e3a8a;color:#93c5fd">해당월<br/>활동</th>'+
      '<th style="background:#14532d;color:#86efac">해당월<br/>성공</th>'+
      '<th>연간<br/>성공</th>'+
      '<th>직접<br/>발굴</th>'+
      '<th>주요 상품</th>'+
    '</tr></thead><tbody>'+
    sorted.map((m,i)=>{
      ...
      const mSuc=ms.success||m.success||0, ySuc=ys.success||m.success||0
      const mDone=ms.mitDone||m.mitDone||0
      const actR=pct(mDone,Math.max(m.mitAssigned||1,1))
      const isZero=mDone===0&&(m.mitAssigned||0)>0
      return '<tr style="'+(i===0?'background:#fefce8':isZero?'background:#fef2f2':'')+'">'+
        '<td style="font-size:'+(i<3?'15':'12')+'px">'+(medals[i]||'<span style="color:#94a3b8;font-weight:600">'+(i+1)+'</span>')+'</td>'+
        '<td><strong style="font-size:'+(i===0?'13':'11')+'px;color:'+(isZero?'#dc2626':'#0f172a')+'">'+m.name+'</strong></td>'+
        '<td style="font-size:10px;color:#64748b">'+m.team+'</td>'+
        '<td>'+m.mitAssigned+'</td>'+
        '<td style="color:#2563eb;font-weight:700;background:#eff6ff">'+mDone+'</td>'+
        '<td style="color:#16a34a;font-weight:700;font-size:'+(i===0?'17':'13')+'px;background:#f0fdf4">'+mSuc+'</td>'+
        '<td style="color:#64748b;font-weight:600">'+ySuc+'</td>'+
        '<td style="color:'+(ms.directDone>0?'#059669':'#94a3b8')+';font-weight:'+(ms.directDone>0?'700':'400')+'">'+(ms.directDone||0)+'</td>'+
        '<td>'+prodBadges(m.prodCounts||{},3)+'</td>'+
      '</tr>'
    }).join('')+
    '</tbody></table>'+
    (sorted.some(m=>(m.monthly?.mitDone||0)===0&&(m.mitAssigned||0)>0)
      ? '<div style="background:#fef2f2;border:1px solid #fecaca;border-radius:7px;padding:8px 14px;font-size:11px;color:#991b1b;font-weight:600;margin-top:4px">⚠ '+sorted.filter(m=>(m.monthly?.mitDone||0)===0&&(m.mitAssigned||0)>0).map(m=>m.name).join(', ')+' — 해당월 활동 0건. 즉각 확인 필요</div>'
      : '')+
    '</div>'+sf(9)+'</div>'

  // ══════════════════════════════════════════════════════
  // 슬라이드 10: 성과 우수자 (월간/연간)
  // ══════════════════════════════════════════════════════
  ...
  const s10='<div class="slide" id="s10">'+
    ...
    '<div>'+
    '<div style="font-size:11px;font-weight:700;color:#64748b;margin-bottom:8px">TOP 성과자 (해당월 기준)</div>'+
    (top5[0]?'<div style="display:flex;align-items:center;gap:12px;padding:12px;background:#fefce8;border:1.5px solid #fde68a;border-radius:10px;margin-bottom:7px">'+
      '<div style="width:36px;height:36px;border-radius:50%;background:#f59e0b;display:flex;align-items:center;justify-content:center;font-size:20px;flex-shrink:0">🥇</div>'+
      '<div style="flex:1">'+
        '<div style="font-size:15px;font-weight:700">'+top5[0].name+'</div>'+
        '<div style="font-size:10px;color:#64748b;margin-bottom:4px">'+top5[0].team+' · MIT '+(top5[0].monthly?.mitDone||0)+'건 활동 · 직접발굴 '+(top5[0].monthly?.directDone||0)+'건</div>'+
        '<div style="display:flex;gap:3px;flex-wrap:wrap">'+prodBadges(top5[0].prodCounts||{},4)+'</div>'+
      '</div>'+
      dualBadge(top5[0].monthly?.success||0, top5[0].annual?.success||0, '건 성공', '#16a34a')+
    '</div>':'<div style="color:#94a3b8;font-size:12px;padding:12px">성과 데이터 없음</div>')+
    ...
    top5.slice(1).map((m,i)=>'<div style="display:flex;align-items:center;gap:8px;padding:9px;background:#f8fafc;border:1px solid #e2e8f0;border-radius:9px">'+
      '<div style="width:28px;height:28px;border-radius:50%;background:'+(i===0?'#94a3b8':i===1?'#b45309':'#cbd5e1')+';display:flex;align-items:center;justify-content:center;font-size:'+(i<2?'15':'11')+'px;font-weight:700;color:#fff;flex-shrink:0">'+(medals[i+1]||(i+2))+'</div>'+
      '<div style="flex:1;min-width:0">'+
        '<div style="font-size:12px;font-weight:700">'+m.name+'</div>'+
        '<div style="display:flex;gap:2px;margin-top:2px;flex-wrap:wrap">'+prodBadges(m.prodCounts||{},2)+'</div>'+
      '</div>'+
      '<div style="text-align:right;flex-shrink:0">'+
        '<div style="font-size:18px;font-weight:700;color:#16a34a">'+(m.monthly?.success||0)+'</div>'+
        '<div style="font-size:9px;color:#94a3b8">연간 '+(m.annual?.success||0)+'건</div>'+
      '</div>'+
    '</div>').join('')+
    '</div></div>'+
    '<div style="display:flex;flex-direction:column;gap:10px">'+
    '<div style="background:#f8fafc;border:1px solid #e2e8f0;border-radius:10px;padding:12px">'+
    '<div style="font-size:11px;font-weight:700;color:#0f172a;margin-bottom:8px">팀별 달성 현황</div>'+
    teams.map(t=>'<div style="margin-bottom:10px">'+
      '<div style="display:flex;justify-content:space-between;font-size:12px;font-weight:700;margin-bottom:3px">'+
        '<span>'+t.label+'</span>'+
        '<span>해당월 <strong style="color:#16a34a">'+(t.monthly?.success||0)+'건</strong> / 연간 <strong style="color:#64748b">'+(t.annual?.success||0)+'건</strong></span>'+
      '</div>'+
      bar(pct(t.monthly?.success||0,Math.max(mTotal,1)),'#16a34a',10)+
      '<div style="font-size:10px;color:#94a3b8;margin-top:2px">MIT 활동률 '+pct(t.monthly?.mitDone||0,Math.max(t.mitLeads||1,1))+'%</div>'+
    '</div>').join('')+
    '</div>'+
    '<div style="background:#f0fdf4;border:1px solid #bbf7d0;border-radius:10px;padding:12px;flex:1">'+
    ...
    (top5[0]?'<div>'+top5[0].name+' 최고 <strong style="color:#16a34a;font-size:14px">'+(top5[0].monthly?.success||0)+'건</strong></div>':'')+
    '<div>해당월 전체 <strong style="color:#16a34a">'+mTotal+'건</strong> 성공</div>'+
    '<div>연간 누적 <strong style="color:#475569">'+yTotal+'건</strong> 성공</div>'+
    '<div>MIT 활동률 <strong style="color:#e67e00">'+pct(mMitAct,Math.max(mMitLd,1))+'%</strong></div>'+
