                ...
                <button onClick={() => {
                  ...
                  const projData = [1,2,3,4].map(ptId => {
                    ...
                    function pStats(filterFn) {
                      ...
                      // 직접발굴: 등록일(created_at) 기준
                      const fdDone=dl.filter(l=>filterFn(l.created_at))
                    ...
                    const projMmStats = myUsers.map(u => {
                      ...
                      function mms(filterFn) {
                        ...
                        // 직접발굴: 등록일(created_at) 기준
                        const fdDone=udl.filter(l=>filterFn(l.created_at))
                        const allF=[...fpDone,...fdDone]
                        return {
                  ...
                  const allMMData = myUsers.map(u => {
                    ...
                    function mmStats(filterFn) {
                      // 배분: 활동완료된 건, 날짜=activity_at 없으면 created_at
                      const mitDone=uLeads.filter(l=>{
                        if(!l.activity_status||l.activity_status==='미처리') return false
                        return filterFn(l.activity_at||l.created_at)
                      })
                      // 직접발굴: 등록일(created_at) 기준
                      const dirDone=uDirect.filter(l=>filterFn(l.created_at))
                      const allDone=[...mitDone,...dirDone]
                      return {
                        mitDone: mitDone.length,
                        directDone: dirDone.length,
                        actAll: mitDone.length+dirDone.length,
                  ...
                  // 직접발굴: 등록일(created_at) 기준으로 항상 활동
                  ...
                  const teamDataFull = (slideAccs.length>0?slideAccs:[{team_id:adminInfo?.teamId,team_label:adminInfo?.name||'우리팀'}]).map(acc=>{
                    ...
                    function teamStats(filterFn) {
                      ...
                      // 직접발굴: 등록일(created_at) 기준으로 항상 활동 인정
                      const directDoneLeads=td.filter(l=>filterFn(l.created_at))
