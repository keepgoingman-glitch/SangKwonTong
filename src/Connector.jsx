...
function mapRow(headers, row) {
  ...
  return {
    ...
    _col1: headers[0] ? row[headers[0]] || '' : '',
    _col2: headers[1] ? row[headers[1]] || '' : '',
    _col3: headers[2] ? row[headers[2]] || '' : '',
    _col4: headers[3] ? row[headers[3]] || '' : '',
...
export default function Connector({ onBack }) {
  ...
  const handleUpload = async () => {
    ...
    setUploading(true)
    try { for (const row of uploadPreview) { const { _allCols, _col1, _col2, _col3, _col4, ...dbRow } = row; await db.post('connector_leads', dbRow) }; t2(uploadPreview.length + '건 업로드 완료!'); setUploadPreview([]); setCsvHeaders([]); load(); setTab('leads') }
