        ...
        <div style="background:#fff;border:1px solid ${col}44;border-radius:10px;padding:12px;flex:1">
          ...
          ${mm.length?`<table><thead><tr>
            ...
            ${mm.map(m=>{const mDm=m.monthly?.mitDone||0,mSm=m.monthly?.success||0,zero=mDm===0&&m.mitAssigned>0
              ...
              <td class="bc" style="font-size:11px">${m.weekly?.actAll||0}</td><td class="gc" style="font-size:11px">${m.weekly?.success||0}</td>
                ...
                <button onClick={() => {
                  ...
                  const projData = [1,2,3,4].map(ptId => {
                    ...
                    const themes = [...new Set(pl.map(l=>l.project_name).filter(Boolean))].map(tn => {
                      ...
                      return {
                        ...
                        weekly:  { done: tl.filter(l=>isThisWeek(l.activity_at)&&l.activity_status&&l.activity_status!=='미처리').length,
                                   success: tl.filter(l=>isThisWeek(l.activity_at)&&l.activity_status==='성공').length },
                        monthly: { done: tl.filter(l=>isThisMonth(l.activity_at)&&l.activity_status&&l.activity_status!=='미처리').length,
                                   success: tl.filter(l=>isThisMonth(l.activity_at)&&l.activity_status==='성공').length }
                      }
                    })
                    ...
                    const projMmStats = myUsers.map(u => {
                      ...
                      function mms(filterFn) {
                        const fpl = upl.filter(l=>filterFn(l.activity_at))
                        const fdl = udl.filter(l=>filterFn(l.activity_at||l.created_at))
                        return {
                          mitDone:    fpl.filter(l=>l.activity_status&&l.activity_status!=='미처리').length,
                          directDone: fdl.length,
                          actAll:     fpl.filter(l=>l.activity_status&&l.activity_status!=='미처리').length + fdl.length,
                          success:    [...fpl,...fdl].filter(l=>l.activity_status==='성공').length
                        }
                      }
                      return {
                        name:u.name, team:acc2?.team_label||'-',
                        mitAssigned:upl.length, directTotal:udl.length,
                        weekly:mms(isThisWeek), monthly:mms(isThisMonth), annual:mms(isThisYear),
                    ...
                    return {
                      ...
                      weekly:pStats(isThisWeek), monthly:pStats(isThisMonth), annual:pStats(isThisYear),
                  ...
                  const teamDataFull = (slideAccs.length>0?slideAccs:[{team_id:adminInfo?.teamId,team_label:adminInfo?.name||'우리팀'}]).map(acc=>{
                    ...
                    function teamStats(filterFn) {
                      ...
                      const fd=td.filter(l=>filterFn(l.activity_at||l.created_at))
