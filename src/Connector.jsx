            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
              ...
              <div style={{ background: '#fff', border: '1.5px solid #bfdbfe', borderRadius: '14px', padding: '18px', boxShadow: '0 1px 4px rgba(0,0,0,0.04)' }}>
                ...
                <button onClick={() => {
                  ...
                  const _diagDow = _diagNow.getDay()
                  const _diagMon = new Date(_diagNow.getFullYear(),_diagNow.getMonth(),
                    _diagNow.getDate()-((_diagDow+6)%7),0,0,0,0)
                  const _diagSun = new Date(_diagMon.getTime()+7*24*60*60*1000-1)
                  const _myLeads = isSuper ? leads : leads.filter(l=>l.team_id===adminInfo?.teamId)
                  const _myDirect= isSuper ? directLeads : directLeads.filter(l=>users.filter(u=>u.team_id===adminInfo?.teamId).map(u=>u.username).includes(l.mm_username))
                  const _wkFn = dt=>{ if(!dt)return false; try{const t=new Date(dt).getTime();return t>=_diagMon.getTime()&&t<=_diagSun.getTime()}catch{return false} }
                  const _wkL = _myLeads.filter(l=>l.activity_status&&l.activity_status!=='미처리'&&_wkFn(l.activity_at))
                  const _wkD = _myDirect.filter(l=>_wkFn(l.activity_at||l.created_at))
                  const _moL = _myLeads.filter(l=>l.activity_status&&l.activity_status!=='미처리'&&new Date(l.activity_at||'').getMonth()+1===_diagNow.getMonth()+1&&new Date(l.activity_at||'').getFullYear()===_diagNow.getFullYear())
                  const _recent5 = [..._myLeads].filter(l=>l.activity_at).sort((a,b)=>new Date(b.activity_at)-new Date(a.activity_at)).slice(0,5)
                  const _diagMsg = [
                    '=== 주간 계산 진단 ===',
                    '이번 주: '+_diagMon.toLocaleDateString('ko-KR')+' ~ '+_diagSun.toLocaleDateString('ko-KR'),
                    '이번 주 배분 활동건: '+_wkL.length+'건',
                    '이번 주 직접발굴: '+_wkD.length+'건',
                    '이번 달 배분 활동건: '+_moL.length+'건',
                    '전체 배분리드: '+_myLeads.length+'건',
                    '',
                    '최근 activity_at 5건:',
                    ..._recent5.map(l=>'  '+l.assigned_to+' | '+l.activity_at+' | '+l.activity_status),
                    '',
                    _wkL.length+_wkD.length>0?'✅ 이번 주 데이터 존재 → 슬라이드에 반영될 것':'❌ 이번 주 데이터 없음 → activity_at이 이번 주 날짜인 건이 없음'
                  ].join('\n')
                  alert(_diagMsg)
                  // ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
