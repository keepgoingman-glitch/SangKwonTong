...
function buildSlideHTML({ period, today, yearStr, teams, projects, allMM, topMM,
  ...
  const PRODS=['인터넷','TV','다량회선','모바일','일반전화','하이오더','로봇','CCTV']
  const PRODLABEL=p=>p==='다량회선'?'TV Biz':p
  ...
  // 상품 5분류: 인터넷 / TV / TV Biz(다량회선) / 모바일 / 기타
  function prodGroup(pc){
    const inet=pc['인터넷']||0
    const tv=pc['TV']||0
    const tvbiz=pc['다량회선']||0
    const mob=pc['모바일']||0
    const etc=Object.entries(pc).filter(([k])=>!['인터넷','TV','다량회선','모바일'].includes(k)).reduce((a,[,v])=>a+v,0)
  ...
  *{margin:0;padding:0;box-sizing:border-box}
  ...
  <div class="body">
    ...
    <table>
      <thead>
        ...
        <tr>
          <th style="background:#d97706">인터넷</th><th style="background:#d97706">TV</th><th style="background:#d97706">TV Biz</th><th style="background:#d97706">모바일</th><th style="background:#d97706">기타</th>
  ...
  function svgLineChart(data, opts){
    const {w=560,h=170,padL=34,padR=14,padT=24,padB=22} = opts||{}
  ...
  function svgCumChart(data, opts){
    const {w=560,h=170,padL=40,padR=14,padT=24,padB=22} = opts||{}
    if(!data.length) return '<div style="color:#94a3b8;font-size:12px;padding:30px;text-align:center">데이터 없음</div>'
    let ca=0, cs=0
  ...
  <div class="body">
    <div style="display:flex;gap:10px;margin-bottom:14px">
      <div style="flex:1;background:#eff6ff;border:1.5px solid #bfdbfe;border-radius:12px;padding:12px;text-align:center"><div style="font-size:24px;font-weight:900;color:#2563eb">${dailyAct}<span style="font-size:13px">건</span></div><div style="font-size:11px;color:#64748b;margin-top:2px">${isPast?'월마감':'월간'} 총 활동</div></div>
      <div style="flex:1;background:#f0fdf4;border:1.5px solid #bbf7d0;border-radius:12px;padding:12px;text-align:center"><div style="font-size:24px;font-weight:900;color:#16a34a">${dailySuc}<span style="font-size:13px">건</span></div><div style="font-size:11px;color:#64748b;margin-top:2px">${isPast?'월마감':'월간'} 총 성공</div></div>
      <div style="flex:1;background:#fff7ed;border:1.5px solid #fed7aa;border-radius:12px;padding:12px;text-align:center"><div style="font-size:24px;font-weight:900;color:#e67e00">${bestDay?bestDay.day+'일':'-'}</div><div style="font-size:11px;color:#64748b;margin-top:2px">최다 활동일 (${bestDay?.act||0}건)</div></div>
      <div style="flex:1;background:#faf5ff;border:1.5px solid #e9d5ff;border-radius:12px;padding:12px;text-align:center"><div style="font-size:24px;font-weight:900;color:#7c3aed">${allDailyStats.length?Math.round(dailyAct/allDailyStats.length*10)/10:0}<span style="font-size:13px">건</span></div><div style="font-size:11px;color:#64748b;margin-top:2px">일평균 활동</div></div>
    </div>
    <div style="display:flex;gap:14px">
      <div style="flex:1;background:#fff;border:1.5px solid #e2e8f0;border-radius:14px;overflow:hidden">
        <div style="background:#16213e;color:#fff;padding:9px 16px;font-size:13.5px;font-weight:800">일별 활동·성공 건수 (${period.replace(' (월마감)','')})</div>
        <div style="padding:12px 14px 6px">
          <div style="display:flex;gap:14px;margin-bottom:4px;font-size:11.5px">
      ...
      <div style="flex:1;background:#fff;border:1.5px solid #e2e8f0;border-radius:14px;overflow:hidden">
        <div style="background:#16213e;color:#fff;padding:9px 16px;font-size:13.5px;font-weight:800">누적 활동·성공 트렌드 (${period.replace(' (월마감)','')})</div>
        <div style="padding:12px 14px 6px">
          <div style="display:flex;gap:14px;margin-bottom:4px;font-size:11.5px">
    ...
    <div style="margin-top:12px;font-size:11px;color:#94a3b8">※ 일자별 집계 기준: 배분건은 활동일(activity_at), 직접발굴은 결과입력일 우선 · 성공건은 개통일자 기준으로 반영</div>
  </div>
  ${ftr(3)}
</div>`
  ...
  <div class="body">
    <table>
      <thead>
        ...
        <tr>
          ...
          <th style="background:#d97706">인터넷</th><th style="background:#d97706">TV</th><th style="background:#d97706">TV Biz</th><th style="background:#d97706">모바일</th><th style="background:#d97706">기타</th><th style="background:#92400e">합계</th>
        </tr>
      </thead>
      <tbody>
        ...
        <tr style="font-weight:900">
          ...
          <td style="background:#16213e;color:#fff">${projRows.reduce((a,r)=>a+(r.p.adminOnly?(r.p.total||0):(r.p.directTotal||0)),0)}건</td>
    ...
    <div style="margin-top:8px;font-size:11px;color:#94a3b8">※ 상품별 실적 = <b style="color:#15803d">월간</b> 성공건의 수주 상품·회선수 · TV Biz=舊 다량회선 · 회선수 미입력 건은 상품당 1회선</div>
    ...
    <div style="display:flex;gap:14px">
      <div style="flex:1.4;overflow:hidden">
        ...
        <div style="display:flex;gap:8px;flex-wrap:wrap">
          ...
          <div style="background:#fef3c7;border:1px solid #fde68a;border-radius:10px;padding:9px 15px;text-align:center;min-width:78px"><div style="font-size:19px;font-weight:900;color:#d97706">${pg.tv}</div><div style="font-size:10.5px;color:#64748b">TV</div></div>
          <div style="background:#fff7ed;border:1px solid #fed7aa;border-radius:10px;padding:9px 15px;text-align:center;min-width:78px"><div style="font-size:19px;font-weight:900;color:#ea580c">${pg.tvbiz}</div><div style="font-size:10.5px;color:#64748b">TV Biz</div></div>
