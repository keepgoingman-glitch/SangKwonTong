...
function buildSlideHTML({ period, today, yearStr, teams, projects, allMM, topMM,
  ...
  *{margin:0;padding:0;box-sizing:border-box;-webkit-print-color-adjust:exact!important;print-color-adjust:exact!important}
  ...
  function mPlusAggForP4(){
    ...
    sites.forEach(m=>{try{const recs=JSON.parse(m.daily_records||'[]');recs.forEach(r=>{net+=Number(r.net||0);tv+=Number(r.tv||0);renew+=Number(r.renew||0);mob+=Number(r.wireChange||0)+Number(r.mnp||0)+Number(r.second||0)})}catch{}})
    ...
    const pg={inet:net, tv:tv, tvbiz:0, mob:mob, etc:renew, total:net+tv+mob+renew}
    return { siteCount:sites.length, active, success, pg }
  }
