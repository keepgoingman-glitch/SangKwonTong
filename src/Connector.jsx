...
export default function Connector({ onBack }) {
  ...
  const handleSaveTeamAdmin = (teamId, form) => {
    ...
    setTeamAccounts(updated); saveTeamAccounts(updated); setEditTeam(null); t2('팀관리자 계정 수정 완료!')
  }
  ...
  const handleUpload = async () => {
    ...
    setUploading(true)
    try {
      for (const row of uploadPreview) { const { _headers, ...dbRow } = row; await db.post('connector_leads', dbRow) }
  ...
  return (
    <div style={{ minHeight: '100vh', background: C.bg, color: C.text, fontFamily: "'Noto Sans KR',sans-serif", maxWidth: '480px', margin: '0 auto', display: 'flex', flexDirection: 'column' }}>
      ...
      {/* 헤더 */}
      <div style={{ background: '#fff', padding: '10px 16px 8px', borderBottom: '1px solid #e2e8f0', position: 'sticky', top: 0, zIndex: 50, boxShadow: '0 2px 8px rgba(0,0,0,0.06)' }}>
          ...
          <div>
            ...
            {isSuper && (
              <div style={{ background: C.blueBg, border: '1px solid #bfdbfe', borderRadius: '12px', padding: '14px', marginBottom: '16px' }}>
                <div style={{ fontSize: '13px', fontWeight: '800', color: C.blue, marginBottom: '10px' }}>🔑 팀관리자 계정 (총괄 전용)</div>
                {Object.entries(teamAccounts).map(([tid, ta]) => (
                  <div key={tid} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '8px' }}>
                    <div><span style={{ fontSize: '14px', fontWeight: '700', color: C.text }}>{ta.name}</span><span style={{ color: C.sub, fontSize: '13px', marginLeft: '8px' }}>ID: {tid}</span></div>
                    <button onClick={() => setEditTeam(tid)} style={{ background: '#fff', border: '1px solid #bfdbfe', color: C.blue, padding: '5px 12px', borderRadius: '8px', fontSize: '13px', cursor: 'pointer', fontFamily: 'inherit' }}>수정</button>
                  </div>
                ))}
              </div>
            )}
            ...
            {uploadPreview.length > 0 && (
              <div>
                ...
                <div style={{ display: 'flex', gap: '10px', marginTop: '12px' }}>
                  ...
                  <button onClick={handleUpload} disabled={uploading} style={{ flex: 2, padding: '13px', background: uploading ? '#e2e8f0' : C.acc, border: 'none', color: uploading ? C.sub : '#fff', borderRadius: '10px', fontSize: '15px', fontWeight: '900', cursor: 'pointer', fontFamily: 'inherit' }}>{uploading ? '업로드 중...' : `${uploadPreview.length}건 업로드`}</button>
                </div>
              </div>
            )}
          </div>
      ...
      {/* 하단 탭 */}
      <div style={{ background: '#fff', borderTop: '1px solid #e2e8f0', display: 'flex', overflowX: 'auto', padding: '2px 0 0', boxShadow: '0 -2px 8px rgba(0,0,0,0.05)' }}>
        {navTabs.map(t => (
          <button key={t.id} onClick={() => setTab(t.id)} style={{ flex: '0 0 auto', background: 'none', border: 'none', color: tab === t.id ? C.acc : '#94a3b8', cursor: 'pointer', padding: '5px 10px 4px', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '2px', fontFamily: 'inherit' }}>
