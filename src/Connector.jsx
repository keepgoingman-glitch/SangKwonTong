...
function mapRow(headers, row) {
  // A~E열(1~5번째 컬럼)을 그대로 col1~col5에 저장
...
export default function Connector({ onBack }) {
  ...
  const load = async (info) => {
    ...
    try {
      ...
      const lq = (ai?.role === 'team' && ai?.teamId)
        ? 'team_id=eq.' + ai.teamId + '&order=created_at.desc'
        : 'order=created_at.desc'
      const mdq = (ai?.role === 'team' && ai?.teamId)
        ? 'team_id=eq.' + ai.teamId + '&order=created_at.desc'
        : 'order=created_at.desc'
  ...
  const handleBulkDelete = async () => {
    ...
    if (!window.confirm(`선택한 ${selectedLeads.size}건을 삭제하시겠습니까?\n배분된 MM 플랫폼에서도 즉시 사라집니다.`)) return
    try {
      for (const id of selectedLeads) await db.del('connector_leads', 'id=eq.' + id)
      t2(selectedLeads.size + '건 삭제 완료'); setSelectedLeads(new Set()); setSelectMode(false); load()
    } catch { t2('삭제 오류') }
  }
  ...
  const handleUpload = async () => {
    ...
    setUploading(true)
    try { for (const row of uploadPreview) { const { _headers, ...dbRow } = row; await db.post('connector_leads', { ...dbRow, team_id: adminInfo?.teamId || '' }) }; t2(uploadPreview.length + '건 업로드 완료!'); setUploadPreview([]); setCsvHeaders([]); load(); setTab('leads') }
                  ...
                  <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap' }}>
                    {[r.col1,r.col2,r.col3,r.col4,r.col5].map((v, ci) => (
                      <div key={ci} style={{ flex: ci < 2 ? '1 1 44%' : '1 1 28%', minWidth: '70px', overflow: 'hidden' }}>
                        <div style={{ fontSize: '10px', color: 'rgba(255,255,255,0.4)', marginBottom: '2px', fontWeight: '600' }}>
                          {r._headers?.[ci] || `${ci+1}열`}
                        </div>
            ...
            <div style={{ marginBottom: '12px' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '8px' }}>
                ...
                <div style={{ display: 'flex', gap: '6px' }}>
                  {unassigned > 0 && !selectMode && <button onClick={bulkAssign} style={{ background: 'linear-gradient(135deg,#f59e0b,#d97706)', border: 'none', color: '#0a0f1e', padding: '8px 12px', borderRadius: '8px', fontSize: '13px', fontWeight: '900', cursor: 'pointer', fontFamily: 'inherit' }}>자동배분({unassigned})</button>}
                  {isSuper && !selectMode && <button onClick={() => { setSelectMode(true); setSelectedLeads(new Set()) }} style={{ background: 'rgba(248,113,113,0.1)', border: '1px solid rgba(248,113,113,0.3)', color: '#f87171', padding: '8px 12px', borderRadius: '8px', fontSize: '13px', fontWeight: '700', cursor: 'pointer', fontFamily: 'inherit' }}>☑ 선택삭제</button>}
                  {isSuper && selectMode && <>
                    <button onClick={() => { const allIds = new Set(filtered.map(l => l.id)); setSelectedLeads(selectedLeads.size === filtered.length ? new Set() : allIds) }} style={{ background: 'rgba(255,255,255,0.08)', border: '1px solid rgba(255,255,255,0.2)', color: C.text, padding: '8px 12px', borderRadius: '8px', fontSize: '13px', fontWeight: '700', cursor: 'pointer', fontFamily: 'inherit' }}>
                      {selectedLeads.size === filtered.length ? '전체해제' : '전체선택'}
                    </button>
                    <button onClick={handleBulkDelete} disabled={selectedLeads.size === 0} style={{ background: selectedLeads.size > 0 ? 'linear-gradient(135deg,#ef4444,#dc2626)' : 'rgba(255,255,255,0.07)', border: 'none', color: selectedLeads.size > 0 ? '#fff' : '#475569', padding: '8px 12px', borderRadius: '8px', fontSize: '13px', fontWeight: '900', cursor: selectedLeads.size > 0 ? 'pointer' : 'not-allowed', fontFamily: 'inherit' }}>
                      {selectedLeads.size > 0 ? `🗑 ${selectedLeads.size}건 삭제` : '선택 없음'}
                    </button>
                    <button onClick={() => { setSelectMode(false); setSelectedLeads(new Set()) }} style={{ background: 'transparent', border: '1px solid rgba(255,255,255,0.15)', color: C.sub, padding: '8px 10px', borderRadius: '8px', fontSize: '13px', cursor: 'pointer', fontFamily: 'inherit' }}>취소</button>
                  </>}
                </div>
              </div>
              {selectMode && <div style={{ background: 'rgba(239,68,68,0.08)', border: '1px solid rgba(239,68,68,0.2)', borderRadius: '8px', padding: '8px 12px', fontSize: '13px', color: '#f87171' }}>
                {selectedLeads.size > 0 ? `${selectedLeads.size}건 선택됨` : '삭제할 항목을 선택하세요 (전체선택 또는 개별 선택)'}
              </div>}
            </div>
                ...
                <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap', marginBottom: '8px' }}>
                  {[l.col1,l.col2,l.col3,l.col4,l.col5].map((v, i) => v ? (
                    <div key={i} style={{ flex: i < 2 ? '1 1 44%' : '1 1 28%', minWidth: '70px', overflow: 'hidden' }}>
                      <div style={{ fontSize: '10px', color: 'rgba(255,255,255,0.35)', marginBottom: '2px' }}>{['A','B','C','D','E'][i]}열</div>
