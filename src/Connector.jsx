...
// ── 관리자 계정 정의 ─────────────────────────────────────────────
// role: 'super' = 총괄관리자, 'team' = 일반관리자
const ADMINS = {
  admin:  { pw: '2026', role: 'super', name: '총괄관리자', teamId: null },
  team1:  { pw: '2026', role: 'team',  name: '1팀 관리자', teamId: 'team1' },
  team2:  { pw: '2026', role: 'team',  name: '2팀 관리자', teamId: 'team2' },
  team3:  { pw: '2026', role: 'team',  name: '3팀 관리자', teamId: 'team3' },
  team4:  { pw: '2026', role: 'team',  name: '4팀 관리자', teamId: 'team4' },
}
...
function mapRow(headers, row) {
  const find = (...cands) => {
    for (const c of cands) { const k = headers.find(k => k.includes(c)); if (k) return row[k] || '' }
    return ''
  }
  return {
        ...
        {isSuper && (
          <div style={{ marginBottom: '11px' }}>
            ...
            <select value={form.team_id} onChange={e => set('team_id', e.target.value)}
              style={{ ...IS, color: form.team_id ? '#f1f5f9' : '#8aa3bc' }}>
...
function EditMMModal({ user, onClose, onSave, isSuper }) {
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.78)', display: 'flex', alignItems: 'flex-end', justifyContent: 'center', zIndex: 998, backdropFilter: 'blur(4px)' }}>
      <div style={{ background: 'linear-gradient(170deg,#0d1f35,#0a1628)', borderRadius: '20px 20px 0 0', padding: '24px 20px 36px', width: '100%', maxWidth: '480px', border: '1px solid rgba(255,255,255,0.1)', position: 'relative' }}>
        <button onClick={onClose} style={{ position: 'absolute', top: '16px', right: '16px', background: 'transparent', border: 'none', color: 'rgba(255,255,255,0.4)', fontSize: '22px', cursor: 'pointer' }}>✕</button>
        <h3 style={{ fontSize: '20px', fontWeight: '900', marginBottom: '16px' }}>계정 수정 — {user.name}</h3>
...
export default function Connector({ onBack }) {
  ...
  const [adminInfo, setAdminInfo] = useState(null) // { role, name, teamId }
  ...
  const load = async (info) => {
    ...
    setLoading(true)
    try {
      // 일반관리자는 자기 팀 MM만, 총괄은 전체
      const uQuery = (ai?.role === 'team' && ai?.teamId)
        ? 'team_id=eq.' + ai.teamId + '&order=created_at.asc'
        : 'order=created_at.asc'
      const [u, l] = await Promise.all([
        db.get('mm_users', uQuery),
  ...
  const handleLogin = () => {
    const a = ADMINS[lid]
    if (a && a.pw === lpw) {
      const info = { role: a.role, name: a.name, teamId: a.teamId }
  ...
  const handleFile = e => {
    ...
    reader.onload = ev => {
      ...
      // address가 없어도 전체 데이터 표시 (주소 컬럼 없어도 업로드 가능)
  ...
  const handleUpload = async () => {
    ...
    try {
      for (const row of uploadPreview) {
        const { _allCols, ...dbRow } = row
        await db.post('connector_leads', dbRow)
      }
  ...
  const downloadPerf = () => dl('상권통_성과현황', [['이름', '아이디', '팀', '지역', '성공', '목표', '달성률']].concat(users.map(u => [u.name, u.username, u.team_id || '-', u.region, u.success, u.goal, Math.round(u.success / u.goal * 100) + '%'])))
  const downloadActivity = () => dl('상권통_활동결과', [['주소', '배분MM', '배분상태', '활동상태', '결과요약', '메모', '연락처']].concat(leads.map(l => [l.address, l.assigned_to, l.assign_status, l.activity_status || '미처리', l.activity_result || '', l.activity_memo || '', l.activity_contact || ''])))
  ...
  if (screen === 'login') return (
    <div style={{ minHeight: '100vh', background: 'linear-gradient(170deg,#06111f 0%,#0a1f3d 40%,#0d1a35 70%,#06111f 100%)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '30px 28px', fontFamily: "'Noto Sans KR',sans-serif", color: C.text }}>
      ...
      <div style={{ width: '100%', maxWidth: '360px', display: 'flex', flexDirection: 'column', gap: '12px' }}>
        <input type="text" placeholder="아이디 (admin / team1~4)" value={lid} onChange={e => { setLid(e.target.value); setLerr('') }} onKeyDown={e => e.key === 'Enter' && handleLogin()} style={{ ...IS, padding: '16px', fontSize: '18px' }} />
      ...
      <div style={{ marginTop: '24px', background: 'rgba(245,158,11,0.06)', border: '1px solid rgba(245,158,11,0.18)', borderRadius: '10px', padding: '14px 18px', textAlign: 'center' }}>
        <div style={{ color: C.acc, fontSize: '13px', fontWeight: '700', marginBottom: '6px' }}>계정 안내</div>
        <div style={{ color: C.sub, fontSize: '13px', lineHeight: 1.8 }}>
          총괄: admin / 2026<br />
          팀관리자: team1~4 / 2026
        </div>
      </div>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.75)', display: 'flex', alignItems: 'flex-end', justifyContent: 'center', zIndex: 998, backdropFilter: 'blur(4px)' }}>
          <div style={{ background: 'linear-gradient(170deg,#0d1f35,#0a1628)', borderRadius: '20px 20px 0 0', padding: '24px 20px 36px', width: '100%', maxWidth: '480px', border: '1px solid rgba(255,255,255,0.1)', position: 'relative' }}>
            ...
            {assignModal.assign_status === '배분완료' && (
              <div style={{ background: 'rgba(245,158,11,0.08)', border: '1px solid rgba(245,158,11,0.2)', borderRadius: '8px', padding: '8px 12px', marginBottom: '12px', fontSize: '13px', color: C.acc }}>현재: {assignModal.assigned_to} MM</div>
            )}
            <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', marginBottom: '16px', maxHeight: '280px', overflowY: 'auto' }}>
              {users.map(u => (
                <button key={u.id} onClick={() => setSelMM(u.name)}
                  style={{ padding: '13px 16px', background: selMM === u.name ? 'rgba(245,158,11,0.18)' : 'rgba(255,255,255,0.05)', border: selMM === u.name ? '1.5px solid ' + C.acc : '1px solid rgba(255,255,255,0.1)', borderRadius: '12px', color: C.text, fontSize: '15px', fontWeight: '700', cursor: 'pointer', textAlign: 'left', fontFamily: 'inherit', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <span>{u.name} · {u.region}</span>
                  <span style={{ color: C.sub, fontSize: '12px' }}>{TEAMS.find(t => t.id === u.team_id)?.label || ''} · 성공 {u.success}건</span>
                </button>
              ))}
            </div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
            ...
            <div>
              <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                <div style={{ fontSize: '17px', fontWeight: '900', color: C.acc, lineHeight: 1 }}>관리자</div>
        ...
        {tab === 'overview' && (
          <div>
            ...
            {users.map(u => {
              ...
              const teamLabel = TEAMS.find(t => t.id === u.team_id)?.label || ''
              return (
                <div key={u.id} style={{ background: 'rgba(13,31,53,0.8)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: '14px', padding: '16px', marginBottom: '10px' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '10px' }}>
                    <div>
                      <div style={{ fontSize: '17px', fontWeight: '800', display: 'flex', alignItems: 'center', gap: '8px' }}>
                        {u.name} MM
                        {teamLabel && <span style={{ background: 'rgba(96,165,250,0.15)', color: '#60a5fa', fontSize: '11px', fontWeight: '700', padding: '2px 8px', borderRadius: '10px' }}>{teamLabel}</span>}
                      </div>
                  ...
                  <div style={{ display: 'flex', gap: '8px' }}>
                    ...
                    <button onClick={() => saveGoal(u.id)}
                      style={{ background: savedGoal === u.id ? '#4ade80' : C.acc, border: 'none', color: '#0a0f1e', padding: '10px 14px', borderRadius: '8px', fontSize: '13px', fontWeight: '800', cursor: 'pointer', fontFamily: 'inherit', whiteSpace: 'nowrap' }}>
        ...
        {tab === 'staff' && (
          <div>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '14px' }}>
              <div>
                <h2 style={{ fontSize: '19px', fontWeight: '900', margin: '0 0 3px', color: C.acc }}>직원 관리</h2>
                <p style={{ color: C.sub, fontSize: '13px', margin: 0 }}>{isSuper ? '전체 MM 계정 관리' : adminInfo?.name + ' 팀 MM 관리'}</p>
              </div>
        ...
        {tab === 'upload' && (
          <div>
            ...
            <div onClick={() => fileRef.current.click()}
              style={{ background: 'rgba(245,158,11,0.05)', border: '2px dashed rgba(245,158,11,0.35)', borderRadius: '14px', padding: '28px 20px', textAlign: 'center', cursor: 'pointer', marginBottom: '14px' }}>
              ...
              <div style={{ background: 'rgba(13,31,53,0.8)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: '12px', padding: '14px', marginBottom: '14px' }}>
                ...
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px' }}>
                  {csvHeaders.slice(0, 5).map((h, i) => (
                    <span key={i} style={{ background: 'rgba(245,158,11,0.12)', color: C.acc, border: '1px solid rgba(245,158,11,0.25)', fontSize: '12px', fontWeight: '700', padding: '4px 10px', borderRadius: '6px' }}>
                      {i + 1}. {h}
                    </span>
                  ))}
            ...
            {uploadPreview.length > 0 && (
              <div>
                <div style={{ fontSize: '14px', fontWeight: '800', marginBottom: '10px', color: '#4ade80' }}>✅ {uploadPreview.length}건 인식됨 — 미리보기</div>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '6px 12px' }}>
                      {r._allCols && r._allCols.slice(0, 6).map((col, ci) => (
                        <div key={ci} style={{ overflow: 'hidden' }}>
                          ...
                          <div style={{ fontSize: '14px', fontWeight: '700', color: ci === 0 ? C.acc : C.text, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                            {col.val || <span style={{ color: 'rgba(255,255,255,0.2)' }}>-</span>}
                          </div>
                        </div>
                      ))}
                    </div>
            ...
            {filtered.map(l => (
              <div key={l.id} style={{ background: 'rgba(13,31,53,0.8)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: '14px', padding: '14px', marginBottom: '10px' }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '4px 12px', marginBottom: '8px' }}>
                  <div>
                    <div style={{ fontSize: '11px', color: 'rgba(255,255,255,0.3)' }}>구분/유형</div>
                    <div style={{ fontSize: '14px', fontWeight: '800', color: C.acc }}>{l.col1 || l.usage_type || '-'}</div>
                  </div>
                  <div>
                    <div style={{ fontSize: '11px', color: 'rgba(255,255,255,0.3)' }}>주소</div>
                    <div style={{ fontSize: '13px', fontWeight: '700', color: C.text, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{l.address || l.col2 || '-'}</div>
                  </div>
                  <div>
                    <div style={{ fontSize: '11px', color: 'rgba(255,255,255,0.3)' }}>일자</div>
                    <div style={{ fontSize: '13px', fontWeight: '600', color: C.sub }}>{l.start_date || l.col3 || '-'}</div>
                  </div>
                  <div>
                    <div style={{ fontSize: '11px', color: 'rgba(255,255,255,0.3)' }}>상품/기타</div>
                    <div style={{ fontSize: '13px', fontWeight: '600', color: C.sub }}>{l.col4 || l.usage_type || '-'}</div>
                  </div>
                </div>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '8px' }}>
                  ...
                  <span style={{ marginLeft: 'auto', background: l.assign_status === '미배분' ? '#2d1d00' : '#052e16', color: l.assign_status === '미배분' ? '#fbbf24' : '#4ade80', border: l.assign_status === '미배분' ? '1px solid #451a03' : '1px solid #14532d', fontSize: '11px', fontWeight: '700', padding: '4px 10px', borderRadius: '20px', whiteSpace: 'nowrap' }}>{l.assign_status}</span>
                </div>
          ...
          <div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
              <div style={{ background: 'rgba(13,31,53,0.8)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: '14px', padding: '18px' }}>
                ...
                <p style={{ color: C.sub, fontSize: '13px', marginBottom: '14px', lineHeight: '1.6' }}>이름 · 아이디 · 팀 · 지역 · 성공건수 · 목표 · 달성률</p>
                <button onClick={downloadPerf} style={{ width: '100%', background: 'linear-gradient(135deg,#f59e0b,#d97706)', border: 'none', color: '#0a0f1e', padding: '13px', borderRadius: '10px', fontSize: '15px', fontWeight: '900', cursor: 'pointer', fontFamily: 'inherit' }}>📥 성과현황 ({users.length}명)</button>
              </div>
              <div style={{ background: 'rgba(13,31,53,0.8)', border: '1px solid rgba(255,255,255,0.07)', borderRadius: '14px', padding: '18px' }}>
                <div style={{ fontSize: '16px', fontWeight: '800', marginBottom: '6px' }}>📋 영업기회 활동결과</div>
                <p style={{ color: C.sub, fontSize: '13px', marginBottom: '14px', lineHeight: '1.6' }}>주소 · 배분MM · 활동상태 · 결과요약 · 메모 · 연락처</p>
                <button onClick={downloadActivity} style={{ width: '100%', background: 'linear-gradient(135deg,#10b981,#059669)', border: 'none', color: '#fff', padding: '13px', borderRadius: '10px', fontSize: '15px', fontWeight: '900', cursor: 'pointer', fontFamily: 'inherit' }}>📥 활동결과 ({leads.length}건)</button>
              </div>
            </div>
          </div>
        )}
