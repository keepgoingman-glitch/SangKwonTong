...
function buildSlideHTML({ period, today, yearStr, teams, projects, allMM, topMM,
  ...
  *{margin:0;padding:0;box-sizing:border-box;-webkit-print-color-adjust:exact!important;print-color-adjust:exact!important}
  ...
  function mPlusSiteSlide(sn){
    const sites = mDiscoveryList||[]
    // 당월 판정용 (psAgencySlide 와 동일 기준, 스코프 독립)
    ...
    const rows = sites.map(m=>{
      const A=aggOf(m,false), M=aggOf(m,true)
      return { name:m.customer_name||m.site_name||'-', stage:m.stage||'접촉중', emp:m.emp_count||'-',
        agency:m.agency_name||'', agencyMgr:m.agency_manager||'', schedule:m.sales_schedule||'',
        industry:m.industry||'', mm:m.registered_by||'', all:A, mon:M }
    }).filter(r=>r.all.total>0 || ['영업중','영업종료'].includes(r.stage))
      .sort((a,b)=>b.all.total-a.all.total)
    const sumOf=k=>rows.reduce((a,r)=>({net:a.net+r[k].net,tv:a.tv+r[k].tv,renew:a.renew+r[k].renew,chg:a.chg+r[k].chg,mnp:a.mnp+r[k].mnp,sec:a.sec+r[k].sec,wire:a.wire+r[k].wire,mob:a.mob+r[k].mob,total:a.total+r[k].total}),{net:0,tv:0,renew:0,chg:0,mnp:0,sec:0,wire:0,mob:0,total:0})
    const sumA=sumOf('all'), sumM=sumOf('mon')
    const stageColor=s=>({'접촉중':'#0891b2','발굴성공':'#7c3aed','영업중':'#d97706','영업종료':'#16a34a','발굴실패':'#dc2626'}[s]||'#64748b')
    ...
    const funnel=stages.map(st=>{
      ...
      return `<div style="flex:1;text-align:center">
        <div style="font-size:20px;font-weight:900;color:${stageColor(st)}">${c}</div>
        <div style="font-size:10px;color:#64748b;margin-bottom:3px">${st}</div>
        <div style="height:5px;background:#eef2f7;border-radius:3px;overflow:hidden"><div style="width:${pct}%;height:100%;background:${stageColor(st)}"></div></div>
    ...
    return `<div class="slide" id="s${sn}">
  ${rh('🔍 M플러스 영업구간별 실적','산단·협단체 임직원 모바일 벌크 — 진행단계 · 상품별 상세',sn)}
  ${dbar}
  <div class="body">
    <div style="display:flex;gap:8px;margin-bottom:9px">
      <div style="flex:1;background:#f8fafc;border:1.5px solid #e2e8f0;border-radius:12px;padding:9px 12px">
        <div style="font-size:11px;font-weight:800;color:#16213e;margin-bottom:6px">▶ 진행단계 현황 <span style="color:#94a3b8;font-weight:400">(총 ${totalSites}개 현장)</span></div>
        <div style="display:flex;gap:6px">${funnel}</div>
      </div>
    </div>
    <div style="display:flex;gap:8px;margin-bottom:10px">
      <div style="flex:1;background:#eef2ff;border:1.5px solid #c7d2fe;border-radius:12px;padding:9px;text-align:center"><div style="font-size:21px;font-weight:900;color:#4338ca">${sumM.total}</div><div style="font-size:10px;color:#64748b">당월 수주 회선</div></div>
      <div style="flex:1;background:#f5f3ff;border:1.5px solid #ddd6fe;border-radius:12px;padding:9px;text-align:center"><div style="font-size:21px;font-weight:900;color:#7c3aed">${sumA.total}</div><div style="font-size:10px;color:#64748b">누적 수주 회선</div></div>
      <div style="flex:1;background:#eff6ff;border:1.5px solid #bfdbfe;border-radius:12px;padding:9px;text-align:center"><div style="font-size:21px;font-weight:900;color:#2563eb">${sumA.wire}</div><div style="font-size:10px;color:#64748b">누적 유선</div></div>
      <div style="flex:1;background:#faf5ff;border:1.5px solid #e9d5ff;border-radius:12px;padding:9px;text-align:center"><div style="font-size:21px;font-weight:900;color:#9333ea">${sumA.mob}</div><div style="font-size:10px;color:#64748b">누적 무선</div></div>
      <div style="flex:1;background:#f0fdf4;border:1.5px solid #bbf7d0;border-radius:12px;padding:9px;text-align:center"><div style="font-size:21px;font-weight:900;color:#16a34a">${rows.filter(r=>r.all.total>0).length}</div><div style="font-size:10px;color:#64748b">실적 발생 현장</div></div>
    </div>
    <div style="font-size:12.5px;font-weight:800;color:#16213e;margin-bottom:5px">영업구간별 상품 상세 <span style="font-size:10.5px;color:#94a3b8;font-weight:400">— 당월 / 누적 병기 · 유선(인터넷·TV·약정갱신) + 무선(기변·MNP·2nd)</span></div>
    <table style="font-size:10.5px">
      <thead><tr>
        <th style="text-align:left;padding-left:8px;width:120px">영업구간(현장)</th>
        <th style="width:52px">단계</th><th style="width:40px">임직원</th><th style="width:34px">구분</th>
        ...
        <th style="background:#15803d">합계</th><th style="text-align:left;width:96px">연계 대리점</th>
      </tr></thead>
      <tbody>
        ${rows.length===0?`<tr><td colspan="12" style="color:#94a3b8;padding:20px">실적이 입력된 영업구간이 없습니다</td></tr>`:rows.slice(0,9).map((r,i)=>`
          <tr style="background:${i%2?'#f8fafc':'#fff'}">
            <td rowspan="2" style="text-align:left;font-weight:700;padding-left:8px;max-width:120px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${r.name}${r.industry?`<div style="font-size:9px;color:#94a3b8;font-weight:400">${r.industry}</div>`:''}</td>
            <td rowspan="2"><span style="font-size:9.5px;padding:2px 6px;border-radius:8px;background:${stageColor(r.stage)}22;color:${stageColor(r.stage)};font-weight:700">${r.stage}</span></td>
            <td rowspan="2" style="font-size:10px">${r.emp}</td>
            <td style="font-size:9.5px;color:#4338ca;font-weight:800;background:#eef2ff">당월</td>
            ${prodCell(r.mon)}
            <td rowspan="2" style="text-align:left;font-size:9.5px;color:#7c3aed;line-height:1.4">${r.agency||'-'}${r.agencyMgr?`<div style="font-size:8.5px;color:#94a3b8">${r.agencyMgr}</div>`:''}</td>
          </tr>
          <tr style="background:${i%2?'#f8fafc':'#fff'}">
            <td style="font-size:9.5px;color:#7c3aed;font-weight:800;background:#f5f3ff">누적</td>
            ${prodCell(r.all)}
          </tr>`).join('')}
        <tr style="background:#eef2ff;font-weight:800;color:#4338ca">
          <td colspan="3" style="text-align:left;padding-left:8px;color:#4338ca">당월 합계</td><td style="font-size:9.5px">당월</td>
          <td>${sumM.net||'-'}</td><td>${sumM.tv||'-'}</td><td>${sumM.renew||'-'}</td><td>${sumM.chg||'-'}</td><td>${sumM.mnp||'-'}</td><td>${sumM.sec||'-'}</td>
          <td style="background:#c7d2fe">${sumM.total||'-'}</td><td></td>
        </tr>
        <tr style="background:#16213e;color:#fff;font-weight:900">
          <td colspan="3" style="color:#fff;text-align:left;padding-left:8px">누적 합계 (${rows.length}개 현장)</td><td style="color:#fff;font-size:9.5px">누적</td>
    ...
    ${rows.length>9?`<div style="font-size:10.5px;color:#94a3b8;text-align:center;margin-top:3px">외 ${rows.length-9}개 현장</div>`:''}
    <div style="margin-top:7px;font-size:10.5px;color:#94a3b8">※ 각 현장의 일별 실적(daily_records) 집계 · 당월 = ${period.replace(' (월마감)','')} · 누적 = 전체 기간 · 유선=인터넷+TV+약정갱신, 무선=기변+MNP+2nd</div>
  </div>
  ${ftr(sn)}
</div>`
  }
  ...
  const s8=mPlusSiteSlide(9)
  const s9=psAgencySlide(10)
...
${s1}${s2}${s3t}${s3}${s4}${s5}${s6}${s7}${s8}${s9}
<script>
function doPrint(){
  try{
