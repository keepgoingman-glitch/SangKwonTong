            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
              ...
              <div style={{ background: '#fff', border: '1.5px solid #bfdbfe', borderRadius: '14px', padding: '18px', boxShadow: '0 1px 4px rgba(0,0,0,0.04)' }}>
                ...
                <button onClick={() => {
                  ...
                  function mPlusProdCountsByTeam(teamId){
                    const pc={}
                  ...
                  const teamDataFull = (slideAccs.length>0?slideAccs:[{team_id:adminInfo?.teamId,team_label:adminInfo?.name||'우리팀'}]).map(acc=>{
                    ...
                    _monthlyStats.prodCounts = mergeProdCounts(_monthlyStats.prodCounts, mPlusProdCountsByTeam(acc.team_id))
                    const _annualStats = teamStats(_inYr)
                    _annualStats.prodCounts = mergeProdCounts(_annualStats.prodCounts, mPlusProdCountsByTeam(acc.team_id))
                    return {
                  ...
                  const globalProdMonthly = mergeProdCounts(calcProdCounts(allSuccM), mPlusProdCountsByTeam(isSuper?null:adminInfo?.teamId))
                  const globalProdAnnual  = mergeProdCounts(calcProdCounts(allSuccY), mPlusProdCountsByTeam(isSuper?null:adminInfo?.teamId))
