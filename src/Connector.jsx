...
function buildSlideHTML({ period, today, yearStr, teams, projects, allMM, topMM,
  ...
  *{margin:0;padding:0;box-sizing:border-box;-webkit-print-color-adjust:exact!important;print-color-adjust:exact!important}
    ...
    <table>
      ...
      <tbody>
        ${byStage.map((b,i)=>`<tr style="background:${i%2?'#f8fafc':'#fff'}">
          ...
          <td style="text-align:left;font-size:11px;color:#64748b">${b.st==='영업중'?'▶ 집중 관리 대상 (10P)':b.st==='영업종료'?'▶ 성과 확정 (11P)':b.st==='접촉중'||b.st==='발굴성공'?'▶ 파이프라인 (12P)':'-'}</td>
        </tr>`).join('')}
        <tr style="background:#16213e;color:#fff;font-weight:900">
  ...
  // ── M플러스 단계별 상세 (구간별 페이지) ──
  function mPlusStageSlide(sn, stageList, title, subtitle, accent){
    const rows = MP.rows.filter(r=>stageList.includes(r.stage))
      .sort((a,b)=>b.all.total-a.all.total)
    const sumA=mpSum(rows,'all'), sumM=mpSum(rows,'mon')
    const prodCell=(g)=>`<td class="bc">${g.net||'-'}</td><td>${g.tv||'-'}</td><td>${g.renew||'-'}</td><td>${g.chg||'-'}</td><td>${g.mnp||'-'}</td><td>${g.sec||'-'}</td><td style="background:#f0fdf4;font-weight:900;color:#15803d">${g.total||'-'}</td>`
    const MAX=8
    return `<div class="slide" id="s${sn}">
  ${rh(title, subtitle, sn)}
  ${dbar}
  <div class="body">
    <div style="display:flex;gap:9px;margin-bottom:11px">
      <div style="flex:1;background:${accent.bg};border:1.5px solid ${accent.br};border-radius:12px;padding:11px;text-align:center"><div style="font-size:24px;font-weight:900;color:${accent.c}">${rows.length}</div><div style="font-size:11px;color:#64748b">해당 현장 수</div></div>
      <div style="flex:1;background:#eef2ff;border:1.5px solid #c7d2fe;border-radius:12px;padding:11px;text-align:center"><div style="font-size:24px;font-weight:900;color:#4338ca">${sumM.total}</div><div style="font-size:11px;color:#64748b">당월 회선</div></div>
      <div style="flex:1;background:#f5f3ff;border:1.5px solid #ddd6fe;border-radius:12px;padding:11px;text-align:center"><div style="font-size:24px;font-weight:900;color:#7c3aed">${sumA.total}</div><div style="font-size:11px;color:#64748b">누적 회선</div></div>
      <div style="flex:1;background:#eff6ff;border:1.5px solid #bfdbfe;border-radius:12px;padding:11px;text-align:center"><div style="font-size:24px;font-weight:900;color:#2563eb">${sumA.wire}</div><div style="font-size:11px;color:#64748b">누적 유선</div></div>
      <div style="flex:1;background:#faf5ff;border:1.5px solid #e9d5ff;border-radius:12px;padding:11px;text-align:center"><div style="font-size:24px;font-weight:900;color:#9333ea">${sumA.mob}</div><div style="font-size:11px;color:#64748b">누적 무선</div></div>
    </div>
    <table style="font-size:11px">
      <thead><tr>
        <th style="text-align:left;padding-left:8px;width:132px">영업구간(현장)</th>
        <th style="width:48px">단계</th><th style="width:44px">임직원</th><th style="width:32px">구분</th>
        <th style="background:#1d4ed8">인터넷</th><th style="background:#0891b2">TV</th><th style="background:#0d9488">약정</th>
        <th style="background:#7c3aed">기변</th><th style="background:#9333ea">MNP</th><th style="background:#c026d3">2nd</th>
        <th style="background:#15803d">합계</th><th style="text-align:left;width:104px">연계 대리점 / 일정</th>
      </tr></thead>
      <tbody>
        ${rows.length===0?`<tr><td colspan="12" style="color:#94a3b8;padding:26px">해당 구간의 현장이 없습니다</td></tr>`:rows.slice(0,MAX).map((r,i)=>`
          <tr style="background:${i%2?'#f8fafc':'#fff'}">
            <td rowspan="2" style="text-align:left;font-weight:700;padding-left:8px;max-width:132px">
              <div style="overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${r.name}</div>
              ${r.industry?`<div style="font-size:9.5px;color:#94a3b8;font-weight:400">${r.industry}</div>`:''}
              ${r.mm?`<div style="font-size:9.5px;color:#2563eb;font-weight:400">담당 ${r.mm}</div>`:''}
            </td>
            <td rowspan="2"><span style="font-size:9.5px;padding:2px 6px;border-radius:8px;background:${mpStageColor(r.stage)}22;color:${mpStageColor(r.stage)};font-weight:700">${r.stage}</span></td>
            <td rowspan="2" style="font-size:10px">${r.emp}</td>
            <td style="font-size:9.5px;color:#4338ca;font-weight:800;background:#eef2ff">당월</td>
            ${prodCell(r.mon)}
            <td rowspan="2" style="text-align:left;font-size:9.5px;color:#7c3aed;line-height:1.45">
              ${r.agency||'<span style="color:#cbd5e1">미지정</span>'}
              ${r.agencyMgr?`<div style="font-size:9px;color:#94a3b8">${r.agencyMgr}</div>`:''}
              ${r.schedule?`<div style="font-size:9px;color:#d97706">📅 ${r.schedule}</div>`:''}
            </td>
          </tr>
          <tr style="background:${i%2?'#f8fafc':'#fff'}">
            <td style="font-size:9.5px;color:#7c3aed;font-weight:800;background:#f5f3ff">누적</td>
            ${prodCell(r.all)}
          </tr>`).join('')}
        ${rows.length>0?`<tr style="background:#16213e;color:#fff;font-weight:900">
          <td colspan="3" style="color:#fff;text-align:left;padding-left:8px">구간 합계 (${rows.length}개 현장)</td><td style="color:#fff;font-size:9.5px">누적</td>
          <td style="color:#60a5fa">${sumA.net||'-'}</td><td style="color:#fff">${sumA.tv||'-'}</td><td style="color:#fff">${sumA.renew||'-'}</td>
          <td style="color:#fff">${sumA.chg||'-'}</td><td style="color:#fff">${sumA.mnp||'-'}</td><td style="color:#fff">${sumA.sec||'-'}</td>
          <td style="background:#166534;color:#fff">${sumA.total||'-'}</td><td style="color:#fff"></td>
        </tr>`:''}
      </tbody>
    </table>
    ${rows.length>MAX?`<div style="font-size:11px;color:#94a3b8;text-align:center;margin-top:4px">외 ${rows.length-MAX}개 현장 (관리자 화면에서 전체 확인)</div>`:''}
  </div>
  ${ftr(sn)}
</div>`
  }
  ...
  const s8b = mPlusStageSlide(10, ['영업중'], '🔍 M플러스 · 진행중 구간', '영업중 현장 — 집중 관리·전략 협의 대상', {bg:'#fffbeb',br:'#fde68a',c:'#d97706'})
  const s8c = mPlusStageSlide(11, ['영업종료'], '🔍 M플러스 · 완료 구간', '영업종료 현장 — 확정 성과 및 BP 도출', {bg:'#f0fdf4',br:'#bbf7d0',c:'#16a34a'})
  const s8d = mPlusStageSlide(12, ['접촉중','발굴성공'], '🔍 M플러스 · 접촉중 구간', '접촉중·발굴성공 현장 — 진입 파이프라인', {bg:'#f0f9ff',br:'#bae6fd',c:'#0891b2'})
  const s9  = psAgencySlide(13)
