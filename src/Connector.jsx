...
function buildSlideHTML({ period, today, yearStr, teams, projects, allMM, topMM,
  ...
  *{margin:0;padding:0;box-sizing:border-box;-webkit-print-color-adjust:exact!important;print-color-adjust:exact!important}
  ...
  const TOTAL_PAGES = 9 + MP_PAGES.run + MP_PAGES.done + MP_PAGES.init + 1 + (reportMode==='annual' ? 2 : 0)
  ...
  function annualTeamSlide(sn){
    const rows = (teams||[]).map(t=>({
      ...
      act: t.annual?.done||0, suc: t.annual?.success||0,
      target: (t.annualTarget||t.yearTarget||0),
      pg: prodGroup(t.annual?.prodCounts||{})
    }))
    const sum = rows.reduce((a,r)=>({act:a.act+r.act,suc:a.suc+r.suc,target:a.target+r.target,
      inet:a.inet+r.pg.inet,tv:a.tv+r.pg.tv,tvbiz:a.tvbiz+r.pg.tvbiz,mob:a.mob+r.pg.mob,etc:a.etc+r.pg.etc}),
      {act:0,suc:0,target:0,inet:0,tv:0,tvbiz:0,mob:0,etc:0})
    const sumLines = sum.inet+sum.tv+sum.tvbiz+sum.mob+sum.etc
    return `<div class="slide" id="s${sn}">
  ${rh('연간 누계 · 팀별 세부 실적', yearStr+' 연초 ~ '+today+' 누계', sn)}
  ${dbar}
  <div class="body">
    <div style="display:flex;gap:10px;margin-bottom:13px">
      <div style="flex:1;background:#f5f3ff;border:1.5px solid #ddd6fe;border-radius:13px;padding:13px;text-align:center"><div style="font-size:28px;font-weight:900;color:#7c3aed">${sum.act}</div><div style="font-size:11.5px;color:#64748b">연간 누적 활동</div></div>
      <div style="flex:1;background:#f0fdf4;border:1.5px solid #bbf7d0;border-radius:13px;padding:13px;text-align:center"><div style="font-size:28px;font-weight:900;color:#16a34a">${sum.suc}</div><div style="font-size:11.5px;color:#64748b">연간 누적 성공</div></div>
      <div style="flex:1;background:#eff6ff;border:1.5px solid #bfdbfe;border-radius:13px;padding:13px;text-align:center"><div style="font-size:28px;font-weight:900;color:#2563eb">${pct(sum.suc,sum.act)}%</div><div style="font-size:11.5px;color:#64748b">누적 성공률</div></div>
      <div style="flex:1;background:#fff7ed;border:1.5px solid #fed7aa;border-radius:13px;padding:13px;text-align:center"><div style="font-size:28px;font-weight:900;color:#d97706">${sumLines}</div><div style="font-size:11.5px;color:#64748b">누적 수주 회선</div></div>
      <div style="flex:1;background:#eef2ff;border:1.5px solid #c7d2fe;border-radius:13px;padding:13px;text-align:center"><div style="font-size:28px;font-weight:900;color:#4338ca">${sum.target||'-'}</div><div style="font-size:11.5px;color:#64748b">연간 목표</div></div>
    </div>
    <div style="font-size:13px;font-weight:800;color:#16213e;margin-bottom:6px">팀별 연간 누계</div>
    <table>
      <thead><tr>
        <th style="text-align:left;padding-left:12px;width:110px">팀</th><th style="width:56px">MM</th>
        <th style="background:#7c3aed">누적 활동</th><th style="background:#16a34a">누적 성공</th><th style="background:#2563eb">성공률</th>
        <th style="background:#d97706">인터넷</th><th style="background:#d97706">TV-M</th><th style="background:#d97706">TV-B</th><th style="background:#d97706">모바일</th><th style="background:#d97706">기타</th>
        <th style="background:#92400e">회선 합계</th>
      </tr></thead>
      <tbody>
        ${rows.map((r,i)=>`<tr style="background:${i%2?'#f8fafc':'#fff'}">
          ...
          <td style="font-weight:700;color:#7c3aed">${r.act}</td><td style="font-weight:900;color:#16a34a">${r.suc}</td><td>${pct(r.suc,r.act)}%</td>
        ...
        <tr style="background:#16213e;color:#fff;font-weight:900">
          <td style="color:#fff;text-align:left;padding-left:12px">전 체</td><td style="color:#fff">${rows.reduce((a,r)=>a+r.mm,0)}명</td>
          <td style="background:#5b21b6;color:#fff">${sum.act}</td><td style="background:#166534;color:#fff">${sum.suc}</td><td style="color:#fff">${pct(sum.suc,sum.act)}%</td>
    ...
    <div style="margin-top:9px;font-size:11px;color:#94a3b8">※ 연간 누계 = ${yearStr} 1월 1일 ~ ${today} 누적 · 회선수는 수주 상품 기준</div>
  </div>
  ${ftr(sn)}
</div>`
  }
  ...
  function annualMMSlide(sn){
    const rows=(allMM||[]).map(m=>({
      ...
      act:m.annual?.done||0, suc:m.annual?.success||0,
    ...
    return `<div class="slide" id="s${sn}">
  ${rh('연간 누계 · MM별 세부 실적', yearStr+' 연초 ~ '+today+' 누계 · 성공 순', sn)}
  ${dbar}
  <div class="body">
    <table style="font-size:11.5px">
      <thead><tr>
        <th style="width:36px">순위</th><th style="text-align:left;padding-left:10px;width:96px">MM</th><th style="width:82px">팀</th>
        <th style="background:#7c3aed">누적 활동</th><th style="background:#16a34a">누적 성공</th><th style="background:#2563eb">성공률</th>
        <th style="background:#d97706">인터넷</th><th style="background:#d97706">TV-M</th><th style="background:#d97706">TV-B</th><th style="background:#d97706">모바일</th><th style="background:#d97706">기타</th>
      ...
      <tbody>
        ${rows.slice(0,16).map((r,i)=>{
          ...
          <td style="color:#7c3aed;font-weight:700">${r.act}</td><td style="color:#16a34a;font-weight:900">${r.suc}</td><td>${pct(r.suc,r.act)}%</td>
          <td>${r.pg.inet||'-'}</td><td>${r.pg.tv||'-'}</td><td>${r.pg.tvbiz||'-'}</td><td>${r.pg.mob||'-'}</td><td>${r.pg.etc||'-'}</td>
    ...
    ${rows.length>16?`<div style="font-size:11px;color:#94a3b8;text-align:center;margin-top:4px">외 ${rows.length-16}명</div>`:''}
  </div>
  ${ftr(sn)}
</div>`
  }
  ...
  const sAnnual = isAnnual ? (annualTeamSlide(_pn) + annualMMSlide(_pn+1)) : ''
...
${s1}${s2}${s3t}${s3}${s4}${s5}${s6}${s7}${s8}${s8b}${s8c}${s8d}${s9}${sAnnual}
<script>
function doPrint(){
  try{
