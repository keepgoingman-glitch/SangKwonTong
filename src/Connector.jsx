...
const C = { bg: '#06111f', card: '#0d1f35', border: '#1a3050', acc: '#f59e0b', text: '#f1f5f9', sub: '#8aa3bc' }
...
export default function Connector({ onBack }) {
  ...
  const handleFile = e => {
    ...
    reader.onload = ev => {
      ...
      const mapped = rows.map(mapRow).filter(r => r.address)
      setUploadPreview(mapped)
      if (!mapped.length) t2('인식된 데이터가 없습니다')
    }
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      t2(uploadPreview.length + '건 업로드 완료!'); setUploadPreview([]); load(); setTab('leads')
  ...
  const downloadActivity = () => dl('상권통_활동결과', [['주소','면적','용도','착공일','배분MM','배분상태','활동상태','결과요약','메모']].concat(leads.map(l => [l.address, l.area, l.usage_type, l.start_date, l.assigned_to, l.assign_status, l.activity_status || '미처리', l.activity_result || '', l.activity_memo || ''])))
  ...
  const InputStyle = { width: '100%', background: 'rgba(255,255,255,0.07)', border: '1px solid rgba(255,255,255,0.15)', color: C.text, padding: '15px 16px', borderRadius: '12px', fontSize: '17px', outline: 'none', boxSizing: 'border-box', fontFamily: 'inherit' }
  ...
  if (screen === 'login') return (
    <div style={{ minHeight: '100vh', background: 'linear-gradient(170deg, #06111f 0%, #0a1f3d 40%, #0d1a35 70%, #06111f 100%)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '30px 28px', fontFamily: "'Noto Sans KR',sans-serif", color: C.text }}>
      <button onClick={onBack} style={{ alignSelf: 'flex-start', background: 'transparent', border: 'none', color: 'rgba(255,255,255,0.4)', fontSize: '15px', cursor: 'pointer', marginBottom: '32px', display: 'flex', alignItems: 'center', gap: '6px' }}>‹ 홈으로</button>
      <KTLogo />
      ...
      <div style={{ width: '100%', maxWidth: '360px', display: 'flex', flexDirection: 'column', gap: '12px' }}>
        ...
        <button onClick={handleLogin} style={{ background: 'linear-gradient(135deg,#f59e0b,#d97706)', border: 'none', color: '#0a0f1e', padding: '18px', borderRadius: '14px', fontSize: '18px', fontWeight: '900', cursor: 'pointer', fontFamily: 'inherit', marginTop: '6px', boxShadow: '0 6px 20px rgba(245,158,11,0.35)' }}>로그인</button>
      </div>
    </div>
  )
  ...
  const Modal = ({ children, onClose }) => (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.75)', display: 'flex', alignItems: 'flex-end', justifyContent: 'center', zIndex: 998, backdropFilter: 'blur(4px)' }}>
      <div style={{ background: 'linear-gradient(170deg,#0d1f35,#0a1628)', borderRadius: '20px 20px 0 0', padding: '24px 20px 36px', width: '100%', maxWidth: '480px', border: '1px solid rgba(255,255,255,0.1)', borderBottom: 'none' }}>
        {children}
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'linear-gradient(170deg,#06111f,#0a1f3d,#06111f)', color: C.text, fontFamily: "'Noto Sans KR',sans-serif", maxWidth: '480px', margin: '0 auto', display: 'flex', flexDirection: 'column' }}>
      ...
      {/* 계정추가 모달 */}
      {addModal && (
        <Modal onClose={() => { setAddModal(false); setAddErr('') }}>
          <h3 style={{ fontSize: '20px', fontWeight: '900', marginBottom: '18px' }}>신규 MM 계정 등록</h3>
          {addErr && <div style={{ color: '#f87171', fontSize: '14px', marginBottom: '12px', background: 'rgba(248,113,113,0.1)', padding: '10px', borderRadius: '8px' }}>{addErr}</div>}
          {[{ k: 'name', l: '이름', p: '홍길동' }, { k: 'username', l: '아이디', p: 'mm003' }, { k: 'password', l: '비밀번호', p: '초기 비밀번호' }, { k: 'region', l: '담당 지역', p: '예: 강남구' }, { k: 'goal', l: '목표(건)', p: '10', t: 'number' }].map(f => (
            <div key={f.k} style={{ marginBottom: '12px' }}>
              <div style={{ fontSize: '13px', color: C.sub, marginBottom: '5px', fontWeight: '600' }}>{f.l}</div>
              <input type={f.t || 'text'} placeholder={f.p} value={newU[f.k]} onChange={e => setNewU({ ...newU, [f.k]: e.target.value })} style={{ ...InputStyle, fontSize: '16px', padding: '13px 14px' }} />
            </div>
          ))}
          <div style={{ display: 'flex', gap: '10px', marginTop: '18px' }}>
            <button onClick={() => { setAddModal(false); setAddErr('') }} style={{ flex: 1, padding: '14px', background: 'rgba(255,255,255,0.07)', border: '1px solid rgba(255,255,255,0.12)', color: C.sub, borderRadius: '12px', fontSize: '16px', cursor: 'pointer', fontFamily: 'inherit' }}>취소</button>
            <button onClick={handleAdd} style={{ flex: 2, padding: '14px', background: 'linear-gradient(135deg,#f59e0b,#d97706)', border: 'none', color: '#0a0f1e', borderRadius: '12px', fontSize: '16px', fontWeight: '900', cursor: 'pointer', fontFamily: 'inherit' }}>계정 생성</button>
          </div>
        </Modal>
      )}
      ...
      {editModal && (
        <Modal onClose={() => setEditModal(null)}>
          <h3 style={{ fontSize: '20px', fontWeight: '900', marginBottom: '18px' }}>계정 수정</h3>
          {[{ k: 'name', l: '이름' }, { k: 'password', l: '비밀번호' }, { k: 'region', l: '담당 지역' }].map(f => (
            <div key={f.k} style={{ marginBottom: '12px' }}>
              <div style={{ fontSize: '13px', color: C.sub, marginBottom: '5px', fontWeight: '600' }}>{f.l}</div>
              <input type="text" value={editModal[f.k] || ''} onChange={e => setEditModal({ ...editModal, [f.k]: e.target.value })} style={{ ...InputStyle, fontSize: '16px', padding: '13px 14px' }} />
            </div>
          ))}
          <div style={{ display: 'flex', gap: '10px', marginTop: '18px' }}>
            <button onClick={() => setEditModal(null)} style={{ flex: 1, padding: '14px', background: 'rgba(255,255,255,0.07)', border: '1px solid rgba(255,255,255,0.12)', color: C.sub, borderRadius: '12px', fontSize: '16px', cursor: 'pointer', fontFamily: 'inherit' }}>취소</button>
            <button onClick={handleEdit} style={{ flex: 2, padding: '14px', background: 'linear-gradient(135deg,#f59e0b,#d97706)', border: 'none', color: '#0a0f1e', borderRadius: '12px', fontSize: '16px', fontWeight: '900', cursor: 'pointer', fontFamily: 'inherit' }}>저장</button>
          </div>
        </Modal>
      )}
      ...
      {/* 배분 모달 */}
      {assignModal && (
        <Modal onClose={() => { setAssignModal(null); setSelMM('') }}>
          ...
          <button onClick={() => handleAssign(assignModal.id)} disabled={!selMM}
      ...
      {/* 헤더 */}
      <div style={{ background: 'rgba(6,17,31,0.9)', padding: '16px 20px 14px', borderBottom: '1px solid rgba(245,158,11,0.25)', backdropFilter: 'blur(10px)', position: 'sticky', top: 0, zIndex: 50 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
            <KTLogo />
            <div>
              <div style={{ fontSize: '18px', fontWeight: '900', color: C.acc, lineHeight: 1 }}>관리자</div>
              <div style={{ fontSize: '11px', color: 'rgba(255,255,255,0.35)', marginTop: '2px' }}>상권통 관리자 플랫폼</div>
            </div>
          </div>
          <div style={{ display: 'flex', gap: '8px' }}>
            <button onClick={load} style={{ background: 'rgba(255,255,255,0.08)', border: '1px solid rgba(255,255,255,0.1)', color: C.sub, padding: '7px 12px', borderRadius: '8px', fontSize: '14px', cursor: 'pointer', fontFamily: 'inherit' }}>🔄</button>
            <button onClick={() => setScreen('login')} style={{ background: 'rgba(255,255,255,0.08)', border: '1px solid rgba(255,255,255,0.1)', color: C.sub, padding: '7px 12px', borderRadius: '8px', fontSize: '13px', cursor: 'pointer', fontFamily: 'inherit' }}>로그아웃</button>
          </div>
        </div>
        <div style={{ display: 'flex', gap: '8px' }}>
          {[{ l: '등록 MM', v: users.length, u: '명', hi: false }, { l: '미배분', v: unassigned, u: '건', hi: unassigned > 0 }, { l: '팀 성공', v: totalSuccess, u: '건', hi: false }].map((s, i) => (
            <div key={i} style={{ flex: 1, background: s.hi ? 'rgba(239,68,68,0.12)' : 'rgba(245,158,11,0.08)', border: '1px solid ' + (s.hi ? 'rgba(239,68,68,0.3)' : 'rgba(245,158,11,0.2)'), borderRadius: '10px', padding: '10px 8px', textAlign: 'center' }}>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '16px' }}>
        ...
        {tab === 'overview' && (
          <div>
            ...
            {totalGoal > 0 && (
              <div style={{ background: 'rgba(245,158,11,0.08)', border: '1px solid rgba(245,158,11,0.2)', borderRadius: '14px', padding: '16px', marginBottom: '14px' }}>
                ...
                <div style={{ color: C.sub, fontSize: '13px', marginTop: '6px' }}>{totalSuccess}건 성공 / 목표 {totalGoal}건</div>
              </div>
            )}
            {users.map(u => {
              ...
              return (
                <div key={u.id} style={{ background: 'rgba(13,31,53,0.8)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '14px', padding: '16px', marginBottom: '10px', backdropFilter: 'blur(10px)' }}>
                  ...
                  <div style={{ display: 'flex', gap: '8px' }}>
                    ...
                    <button onClick={() => saveGoal(u.id)}
                      style={{ background: savedGoal === u.id ? '#4ade80' : C.acc, border: 'none', color: '#0a0f1e', padding: '10px 16px', borderRadius: '8px', fontSize: '14px', fontWeight: '800', cursor: 'pointer', fontFamily: 'inherit', whiteSpace: 'nowrap' }}>
        ...
        {tab === 'staff' && (
          <div>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '14px' }}>
              ...
              <button onClick={() => { setAddModal(true); setAddErr('') }}
            ...
            {users.map(u => (
              <div key={u.id} style={{ background: 'rgba(13,31,53,0.8)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '14px', padding: '16px', marginBottom: '10px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px' }}>
                  <div>
                    <div style={{ fontSize: '17px', fontWeight: '800' }}>{u.name} MM</div>
                  ...
                  <div style={{ display: 'flex', gap: '6px' }}>
                    <button onClick={() => setEditModal({ ...u })} style={{ background: 'rgba(96,165,250,0.12)', border: '1px solid rgba(96,165,250,0.3)', color: '#60a5fa', padding: '7px 14px', borderRadius: '8px', fontSize: '14px', cursor: 'pointer', fontFamily: 'inherit' }}>수정</button>
                    <button onClick={() => handleDelete(u.id)} style={{ background: 'rgba(248,113,113,0.1)', border: '1px solid rgba(248,113,113,0.3)', color: '#f87171', padding: '7px 14px', borderRadius: '8px', fontSize: '14px', cursor: 'pointer', fontFamily: 'inherit' }}>삭제</button>
                  </div>
                </div>
        ...
        {tab === 'upload' && (
          <div>
            ...
            <div onClick={() => fileRef.current.click()}
              style={{ background: 'rgba(245,158,11,0.05)', border: '2px dashed rgba(245,158,11,0.4)', borderRadius: '14px', padding: '32px 20px', textAlign: 'center', cursor: 'pointer', marginBottom: '14px' }}>
              <div style={{ fontSize: '40px', marginBottom: '10px' }}>📂</div>
              <div style={{ fontSize: '16px', fontWeight: '800', color: C.acc, marginBottom: '4px' }}>CSV 파일 선택</div>
              <div style={{ color: C.sub, fontSize: '13px' }}>클릭하여 파일 선택</div>
              <input ref={fileRef} type="file" accept=".csv,.txt" style={{ display: 'none' }} onChange={handleFile} />
            </div>
            <div style={{ background: 'rgba(13,31,53,0.8)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '12px', padding: '14px', marginBottom: '16px' }}>
              <div style={{ fontSize: '13px', fontWeight: '700', color: C.acc, marginBottom: '10px' }}>📋 자동 인식 컬럼명</div>
              {[['주소/대지위치/위치', '고객 주소 (필수)'], ['면적/건축면적(㎡)', '건물 규모'], ['용도/건축', '단독주택 / 공장 등'], ['착공/일자/날짜', '착공 예정일']].map(([col, desc], i) => (
                <div key={i} style={{ display: 'flex', gap: '10px', alignItems: 'center', marginBottom: '7px' }}>
                  <span style={{ background: 'rgba(245,158,11,0.1)', color: C.acc, fontSize: '12px', fontWeight: '700', padding: '3px 10px', borderRadius: '4px', whiteSpace: 'nowrap' }}>{col}</span>
                  <span style={{ color: C.sub, fontSize: '13px' }}>{desc}</span>
                </div>
              ))}
            </div>
            {uploadPreview.length > 0 && (
              <div>
                ...
                {uploadPreview.slice(0, 4).map((r, i) => (
                  <div key={i} style={{ background: 'rgba(13,31,53,0.8)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '8px', padding: '10px 12px', marginBottom: '6px' }}>
                ...
                {uploadPreview.length > 4 && <div style={{ color: C.sub, fontSize: '13px', textAlign: 'center', marginBottom: '12px' }}>외 {uploadPreview.length - 4}건 더...</div>}
                <div style={{ display: 'flex', gap: '10px', marginTop: '12px' }}>
                  <button onClick={() => setUploadPreview([])} style={{ flex: 1, padding: '13px', background: 'rgba(255,255,255,0.07)', border: '1px solid rgba(255,255,255,0.1)', color: C.sub, borderRadius: '10px', fontSize: '15px', cursor: 'pointer', fontFamily: 'inherit' }}>취소</button>
        ...
        {tab === 'leads' && (
          <div>
            ...
            {filtered.length === 0 && <div style={{ textAlign: 'center', padding: '50px 20px', color: C.sub, fontSize: '16px' }}>영업기회가 없습니다<br /><span style={{ fontSize: '14px', opacity: 0.6 }}>업로드 탭에서 추가하세요</span></div>}
            {filtered.map(l => (
              <div key={l.id} style={{ background: 'rgba(13,31,53,0.8)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '14px', padding: '14px', marginBottom: '10px' }}>
                ...
                {l.products?.length > 0 && (
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: '4px', marginBottom: '10px' }}>
                    {l.products.map(p => <span key={p} style={{ background: 'rgba(245,158,11,0.12)', color: C.acc, border: '1px solid rgba(245,158,11,0.25)', fontSize: '11px', fontWeight: '700', padding: '3px 8px', borderRadius: '4px' }}>{p}</span>)}
                  </div>
                )}
                {l.assign_status === '배분완료'
                  ? <div style={{ color: '#4ade80', fontSize: '13px', fontWeight: '700' }}>👤 {l.assigned_to} MM · <span style={{ color: C.sub }}>{l.activity_status || '미처리'}</span></div>
                  : <button onClick={() => setAssignModal(l)} style={{ width: '100%', background: 'transparent', border: '1px solid rgba(245,158,11,0.4)', color: C.acc, padding: '10px', borderRadius: '8px', fontSize: '14px', fontWeight: '700', cursor: 'pointer', fontFamily: 'inherit' }}>MM 배분하기</button>}
              </div>
            ))}
          </div>
        )}
        ...
        {tab === 'download' && (
          <div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
              <div style={{ background: 'rgba(13,31,53,0.8)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '14px', padding: '18px' }}>
              ...
              <div style={{ background: 'rgba(13,31,53,0.8)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '14px', padding: '18px' }}>
                ...
                <p style={{ color: C.sub, fontSize: '13px', marginBottom: '14px', lineHeight: '1.6' }}>주소 · 배분MM · 활동상태 · 결과요약 · 메모 포함</p>
      ...
      {/* 하단 탭 */}
      <div style={{ background: 'rgba(6,17,31,0.95)', borderTop: '1px solid rgba(255,255,255,0.08)', display: 'flex', padding: '8px 0 8px', backdropFilter: 'blur(10px)' }}>
        {navTabs.map(t => (
