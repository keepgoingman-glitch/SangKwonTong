...
function buildSlideHTML({ period, teams, projects, allMM, topMM, mDiscoveryCount, globalProdCounts = {} }) {
  const total = teams.reduce((a,t)=>a+t.success,0)
  const mitT  = teams.reduce((a,t)=>a+t.mitDone,0)
  const pct   = (n,d) => d>0 ? Math.round(n/d*100) : 0
  const PRODS = ['인터넷','TV','다량회선','모바일','일반전화','하이오더','로봇','CCTV']
  const PT_COLOR = {1:'#2563eb',2:'#7c3aed',3:'#0891b2',4:'#059669'}
  const PT_LIGHT = {1:'#eff6ff',2:'#f5f3ff',3:'#ecfeff',4:'#ecfdf5'}
  const PT_BORDER= {1:'#bfdbfe',2:'#c4b5fd',3:'#a5f3fc',4:'#6ee7b7'}
  const PT_TEXT  = {1:'#1e40af',2:'#5b21b6',3:'#0e7490',4:'#047857'}

  // SVG 도넛
  function donut(act,success,total2,color,size=120,sw=20) {
    const r=(size-sw)/2, cx=size/2, cy=size/2, circ=2*Math.PI*r
    const actDash  = pct(act,Math.max(total2,1))/100*circ
    const sucDash  = pct(success,Math.max(total2,1))/100*circ
    return '<svg width="'+size+'" height="'+size+'" viewBox="0 0 '+size+' '+size+'">'+
      '<circle cx="'+cx+'" cy="'+cy+'" r="'+r+'" fill="none" stroke="'+PT_BORDER[1]+'" stroke-width="'+sw+'"/>'+
      '<circle cx="'+cx+'" cy="'+cy+'" r="'+r+'" fill="none" stroke="'+color+'" stroke-width="'+sw+'" stroke-dasharray="'+actDash+' '+(circ-actDash)+'" stroke-dashoffset="0" transform="rotate(-90 '+cx+' '+cy+')" style="transition:stroke-dasharray 0.4s"/>'+
      '<circle cx="'+cx+'" cy="'+cy+'" r="'+r+'" fill="none" stroke="#16a34a" stroke-width="'+sw+'" stroke-dasharray="'+sucDash+' '+(circ-sucDash)+'" stroke-dashoffset="'+(-actDash)+'" transform="rotate(-90 '+cx+' '+cy+')" style="transition:stroke-dasharray 0.4s"/>'+
      '</svg>'
  }
  // 상품 도넛
  function prodDonut(pc,total2) {
    const PCOLS=['#2563eb','#3b82f6','#16a34a','#22c55e','#e67e00','#f59e0b','#7c3aed','#0891b2']
    const r=54,cx=70,cy=70,sw=22,circ=2*Math.PI*r
    let svg='<svg width="140" height="140" viewBox="0 0 140 140"><circle cx="70" cy="70" r="'+r+'" fill="none" stroke="#e8edf2" stroke-width="'+sw+'"/>'
    let off=0
    PRODS.forEach((p,i)=>{
      if(!pc[p]) return
      const dash=pct(pc[p],Math.max(total2,1))/100*circ
      svg+='<circle cx="'+cx+'" cy="'+cy+'" r="'+r+'" fill="none" stroke="'+PCOLS[i%PCOLS.length]+'" stroke-width="'+sw+'" stroke-dasharray="'+dash+' '+(circ-dash)+'" stroke-dashoffset="'+(-off)+'" transform="rotate(-90 '+cx+' '+cy+')" />'
      off+=dash
    })
    svg+='</svg>'
    return svg
  ...
  // 바
  function bar(val,max,col,h=8) {
    const w=Math.min(pct(val,Math.max(max,1)),100)
    return '<div style="background:#e8edf2;border-radius:3px;overflow:hidden;height:'+h+'px"><div style="width:'+w+'%;background:'+col+';height:'+h+'px;border-radius:3px"></div></div>'
  }
  // 배지
  function pd(txt,bg,col) { return '<span style="background:'+bg+';color:'+col+';padding:2px 8px;border-radius:5px;font-size:11px;font-weight:700;margin:2px;display:inline-block">'+txt+'</span>' }
  function badge(txt,type) {
    const map={green:['#dcfce7','#166534'],blue:['#dbeafe','#1e40af'],red:['#fee2e2','#991b1b'],orange:['#ffedd5','#9a3412'],purple:['#ede9fe','#5b21b6'],cyan:['#cffafe','#155e75']}
    const c=map[type]||['#f1f5f9','#475569']
    return '<span style="background:'+c[0]+';color:'+c[1]+';padding:2px 8px;border-radius:14px;font-size:11px;font-weight:600">'+txt+'</span>'
  }
  // 상품 배지 자동
  function prodBadges(pc,limit=5) {
    return PRODS.filter(p=>pc[p]).slice(0,limit).map(p=>{
      const col=p==='인터넷'||p==='TV'||p==='다량회선'?'blue':p==='모바일'?'green':p==='하이오더'?'orange':'purple'
      return pd(p+' '+pc[p], col==='blue'?'#dbeafe':col==='green'?'#dcfce7':col==='orange'?'#ffedd5':'#ede9fe', col==='blue'?'#1e40af':col==='green'?'#166534':col==='orange'?'#9a3412':'#5b21b6')
    }).join('')
  }

  const sf = (n,tot) => '<div style="padding:6px 34px;background:#f8fafc;border-top:1px solid #e2e8f0;display:flex;justify-content:space-between;font-size:10px;color:#94a3b8;flex-shrink:0"><span>상권통 영업 보고서 · '+period+'</span><span>'+n+' / '+tot+'</span></div>'
  const sh = (title,sub,n,tot,accentColor) => '<div style="display:flex;justify-content:space-between;align-items:center;padding:11px 34px 10px;border-bottom:3px solid '+(accentColor||'#e67e00')+';flex-shrink:0"><div><div style="font-size:18px;font-weight:700;color:#0f172a">'+title+'</div>'+(sub?'<div style="font-size:11px;color:#64748b;margin-top:1px">'+sub+'</div>':'')+' </div><div style="font-size:10px;color:#94a3b8;text-align:right;line-height:1.7">'+period+'<br/>'+n+' / '+tot+'</div></div>'
  const stb = (title,sub,n,tot,col) => '<div style="display:flex;justify-content:space-between;align-items:center;padding:11px 34px 10px;border-bottom:3px solid '+col+';flex-shrink:0"><div><div style="font-size:22px;font-weight:700;color:'+col+'">'+title+'</div><div style="font-size:11px;color:#64748b;margin-top:1px">'+sub+'</div></div><div style="font-size:10px;color:#94a3b8;text-align:right;line-height:1.7">'+period+'<br/>'+n+' / 10</div></div>'
  const slop = '<div style="padding:14px 34px 16px;flex:1;display:flex;flex-direction:column;gap:11px">'
  const css = `
:root{--acc:#e67e00}
*{margin:0;padding:0;box-sizing:border-box}
body{font-family:"Apple SD Gothic Neo","Noto Sans KR",Malgun Gothic,sans-serif;background:#0f1923;color:#1e293b;-webkit-print-color-adjust:exact;print-color-adjust:exact}
.slide{width:297mm;min-height:210mm;background:#fff;position:relative;overflow:hidden;display:flex;flex-direction:column;margin:0 auto 24px;page-break-after:always;page-break-inside:avoid}
@media screen{body{padding:24px;display:flex;flex-direction:column;align-items:center;gap:24px}.slide{box-shadow:0 12px 48px rgba(0,0,0,0.5)}}
...
.kpi{background:#f8fafc;border:1px solid #e2e8f0;border-radius:10px;padding:12px 10px;text-align:center;position:relative}
.kt{position:absolute;top:0;left:0;right:0;height:3px;border-radius:10px 10px 0 0}
...
th{padding:7px 9px;text-align:left;font-weight:600;font-size:10px;letter-spacing:0.3px}
...
.nav{position:fixed;bottom:14px;left:50%;transform:translateX(-50%);display:flex;gap:4px;background:rgba(10,15,30,0.95);padding:8px 14px;border-radius:50px;z-index:100;backdrop-filter:blur(10px)}
.nb{width:24px;height:24px;border-radius:50%;background:rgba(255,255,255,0.1);border:none;color:#fff;font-size:10px;font-weight:600;cursor:pointer}
.nb:hover{background:rgba(255,255,255,0.25)}
...
`
  ...
  const s1='<div class="slide" id="s1">'+
    '<div style="background:#0d1b2a;min-height:210mm;display:flex;flex-direction:column;align-items:center;justify-content:center;text-align:center;padding:44px 60px;position:relative">'+
    '<div style="position:absolute;top:0;left:0;right:0;height:5px;background:linear-gradient(90deg,#e67e00,#f59e0b,#e67e00)"></div>'+
    '<div style="position:absolute;bottom:0;left:0;right:0;height:5px;background:linear-gradient(90deg,#e67e00,#f59e0b,#e67e00)"></div>'+
    '<div style="font-size:11px;font-weight:600;color:rgba(230,126,0,0.8);letter-spacing:6px;margin-bottom:22px;text-transform:uppercase">KT 상권통 플랫폼</div>'+
    '<div style="font-size:56px;font-weight:700;color:#fff;line-height:1;margin-bottom:12px">영업 성과 보고서</div>'+
    '<div style="font-size:16px;color:rgba(255,255,255,0.35);margin-bottom:36px">4대 프로젝트 기반 영업활동 현황 분석</div>'+
    '<div style="background:rgba(230,126,0,0.12);border:1.5px solid rgba(230,126,0,0.35);color:#f59e0b;font-size:15px;font-weight:600;padding:10px 32px;border-radius:32px;display:inline-block;margin-bottom:46px">'+period+' 기준</div>'+
    '<div style="display:flex;gap:24px;justify-content:center;margin-bottom:36px">'+
    teams.map(t=>'<div style="background:rgba(255,255,255,0.05);border:1px solid rgba(255,255,255,0.1);border-radius:14px;padding:20px 36px"><div style="font-size:14px;font-weight:600;color:#f59e0b;margin-bottom:8px">'+t.label+'</div><div style="font-size:40px;font-weight:700;color:#4ade80;line-height:1">'+t.success+'<span style="font-size:16px">건</span></div><div style="font-size:11px;color:rgba(255,255,255,0.35);margin-top:4px">MM '+t.mm+'명 · MIT활동률 '+pct(t.mitDone,Math.max(t.mitLeads,1))+'%</div></div>').join('')+
    '</div>'+
    '<div style="display:flex;gap:32px;font-size:13px;color:rgba(255,255,255,0.25)">'+
    '<span>참여 MM <strong style="color:rgba(255,255,255,0.5)">'+allMM.length+'명</strong></span>'+
    '<span>MIT배분 <strong style="color:rgba(255,255,255,0.5)">'+teams.reduce((a,t)=>a+t.mitLeads,0)+'건</strong></span>'+
    '<span>총 성공 <strong style="color:#4ade80">'+total+'건</strong></span>'+
    '<span>MIT활동률 <strong style="color:rgba(255,255,255,0.5)">'+pct(mitT,Math.max(teams.reduce((a,t)=>a+t.mitLeads,0),1))+'%</strong></span>'+
    '</div>'+
    '</div></div>'
  ...
  // 슬라이드 2: 핵심 성과 + 상품별
  // ────────────────────────────────────────────────────────────
  const mitLeadsTotal=teams.reduce((a,t)=>a+t.mitLeads,0)
  const mitActTotal=mitT
  const prodEntries=PRODS.filter(p=>globalProdCounts[p]).map(p=>[p,globalProdCounts[p]])
  const maxProd=prodEntries.length>0?Math.max(...prodEntries.map(([,v])=>v)):1
  const PCOLS_MAP={인터넷:'#2563eb',TV:'#3b82f6',다량회선:'#22c55e',모바일:'#16a34a',일반전화:'#64748b',하이오더:'#e67e00',로봇:'#7c3aed',CCTV:'#0891b2'}
  const s2='<div class="slide" id="s2">'+
    sh('핵심 성과 요약','전체 팀 통합 기준 · 상품별 세분화',2,10)+
    slop+
    '<div class="g4">'+
    '<div class="kpi"><div class="kt" style="background:#16a34a"></div><div style="font-size:40px;font-weight:700;color:#16a34a;line-height:1">'+total+'<span style="font-size:16px">건</span></div><div style="font-size:11px;color:#64748b;margin-top:4px">총 성공</div><div style="font-size:10px;color:#94a3b8;margin-top:2px">전체 팀 합산</div></div>'+
    '<div class="kpi"><div class="kt" style="background:#2563eb"></div><div style="font-size:40px;font-weight:700;color:#2563eb;line-height:1">'+mitActTotal+'<span style="font-size:16px">건</span></div><div style="font-size:11px;color:#64748b;margin-top:4px">MIT 활동 완료</div><div style="font-size:10px;color:#94a3b8;margin-top:2px">배분 '+mitLeadsTotal+'건 중</div></div>'+
    '<div class="kpi"><div class="kt" style="background:#e67e00"></div><div style="font-size:40px;font-weight:700;color:#e67e00;line-height:1">'+pct(mitActTotal,Math.max(mitLeadsTotal,1))+'<span style="font-size:16px">%</span></div><div style="font-size:11px;color:#64748b;margin-top:4px">MIT 활동률</div><div style="font-size:10px;color:#94a3b8;margin-top:2px">'+mitActTotal+' / '+mitLeadsTotal+'건</div></div>'+
    '<div class="kpi"><div class="kt" style="background:#7c3aed"></div><div style="font-size:40px;font-weight:700;color:#7c3aed;line-height:1">'+pct(total,Math.max(mitActTotal,1))+'<span style="font-size:16px">%</span></div><div style="font-size:11px;color:#64748b;margin-top:4px">성공 전환율</div><div style="font-size:10px;color:#94a3b8;margin-top:2px">'+total+' / '+mitActTotal+'건</div></div>'+
    '</div>'+
    '<div style="background:#f8fafc;border:1px solid #e2e8f0;border-radius:12px;padding:14px 18px;flex:1">'+
    '<div style="font-size:14px;font-weight:700;color:#0f172a;margin-bottom:14px">📦 상품별 성공 실적 세분화</div>'+
    '<div style="display:grid;grid-template-columns:1fr 1fr;gap:24px;align-items:center">'+
    '<div>'+
    (prodEntries.length>0
      ? prodEntries.slice(0,6).map(([p,cnt])=>
          '<div style="display:flex;align-items:center;gap:8px;margin-bottom:7px"><span style="font-size:13px;font-weight:600;min-width:64px">'+p+'</span><div style="flex:1;background:#e8edf2;border-radius:3px;overflow:hidden"><div style="width:'+pct(cnt,maxProd)+'%;height:20px;background:'+(PCOLS_MAP[p]||'#64748b')+';border-radius:3px"></div></div><span style="font-size:15px;font-weight:700;color:'+(PCOLS_MAP[p]||'#64748b')+';min-width:28px;text-align:right">'+cnt+'</span></div>'
        ).join('')
      : '<div style="text-align:center;padding:20px;color:#94a3b8;font-size:13px">상품 데이터 없음<br/><span style="font-size:11px">MM 활동 결과 입력 시 자동 집계됩니다</span></div>'+
        '<div style="font-size:12px;color:#94a3b8;margin-top:8px">현재 등록된 상품 데이터가 없습니다.</div>'
    )+
    '</div>'+
    '<div style="display:flex;align-items:center;gap:18px;justify-content:center">'+
    '<div class="dw">'+prodDonut(globalProdCounts,total)+'<div class="dc"><div style="font-size:30px;font-weight:700;color:#0f172a">'+total+'</div><div style="font-size:11px;color:#64748b">성공건</div></div></div>'+
    '<div>'+
    (prodEntries.length>0
      ? prodEntries.slice(0,6).map(([p,cnt])=>
          '<div style="display:flex;align-items:center;gap:6px;margin-bottom:8px"><span style="width:10px;height:10px;border-radius:2px;background:'+(PCOLS_MAP[p]||'#64748b')+';display:inline-block"></span><span style="font-size:13px;font-weight:600">'+p+'</span><span style="margin-left:auto;font-size:14px;font-weight:700;color:'+(PCOLS_MAP[p]||'#64748b')+';padding-left:16px">'+cnt+'건 ('+pct(cnt,total)+'%)</span></div>'
        ).join('')
      : '<div style="color:#94a3b8;font-size:12px">데이터 없음</div>'
    )+
    '</div></div></div>'+
    '</div>'+
    sf(2,10)+'</div>'
  ...
  // 슬라이드 3: 4대 프로젝트 개요
  // ────────────────────────────────────────────────────────────
  const PT_ICONS={1:'🏭',2:'🏗️',3:'🏢',4:'🤝'}
  const PT_LABELS={1:'기업체 통신환경 개선공사',2:'세움터 신축건물',3:'빌딩/상가 공략',4:'직접 판매'}
  const allProjTotal=projects.reduce((a,p)=>a+(p.total+p.directTotal),0)
  const allProjAct=projects.reduce((a,p)=>a+p.done,0)
  const allProjSuc=projects.reduce((a,p)=>a+p.success,0)
  const s3='<div class="slide" id="s3">'+
    sh('4대 프로젝트 성과 개요','프로젝트별 활동 현황 한눈에 보기',3,10)+
    slop+
    '<div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;flex:1">'+
    projects.map(p=>{
      const actR=pct(p.done,Math.max(p.total,1))
      const col=PT_COLOR[p.id]||'#2563eb'
      const light=PT_LIGHT[p.id]||'#eff6ff'
      const border=PT_BORDER[p.id]||'#bfdbfe'
      const textc=PT_TEXT[p.id]||'#1e40af'
      const dtot=p.total+p.directTotal
      return '<div style="background:'+light+';border:2px solid '+border+';border-radius:12px;padding:14px">'+
        '<div style="font-size:11px;font-weight:700;color:'+textc+';margin-bottom:8px">'+PT_ICONS[p.id]+' '+p.id+'. '+PT_LABELS[p.id]+(p.adminOnly?' — 관리자 배분':' — MM 직접발굴')+'</div>'+
        '<div style="display:flex;align-items:center;gap:14px;margin-bottom:10px">'+
        '<div class="dw">'+donut(p.done,p.success,Math.max(dtot,1),col,100,18)+
        '<div class="dc"><div style="font-size:20px;font-weight:700;color:'+col+'">'+pct(p.done,Math.max(dtot,1))+'%</div><div style="font-size:9px;color:#64748b">활동률</div></div></div>'+
        '<div>'+
        '<div style="font-size:36px;font-weight:700;color:'+col+';line-height:1.1">'+(p.adminOnly?p.done:p.done)+'<span style="font-size:14px;color:#64748b">건</span></div>'+
        '<div style="font-size:11px;color:#64748b;margin-bottom:6px">'+(p.adminOnly?'배분 '+p.total:'직접등록 '+p.directTotal)+'건 중 활동</div>'+
        '<div style="font-size:32px;font-weight:700;color:#16a34a;line-height:1.1">'+p.success+'<span style="font-size:14px;color:#64748b">건</span></div>'+
        '<div style="font-size:11px;color:#64748b">성공 전환</div>'+
        '</div></div>'+
        '<div style="display:flex;gap:4px;flex-wrap:wrap">'+prodBadges(p.prodCounts||{})+'</div>'+
        '</div>'
    }).join('')+
    '</div>'+
    '<div style="background:#f8fafc;border:1px solid #e2e8f0;border-radius:8px;padding:9px 16px;display:flex;justify-content:space-around;align-items:center">'+
    '<div style="text-align:center"><div style="font-size:22px;font-weight:700;color:#0f172a">'+allProjTotal+'</div><div style="font-size:10px;color:#64748b">총 배분+직접</div></div>'+
    '<div style="width:1px;background:#e2e8f0;height:28px"></div>'+
    '<div style="text-align:center"><div style="font-size:22px;font-weight:700;color:#2563eb">'+allProjAct+'</div><div style="font-size:10px;color:#64748b">총 활동완료</div></div>'+
    '<div style="width:1px;background:#e2e8f0;height:28px"></div>'+
    '<div style="text-align:center"><div style="font-size:22px;font-weight:700;color:#16a34a">'+allProjSuc+'</div><div style="font-size:10px;color:#64748b">총 성공</div></div>'+
    '<div style="width:1px;background:#e2e8f0;height:28px"></div>'+
    '<div style="text-align:center"><div style="font-size:22px;font-weight:700;color:#e67e00">'+pct(allProjAct,Math.max(allProjTotal,1))+'%</div><div style="font-size:10px;color:#64748b">전체 활동률</div></div>'+
    '<div style="width:1px;background:#e2e8f0;height:28px"></div>'+
    '<div style="text-align:center"><div style="font-size:22px;font-weight:700;color:#7c3aed">'+pct(allProjSuc,Math.max(allProjAct,1))+'%</div><div style="font-size:10px;color:#64748b">성공 전환율</div></div>'+
    '</div>'+
    '</div>'+sf(3,10)+'</div>'

  // ────────────────────────────────────────────────────────────
  // 슬라이드 4~7: 프로젝트별 상세 (1장씩)
  // ────────────────────────────────────────────────────────────
  function projSlide(p, slideNum) {
    const col=PT_COLOR[p.id]||'#2563eb'
    const light=PT_LIGHT[p.id]||'#eff6ff'
    const border=PT_BORDER[p.id]||'#bfdbfe'
    const textc=PT_TEXT[p.id]||'#1e40af'
    const dtot=p.adminOnly?p.total:p.directTotal
    const remaining=dtot-p.done
    const pc=p.prodCounts||{}
    const prodE=PRODS.filter(x=>pc[x]).map(x=>[x,pc[x]])
    const maxP=prodE.length>0?Math.max(...prodE.map(([,v])=>v)):1

    // 담당 MM 목록 (해당 프로젝트 활동한 MM)
    const projMM=allMM.filter(m=>m.mitAssigned>0).slice(0,5)

    return '<div class="slide" id="s'+slideNum+'">'+
      stb(PT_ICONS[p.id]+' '+PT_LABELS[p.id], (p.adminOnly?'관리자 배분 전용 · MIT 프로젝트':'MM 직접발굴 · 건물/상가 또는 일반 직접 영업')+(p.themes&&p.themes.length>0?' · 테마 '+p.themes.length+'개 운영':''), slideNum, 10, col)+
      slop+
      '<div class="g4">'+
      '<div class="kpi" style="background:'+light+';border-color:'+border+'"><div class="kt" style="background:'+col+'"></div><div style="font-size:36px;font-weight:700;color:'+col+';line-height:1">'+dtot+'<span style="font-size:14px">건</span></div><div style="font-size:11px;color:'+textc+';margin-top:4px;font-weight:600">'+(p.adminOnly?'MIT 배분':'직접 등록')+'</div></div>'+
      '<div class="kpi" style="background:'+light+';border-color:'+border+'"><div class="kt" style="background:'+col+'"></div><div style="font-size:36px;font-weight:700;color:'+col+';line-height:1">'+p.done+'<span style="font-size:14px">건</span></div><div style="font-size:11px;color:'+textc+';margin-top:4px;font-weight:600">활동 완료 ('+pct(p.done,Math.max(dtot,1))+'%)</div></div>'+
      '<div class="kpi" style="background:#f0fdf4;border-color:#86efac"><div class="kt" style="background:#16a34a"></div><div style="font-size:36px;font-weight:700;color:#16a34a;line-height:1">'+p.success+'<span style="font-size:14px">건</span></div><div style="font-size:11px;color:#166534;margin-top:4px;font-weight:600">성공 전환 ('+pct(p.success,Math.max(p.done,1))+'%)</div></div>'+
      '<div class="kpi"><div class="kt" style="background:#e67e00"></div><div style="font-size:36px;font-weight:700;color:#e67e00;line-height:1">'+remaining+'<span style="font-size:14px">건</span></div><div style="font-size:11px;color:#64748b;margin-top:4px;font-weight:600">미활동 잔여</div></div>'+
      '</div>'+
      '<div class="g2" style="flex:1">'+
      // 좌: 도넛 + 상품별
      '<div style="background:'+light+';border:1px solid '+border+';border-radius:12px;padding:14px">'+
      '<div style="font-size:13px;font-weight:700;color:'+textc+';margin-bottom:12px">활동 현황 및 상품별 성공 실적</div>'+
      '<div style="display:flex;align-items:center;gap:14px;margin-bottom:12px">'+
      '<div class="dw">'+donut(p.done,p.success,Math.max(dtot,1),col,120,20)+
      '<div class="dc"><div style="font-size:22px;font-weight:700;color:'+col+'">'+pct(p.done,Math.max(dtot,1))+'%</div><div style="font-size:9px;color:#64748b">활동률</div></div></div>'+
      '<div>'+
      '<div style="display:flex;align-items:center;gap:6px;margin-bottom:6px"><span style="width:10px;height:10px;background:'+col+';border-radius:2px;display:inline-block"></span><span style="font-size:12px;font-weight:600">활동 완료</span><strong style="color:'+col+';margin-left:auto;padding-left:12px">'+p.done+'건</strong></div>'+
      '<div style="display:flex;align-items:center;gap:6px;margin-bottom:6px"><span style="width:10px;height:10px;background:#16a34a;border-radius:2px;display:inline-block"></span><span style="font-size:12px;font-weight:600">성공 전환</span><strong style="color:#16a34a;margin-left:auto;padding-left:12px">'+p.success+'건</strong></div>'+
      '<div style="display:flex;align-items:center;gap:6px"><span style="width:10px;height:10px;background:'+border+';border-radius:2px;display:inline-block"></span><span style="font-size:12px;font-weight:600">미활동</span><strong style="color:#94a3b8;margin-left:auto;padding-left:12px">'+remaining+'건</strong></div>'+
      '</div></div>'+
      '<div style="font-size:12px;font-weight:700;color:'+textc+';margin-bottom:7px">상품별 성공 실적</div>'+
      (prodE.length>0
        ? prodE.slice(0,5).map(([pp,cnt])=>
            '<div style="display:flex;align-items:center;gap:8px;margin-bottom:5px"><span style="font-size:12px;font-weight:600;min-width:60px">'+pp+'</span><div style="flex:1;background:#e8edf2;border-radius:3px;overflow:hidden"><div style="width:'+pct(cnt,maxP)+'%;height:14px;background:'+(PCOLS_MAP[pp]||col)+';border-radius:3px"></div></div><span style="font-weight:700;color:'+(PCOLS_MAP[pp]||col)+';font-size:13px;min-width:22px;text-align:right">'+cnt+'</span></div>'
          ).join('')
        : '<div style="text-align:center;padding:16px;color:#94a3b8;font-size:12px">상품 데이터 없음<br/>활동 결과 입력 시 자동 집계됩니다</div>'
      )+
      '</div>'+
      // 우: 테마 + MM별
      '<div style="display:flex;flex-direction:column;gap:10px">'+
      (p.themes&&p.themes.length>0
        ? '<div style="background:#fff;border:1px solid '+border+';border-radius:10px;padding:12px">'+
          '<div style="font-size:12px;font-weight:700;color:'+textc+';margin-bottom:8px">🏷️ 테마별 활동 현황</div>'+
          '<div style="border:1px solid '+border+';border-radius:8px;overflow:hidden">'+
          '<div style="display:grid;grid-template-columns:1fr 50px 50px 50px;background:'+col+';color:#fff;padding:5px 10px;font-size:10px;font-weight:600;gap:10px"><span>테마명</span><span style="text-align:center">배분</span><span style="text-align:center">활동</span><span style="text-align:center;color:#4ade80">성공</span></div>'+
          p.themes.map((t,i)=>'<div style="display:grid;grid-template-columns:1fr 50px 50px 50px;padding:7px 10px;gap:10px;font-size:11px;align-items:center;'+(i%2===1?'background:#f8fafc':'')+' "><span style="font-weight:600">'+t.name+'</span><span style="text-align:center;color:#64748b">'+t.total+'</span><span style="text-align:center;color:'+col+';font-weight:700">'+t.done+'</span><span style="text-align:center;color:#16a34a;font-weight:700;font-size:14px">'+t.success+'</span></div>').join('')+
          '</div></div>'
        : '<div style="background:#fff;border:1px solid '+border+';border-radius:10px;padding:12px">'+
          '<div style="font-size:12px;font-weight:700;color:'+textc+';margin-bottom:6px">활동 현황</div>'+
          '<div style="font-size:12px;color:#64748b">전체 활동률 <strong style="color:'+col+'">'+pct(p.done,Math.max(dtot,1))+'%</strong> · 성공 전환율 <strong style="color:#16a34a">'+pct(p.success,Math.max(p.done,1))+'%</strong></div>'+
          (remaining>0?'<div style="margin-top:8px;background:#fef3c7;border:1px solid #fcd34d;border-radius:7px;padding:7px 10px;font-size:11px;color:#92400e">⚠ 미활동 '+remaining+'건 추가 접촉 필요</div>':'<div style="margin-top:8px;background:#f0fdf4;border:1px solid #86efac;border-radius:7px;padding:7px 10px;font-size:11px;color:#166534">✅ 전체 활동 완료</div>')+
          '</div>'
      )+
      '<div style="background:#fff;border:1px solid '+border+';border-radius:10px;padding:12px;flex:1">'+
      '<div style="font-size:12px;font-weight:700;color:'+textc+';margin-bottom:8px">👤 담당 MM 활동 현황</div>'+
      (allMM.length>0
        ? allMM.slice(0,6).map(m=>'<div style="display:flex;align-items:center;gap:7px;margin-bottom:6px"><span style="font-size:12px;font-weight:600;min-width:52px;color:'+(m.mitDone===0?'#dc2626':'#0f172a')+'">'+m.name+'</span><div style="flex:1;background:#e8edf2;border-radius:3px;overflow:hidden"><div style="width:'+Math.min(pct(m.mitDone,Math.max(m.mitAssigned,1)),100)+'%;height:10px;background:'+(m.mitDone===0?'#fca5a5':col)+';border-radius:3px"></div></div><span style="font-size:10px;color:#64748b;min-width:80px;text-align:right">활동 '+m.mitDone+' · 성공 <strong style="color:'+(m.success>0?'#16a34a':'#94a3b8')+'">'+m.success+'</strong></span></div>').join('')
        : '<div style="color:#94a3b8;font-size:12px">데이터 없음</div>'
      )+
      '</div>'+
      '</div>'+
  ...
  const s4=projSlide(projects[0]||{id:1,total:0,directTotal:0,done:0,success:0,prodCounts:{},themes:[]},4)
  const s5=projSlide(projects[1]||{id:2,total:0,directTotal:0,done:0,success:0,prodCounts:{},themes:[]},5)
  const s6=projSlide(projects[2]||{id:3,total:0,directTotal:0,done:0,success:0,prodCounts:{},themes:[]},6)
  const s7=projSlide(projects[3]||{id:4,total:0,directTotal:0,done:0,success:0,prodCounts:{},themes:[]},7)

  // ────────────────────────────────────────────────────────────
  // 슬라이드 8: 팀별 성과
  // ────────────────────────────────────────────────────────────
  const s8='<div class="slide" id="s8">'+
    sh('팀별 성과 현황','프로젝트별 · 상품별 세부 비교',8,10)+
    slop+
    '<div class="g2">'+
    teams.map((t,ti)=>{
      const bgHead=ti===0?'#0f172a':'#1e3a5f'
      const pc=t.prodCounts||{}
      return '<div style="background:#fff;border:1px solid #e2e8f0;border-radius:12px;overflow:hidden">'+
        '<div style="background:'+bgHead+';color:#fff;padding:10px 16px;display:flex;justify-content:space-between;align-items:center">'+
        '<div><span style="font-size:17px;font-weight:700">'+t.label+'</span><span style="font-size:11px;opacity:0.45;margin-left:10px">MM '+t.mm+'명</span></div>'+
        '<div style="font-size:28px;font-weight:700;color:#4ade80">'+t.success+'<span style="font-size:12px">건</span></div>'+
        '</div>'+
        '<div style="padding:13px 16px">'+
        '<div class="g3" style="margin-bottom:12px">'+
        '<div style="text-align:center;background:#f0fdf4;border-radius:8px;padding:8px"><div style="font-size:22px;font-weight:700;color:#16a34a">'+t.success+'</div><div style="font-size:10px;color:#64748b">성공건</div></div>'+
        '<div style="text-align:center;background:#eff6ff;border-radius:8px;padding:8px"><div style="font-size:22px;font-weight:700;color:#2563eb">'+t.mitDone+'</div><div style="font-size:10px;color:#64748b">MIT활동</div></div>'+
        '<div style="text-align:center;background:#fff7ed;border-radius:8px;padding:8px"><div style="font-size:22px;font-weight:700;color:#e67e00">'+pct(t.mitDone,Math.max(t.mitLeads,1))+'%</div><div style="font-size:10px;color:#64748b">활동률</div></div>'+
        '</div>'+
        '<div style="font-size:11px;font-weight:700;color:#64748b;margin-bottom:7px">프로젝트별 활동</div>'+
        projects.filter(p=>p.total>0||p.directTotal>0).map(p=>{
          const dtot=p.adminOnly?p.total:p.directTotal
          return '<div style="display:flex;align-items:center;gap:6px;margin-bottom:5px;font-size:12px">'+
            '<span style="width:8px;height:8px;background:'+(PT_COLOR[p.id]||'#2563eb')+';border-radius:2px;display:inline-block"></span>'+
            '<span style="font-weight:600;min-width:80px;font-size:11px">'+PT_ICONS[p.id]+' '+PT_LABELS[p.id].slice(0,8)+'</span>'+
            '<div style="flex:1;background:#e8edf2;border-radius:3px;overflow:hidden"><div style="width:'+Math.min(pct(p.done,Math.max(dtot,1)),100)+'%;height:8px;background:'+(PT_COLOR[p.id]||'#2563eb')+';border-radius:3px"></div></div>'+
            '<span style="font-size:10px;color:#64748b;padding-left:5px">'+p.done+'건 활동 · <strong style="color:#16a34a">'+p.success+'건 성공</strong></span>'+
            '</div>'
        }).join('')+
        '<div style="font-size:11px;font-weight:700;color:#64748b;margin-bottom:7px;margin-top:10px">상품별 성공</div>'+
        '<div style="display:flex;gap:4px;flex-wrap:wrap">'+prodBadges(pc)+'</div>'+
        '</div></div>'
    }).join('')+
    '</div>'+
    '<div style="background:#f8fafc;border:1px solid #e2e8f0;border-radius:8px;padding:9px 16px;display:grid;grid-template-columns:1fr 1fr;gap:16px">'+
    '<div><div style="font-size:11px;font-weight:700;color:#64748b;margin-bottom:6px">MIT 활동률 비교</div>'+
    teams.map(t=>'<div style="display:flex;align-items:center;gap:8px;margin-bottom:5px;font-size:12px"><span style="font-weight:700;min-width:50px">'+t.label+'</span><div style="flex:1;background:#e8edf2;border-radius:3px;overflow:hidden"><div style="width:'+pct(t.mitDone,Math.max(t.mitLeads,1))+'%;height:9px;background:#2563eb;border-radius:3px"></div></div><strong style="color:#2563eb;padding-left:6px">'+pct(t.mitDone,Math.max(t.mitLeads,1))+'%</strong></div>').join('')+
    '</div>'+
    '<div><div style="font-size:11px;font-weight:700;color:#64748b;margin-bottom:6px">성공 전환율 비교</div>'+
    teams.map(t=>'<div style="display:flex;align-items:center;gap:8px;margin-bottom:5px;font-size:12px"><span style="font-weight:700;min-width:50px">'+t.label+'</span><div style="flex:1;background:#e8edf2;border-radius:3px;overflow:hidden"><div style="width:'+pct(t.success,Math.max(t.mitDone,1))+'%;height:9px;background:#7c3aed;border-radius:3px"></div></div><strong style="color:#7c3aed;padding-left:6px">'+pct(t.success,Math.max(t.mitDone,1))+'%</strong></div>').join('')+
    '</div></div>'+
    '</div>'+sf(8,10)+'</div>'
  ...
  // 슬라이드 9: 전직원 세부
  // ────────────────────────────────────────────────────────────
  const sorted=[...allMM].sort((a,b)=>b.success-a.success)
  const medals=['🥇','🥈','🥉']
  const s9='<div class="slide" id="s9">'+
    sh('전직원 세부 활동 실적','전원 · 프로젝트별 · 상품별 세부 · 성공 실적 내림차순',9,10)+
    slop+
    '<table>'+
    '<thead><tr><th style="width:28px">#</th><th>이름</th><th>팀</th><th>MIT배분</th><th>MIT활동</th><th>활동률</th><th>직접발굴</th><th style="font-size:13px">성공</th><th>주요 상품</th><th>주요 프로젝트</th></tr></thead><tbody>'+
    sorted.map((m,i)=>{
      const actR=pct(m.mitDone,Math.max(m.mitAssigned,1))
      const pc=m.prodCounts||{}
      const isZero=m.mitDone===0&&m.mitAssigned>0
      return '<tr style="'+(i===0?'background:#fefce8':isZero?'background:#fef2f2':'')+'">'+
        '<td style="font-size:'+(i<3?'16':'13')+'px">'+(medals[i]||'<span style="color:#94a3b8;font-weight:600">'+(i+1)+'</span>')+'</td>'+
        '<td><strong style="font-size:'+(i===0?'13':'11')+'px">'+m.name+'</strong></td>'+
        '<td>'+m.team+'</td>'+
        '<td>'+m.mitAssigned+'</td>'+
        '<td style="color:#2563eb;font-weight:700">'+m.mitDone+'</td>'+
        '<td><span style="background:'+(actR>=70?'#dbeafe':actR>0?'#ffedd5':'#fee2e2')+';color:'+(actR>=70?'#1e40af':actR>0?'#9a3412':'#991b1b')+';padding:2px 8px;border-radius:14px;font-size:11px;font-weight:600">'+actR+'%</span></td>'+
        '<td style="color:'+(m.directDone>0?'#059669':'#94a3b8')+';font-weight:'+(m.directDone>0?'700':'400')+'">'+m.directDone+'</td>'+
        '<td style="font-size:'+(i===0?'19':'15')+'px;font-weight:700;color:'+(m.success>0?'#16a34a':'#94a3b8')+'">'+m.success+'</td>'+
        '<td>'+prodBadges(pc,3)+'</td>'+
        '<td>'+(isZero?'<span style="background:#fee2e2;color:#991b1b;padding:2px 7px;border-radius:4px;font-size:10px;font-weight:600">⚠ 즉각확인</span>':'<span style="font-size:10px;color:#64748b">-</span>')+'</td>'+
        '</tr>'
    }).join('')+
    '</tbody></table>'+
    (sorted.some(m=>m.mitDone===0&&m.mitAssigned>0)?'<div style="background:#fef2f2;border:1px solid #fecaca;border-radius:7px;padding:8px 14px;font-size:12px;color:#991b1b;font-weight:600;margin-top:4px">⚠ '+sorted.filter(m=>m.mitDone===0&&m.mitAssigned>0).map(m=>m.name).join(', ')+' MM — 이달 활동 0건. 즉각 확인 및 지원 필요</div>':'')+
    '</div>'+sf(9,10)+'</div>'

  // ────────────────────────────────────────────────────────────
  // 슬라이드 10: 성과 우수자
  // ────────────────────────────────────────────────────────────
  const top5=[...allMM].sort((a,b)=>b.success-a.success).slice(0,5)
  const medalBgs=['#fef9c3','#f1f5f9','#fdf4e7','#f8fafc','#f8fafc']
  const medalBords=['#fde68a','#e2e8f0','#e2e8f0','#e2e8f0','#e2e8f0']
  const medalCirc=['#f59e0b','#94a3b8','#b45309','#cbd5e1','#cbd5e1']
  const s10='<div class="slide" id="s10">'+
    sh('성과 우수자 현황','성공 실적 기준 · 상품별 상세',10,10)+
    '<div style="padding:14px 34px 16px;flex:1;display:grid;grid-template-columns:1.3fr 0.7fr;gap:18px">'+
    '<div>'+
    '<div style="font-size:12px;font-weight:700;color:#64748b;margin-bottom:10px">TOP 성과자</div>'+
    (top5[0]?'<div style="display:flex;align-items:center;gap:13px;padding:13px;background:#fefce8;border:1.5px solid #fde68a;border-radius:10px;margin-bottom:8px">'+
      ...
      '<div style="flex:1"><div style="font-size:16px;font-weight:700">'+top5[0].name+'</div><div style="font-size:11px;color:#64748b;margin-bottom:5px">'+top5[0].team+' · MIT '+top5[0].mitDone+'건 활동 ('+pct(top5[0].mitDone,Math.max(top5[0].mitAssigned,1))+'%) · 직접발굴 '+top5[0].directDone+'건</div>'+
      '<div style="display:flex;gap:4px">'+prodBadges(top5[0].prodCounts||{},5)+'</div>'+
      '</div>'+
      '<div style="text-align:right"><div style="font-size:36px;font-weight:700;color:#16a34a">'+top5[0].success+'<span style="font-size:14px">건</span></div></div>'+
      '</div>':'<div style="color:#94a3b8;font-size:12px">성과 데이터 없음</div>')+
    '<div style="display:grid;grid-template-columns:1fr 1fr;gap:7px">'+
    top5.slice(1).map((m,i)=>'<div style="display:flex;align-items:center;gap:9px;padding:10px;background:'+medalBgs[i+1]+';border:1px solid '+medalBords[i+1]+';border-radius:9px">'+
      '<div style="width:30px;height:30px;border-radius:50%;background:'+medalCirc[i+1]+';display:flex;align-items:center;justify-content:center;font-size:'+(i<2?'16':'12')+'px;font-weight:700;color:#fff;flex-shrink:0">'+(medals[i+1]||(i+2))+'</div>'+
      '<div style="flex:1"><div style="font-size:13px;font-weight:700">'+m.name+'</div><div style="display:flex;gap:3px;margin-top:3px;flex-wrap:wrap">'+prodBadges(m.prodCounts||{},2)+'</div></div>'+
      '<div style="font-size:22px;font-weight:700;color:#16a34a">'+m.success+'</div>'+
      '</div>').join('')+
    '</div></div>'+
    '<div style="display:flex;flex-direction:column;gap:10px">'+
    '<div style="background:#f8fafc;border:1px solid #e2e8f0;border-radius:10px;padding:14px">'+
    '<div style="font-size:12px;font-weight:700;color:#0f172a;margin-bottom:10px">팀별 달성 현황</div>'+
    teams.map(t=>'<div style="margin-bottom:10px"><div style="display:flex;justify-content:space-between;font-size:13px;font-weight:700;margin-bottom:4px"><span>'+t.label+'</span><span style="color:#16a34a">'+t.success+'건</span></div><div style="background:#e8edf2;border-radius:3px;overflow:hidden;height:11px"><div style="width:'+pct(t.success,Math.max(total,1))+'%;height:11px;background:#16a34a;border-radius:3px"></div></div><div style="font-size:10px;color:#94a3b8;margin-top:2px">MIT 활동률 '+pct(t.mitDone,Math.max(t.mitLeads,1))+'% · 전환율 '+pct(t.success,Math.max(t.mitDone,1))+'%</div></div>').join('')+
    '</div>'+
    '<div style="background:#f0fdf4;border:1px solid #bbf7d0;border-radius:10px;padding:14px;flex:1">'+
    '<div style="font-size:12px;font-weight:700;color:#166534;margin-bottom:10px">이달 핵심 성과</div>'+
    '<div style="font-size:12px;color:#374151;line-height:2.1">'+
    (top5[0]?'<div>'+top5[0].name+' MM 최고 <strong style="color:#16a34a;font-size:14px">'+top5[0].success+'건</strong> 달성</div>':'')+
    (Object.keys(globalProdCounts).length>0?'<div>상품 1위 <strong style="color:#2563eb">'+PRODS.find(p=>globalProdCounts[p]===Math.max(...Object.values(globalProdCounts)))||'-'+'</strong></div>':'')+
    '<div>MIT 전체 활동률 <strong style="color:#e67e00">'+pct(mitT,Math.max(teams.reduce((a,t)=>a+t.mitLeads,0),1))+'%</strong></div>'+
    '<div>성공 전환율 <strong style="color:#7c3aed">'+pct(total,Math.max(mitT,1))+'%</strong></div>'+
    (sorted.some(m=>m.mitDone===0&&m.mitAssigned>0)?'<div style="color:#dc2626">'+sorted.filter(m=>m.mitDone===0&&m.mitAssigned>0).map(m=>m.name).join('·')+' 미활동 확인 필요</div>':'')+
    '</div></div></div>'+
    '</div>'+
    sf(10,10)+'</div>'

  // 네비
  const navBtns=Array.from({length:10},(_,i)=>'<button class="nb" onclick="document.getElementById(\'s'+(i+1)+'\').scrollIntoView({behavior:\'smooth\'})">'+(i+1)+'</button>').join('')
