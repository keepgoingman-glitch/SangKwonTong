...
function buildSlideHTML({ period, today, yearStr, teams, projects, allMM, topMM,
  ...
  *{margin:0;padding:0;box-sizing:border-box;-webkit-print-color-adjust:exact!important;print-color-adjust:exact!important}
  ...
  function psAgencySlide(sn){
    ...
    function agLines(l){try{const wp=JSON.parse(l.won_products||'null');if(wp&&!Array.isArray(wp))return Object.values(wp).reduce((a,v)=>a+(+v||0),0);if(Array.isArray(wp))return wp.length}catch{}return l.won_lines||1}
    ...
    // 성공+대리점 건 수집 (배분+직접발굴), 월간여부 플래그 포함
    const collect = (onlyMonth) => {
      const arr=[]
      ;(D.leads||[]).filter(l=>l.activity_status==='성공'&&myNM2.includes(l.assigned_to)&&l.agency_name&&(!onlyMonth||inMonth(l))).forEach(l=>arr.push({type:l.agency_type||'미분류',name:l.agency_name,lines:agLines(l)}))
      ;(D.directLeads||[]).filter(l=>l.activity_status==='성공'&&myUN2.includes(l.mm_username)&&l.agency_name&&(!onlyMonth||inMonth(l))).forEach(l=>arr.push({type:l.agency_type||'미분류',name:l.agency_name,lines:agLines(l)}))
      return arr
    }
    const aggregate = (arr) => {
      const byType={}
      arr.forEach(s=>{if(!byType[s.type])byType[s.type]={count:0,lines:0,agencies:{}};byType[s.type].count++;byType[s.type].lines+=s.lines;byType[s.type].agencies[s.name]=(byType[s.type].agencies[s.name]||0)+1})
      return { byType, totCount:arr.length, totLines:arr.reduce((a,s)=>a+s.lines,0) }
    }
    ...
    const renderTable = (data, label, accent) => {
      const types=Object.keys(data.byType).sort((a,b)=>data.byType[b].count-data.byType[a].count)
      const dispTypes=[...new Set([...types,...allTypes])]
      return `
      <div style="font-size:13px;font-weight:800;color:${accent};margin-bottom:6px">${label} <span style="font-size:11px;color:#94a3b8;font-weight:400">— 성공 ${data.totCount}건 · ${data.totLines}회선</span></div>
      <table>
        <thead><tr><th style="text-align:left;padding-left:10px">대리점 유형</th><th>성공 건수</th><th style="background:#15803d">수주 회선</th><th>비중</th></tr></thead>
        <tbody>
          ${dispTypes.length===0?`<tr><td colspan="4" style="color:#94a3b8;padding:14px">실적 없음</td></tr>`:dispTypes.map((t,i)=>{
            const d=data.byType[t]||{count:0,lines:0}
            return `<tr style="background:${i%2?'#f8fafc':'#fff'}">
              ...
              <td style="font-weight:800">${d.count||'-'}</td>
              <td style="background:#f0fdf4;font-weight:900;color:#15803d">${d.lines||'-'}</td>
              <td>${data.totCount>0?pct(d.count,data.totCount):0}%</td>
            </tr>`}).join('')}
          <tr style="background:#16213e;color:#fff;font-weight:900">
            <td style="color:#fff;text-align:left;padding-left:10px">전 체</td>
            <td style="color:#fff">${data.totCount||'-'}</td>
            <td style="background:#166534;color:#fff">${data.totLines||'-'}</td>
    ...
    return `<div class="slide" id="s${sn}">
  ${rh('🤝 P&S 대리점 유형별 성과','패스앤슈팅 대리점 성과 현황 (월간 / 누적)',sn)}
  ${dbar}
  <div class="body">
    <div style="display:flex;gap:10px;margin-bottom:12px">
      <div style="flex:1;background:#eff6ff;border:1.5px solid #bfdbfe;border-radius:12px;padding:11px;text-align:center"><div style="font-size:22px;font-weight:900;color:#2563eb">${monthData.totCount}<span style="font-size:12px">건</span></div><div style="font-size:10.5px;color:#64748b">월간 P&S 성공</div></div>
      <div style="flex:1;background:#f0fdf4;border:1.5px solid #bbf7d0;border-radius:12px;padding:11px;text-align:center"><div style="font-size:22px;font-weight:900;color:#16a34a">${monthData.totLines}<span style="font-size:12px">회선</span></div><div style="font-size:10.5px;color:#64748b">월간 수주 회선</div></div>
      <div style="flex:1;background:#faf5ff;border:1.5px solid #e9d5ff;border-radius:12px;padding:11px;text-align:center"><div style="font-size:22px;font-weight:900;color:#7c3aed">${cumData.totCount}<span style="font-size:12px">건</span></div><div style="font-size:10.5px;color:#64748b">누적 P&S 성공</div></div>
      <div style="flex:1;background:#fff7ed;border:1.5px solid #fed7aa;border-radius:12px;padding:11px;text-align:center"><div style="font-size:22px;font-weight:900;color:#e67e00">${cumData.totLines}<span style="font-size:12px">회선</span></div><div style="font-size:10.5px;color:#64748b">누적 수주 회선</div></div>
    ...
    <div style="display:flex;gap:16px">
      <div style="flex:1">${renderTable(monthData,'▶ 월간 유형별 성과','#15803d')}</div>
      <div style="flex:1">${renderTable(cumData,'▶ 누적 유형별 성과','#7c3aed')}</div>
    </div>
    <div style="margin-top:12px">
      <div style="font-size:13px;font-weight:800;color:#16213e;margin-bottom:6px">대리점별 성공 건수 (누적 상위)</div>
      ${(()=>{
        const types=Object.keys(cumData.byType)
        const allAg=[]
        types.forEach(t=>{Object.entries(cumData.byType[t].agencies).forEach(([name,cnt])=>allAg.push({type:t,name,cnt}))})
        allAg.sort((a,b)=>b.cnt-a.cnt)
        if(allAg.length===0)return '<div style="color:#94a3b8;font-size:13px;padding:14px;text-align:center;background:#f8fafc;border-radius:10px">대리점별 실적이 없습니다</div>'
        const half=Math.ceil(Math.min(allAg.length,12)/2)
        const left=allAg.slice(0,half), right=allAg.slice(half,12)
        const col=(list)=>`<table><thead><tr><th style="text-align:left;padding-left:10px">대리점명</th><th>유형</th><th style="background:#15803d">성공</th></tr></thead><tbody>${list.map((a,i)=>`<tr style="background:${i%2?'#f8fafc':'#fff'}"><td style="text-align:left;font-weight:700;padding-left:10px;font-size:11.5px">${a.name}</td><td style="font-size:10px;color:#64748b">${a.type}</td><td style="background:#f0fdf4;font-weight:900;color:#15803d">${a.cnt}</td></tr>`).join('')}</tbody></table>`
        return `<div style="display:flex;gap:16px"><div style="flex:1">${col(left)}</div><div style="flex:1">${right.length?col(right):''}</div></div>${allAg.length>12?`<div style="font-size:11px;color:#94a3b8;text-align:center;margin-top:4px">외 ${allAg.length-12}개</div>`:''}`
      })()}
    </div>
    <div style="margin-top:8px;font-size:11px;color:#94a3b8">※ 월간 = 당월 개통/활동 성공건 · 누적 = 전체 기간 성공건 · 회선수는 수주 상품 기준</div>
  </div>
  ${ftr(sn)}
</div>`
  }
