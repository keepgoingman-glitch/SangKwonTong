...
export default function Connector({ onBack }) {
  ...
  const handleUpload = async () => {
    ...
    setUploading(true)
    try { for (const row of uploadPreview) { const { _headers, ...dbRow } = row; await db.post('connector_leads', dbRow) }; t2(uploadPreview.length + '건 업로드 완료!'); setUploadPreview([]); setCsvHeaders([]); load(); setTab('leads') }
