...
function buildSlideHTML({ period, teams, projects, allMM, topMM, mDiscoveryCount, globalProdCounts = {} }) {
  ...
  const N = 5 + (allMM.length>0?1:0)  // 표지+요약+팀별+4대프로젝트+전직원(옵션)+우수자+종합
...
`
  ...
  // ── 슬라이드 4: 프로젝트 현황 ────────────────────────────────
  // ── 슬라이드 4: 4대 프로젝트별 성과 ───────────────────────────
  const projSlide = '<div class="slide" id="s4">'+
    slideHead(4,'4대 프로젝트별 성과','프로젝트 기반 영업 활동 현황')+
    '<table><thead><tr><th>프로젝트</th><th>MIT배분</th><th>직접발굴</th><th>활동</th><th>성공</th><th>활동률</th><th>성공 상품</th></tr></thead><tbody>'+
    '<div class="sb">'+
    projects.map(p=>{
      const ar=pct(p.done,Math.max(p.total,1))
      const dr=pct(p.success,Math.max(p.done+p.directTotal,1))
      const pc=p.prodCounts||{}
      const topP=PRODS_ORDER.filter(x=>pc[x]).slice(0,4).map(x=>'<span style="background:#f0fdf4;border:1px solid #bbf7d0;border-radius:5px;padding:2px 8px;font-size:10px;font-weight:800;color:#15803d;margin-right:3px">'+x+' '+pc[x]+'</span>').join('')
      const themeRows=(p.themes||[]).map(t=>'<tr style="background:#f8fafc"><td style="padding-left:20px;font-size:11px;color:#64748b">└ '+t.name+'</td><td colspan="2" style="font-size:11px;color:#64748b">'+t.total+'건</td><td style="font-size:11px;color:#2563eb;font-weight:700">'+t.done+'건</td><td style="font-size:11px;color:#16a34a;font-weight:800">'+t.success+'건</td><td colspan="2"></td></tr>').join('')
      return '<tr>'+
        '<td><div style="font-weight:900;font-size:13px">'+p.icon+' '+p.id+'. '+p.name+'</div>'+(p.adminOnly?'<div style="font-size:10px;color:#94a3b8">관리자 배분</div>':'<div style="font-size:10px;color:#7c3aed">MM 직접등록 가능</div>')+'</td>'+
        '<td style="font-weight:700">'+p.total+'건</td>'+
        '<td style="color:#7c3aed;font-weight:700">'+p.directTotal+'건</td>'+
        '<td style="color:#2563eb;font-weight:700">'+p.done+'건</td>'+
        '<td style="color:#16a34a;font-weight:900;font-size:15px">'+p.success+'건</td>'+
        ...
        '<td>'+(topP||'<span style="color:#94a3b8;font-size:10px">없음</span>')+'</td></tr>'+themeRows
    }).join('')+'</tbody></table>'+
    '</div>'+sf+'</div>'

  // ── 슬라이드 5: 전직원 세부 활동 ─────────────────────────────
  const slideNum_detail = 5  // 표지(1)+요약(2)+팀별(3)+4대프로젝트(4)+전직원(5)
