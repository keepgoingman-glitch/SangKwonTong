...
export default function Connector({ onBack }) {
  ...
  const [mDiscStage, setMDiscStage] = useState('전체')  // M발굴 단계 필터
...
function buildSlideHTML({ period, today, yearStr, teams, projects, allMM, topMM,
  ...
  bizDays=0, perMMTarget=0, workDaysAll={}, allDailyStats=[], _debug={}, reportMode='current' }) {
  // reportMode: 'current' = 주간/월간/누적, 'past' = 월마감/누적
  ...
  // 상품 분류: 인터넷/TV/모바일/기타 4분류
  function prodGroup(pc){
    const inet=(pc['인터넷']||0)+(pc['다량회선']||0)
    const tv=pc['TV']||0
    const mob=pc['모바일']||0
    const etc=Object.entries(pc).filter(([k])=>!['인터넷','다량회선','TV','모바일'].includes(k)).reduce((a,[,v])=>a+v,0)
    return {inet,tv,mob,etc,total:inet+tv+mob+etc}
  }
  ...
  *{margin:0;padding:0;box-sizing:border-box}
  ...
  th{background:#16213e;color:#fff;padding:9px 8px;font-size:12px;font-weight:700;border:1px solid #2a3a5c;text-align:center}
  td{padding:8px;font-size:12px;border:1px solid #e2e8f0;text-align:center;color:#1e293b}
  .rh{padding:22px 40px 12px;border-bottom:3px solid #e67e00;display:flex;justify-content:space-between;align-items:flex-end}
  .rt{font-size:25px;font-weight:900;color:#16213e}
  .rb{font-size:13px;background:#fff7ed;color:#e67e00;border:1px solid #fed7aa;padding:3px 12px;border-radius:20px;font-weight:700;margin-left:10px}
  .rd{font-size:12px;color:#94a3b8}
  .body{flex:1;padding:16px 40px;overflow:hidden}
  ...
  .kbox{flex:1;background:#f8fafc;border:1.5px solid #e2e8f0;border-radius:12px;padding:14px;text-align:center}
  .kv{font-size:26px;font-weight:900}
  .kl{font-size:11px;color:#64748b;margin-top:3px}
  @media print{body{background:#fff}.slide{margin:0}.print-btn{display:none!important}}
  ...
  function rh(title, badge, num){
    return `<div class="rh"><div><span class="rt">${title}</span>${badge?`<span class="rb">${badge}</span>`:''}</div><div class="rd">${today} · ${num}/7</div></div>`
  }
  ...
  <div style="text-align:center">
    ...
    <div style="font-size:20px;color:rgba(255,255,255,0.7);margin-bottom:36px">${yearStr} ${period} 실적 현황</div>
    <div style="display:flex;gap:16px;justify-content:center">
      ${teams.map(t=>`<div style="background:rgba(255,255,255,0.08);border:1px solid rgba(255,255,255,0.15);border-radius:14px;padding:18px 30px;min-width:200px">
        ...
        <div style="font-size:13px;color:rgba(255,255,255,0.85);line-height:1.9">MM ${t.mm}명 · 목표 ${t.goal}건<br/>월간활동 <b style="color:#60a5fa">${(t.monthly?.mitDone||0)+(t.monthly?.directDone||0)}건</b> · 성공 <b style="color:#4ade80">${t.monthly?.success||0}건</b></div>
      </div>`).join('')}
    </div>
  </div>
  ...
  const projRows = projects.map(p=>{
    // 프로젝트별 주간 (mmStats의 프로젝트별 weekly 합산)
    ...
    const pg = prodGroup(p.prodCounts||{})
  ...
  const sumRow = projRows.reduce((a,r)=>({
    ...
    inet:a.inet+r.pg.inet, tv:a.tv+r.pg.tv, mob:a.mob+r.pg.mob, etc:a.etc+r.pg.etc
  }),{wD:0,wS:0,mD:0,mS:0,yD:0,yS:0,inet:0,tv:0,mob:0,etc:0})

  const s2=`<div class="slide" id="s2">
  ${rh('프로젝트별 종합 현황','주간 → 월간 → 누적 + 상품별 실적',2)}
  <div class="body">
    <div class="kpi">
      ${isPast?'':`<div class="kbox"><div class="kv" style="color:#2563eb">${wAct}<span style="font-size:14px">건</span></div><div class="kl">주간 활동</div></div>
      <div class="kbox"><div class="kv" style="color:#16a34a">${wSuc}<span style="font-size:14px">건</span></div><div class="kl">주간 성공</div></div>`}
      <div class="kbox"><div class="kv" style="color:#2563eb">${mAct}<span style="font-size:14px">건</span></div><div class="kl">${isPast?'월마감 활동':'월간 활동'} (목표 ${totalMonthTarget}건 · ${pct(mAct,totalMonthTarget)}%)</div></div>
      <div class="kbox"><div class="kv" style="color:#16a34a">${mSuc}<span style="font-size:14px">건</span></div><div class="kl">${isPast?'월마감 성공':'월간 성공'} (성공률 ${sucRate(mSuc,mAct)}%)</div></div>
      <div class="kbox"><div class="kv" style="color:#7c3aed">${yAct}<span style="font-size:14px">건</span></div><div class="kl">누적 활동</div></div>
      <div class="kbox"><div class="kv" style="color:#7c3aed">${ySuc}<span style="font-size:14px">건</span></div><div class="kl">누적 성공</div></div>
    </div>
    <table>
      <thead>
        <tr>
          <th rowspan="2" style="width:200px">프로젝트</th>
          <th rowspan="2">대상건수</th>
          ${isPast?'':'<th colspan="2" style="background:#1e3a5f">주간</th>'}
          <th colspan="3" style="background:#1e3a5f">${isPast?'월마감':'월간'}</th>
          <th colspan="2" style="background:#1e3a5f">누적</th>
          <th colspan="5" style="background:#14532d">상품별 실적 (회선)</th>
        </tr>
        <tr>
          ${isPast?'':'<th>활동</th><th>성공</th>'}
          <th>활동</th><th>성공</th><th>성공률</th>
          <th>활동</th><th>성공</th>
          <th>인터넷</th><th>TV</th><th>모바일</th><th>기타</th><th style="background:#166534">합계</th>
        </tr>
      </thead>
      <tbody>
        ${projRows.map(({p,wD,wS,mD,mS,yD,yS,pg},i)=>`
        <tr style="background:${i%2?'#f8fafc':'#fff'}">
          <td style="text-align:left;font-weight:800;color:${PC[p.id]||'#1e293b'}">${p.icon} ${p.name}</td>
          <td>${p.adminOnly?(p.total||0):(p.directTotal||0)}건</td>
          ${isPast?'':`<td class="bc">${wD}</td><td class="gc">${wS}</td>`}
          <td class="bc">${mD}</td><td class="gc">${mS}</td><td>${sucRate(mS,mD)}%</td>
          <td class="bc">${yD}</td><td class="gc">${yS}</td>
          <td>${pg.inet||'-'}</td><td>${pg.tv||'-'}</td><td>${pg.mob||'-'}</td><td>${pg.etc||'-'}</td>
          <td style="background:#f0fdf4;font-weight:900;color:#15803d">${pg.total||'-'}</td>
        </tr>`).join('')}
        <tr style="background:#16213e;color:#fff;font-weight:900">
          <td style="text-align:left;color:#fff">합 계</td>
          <td style="color:#fff">${projRows.reduce((a,r)=>a+(r.p.adminOnly?(r.p.total||0):(r.p.directTotal||0)),0)}건</td>
          ${isPast?'':`<td style="color:#60a5fa">${sumRow.wD}</td><td style="color:#4ade80">${sumRow.wS}</td>`}
          <td style="color:#60a5fa">${sumRow.mD}</td><td style="color:#4ade80">${sumRow.mS}</td><td style="color:#fff">${sucRate(sumRow.mS,sumRow.mD)}%</td>
          <td style="color:#60a5fa">${sumRow.yD}</td><td style="color:#4ade80">${sumRow.yS}</td>
          <td style="color:#fff">${sumRow.inet}</td><td style="color:#fff">${sumRow.tv}</td><td style="color:#fff">${sumRow.mob}</td><td style="color:#fff">${sumRow.etc}</td>
          <td style="background:#166534;color:#fff">${sumRow.inet+sumRow.tv+sumRow.mob+sumRow.etc}</td>
        </tr>
      </tbody>
    </table>
    <div style="margin-top:10px;font-size:11px;color:#94a3b8">※ 상품별 실적은 성공건의 수주 상품·회선수 기준 (회선수 미입력 건은 상품당 1회선으로 집계) · 인터넷=인터넷+다량회선</div>
  </div>
</div>`

  // ════ S3~S7: 프로젝트별 세부 (1장씩) ════
  function projDetailSlide(p, sn){
    if(!p) return `<div class="slide" id="s${sn}"><div class="rh"><div class="rt">프로젝트 데이터 없음</div></div></div>`
    const col=PC[p.id]||'#2563eb'
    const wD=(p.mmStats||[]).reduce((a,m)=>a+(m.weekly?.actAll||0),0)
    const wS=(p.mmStats||[]).reduce((a,m)=>a+(m.weekly?.success||0),0)
    const mD=p.monthly?.done||0, mS=p.monthly?.success||0
    const yD2=p.annual?.done||0, yS2=p.annual?.success||0
    const dtot=p.adminOnly?(p.total||0):(p.directTotal||0)
    const pg=prodGroup(p.prodCounts||{})
    const prodE=PRODS.filter(x=>(p.prodCounts||{})[x]).map(x=>[x,p.prodCounts[x]])
    const mm=p.mmStats||[]
  ...
  <div class="body">
    <div class="kpi">
      <div class="kbox"><div class="kv" style="color:${col}">${dtot}<span style="font-size:13px">건</span></div><div class="kl">${p.adminOnly?'배분 건수':'직접등록 건수'}</div></div>
      ${isPast?'':`<div class="kbox"><div class="kv" style="color:#2563eb">${wD}<span style="font-size:13px">건</span></div><div class="kl">주간 활동 (성공 ${wS})</div></div>`}
      <div class="kbox"><div class="kv" style="color:#2563eb">${mD}<span style="font-size:13px">건</span></div><div class="kl">${isPast?'월마감 활동':'월간 활동'} (활동률 ${pct(mD,Math.max(dtot,1))}%)</div></div>
      <div class="kbox"><div class="kv" style="color:#16a34a">${mS}<span style="font-size:13px">건</span></div><div class="kl">${isPast?'월마감 성공':'월간 성공'} (성공률 ${sucRate(mS,mD)}%)</div></div>
      <div class="kbox"><div class="kv" style="color:#7c3aed">${yS2}<span style="font-size:13px">건</span></div><div class="kl">누적 성공 (활동 ${yD2})</div></div>
    </div>
    <div style="display:flex;gap:16px;height:480px">
      <div style="flex:1.3;display:flex;flex-direction:column;gap:10px;overflow:hidden">
        <div style="font-size:14px;font-weight:800;color:#16213e">담당 MM별 활동 현황</div>
        <table>
          <thead><tr><th style="text-align:left">MM</th><th>팀</th><th>배분/등록</th>${isPast?'':'<th>주간활동</th><th>주간성공</th>'}<th>${isPast?'월마감활동':'월간활동'}</th><th>${isPast?'월마감성공':'월간성공'}</th><th>성공률</th></tr></thead>
          <tbody>
            ${mm.length===0?'<tr><td colspan="${isPast?6:8}" style="color:#94a3b8;padding:20px">활동 MM 없음</td></tr>':mm.map((m,i)=>{
              const mwk=m.weekly||{}
              const mD2=m.monthly?.done||m.monthly?.mitDone||0, mS2=m.monthly?.success||0
              const zero=mD2===0&&(m.mitAssigned||0)>0
              return `<tr style="background:${zero?'#fef2f2':i%2?'#f8fafc':'#fff'}">
                <td style="text-align:left;font-weight:700;color:${zero?'#dc2626':'#1e293b'}">${m.name}</td>
                <td style="font-size:11px">${m.team||'-'}</td>
                <td>${(m.mitAssigned||0)+(m.directTotal||0)}건</td>
                ${isPast?'':`<td class="bc">${mwk.actAll||0}</td><td class="gc">${mwk.success||0}</td>`}
                <td class="${zero?'rc':'bc'}">${mD2}</td><td class="gc">${mS2}</td>
                <td>${sucRate(mS2,mD2)}%</td>
              </tr>`
            }).join('')}
          </tbody>
        </table>
        <div style="font-size:14px;font-weight:800;color:#16213e;margin-top:4px">상품별 수주 실적</div>
        <div style="display:flex;gap:8px;flex-wrap:wrap">
          <div style="background:#eff6ff;border:1px solid #bfdbfe;border-radius:10px;padding:10px 16px;text-align:center;min-width:90px"><div style="font-size:20px;font-weight:900;color:#2563eb">${pg.inet}</div><div style="font-size:11px;color:#64748b">인터넷</div></div>
          <div style="background:#fef3c7;border:1px solid #fde68a;border-radius:10px;padding:10px 16px;text-align:center;min-width:90px"><div style="font-size:20px;font-weight:900;color:#d97706">${pg.tv}</div><div style="font-size:11px;color:#64748b">TV</div></div>
          <div style="background:#ede9fe;border:1px solid #ddd6fe;border-radius:10px;padding:10px 16px;text-align:center;min-width:90px"><div style="font-size:20px;font-weight:900;color:#7c3aed">${pg.mob}</div><div style="font-size:11px;color:#64748b">모바일</div></div>
          <div style="background:#f1f5f9;border:1px solid #e2e8f0;border-radius:10px;padding:10px 16px;text-align:center;min-width:90px"><div style="font-size:20px;font-weight:900;color:#475569">${pg.etc}</div><div style="font-size:11px;color:#64748b">기타</div></div>
          <div style="background:#f0fdf4;border:1.5px solid #86efac;border-radius:10px;padding:10px 16px;text-align:center;min-width:90px"><div style="font-size:20px;font-weight:900;color:#15803d">${pg.total}</div><div style="font-size:11px;color:#15803d;font-weight:700">합계 회선</div></div>
        </div>
        ${prodE.length?`<div style="display:flex;gap:5px;flex-wrap:wrap;margin-top:2px">${prodE.map(([k,v])=>`<span style="background:#f8fafc;border:1px solid #e2e8f0;border-radius:16px;padding:2px 10px;font-size:11px;color:#475569">${k} ${v}</span>`).join('')}</div>`:''}
      </div>
      <div style="flex:1;display:flex;flex-direction:column;gap:8px;overflow:hidden">
        <div style="font-size:14px;font-weight:800;color:#16213e">이번 달 성공 건 세부 내역</div>
        ...
        <table>
          <thead><tr><th>일자</th><th>MM</th><th style="text-align:left">고객/주소</th><th>상품</th></tr></thead>
          <tbody>
            ${cases.slice(0,12).map((c,i)=>`<tr style="background:${i%2?'#f8fafc':'#fff'}">
              <td style="font-size:11px">${c.date}</td>
              <td style="font-size:11px;font-weight:700">${c.mm}</td>
              <td style="text-align:left;font-size:11px;max-width:180px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${c.address}</td>
              <td style="font-size:10px">${c.products}</td>
            </tr>`).join('')}
          </tbody>
        </table>
        ${cases.length>12?`<div style="font-size:11px;color:#94a3b8;text-align:center">외 ${cases.length-12}건</div>`:''}`}
      </div>
    </div>
  </div>
</div>`
  ...
  const s3=projDetailSlide(projects[0],3)
  const s4=projDetailSlide(projects[1],4)
  const s5=projDetailSlide(projects[2],5)
  const s6=projDetailSlide(projects[3],6)
  const s7=projDetailSlide(projects[4],7)
        ...
        {tab === 'activity' && (
          <div>
            ...
            {(() => {
              ...
              // 프로젝트별 필터 (1=기업체, 2=세움터, 3=빌딩/상가, 4=직접판매, 5=M발굴)
              if (actView === '프로젝트별' && actProj !== '전체') {
                const pid = Number(actProj)
                if (pid === 5) {
                  // M발굴은 m_discovery 기반 → baseLeads/baseDirect 비움, M발굴 별도 처리
                ...
                <>
                  ...
                  {actView === '프로젝트별' && (
                    <div style={{ display:'flex', gap:'5px', marginBottom:'6px', overflowX:'auto', paddingBottom:'4px' }}>
                      {[{id:'전체',label:'전체',icon:'📋'},{id:'1',label:'기업체',icon:'🏭'},{id:'2',label:'세움터',icon:'🏗️'},{id:'3',label:'빌딩/상가',icon:'🏢'},{id:'4',label:'직접판매',icon:'🤝'},{id:'5',label:'M발굴',icon:'🔍'}].map(p => (
        ...
        {tab === 'mdiscovery' && (
          <div>
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'4px'}}>
              <h2 style={{ fontSize: '19px', fontWeight: '900', color: C.text }}>🔍 M발굴 관리</h2>
            </div>
            <p style={{ color: C.sub, fontSize: '13px', marginBottom: '12px' }}>MM이 등록한 모바일 벌크영업 사이트 현황</p>
            {/* 단계별 요약 카드 */}
            <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:'6px',marginBottom:'14px'}}>
              {[['접촉중','#0891b2','#e0f2fe'],['발굴성공','#7c3aed','#ede9fe'],['영업중','#d97706','#fef3c7'],['영업종료','#16a34a','#dcfce7']].map(([s,c,bg])=>(
                <div key={s} style={{background:bg,borderRadius:'10px',padding:'10px',textAlign:'center'}}>
                  <div style={{fontSize:'18px',fontWeight:'900',color:c}}>{mDiscovery.filter(m=>m.stage===s).length}</div>
            ...
            <div style={{display:'flex',gap:'6px',marginBottom:'12px',overflowX:'auto',paddingBottom:'4px'}}>
              {['전체','접촉중','발굴성공','영업중','영업종료'].map(s=>(
                <button key={s} onClick={()=>setMDiscStage(s)} style={{flex:'none',padding:'5px 12px',background:mDiscStage===s?'#1e293b':'#fff',border:'1px solid '+(mDiscStage===s?'#1e293b':'#e2e8f0'),borderRadius:'20px',fontSize:'12px',fontWeight:mDiscStage===s?'700':'400',color:mDiscStage===s?'#fff':'#64748b',cursor:'pointer',fontFamily:'inherit'}}>{s}</button>
              ))}
            </div>
            {/* 목록 */}
            {(mDiscovery.filter(m=>!mDiscStage||mDiscStage==='전체'||m.stage===mDiscStage)).map(m=>(
              <div key={m.id} style={{background:'#fff',border:'1px solid #e2e8f0',borderRadius:'12px',padding:'12px',marginBottom:'8px',boxShadow:'0 1px 3px rgba(0,0,0,0.04)'}}>
                <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'6px'}}>
                  <span style={{fontWeight:'800',fontSize:'14px'}}>{m.site_name}</span>
                  <span style={{fontSize:'11px',padding:'2px 8px',borderRadius:'10px',fontWeight:'700',
                    background:m.stage==='접촉중'?'#e0f2fe':m.stage==='발굴성공'?'#ede9fe':m.stage==='영업중'?'#fef3c7':'#dcfce7',
                    color:m.stage==='접촉중'?'#0369a1':m.stage==='발굴성공'?'#6d28d9':m.stage==='영업중'?'#92400e':'#15803d'}}>
                    {m.stage||'접촉중'}
                  </span>
                </div>
                <div style={{fontSize:'12px',color:C.sub}}>👤 {m.registered_by} &nbsp;|&nbsp; 📍 {m.address||'-'}</div>
                {m.result&&<div style={{fontSize:'12px',color:'#374151',marginTop:'4px',padding:'4px 8px',background:'#f8fafc',borderRadius:'6px'}}>결과: {m.result}</div>}
              </div>
            ))}
            {mDiscovery.length===0&&<div style={{textAlign:'center',padding:'40px',color:C.sub}}>등록된 M발굴 현장이 없습니다</div>}
          </div>
        )}
        {/* ── 다운로드 ─────────────────────────────────────────────── */}
        {tab === 'download' && (
