            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
              ...
              <div style={{ background: '#fff', border: '1.5px solid #bfdbfe', borderRadius: '14px', padding: '18px', boxShadow: '0 1px 4px rgba(0,0,0,0.04)' }}>
                ...
                <button onClick={() => {
                  ...
                  const projData = [1,2,3,4].map(ptId => {
                    ...
                    const themes = [...new Set(pl.map(l=>l.project_name).filter(Boolean))].map(tn => {
                      ...
                      return {
                        ...
                        weekly:  { done: tl.filter(l=>l.activity_status&&l.activity_status!=='미처리'&&isThisWeek(l.activity_at||l.created_at)).length,
                                   success: tl.filter(l=>l.activity_status==='성공'&&isThisWeek(l.activity_at||l.created_at)).length },
                        monthly: { done: tl.filter(l=>l.activity_status&&l.activity_status!=='미처리'&&isThisMonth(l.activity_at||l.created_at)).length,
                                   success: tl.filter(l=>l.activity_status==='성공'&&isThisMonth(l.activity_at||l.created_at)).length }
                      }
                    })
                  ...
                  // 전직원 세부 데이터 (슬라이드 선택 팀만 포함)
                  const slideTeamIdSet = new Set(slideAccs.map(a=>a.team_id))
                  const slideOnlyUsers = isSuper
                    ? users.filter(u => slideTeamIdSet.has(u.team_id))
                    : users.filter(u => u.team_id === adminInfo?.teamId)

                  // 기간 체크 헬퍼 (클로저 의존 없이 직접 계산)
                  const nowD = new Date()
                  const nowY = nowD.getFullYear()
                  const nowM = nowD.getMonth()+1
                  const monD = new Date(nowD); monD.setHours(0,0,0,0); monD.setDate(nowD.getDate()-((nowD.getDay()+6)%7))
                  const sunD = new Date(monD); sunD.setDate(monD.getDate()+6); sunD.setHours(23,59,59,999)
                  function isThisWeek(dt)  { if(!dt)return false; try{const d=new Date(dt);return d>=monD&&d<=sunD}catch{return false} }
                  function isThisMonth(dt) { if(!dt)return false; try{const d=new Date(dt);return d.getFullYear()===nowY&&d.getMonth()+1===nowM}catch{return false} }
                  function isThisYear(dt)  { if(!dt)return false; try{const d=new Date(dt);return d.getFullYear()===nowY}catch{return false} }
                  ...
                  const allMMData = slideOnlyUsers.map(u => {
                    ...
                    // 인라인 계산 - 클로저 문제 방지
                    function calcStats(chkFn) {
                      // 배분: 활동완료, 날짜=activity_at 없으면 created_at
                      const md = uLeads.filter(l=>{
                        if(!l.activity_status||l.activity_status==='미처리') return false
                        return chkFn(l.activity_at||l.created_at)
                      })
                      // 직접발굴: activity_at(결과입력일) 우선, 없으면 created_at(등록일)
                      const dd = uDirect.filter(l=>chkFn(l.activity_at||l.created_at))
                      const all=[...md,...dd]
                      return {
                        mitDone: md.length, directDone: dd.length,
                        actAll: md.length+dd.length,
                    ...
                    return {
                      ...
                      weekly:  calcStats(isThisWeek),
                      monthly: calcStats(isThisMonth),
                      annual:  calcStats(isThisYear)
                    }
                  })
