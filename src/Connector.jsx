...
function buildSlideHTML({ period, today, yearStr, teams, projects, allMM, topMM,
  ...
  bizDays=0, perMMTarget=0, workDaysAll={}, allDailyStats=[], _debug={}, reportMode='current', momData=null, mDiscoveryList=[], agenciesList=[], allLeadsForPS=null }) {
  ...
  *{margin:0;padding:0;box-sizing:border-box;-webkit-print-color-adjust:exact!important;print-color-adjust:exact!important}
  ...
  function psAgencySlide(sn){
    ...
    // 성공 건 중 대리점 정보가 있는 것 수집 (배분 + 직접발굴)
    const successWithAgency = []
    ;(D.leads||[]).filter(l=>l.activity_status==='성공'&&myNM2.includes(l.assigned_to)).forEach(l=>{
      if(l.agency_name) successWithAgency.push({type:l.agency_type||'미분류',name:l.agency_name,lines:agLines(l)})
    })
    ;(D.directLeads||[]).filter(l=>l.activity_status==='성공'&&myUN2.includes(l.mm_username)).forEach(l=>{
      if(l.agency_name) successWithAgency.push({type:l.agency_type||'미분류',name:l.agency_name,lines:agLines(l)})
    })
    ...
    // 유형별 집계
    const byType={}
    successWithAgency.forEach(s=>{
      if(!byType[s.type])byType[s.type]={count:0,lines:0,agencies:{}}
      byType[s.type].count++; byType[s.type].lines+=s.lines
      byType[s.type].agencies[s.name]=(byType[s.type].agencies[s.name]||0)+1
    })
    const types=Object.keys(byType).sort((a,b)=>byType[b].count-byType[a].count)
    const totCount=successWithAgency.length
    const totLines=successWithAgency.reduce((a,s)=>a+s.lines,0)
    const typeColor=['#2563eb','#16a34a','#d97706','#7c3aed','#0891b2','#dc2626','#475569']
    // 등록된 대리점 유형 전체 (실적 없어도 표시)
    const allTypes=[...new Set((agenciesList||[]).map(a=>a.type))]
    const displayTypes=[...new Set([...types,...allTypes])]
    return `<div class="slide" id="s${sn}">
  ${rh('🤝 P&S 대리점 유형별 성과','패스앤슈팅 대리점 성과 현황',sn)}
  ${dbar}
  <div class="body">
    <div style="display:flex;gap:10px;margin-bottom:14px">
      <div style="flex:1;background:#eff6ff;border:1.5px solid #bfdbfe;border-radius:12px;padding:12px;text-align:center"><div style="font-size:24px;font-weight:900;color:#2563eb">${totCount}<span style="font-size:13px">건</span></div><div style="font-size:11px;color:#64748b">P&S 성공 건수</div></div>
      <div style="flex:1;background:#f0fdf4;border:1.5px solid #bbf7d0;border-radius:12px;padding:12px;text-align:center"><div style="font-size:24px;font-weight:900;color:#16a34a">${totLines}<span style="font-size:13px">회선</span></div><div style="font-size:11px;color:#64748b">총 수주 회선</div></div>
      <div style="flex:1;background:#faf5ff;border:1.5px solid #e9d5ff;border-radius:12px;padding:12px;text-align:center"><div style="font-size:24px;font-weight:900;color:#7c3aed">${displayTypes.length}</div><div style="font-size:11px;color:#64748b">대리점 유형 수</div></div>
      <div style="flex:1;background:#fff7ed;border:1.5px solid #fed7aa;border-radius:12px;padding:12px;text-align:center"><div style="font-size:24px;font-weight:900;color:#e67e00">${(agenciesList||[]).length}</div><div style="font-size:11px;color:#64748b">등록 대리점 수</div></div>
    </div>
    <div style="display:flex;gap:14px">
      <div style="flex:1">
        <div style="font-size:13.5px;font-weight:800;color:#16213e;margin-bottom:6px">유형별 성과 요약</div>
        <table>
          ...
          <tbody>
            ${displayTypes.length===0?`<tr><td colspan="4" style="color:#94a3b8;padding:20px">P&S 성공 실적이 없습니다</td></tr>`:displayTypes.map((t,i)=>{
              const d=byType[t]||{count:0,lines:0}
              return `<tr style="background:${i%2?'#f8fafc':'#fff'}">
                ...
                <td>${totCount>0?pct(d.count,totCount):0}%</td>
              </tr>`}).join('')}
            <tr style="background:#16213e;color:#fff;font-weight:900">
              <td style="color:#fff;text-align:left;padding-left:10px">전 체</td>
              <td style="color:#fff">${totCount||'-'}</td>
              <td style="background:#166534;color:#fff">${totLines||'-'}</td>
      ...
      <div style="flex:1">
        <div style="font-size:13.5px;font-weight:800;color:#16213e;margin-bottom:6px">대리점별 성공 건수 (상위)</div>
        ${(()=>{
          const allAg=[]
          types.forEach(t=>{Object.entries(byType[t].agencies).forEach(([name,cnt])=>allAg.push({type:t,name,cnt}))})
          allAg.sort((a,b)=>b.cnt-a.cnt)
          if(allAg.length===0)return '<div style="color:#94a3b8;font-size:13px;padding:20px;text-align:center;background:#f8fafc;border-radius:10px">대리점별 실적이 없습니다</div>'
          return `<table>
            <thead><tr><th style="text-align:left;padding-left:10px">대리점명</th><th>유형</th><th style="background:#15803d">성공</th></tr></thead>
            <tbody>${allAg.slice(0,12).map((a,i)=>`<tr style="background:${i%2?'#f8fafc':'#fff'}">
              <td style="text-align:left;font-weight:700;padding-left:10px;font-size:12px">${a.name}</td>
              <td style="font-size:10.5px;color:#64748b">${a.type}</td>
              <td style="background:#f0fdf4;font-weight:900;color:#15803d">${a.cnt}</td>
            </tr>`).join('')}</tbody>
          </table>${allAg.length>12?`<div style="font-size:11px;color:#94a3b8;text-align:center;margin-top:4px">외 ${allAg.length-12}개</div>`:''}`
        })()}
      </div>
    </div>
    <div style="margin-top:10px;font-size:11px;color:#94a3b8">※ P&S 성과 = 활동결과 '성공' 건 중 패스앤슈팅 대리점이 선택된 건의 집계 · 회선수는 수주 상품 기준</div>
  </div>
  ${ftr(sn)}
</div>`
  }
