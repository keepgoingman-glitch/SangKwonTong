...
export default function Connector({ onBack, initialTab }) {
  ...
  const [annualMode, setAnnualMode] = useState(false)  // 연간 누계 전용 보고서 여부
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
              ...
              <div style={{ background: '#fff', border: '1.5px solid #bfdbfe', borderRadius: '14px', padding: '18px', boxShadow: '0 1px 4px rgba(0,0,0,0.04)' }}>
                ...
                <button onClick={() => {
                  ...
                  const html = buildSlideHTML({ _debug: dbgInfo, reportMode: annualMode?'annual':(_isPastMonth?'past':'current'),
                    bizDays: curBizDays, perMMTarget, workDaysAll: workDays,
                    allDailyStats,
                    period: annualMode ? (curYear+'년 연간 누계') : periodStr, today: todayStr, yearStr: curYear+'년',
                    teams: teamDataFull, projects: projDataFull,
                    allMM: allMMData, topMM: [...allMMData].sort((a,b)=>(annualMode?((b.annual?.success||0)-(a.annual?.success||0)):((b.monthly?.success||0)-(a.monthly?.success||0)))).slice(0,5),
                    mDiscoveryCount: mDiscoveryTotal,
                    globalProdCounts: annualMode ? globalProdAnnual : globalProdMonthly,
                    globalProdAnnual,
                    momData,
                  ...
                  a.download = annualMode
                    ? ('상권통_연간누계보고서_' + curYear + '.html')
                    : ('상권통_보고서_' + curYear + '-' + String(slideMonth||curMonth).padStart(2,'0') + (_isPastMonth?'_월마감':'') + '.html')
                  a.click()
                  t2(annualMode ? '연간 누계 보고서 다운로드 완료!' : '슬라이드 장표 다운로드 완료!')
                  } catch(err) { t2('슬라이드 오류: ' + err.message); console.error('Slide err:', err) }
                  finally { setAnnualMode(false) }
                }} id="__slideDlBtn" style={{ width: '100%', background: C.blue, border: 'none', color: '#fff', padding: '13px', borderRadius: '10px', fontSize: '15px', fontWeight: '900', cursor: 'pointer', fontFamily: 'inherit' }}>
                  📥 슬라이드 장표 다운로드 (월간)
                </button>
                {/* 연간 누계 전용 보고서 */}
                <button onClick={() => {
                  setAnnualMode(true)
                  // state 반영 후 동일 생성 로직 실행
                  setTimeout(()=>{ const b=document.getElementById('__slideDlBtn'); if(b) b.click() }, 0)
                  ...
                  📊 연간 누계 보고서 다운로드 ({new Date().getFullYear()}년)
                </button>
                <div style={{ fontSize: '11.5px', color: C.sub, marginTop: '6px', lineHeight: 1.5 }}>
                  ※ 연간 누계 보고서: 주간·월간 대신 <b>연초부터 현재까지 누계</b> 기준으로 팀·MM·프로젝트·상품별 세부 실적을 정리합니다.
                </div>
              </div>
