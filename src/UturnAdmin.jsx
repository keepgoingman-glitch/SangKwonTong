...
const UGROUPS = [
  { gid:'g_mkt',  label:'마케팅팀',      teamIds:['team5'],         color:'#db2777' },
  { gid:'g_syn1', label:'시너지영업1팀', teamIds:['team3'],         color:'#ea580c' },
  { gid:'g_syn2', label:'시너지영업2팀', teamIds:['team1','team2'], color:'#16a34a' },
  { gid:'g_cs',   label:'CS운영팀',      teamIds:['team4'],         color:'#2563eb' },
  { gid:'g_plan', label:'고객기획팀',    teamIds:['team6'],         color:'#9333ea' },
]
...
const C = { text:'#1e293b', sub:'#64748b', card:'#ffffff',
  border:'#e2e8f0', red:'#E31937', u:'#9333ea', i:'#2563eb', t:'#d97706', green:'#16a34a' }
const IS = { width:'100%', background:'#f8fafc', border:'1.5px solid #e2e8f0',
  color:'#1e293b', padding:'12px 14px', borderRadius:'10px', fontSize:'15px', outline:'none', boxSizing:'border-box', fontFamily:'inherit' }
...
function MemberModal({ editUser, isSuper, myTeamId, onClose, onSaved }) {
  ...
  return (
    <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.6)', zIndex:1001, display:'flex', alignItems:'flex-end', justifyContent:'center', backdropFilter:'blur(4px)' }}>
      <div style={{ background:'#ffffff', borderRadius:'20px 20px 0 0', padding:'22px 18px 32px', width:'100%', maxWidth:'480px', maxHeight:'92vh', overflowY:'auto', position:'relative', border:'1px solid #e2e8f0' }}>
        ...
        {err && <div style={{ color:'#E31937', fontSize:'13px', background:'rgba(227,25,55,0.12)', border:'1px solid rgba(227,25,55,0.3)', padding:'9px 11px', borderRadius:'8px', marginBottom:'10px' }}>{err}</div>}
        ...
        <button onClick={save} disabled={saving} style={{ width:'100%', padding:'14px', background:saving?'#e2e8f0':C.red, border:'none', color:'#fff', borderRadius:'12px', fontSize:'16px', fontWeight:'900', cursor:saving?'wait':'pointer', fontFamily:'inherit' }}>{saving?'저장 중...':(editUser?'수정 저장':'팀원 생성')}</button>
      </div>
    </div>
  )
}
...
function StrategyModal({ editStrat, isSuper, myTeamId, users, adminName, onClose, onSaved }) {
  ...
  return (
    <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.6)', zIndex:1001, display:'flex', alignItems:'flex-end', justifyContent:'center', backdropFilter:'blur(4px)' }}>
      <div style={{ background:'#ffffff', borderRadius:'20px 20px 0 0', padding:'22px 18px 32px', width:'100%', maxWidth:'480px', maxHeight:'94vh', overflowY:'auto', position:'relative', border:'1px solid #e2e8f0' }}>
        <button onClick={onClose} style={{ position:'absolute', top:'14px', right:'14px', background:'#e2e8f0', border:'none', color:C.text, width:'28px', height:'28px', borderRadius:'50%', fontSize:'16px', cursor:'pointer' }}>✕</button>
        ...
        <div style={{ background:'rgba(227,25,55,0.08)', border:'1px solid rgba(227,25,55,0.3)', borderRadius:'12px', padding:'12px', marginBottom:'12px' }}>
          <div style={{ fontSize:'12.5px', fontWeight:'900', color:'#E31937', marginBottom:'8px' }}>⤴ UIT 목표</div>
        ...
        <button onClick={save} disabled={saving} style={{ width:'100%', padding:'14px', background:saving?'#e2e8f0':C.red, border:'none', color:'#fff', borderRadius:'12px', fontSize:'16px', fontWeight:'900', cursor:saving?'wait':'pointer', fontFamily:'inherit' }}>{saving?'저장 중...':(editStrat?'수정 저장':'전략활동 배분')}</button>
      </div>
    </div>
  )
}
    ...
    <div style={{ minHeight:'100vh', background:'#f4f6fb', color:C.text, fontFamily:"'Noto Sans KR',sans-serif", paddingBottom:'40px' }}>
      ...
      <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'16px 18px 12px' }}>
        ...
        <div style={{ textAlign:'center' }}>
          <div style={{ fontSize:'16px', fontWeight:'900', color:'#E31937' }}>⤴ U-TURN 관리</div>
      ...
      <div style={{ maxWidth:'460px', margin:'0 auto', padding:'0 16px' }}>
        ...
        <div style={{ display:'flex', gap:'6px', marginBottom:'14px' }}>
          {[['perf','📊 성과현황'],['strategies','🎯 전략활동'],['members','👥 팀원관리']].map(([id,lb])=>(
            <button key={id} onClick={()=>setTab(id)} style={{ flex:1, padding:'11px 4px', background:tab===id?'rgba(227,25,55,0.15)':'#ffffff', border:'1.5px solid '+(tab===id?'rgba(227,25,55,0.4)':C.border), color:tab===id?'#ff5a6e':C.sub, borderRadius:'11px', fontSize:'13.5px', fontWeight:tab===id?'800':'500', cursor:'pointer', fontFamily:'inherit' }}>{lb}</button>
          ))}
        </div>
            ...
            <div key={g.gid} style={{ background:C.card, border:'1px solid '+C.border, borderLeft:'4px solid '+g.color, borderRadius:'14px', padding:'14px', marginBottom:'11px' }}>
              <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:'10px' }}>
                <div style={{ display:'flex', alignItems:'center', gap:'8px' }}>
                  {isSuper && <span style={{ fontSize:'14px', fontWeight:'900', color:idx===0?'#d97706':C.sub }}>{['🥇','🥈','🥉','4','5'][idx]}</span>}
                ...
                <div key={s.id} style={{ background:C.card, border:'1px solid '+C.border, borderLeft:'4px solid '+stC, borderRadius:'13px', padding:'13px', marginBottom:'10px' }}>
                  <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:'6px' }}>
                    <div style={{ flex:1 }}>
                      <div style={{ display:'flex', gap:'5px', alignItems:'center', marginBottom:'3px', flexWrap:'wrap' }}>
                        <span style={{ fontSize:'10px', fontWeight:'800', padding:'2px 7px', borderRadius:'9px', background:'#f1f5f9', color:stC }}>{s.status}</span>
                  ...
                  <div style={{ display:'flex', gap:'6px', marginTop:'8px', marginBottom:'8px' }}>
                    {[['U',u,s.target_u,C.u],['I',i,s.target_i,C.i],['T',t,s.target_t,C.t]].map(([lb,v,goal,col])=>(
                      <div key={lb} style={{ flex:1, textAlign:'center', background:'#ffffff', borderRadius:'8px', padding:'6px' }}>
                  ...
                  : tu.map(u=>(
                    <div key={u.id} style={{ background:C.card, border:'1px solid '+C.border, borderRadius:'11px', padding:'11px 13px', marginBottom:'7px', display:'flex', justifyContent:'space-between', alignItems:'center' }}>
                      ...
                      <div style={{ display:'flex', gap:'8px' }}>
                        ...
                        <button onClick={()=>delMember(u)} style={{ background:'rgba(227,25,55,0.12)', border:'none', color:'#E31937', padding:'6px 11px', borderRadius:'8px', fontSize:'12px', fontWeight:'700', cursor:'pointer', fontFamily:'inherit' }}>삭제</button>
                      </div>
                    </div>
                  ))
                }
