...
function MMDashboard({cLeads,mLeads,user,period}){
  const now=new Date();const allLeads=[...cLeads,...mLeads];const assigned=cLeads.length;const directCount=mLeads.length;const directAct=mLeads.filter(l=>l.activity_status&&l.activity_status!=='미처리').length;const directSuc=mLeads.filter(l=>l.activity_status==='성공').length
  ...
  return(<div>
    ...
    {directCount>0&&card(<>
      ...
      <div style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr',gap:'8px',marginBottom:'8px'}}>{[{l:'직접등록',v:directCount,c:'#0891b2'},{l:'활동완료',v:directAct,c:C.acc},{l:'성공',v:directSuc,c:C.green}].map(({l,v,c})=>(<div key={l} style={{background:'#f8fafc',border:'1px solid #e2e8f0',borderRadius:'10px',padding:'10px',textAlign:'center'}}><div style={{fontSize:'24px',fontWeight:'900',color:c}}>{v}</div><div style={{fontSize:'11px',color:C.sub,marginTop:'2px'}}>{l}</div></div>))}</div>
    </>)}
    {card(<>
