...
const S = { bg: '#0c1524', card: '#131e30', border: '#1e3050', acc: '#4ade80', text: '#f1f5f9', sub: '#94a3b8' }
...
export default function MM({ onBack }) {
  ...
  if (screen === 'login') return (
    <div style={{ minHeight: '100vh', background: 'linear-gradient(160deg,#0c1524,#091809)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '30px 24px', fontFamily: "'Noto Sans KR',sans-serif", color: S.text }}>
      ...
      <div style={{ textAlign: 'center', marginBottom: '32px' }}>
        <div style={{ background: S.acc, color: '#0c1524', fontWeight: '900', fontSize: '14px', padding: '4px 14px', borderRadius: '4px', display: 'inline-block', marginBottom: '12px' }}>상권통</div>
      ...
      <div style={{ width: '100%', maxWidth: '360px', display: 'flex', flexDirection: 'column', gap: '14px' }}>
        ...
        <button onClick={handleLogin} disabled={lding} style={{ background: lding ? '#1e293b' : S.acc, border: 'none', color: lding ? '#475569' : '#0c1524', padding: '20px', borderRadius: '12px', fontSize: '20px', fontWeight: '900', cursor: lding ? 'not-allowed' : 'pointer', fontFamily: 'inherit' }}>{lding ? '로그인 중...' : '로그인'}</button>
      </div>
  ...
  return (
    <div style={{ minHeight: '100vh', background: S.bg, color: S.text, fontFamily: "'Noto Sans KR',sans-serif", maxWidth: '420px', margin: '0 auto', display: 'flex', flexDirection: 'column' }}>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.88)', display: 'flex', alignItems: 'flex-end', justifyContent: 'center', zIndex: 999 }}>
          <div style={{ background: S.card, borderRadius: '16px 16px 0 0', padding: '24px 20px 32px', width: '100%', maxWidth: '420px' }}>
            ...
            <div style={{ marginBottom: '14px' }}>
              ...
              <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
                {['미처리', '접촉완료', '성공', '실패'].map(s => (
                  <button key={s} onClick={() => setRf({ ...rf, status: s })} style={{ padding: '11px 18px', background: rf.status === s ? S.acc : 'transparent', border: rf.status === s ? 'none' : '1px solid ' + S.border, color: rf.status === s ? '#0c1524' : S.sub, borderRadius: '8px', fontSize: '15px', fontWeight: '700', cursor: 'pointer', fontFamily: 'inherit' }}>{s}</button>
                ))}
              </div>
            </div>
            ...
            <div style={{ display: 'flex', gap: '10px' }}>
              ...
              <button onClick={saveResult} style={{ flex: 2, padding: '14px', background: S.acc, border: 'none', color: '#0c1524', borderRadius: '10px', fontSize: '16px', fontWeight: '900', cursor: 'pointer', fontFamily: 'inherit' }}>저장하기</button>
            </div>
          </div>
        </div>
      )}
      ...
      {/* 헤더 */}
      <div style={{ background: 'linear-gradient(135deg,#091809,#0c1524)', padding: '18px 20px 14px', borderBottom: '3px solid ' + S.acc }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <div>
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '4px' }}>
              <span style={{ background: S.acc, color: '#0c1524', fontWeight: '900', fontSize: '11px', padding: '3px 8px', borderRadius: '4px' }}>상권통</span>
              <span style={{ color: S.acc, fontWeight: '900', fontSize: '16px' }}>상권마스터 플랫폼</span>
            </div>
            <div style={{ color: S.sub, fontSize: '16px' }}><strong style={{ color: S.text, fontSize: '18px' }}>{user?.name}</strong> MM <span style={{ color: '#64748b', fontSize: '13px', marginLeft: '8px' }}>{user?.region}</span></div>
          </div>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '18px' }}>
          ...
          <div>
            <div style={{ background: 'rgba(74,222,128,0.06)', border: '2px solid ' + lv.color, borderRadius: '14px', padding: '18px', marginBottom: '16px' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '12px' }}>
                ...
                <div><div style={{ fontSize: '13px', color: S.sub }}>현재 레벨</div><div style={{ fontSize: '26px', fontWeight: '900', color: lv.color }}>Lv.{lv.lv} {lv.name}</div></div>
              </div>
              {nextLv && (
                <div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px', marginBottom: '16px' }}>
              {[{ l: '커낵터 배분', v: cLeads.length, c: '#60a5fa' }, { l: '직접 발굴', v: mLeads.length, c: S.acc }, { l: '전체 성공', v: sc, c: lv.color }, { l: '이달 목표', v: goal, c: '#f59e0b' }].map((s, i) => (
        ...
        {tab === 'leads' && (
          <div>
            ...
            <div style={{ display: 'flex', background: S.bg, borderRadius: '10px', padding: '4px', marginBottom: '16px', border: '1px solid ' + S.border }}>
              ...
              <button onClick={() => setLeadTab('my')} style={{ flex: 1, padding: '11px', background: leadTab === 'my' ? S.acc : 'transparent', color: leadTab === 'my' ? '#0c1524' : S.sub, border: 'none', borderRadius: '8px', fontSize: '14px', fontWeight: '800', cursor: 'pointer', fontFamily: 'inherit' }}>🔍 직접발굴 ({mLeads.length})</button>
            </div>
            ...
            {leadTab === 'my' && (mLeads.length === 0
              ...
              : mLeads.map(l => (
                <div key={l.id} style={{ background: S.card, border: '1px solid ' + S.border, borderLeft: '3px solid ' + S.acc, borderRadius: '10px', padding: '14px', marginBottom: '10px' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '8px' }}>
                    <div style={{ flex: 1, marginRight: '8px' }}>
                      <div style={{ fontSize: '12px', color: S.acc, fontWeight: '700', marginBottom: '4px' }}>🔍 직접 발굴</div>
                  ...
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: '5px', marginBottom: '10px' }}>
                    {l.products?.map(p => <span key={p} style={{ background: 'rgba(74,222,128,0.15)', color: S.acc, border: '1px solid rgba(74,222,128,0.3)', fontSize: '12px', fontWeight: '700', padding: '3px 9px', borderRadius: '4px' }}>{p}</span>)}
                  </div>
                  {l.activity_memo && <div style={{ background: S.bg, borderRadius: '6px', padding: '8px', marginBottom: '10px', fontSize: '13px', color: S.sub }}>💬 {l.activity_memo}</div>}
                  <button onClick={() => { setRm({ ...l, _src: 'm' }); setRf({ status: l.activity_status || '미처리', result: l.activity_result || '', memo: l.activity_memo || '' }) }} style={{ width: '100%', background: 'transparent', border: '1px solid ' + S.acc, color: S.acc, padding: '10px', borderRadius: '8px', fontSize: '14px', fontWeight: '700', cursor: 'pointer', fontFamily: 'inherit' }}>활동 결과 입력</button>
                </div>
              ))
            )}
          </div>
        ...
        {tab === 'register' && (
          <div>
            ...
            {regDone
              ? <div style={{ textAlign: 'center', padding: '50px 20px', background: 'rgba(5,46,22,0.4)', border: '1px solid #14532d', borderRadius: '14px' }}><div style={{ fontSize: '54px', marginBottom: '12px' }}>✅</div><div style={{ fontSize: '22px', fontWeight: '900', color: S.acc }}>등록 완료!</div></div>
                ...
                <div>
                  ...
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px' }}>
                    {PRODUCTS.map(p => {
                      ...
                      return <button key={p} onClick={() => toggleP(p)} style={{ padding: '13px 18px', background: sel ? S.acc : S.card, border: sel ? 'none' : '1px solid ' + S.border, color: sel ? '#0c1524' : S.sub, borderRadius: '8px', fontSize: '16px', fontWeight: '800', cursor: 'pointer', fontFamily: 'inherit' }}>{sel ? '✓ ' : ''}{p}</button>
                    })}
                  </div>
                </div>
                <button onClick={handleReg} disabled={!reg.customer || !reg.products.length} style={{ background: reg.customer && reg.products.length ? S.acc : '#1e293b', border: 'none', color: reg.customer && reg.products.length ? '#0c1524' : '#475569', padding: '18px', borderRadius: '12px', fontSize: '18px', fontWeight: '900', cursor: 'pointer', marginTop: '6px', fontFamily: 'inherit' }}>등록하기</button>
              </div>
            }
          </div>
        )}
      ...
      <div style={{ background: '#060c17', borderTop: '1px solid ' + S.border, display: 'flex', padding: '8px 0 6px' }}>
        {navTabs.map(t => (
          <button key={t.id} onClick={() => setTab(t.id)} style={{ flex: 1, background: 'none', border: 'none', color: tab === t.id ? S.acc : '#475569', cursor: 'pointer', padding: '6px 0', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '3px', fontFamily: 'inherit' }}>
