...
function ResultModal({lead,onClose,onSave}){
  const [form,setForm]=useState({status:lead.activity_status||'미처리',result:lead.activity_result||'',memo:lead.activity_memo||'',contact:lead.activity_contact||'',photos:(()=>{try{return JSON.parse(lead.photos||'[]')}catch{return[]}})(),wonProds:[],contractExpiry:'',wonLines:1})
  ...
  return(<div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.3)',display:'flex',alignItems:'flex-end',justifyContent:'center',zIndex:999,backdropFilter:'blur(3px)'}}>
    <div style={{background:'#fff',borderRadius:'20px 20px 0 0',padding:'24px 20px 36px',width:'100%',maxWidth:'420px',boxShadow:'0 -4px 24px rgba(0,0,0,0.12)',position:'relative',maxHeight:'92vh',overflowY:'auto'}}>
      ...
      <button onClick={()=>{if(form.status==='성공'){setShowChecklist(true)}else{onSave(form)}}} style={{width:'100%',padding:'15px',background:C.acc,border:'none',color:'#fff',borderRadius:'12px',fontSize:'17px',fontWeight:'900',cursor:'pointer',fontFamily:'inherit'}}>저장하기</button>
    </div>
  </div>)
}
...
export default function MM({onBack}){
  ...
  const handleLogin=async()=>{
    ...
    try{
      ...
      const [cl,ml,disc]=await Promise.all([
        db.get('connector_leads','select=id,col1,col2,col3,col4,col5,address,products,assigned_to,assign_status,activity_status,activity_result,activity_memo,activity_contact,activity_at,team_id,project_name,theme_name,project_type&assigned_to=eq.'+encodeURIComponent(u.name)+'&order=created_at.desc&limit=500'),
        db.get('mm_direct_leads','select=id,mm_username,customer,address,contact,products,note,project_type,activity_status,activity_result,activity_memo,activity_contact,activity_at,created_at&mm_username=eq.'+u.username+'&order=created_at.desc&limit=200')
      ])
  ...
  const saveResult=async(form)=>{
    ...
    const upd={activity_status:form.status,activity_result:form.result,activity_memo:form.memo,activity_contact:form.contact,photos:JSON.stringify(form.photos),activity_at:now,
      ...(form.status==='성공'&&{won_products:JSON.stringify(form.wonLinesByProd&&Object.keys(form.wonLinesByProd).length>0?form.wonLinesByProd:(form.wonProds||[])),won_lines:Object.values(form.wonLinesByProd||{}).reduce((a,v)=>a+v,0)||1}),
