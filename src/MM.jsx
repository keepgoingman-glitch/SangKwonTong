  ...
  const saveResult=async(form)=>{
    ...
    try{
      if(rm._src==='c'){
        ...
        const allC=await db.get('connector_leads','assigned_to=eq.'+encodeURIComponent(user.name))
        const allM=await db.get('mm_direct_leads','mm_username=eq.'+user.username)
