...
function ResultModal({ lead, onClose, onSave }) {
  ...
  const statusColors = { '미처리': '#f59e0b', '접촉완료': '#60a5fa', '성공': '#4ade80', '실패': '#f87171' }
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.75)', display: 'flex', alignItems: 'flex-end', justifyContent: 'center', zIndex: 999, backdropFilter: 'blur(4px)' }}>
      <div style={{ background: 'linear-gradient(170deg,#0d1f35,#0a1628)', borderRadius: '20px 20px 0 0', padding: '24px 20px 36px', width: '100%', maxWidth: '420px', border: '1px solid rgba(255,255,255,0.1)', borderBottom: 'none', position: 'relative' }}>
        ...
        <h3 style={{ fontSize: '18px', fontWeight: '900', marginBottom: '4px' }}>활동 결과 {lead.activity_status && lead.activity_status !== '미처리' ? '수정' : '입력'}</h3>
        <p style={{ color: S.sub, fontSize: '13px', marginBottom: '18px', lineHeight: '1.5' }}>{lead._src === 'c' ? lead.address : lead.customer}</p>
          ...
          <div style={{ display: 'flex', gap: '7px', flexWrap: 'wrap' }}>
            {['미처리', '접촉완료', '성공', '실패'].map(s => (
              <button key={s} onClick={() => set('status', s)}
                style={{ padding: '10px 16px', background: form.status === s ? statusColors[s] : 'rgba(255,255,255,0.06)', border: form.status === s ? 'none' : '1px solid rgba(255,255,255,0.1)', color: form.status === s ? '#06111f' : S.sub, borderRadius: '10px', fontSize: '15px', fontWeight: '700', cursor: 'pointer', fontFamily: 'inherit' }}>{s}</button>
            ))}
          </div>
        </div>
        ...
        <div style={{ marginBottom: '11px' }}>
          ...
          <input type="text" placeholder="예: 계약완료, 다음 주 재방문 예정" value={form.result} onChange={e => set('result', e.target.value)} style={IS} />
        </div>
        ...
        <div style={{ marginBottom: '20px' }}>
          ...
          <textarea placeholder="특이사항, 다음 액션 등" value={form.memo} onChange={e => set('memo', e.target.value)} rows={3}
    ...
    <div style={{ minHeight: '100vh', background: 'linear-gradient(170deg,#06111f 0%,#0a1f3d 40%,#0d1a35 70%,#06111f 100%)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '30px 28px', fontFamily: "'Noto Sans KR',sans-serif", color: S.text }}>
      ...
      <div style={{ width: '100%', maxWidth: '360px', display: 'flex', flexDirection: 'column', gap: '12px' }}>
        ...
        <button onClick={handleLogin} disabled={lding}
          style={{ background: lding ? 'rgba(255,255,255,0.1)' : 'linear-gradient(135deg,#4ade80,#16a34a)', border: 'none', color: lding ? '#475569' : '#06111f', padding: '18px', borderRadius: '14px', fontSize: '18px', fontWeight: '900', cursor: lding ? 'not-allowed' : 'pointer', fontFamily: 'inherit', marginTop: '6px' }}>
          {lding ? '로그인 중...' : '로그인'}
        </button>
      </div>
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'linear-gradient(170deg,#06111f,#0a1f3d,#06111f)', color: S.text, fontFamily: "'Noto Sans KR',sans-serif", maxWidth: '420px', margin: '0 auto', display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '16px' }}>
        ...
        {tab === 'home' && (
          <div>
            <div style={{ background: `rgba(13,31,53,0.9)`, border: '1.5px solid ' + lv.color + '44', borderRadius: '18px', padding: '20px', marginBottom: '14px' }}>
            ...
            {unproc > 0 && (
              <div style={{ background: 'rgba(245,158,11,0.07)', border: '1.5px solid rgba(245,158,11,0.3)', borderRadius: '14px', padding: '14px 16px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <div>
                  <div style={{ fontSize: '15px', fontWeight: '800', color: '#f59e0b' }}>⚠️ 미처리 기회 {unproc}건</div>
                  <div style={{ color: S.sub, fontSize: '12px', marginTop: '3px' }}>확인이 필요한 배분 기회</div>
                </div>
                <button onClick={() => { setTab('leads'); setLeadTab('connector') }}
                  style={{ background: '#f59e0b', border: 'none', color: '#06111f', padding: '9px 14px', borderRadius: '8px', fontSize: '13px', fontWeight: '900', cursor: 'pointer', fontFamily: 'inherit' }}>확인 →</button>
              </div>
            )}
          </div>
        )}
            ...
            {leadTab === 'connector' && (cLeads.length === 0
              ...
              : cLeads.map(l => (
                <div key={l.id} style={{ background: 'rgba(13,31,53,0.8)', border: '1px solid rgba(255,255,255,0.07)', borderLeft: '3px solid #60a5fa', borderRadius: '14px', padding: '14px', marginBottom: '10px' }}>
                  ...
                  {l.activity_result && (
                    <div style={{ background: 'rgba(255,255,255,0.04)', borderRadius: '8px', padding: '8px 10px', marginBottom: '8px', fontSize: '13px', color: S.sub }}>
                  ...
                  {/* 항상 입력/수정 버튼 표시 */}
                  <button onClick={() => setRm({ ...l, _src: 'c' })}
                    style={{ width: '100%', background: 'transparent', border: '1px solid rgba(96,165,250,0.35)', color: '#60a5fa', padding: '10px', borderRadius: '8px', fontSize: '14px', fontWeight: '700', cursor: 'pointer', fontFamily: 'inherit' }}>
                ...
                <div key={l.id} style={{ background: 'rgba(13,31,53,0.8)', border: '1px solid rgba(255,255,255,0.07)', borderLeft: '3px solid #4ade80', borderRadius: '14px', padding: '14px', marginBottom: '10px' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '8px' }}>
                    <div style={{ flex: 1, marginRight: '10px' }}>
                      ...
                      <div style={{ color: S.sub, fontSize: '12px', marginTop: '2px' }}>
                        {l.address}{l.contact && ' · 📞 ' + l.contact}
                      </div>
                  ...
                  {/* 항상 입력/수정 버튼 표시 */}
                  <button onClick={() => setRm({ ...l, _src: 'm' })}
                    style={{ width: '100%', background: 'transparent', border: '1px solid rgba(74,222,128,0.35)', color: S.acc, padding: '10px', borderRadius: '8px', fontSize: '14px', fontWeight: '700', cursor: 'pointer', fontFamily: 'inherit' }}>
                    {l.activity_status && l.activity_status !== '미처리' ? '✏️ 결과 수정' : '활동 결과 입력'}
                  </button>
                </div>
              ))
          ...
          <div>
            ...
            {regDone
              ...
              : <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
                  {/* 고객명 */}
                  <div>
                    <label style={{ fontSize: '14px', color: S.sub, fontWeight: '700', display: 'block', marginBottom: '7px' }}>고객명 <span style={{ color: '#f87171' }}>*</span></label>
                    <input type="text" placeholder="예: 행복분식, 우리빌딩" value={reg.customer}
                      onChange={e => setReg(prev => ({ ...prev, customer: e.target.value }))}
                      style={{ ...IS, fontSize: '17px' }} />
                  </div>
                  {/* 주소 */}
                  <div>
                    <label style={{ fontSize: '14px', color: S.sub, fontWeight: '700', display: 'block', marginBottom: '7px' }}>주소</label>
                    <input type="text" placeholder="예: 강남구 역삼동 123" value={reg.address}
                      onChange={e => setReg(prev => ({ ...prev, address: e.target.value }))}
                      style={IS} />
                  </div>
                  {/* 연락처 */}
                  <div>
                    <label style={{ fontSize: '14px', color: S.sub, fontWeight: '700', display: 'block', marginBottom: '7px' }}>연락처</label>
                    <input type="tel" placeholder="010-0000-0000" value={reg.contact}
                      onChange={e => setReg(prev => ({ ...prev, contact: e.target.value }))}
                      style={IS} />
                  </div>
                  {/* 상품군 */}
                  <div>
                    <label style={{ fontSize: '14px', color: S.sub, fontWeight: '700', display: 'block', marginBottom: '9px' }}>
                      상품군 <span style={{ color: '#f87171' }}>*</span>
                      <span style={{ fontSize: '12px', color: 'rgba(255,255,255,0.25)', fontWeight: '400', marginLeft: '6px' }}>(중복 선택 가능)</span>
                    </label>
                  ...
                  <div>
                    ...
                    <textarea placeholder="특이사항, 상세 정보 등" value={reg.note}
                      onChange={e => setReg(prev => ({ ...prev, note: e.target.value }))}
                      rows={3}
                      style={{ ...IS, resize: 'none' }} />
                  </div>
                  <button onClick={handleReg} disabled={!reg.customer || !reg.products.length}
                    style={{ background: reg.customer && reg.products.length ? 'linear-gradient(135deg,#4ade80,#16a34a)' : 'rgba(255,255,255,0.08)', border: 'none', color: reg.customer && reg.products.length ? '#06111f' : '#475569', padding: '18px', borderRadius: '14px', fontSize: '18px', fontWeight: '900', cursor: 'pointer', marginTop: '4px', fontFamily: 'inherit' }}>
                    등록하기
                  </button>
                </div>
            }
                ...
                {LEVELS.map((l, i) => {
                  ...
                  return (
                    <div key={l.lv} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '4px' }}>
                      ...
                      <div style={{ width: '46px', height: h + 'px', background: reached ? l.color : 'rgba(255,255,255,0.07)', borderRadius: '6px 6px 0 0', display: 'flex', alignItems: 'flex-start', justifyContent: 'center', paddingTop: '6px', border: cur ? '2px solid ' + l.color : 'none', boxShadow: cur ? '0 0 14px ' + l.color + '44' : 'none' }}>
