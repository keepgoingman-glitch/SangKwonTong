    ...
    <div style={{background:'#fff',borderRadius:'20px 20px 0 0',padding:'24px 20px 36px',width:'100%',maxWidth:'420px',boxShadow:'0 -4px 24px rgba(0,0,0,0.12)',position:'relative',maxHeight:'92vh',overflowY:'auto'}}>
      ...
      {form.status==='성공'&&<div style={{background:'#f0fdf4',border:'1px solid #bbf7d0',borderRadius:'12px',padding:'14px',marginBottom:'12px'}}>
        ...
        <div style={{display:'flex',alignItems:'center',gap:'10px'}}>
          <span style={{fontSize:'12px',color:'#374151',fontWeight:'600'}}>총 회선수:</span>
          <input type="number" min="1" value={form.wonLines||1} onChange={e=>setForm(f=>({...f,wonLines:Number(e.target.value)}))}
            style={{width:'70px',border:'1px solid #bbf7d0',borderRadius:'8px',padding:'6px 10px',fontSize:'14px',fontWeight:'700',textAlign:'center',fontFamily:'inherit'}}/>
...
export default function MM({onBack}){
  ...
  const saveResult=async(form)=>{
    ...
    const upd={activity_status:form.status,activity_result:form.result,activity_memo:form.memo,activity_contact:form.contact,photos:JSON.stringify(form.photos),activity_at:now,
      ...(form.status==='성공'&&{won_products:JSON.stringify(form.wonProds||[]),won_lines:form.wonLines||1}),
      ...
      <div style={{flex:1,overflowY:'auto',padding:'14px'}}>
        ...
        {tab==='leads'&&(
          <div>
            ...
            {leadTab==='my'&&(()=>{
              const filteredM=mLeads.filter(l=>filterStatus==='전체'||(l.activity_status||'미처리')===filterStatus)
            ...
            <div style={{display:'flex',gap:'6px',marginBottom:'14px',overflowX:'auto',paddingBottom:'4px'}}>
              {['전체','접촉중','발굴성공','영업중','영업종료'].map(s=>(
                <button key={s} onClick={()=>setDiscStage(s)} style={{flex:'none',padding:'6px 14px',background:discStage===s?(s==='접촉중'?'#0891b2':s==='발굴성공'?'#7c3aed':s==='영업중'?'#d97706':s==='영업종료'?'#16a34a':'#1e293b'):'#fff',border:'1px solid '+(discStage===s?'transparent':'#e2e8f0'),borderRadius:'20px',fontSize:'12px',fontWeight:discStage===s?'700':'400',color:discStage===s?'#fff':'#64748b',cursor:'pointer',fontFamily:'inherit'}}>
                  {s==='전체'?'전체':s==='접촉중'?'1️⃣ 접촉중':s==='발굴성공'?'2️⃣ 발굴성공':s==='영업중'?'3️⃣ 영업중':'4️⃣ 영업종료'}
                </button>
              ))}
            </div>
            {/* M발굴 목록 */}
                  ...
                  <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',marginBottom:'8px'}}>
                    <div>
                      ...
                      <span style={{marginLeft:'8px',fontSize:'11px',padding:'2px 8px',borderRadius:'10px',fontWeight:'700',
                        ...
                        {d.stage==='접촉중'?'1️⃣ 접촉중':d.stage==='발굴성공'?'2️⃣ 발굴성공':d.stage==='영업중'?'3️⃣ 영업중':d.stage==='영업종료'?'4️⃣ 영업종료':'접촉중'}
                      </span>
                    </div>
                    {/* 단계 진행 버튼 */}
