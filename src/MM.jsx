...
export default function MM({onBack}){
  ...
  const handleReg=async()=>{
    ...
    const newLead={id:'m'+Date.now(),mm_username:user.username,customer:reg.customer,address:reg.address,contact:reg.contact,products:reg.products,note:reg.note,project_type:reg.project_type||3,photos:JSON.stringify(reg.photos),activity_status:'미처리',activity_result:'',activity_memo:'',activity_contact:reg.contact,activity_at:''}
    try{await db.post('mm_direct_leads',newLead);setMLeads([newLead,...mLeads]);setRegDone(true);t2('등록 완료!');setTimeout(()=>{setRegDone(false);setReg({customer:'',address:'',contact:'',products:[],note:'',photos:[],project_type:3});setTab('leads');setLeadTab('my')},1800)}
            ...
            {regDone
              ...
              <div style={{marginBottom:'16px'}}>
                ...
                {PT.filter(p=>!p.adminOnly).map(p=>(
                  <button key={p.id} onClick={()=>setReg(r=>({...r,project_type:p.id}))}
                    style={{width:'100%',padding:'11px 14px',background:reg.project_type===p.id?C.text:'#f8fafc',border:'1.5px solid '+(reg.project_type===p.id?C.text:'#e2e8f0'),color:reg.project_type===p.id?'#fff':C.text,borderRadius:'10px',fontSize:'14px',fontWeight:reg.project_type===p.id?'800':'500',cursor:'pointer',textAlign:'left',fontFamily:'inherit',display:'flex',alignItems:'center',gap:'8px',marginBottom:'6px'}}>
