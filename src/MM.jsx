...
function ResultModal({lead,onClose,onSave}){
  const [form,setForm]=useState({status:lead.activity_status||'미처리',result:lead.activity_result||'',memo:lead.activity_memo||'',contact:lead.activity_contact||'',photos:(()=>{try{return JSON.parse(lead.photos||'[]')}catch{return[]}})(),wonProds:[],contractExpiry:'',wonLines:1,activation_date:lead.activation_date||'',circuit_number:lead.circuit_number||'',project_type:lead.project_type||null,wonLinesByProd:(()=>{try{const p=JSON.parse(lead.won_products||'null');if(p&&!Array.isArray(p))return p}catch{}})()||{}})
...
export default function MM({onBack}){
  ...
  const [discForm,setDiscForm]=useState({site_name:'',address:'',contact:'',capacity:'',note:''})
        ...
        {tab==='discovery'&&(
          <div>
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'12px'}}>
              ...
              <button onClick={()=>setShowDiscForm(true)} style={{background:C.acc,color:'#fff',border:'none',borderRadius:'10px',padding:'8px 14px',fontSize:'13px',fontWeight:'700',cursor:'pointer',fontFamily:'inherit'}}>+ 신규 등록</button>
            </div>
            {/* 단계 필터 */}
            <div style={{display:'flex',gap:'6px',marginBottom:'14px',overflowX:'auto',paddingBottom:'4px'}}>
              {['전체','접촉중','발굴성공','영업중','영업종료'].map(s=>(
                <button key={s} onClick={()=>setDiscStage(s)} style={{flex:'none',padding:'6px 14px',background:discStage===s?(s==='접촉중'?'#0891b2':s==='발굴성공'?'#7c3aed':s==='영업중'?'#d97706':s==='영업종료'?'#16a34a':'#1e293b'):'#fff',border:'1px solid '+(discStage===s?'transparent':'#e2e8f0'),borderRadius:'20px',fontSize:'12px',fontWeight:discStage===s?'700':'400',color:discStage===s?'#fff':'#64748b',cursor:'pointer',fontFamily:'inherit'}}>
                  {s==='전체'?'전체':s==='접촉중'?'1 접촉중':s==='발굴성공'?'2 발굴성공':s==='영업중'?'3 영업중':'4 영업종료'}
                </button>
              ))}
            </div>
            {/* M발굴 목록 */}
            {(discStage==='전체'?myDiscovery:myDiscovery.filter(d=>d.stage===discStage)).length===0
              ?<div style={{textAlign:'center',padding:'40px 0',color:C.sub,fontSize:'14px'}}>등록된 M발굴 현장이 없습니다</div>
              :(discStage==='전체'?myDiscovery:myDiscovery.filter(d=>d.stage===discStage)).map(d=>(
                <div key={d.id} style={{background:'#fff',border:'1px solid #e2e8f0',borderRadius:'14px',padding:'14px',marginBottom:'10px',boxShadow:'0 1px 4px rgba(0,0,0,0.04)'}}>
                  <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',marginBottom:'8px'}}>
                    <div>
                      <span style={{fontSize:'15px',fontWeight:'800',color:C.text}}>{d.site_name||'현장명 없음'}</span>
                      <span style={{marginLeft:'8px',fontSize:'11px',padding:'2px 8px',borderRadius:'10px',fontWeight:'700',
                        background:d.stage==='접촉중'?'#e0f2fe':d.stage==='발굴성공'?'#ede9fe':d.stage==='영업중'?'#fef3c7':'#dcfce7',
                        color:d.stage==='접촉중'?'#0369a1':d.stage==='발굴성공'?'#6d28d9':d.stage==='영업중'?'#92400e':'#15803d'}}>
                        {d.stage==='접촉중'?'1 접촉중':d.stage==='발굴성공'?'2 발굴성공':d.stage==='영업중'?'3 영업중':d.stage==='영업종료'?'4 영업종료':'접촉중'}
                      </span>
                    </div>
                    {/* 단계 진행 버튼 */}
                    {d.stage!=='영업종료'&&<button onClick={async()=>{
                      const next=d.stage==='접촉중'?'발굴성공':d.stage==='발굴성공'?'영업중':'영업종료'
                      await db.patch('m_discovery',d.id,{stage:next,updated_at:new Date().toISOString()})
                      setMyDiscovery(prev=>prev.map(x=>x.id===d.id?{...x,stage:next}:x))
                    }} style={{background:d.stage==='접촉중'?'#0891b2':d.stage==='발굴성공'?'#7c3aed':'#d97706',color:'#fff',border:'none',borderRadius:'8px',padding:'5px 10px',fontSize:'11px',fontWeight:'700',cursor:'pointer',fontFamily:'inherit'}}>
                      {d.stage==='접촉중'?'발굴성공 →':d.stage==='발굴성공'?'영업중 →':'영업종료 →'}
                    </button>}
                  </div>
                  <div style={{fontSize:'12px',color:C.sub,marginBottom:'4px'}}>📍 {d.address||'-'}</div>
                  {d.contact&&<div style={{fontSize:'12px',color:C.sub,marginBottom:'4px'}}>📞 {d.contact}</div>}
                  {d.capacity&&<div style={{fontSize:'12px',color:C.sub,marginBottom:'4px'}}>👥 수용인원: {d.capacity}명</div>}
                  {d.note&&<div style={{fontSize:'12px',color:'#374151',padding:'6px 10px',background:'#f8fafc',borderRadius:'8px',marginTop:'6px'}}>{d.note}</div>}
                  {/* 영업종료 시 결과 입력 */}
                  {d.stage==='영업종료'&&<div style={{marginTop:'10px',padding:'8px',background:'#f0fdf4',borderRadius:'8px',fontSize:'12px'}}>
                    <div style={{fontWeight:'700',color:'#15803d',marginBottom:'6px'}}>영업 결과</div>
                    <div style={{color:'#374151'}}>{d.result||'결과 미입력'}</div>
                    {!d.result&&<button onClick={()=>{
                      const r=window.prompt('영업 결과를 입력하세요')
                      if(r)db.patch('m_discovery',d.id,{result:r}).then(()=>setMyDiscovery(prev=>prev.map(x=>x.id===d.id?{...x,result:r}:x)))
                    }} style={{marginTop:'6px',background:'#16a34a',color:'#fff',border:'none',borderRadius:'6px',padding:'4px 10px',fontSize:'11px',cursor:'pointer',fontFamily:'inherit'}}>결과 입력</button>}
                  </div>}
                </div>
              ))
            }
            ...
            {showDiscForm&&(
              <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.4)',zIndex:100,display:'flex',alignItems:'flex-end'}}>
                <div style={{background:'#fff',borderRadius:'20px 20px 0 0',padding:'20px',width:'100%',maxWidth:'420px',margin:'0 auto'}}>
                  <div style={{fontWeight:'900',fontSize:'17px',marginBottom:'16px',color:C.text}}>📍 M발굴 현장 등록</div>
                  {[['site_name','현장명 *'],['address','주소'],['contact','담당자 연락처'],['capacity','수용 인원'],['note','메모']].map(([k,label])=>(
                    <div key={k} style={{marginBottom:'10px'}}>
                      <div style={{fontSize:'12px',color:C.sub,marginBottom:'4px'}}>{label}</div>
                      <input value={discForm[k]||''} onChange={e=>setDiscForm(p=>({...p,[k]:e.target.value}))}
                        style={{width:'100%',border:'1px solid #e2e8f0',borderRadius:'8px',padding:'8px 10px',fontSize:'14px',boxSizing:'border-box',fontFamily:'inherit'}}/>
                    </div>
                  ))}
                  <div style={{display:'flex',gap:'8px',marginTop:'14px'}}>
                    <button onClick={()=>{setShowDiscForm(false);setDiscForm({site_name:'',address:'',contact:'',capacity:'',note:''})}}
                      style={{flex:1,padding:'12px',background:'#f1f5f9',border:'none',borderRadius:'10px',fontWeight:'700',cursor:'pointer',fontFamily:'inherit'}}>취소</button>
                    <button onClick={async()=>{
                      if(!discForm.site_name){alert('현장명을 입력하세요');return}
                      const newDisc={...discForm,registered_by:user?.name,team_id:user?.team_id,stage:'접촉중',created_at:new Date().toISOString()}
                      const res=await db.post('m_discovery',newDisc)
                      if(res){setMyDiscovery(p=>[{...newDisc,id:res.id||Date.now()},...p]);setShowDiscForm(false);setDiscForm({site_name:'',address:'',contact:'',capacity:'',note:''})}
