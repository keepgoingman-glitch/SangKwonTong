...
export default function UturnGate({ onBack }) {
  ...
  // 로그인 화면
  const IS = { width:'100%', background:'rgba(255,255,255,0.06)', border:'1.5px solid rgba(255,255,255,0.15)', color:'#f1f5f9', padding:'14px 15px', borderRadius:'11px', fontSize:'16px', outline:'none', boxSizing:'border-box', fontFamily:'inherit' }
  return (
    <div style={{ minHeight:'100vh', background:'linear-gradient(165deg,#0a0e1a 0%,#0f1a30 45%,#1a0e18 100%)', display:'flex', flexDirection:'column', fontFamily:"'Noto Sans KR',sans-serif", color:'#f1f5f9' }}>
      <div style={{ padding:'18px 20px' }}>
        <button onClick={onBack} style={{ background:'rgba(255,255,255,0.08)', border:'none', color:'#f1f5f9', fontSize:'14px', padding:'7px 13px', borderRadius:'9px', cursor:'pointer', fontFamily:'inherit' }}>‹ 홈으로</button>
      </div>
      <div style={{ flex:1, display:'flex', flexDirection:'column', justifyContent:'center', alignItems:'center', padding:'0 24px 80px' }}>
        ...
        <h1 style={{ fontSize:'26px', fontWeight:'900', margin:'8px 0 4px', color:'#ff5a6e' }}>U-TURN 로그인</h1>
        <p style={{ color:'rgba(255,255,255,0.5)', fontSize:'13.5px', marginBottom:'32px', textAlign:'center' }}>관리자·팀원 계정으로 로그인하세요</p>
        <div style={{ width:'100%', maxWidth:'360px', display:'flex', flexDirection:'column', gap:'12px' }}>
          ...
          {err && <div style={{ color:'#ff5a6e', fontSize:'13.5px', background:'rgba(227,25,55,0.12)', border:'1px solid rgba(227,25,55,0.3)', padding:'11px', borderRadius:'10px' }}>{err}</div>}
          <button onClick={handleLogin} disabled={busy} style={{ padding:'15px', background:busy?'rgba(255,255,255,0.1)':'#E31937', border:'none', color:'#fff', borderRadius:'12px', fontSize:'17px', fontWeight:'900', cursor:busy?'wait':'pointer', fontFamily:'inherit', marginTop:'4px' }}>{busy?'확인 중...':'로그인'}</button>
        </div>
      </div>
    </div>
  )
