...
const C = { bg:'#0a0e1a', card:'#ffffff', border:'#e2e8f0',
  text:'#1e293b', sub:'#64748b', red:'#E31937',
  u:'#9333ea', i:'#2563eb', t:'#d97706', green:'#16a34a' }
const IS = { width:'100%', background:'#f8fafc', border:'1.5px solid #e2e8f0',
  color:'#1e293b', padding:'12px 14px', borderRadius:'10px', fontSize:'15px', outline:'none',
  boxSizing:'border-box', fontFamily:'inherit' }
const ST = ['성공','접촉완료','미처리','실패']
const SC = {
  '성공':    { bg:'rgba(74,222,128,0.15)', text:'#4ade80', bd:'rgba(74,222,128,0.4)' },
  '접촉완료':{ bg:'rgba(96,165,250,0.15)', text:'#2563eb', bd:'rgba(96,165,250,0.4)' },
  '미처리':  { bg:'rgba(251,191,36,0.15)', text:'#d97706', bd:'rgba(251,191,36,0.4)' },
...
function StrategyCard({ s, acts, onOpen }) {
  ...
  const prC = { '높음':'#ff5a6e', '보통':'#d97706', '낮음':'#94a3b8' }[s.priority] || '#94a3b8'
  const stC = { '진행중':'#2563eb', '완료':'#4ade80', '보류':'#94a3b8' }[s.status] || '#2563eb'
  return (
    <div onClick={() => onOpen(s)} style={{ background:C.card, border:'1.5px solid '+C.border, borderLeft:'4px solid '+stC, borderRadius:'14px', padding:'15px', marginBottom:'12px', cursor:'pointer' }}>
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:'8px' }}>
        <div style={{ flex:1 }}>
          <div style={{ display:'flex', gap:'6px', alignItems:'center', marginBottom:'4px', flexWrap:'wrap' }}>
            <span style={{ fontSize:'10px', fontWeight:'800', padding:'2px 8px', borderRadius:'10px', background:'#f1f5f9', color:prC }}>{s.priority}</span>
            <span style={{ fontSize:'10px', fontWeight:'800', padding:'2px 8px', borderRadius:'10px', background:'#f1f5f9', color:stC }}>{s.status}</span>
      ...
      <div style={{ display:'flex', gap:'8px', marginTop:'10px' }}>
        {[['U',u,goalU,C.u],['I',i,goalI,C.i],['T',t,goalT,C.t]].map(([lb,val,goal,col])=>(
          <div key={lb} style={{ flex:1, background:'#ffffff', borderRadius:'9px', padding:'8px', textAlign:'center' }}>
            ...
            <div style={{ background:'#e2e8f0', borderRadius:'3px', height:'4px', marginTop:'4px', overflow:'hidden' }}>
...
function ActivityModal({ strategy, editAct, mmInfo, onClose, onSaved }) {
  ...
  return (
    <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.6)', zIndex:1001, display:'flex', alignItems:'flex-end', justifyContent:'center', backdropFilter:'blur(4px)' }}>
      <div style={{ background:'#ffffff', borderRadius:'20px 20px 0 0', padding:'20px 18px 32px', width:'100%', maxWidth:'480px', maxHeight:'94vh', overflowY:'auto', position:'relative', border:'1px solid #e2e8f0' }}>
        <button onClick={onClose} style={{ position:'absolute', top:'14px', right:'14px', background:'#e2e8f0', border:'none', color:C.text, width:'28px', height:'28px', borderRadius:'50%', fontSize:'16px', cursor:'pointer' }}>✕</button>
        ...
        <div style={{ background:'rgba(227,25,55,0.08)', border:'1px solid rgba(227,25,55,0.3)', borderRadius:'12px', padding:'13px', marginBottom:'16px' }}>
          <div style={{ fontSize:'13px', fontWeight:'900', color:'#E31937', marginBottom:'10px' }}>⤴ UIT 성과 <span style={{ fontWeight:'400', color:C.sub, fontSize:'11px' }}>· 기변은 실적 제외</span></div>
        ...
        <div style={{ display:'flex', gap:'6px', marginBottom:'12px' }}>
          {ST.map(s=>{ const c=SC[s]; const on=f.activity_status===s; return (
            <button key={s} onClick={()=>set('activity_status',s)} style={{ flex:1, padding:'10px 4px', background:on?c.bg:'#ffffff', border:'1.5px solid '+(on?c.bd:'#e2e8f0'), color:on?c.text:C.sub, borderRadius:'9px', fontSize:'13px', fontWeight:on?'800':'500', cursor:'pointer', fontFamily:'inherit' }}>{s}</button>
          )})}
        </div>
        ...
        <button onClick={handleSave} disabled={saving} style={{ width:'100%', padding:'15px', background:saving?'#e2e8f0':C.red, border:'none', color:'#fff', borderRadius:'12px', fontSize:'16px', fontWeight:'900', cursor:saving?'wait':'pointer', fontFamily:'inherit' }}>
...
export default function UturnMember({ mmInfo, onBack }) {
  ...
  return (
    <div style={{ minHeight:'100vh', background:'#f4f6fb', color:C.text, fontFamily:"'Noto Sans KR',sans-serif", paddingBottom:'40px' }}>
      ...
      <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'16px 18px 12px' }}>
        <button onClick={onBack} style={{ background:'#f1f5f9', border:'1px solid #e2e8f0', color:'#64748b', fontSize:'14px', padding:'7px 13px', borderRadius:'9px', cursor:'pointer', fontFamily:'inherit' }}>‹ 홈</button>
        <div style={{ textAlign:'center' }}>
          <div style={{ fontSize:'16px', fontWeight:'900', color:'#E31937' }}>⤴ U-TURN</div>
          <div style={{ fontSize:'11px', color:C.sub }}>{mmInfo.name} · {mmInfo.region||'팀원'}</div>
        </div>
        <button onClick={load} style={{ background:'#f1f5f9', border:'1px solid #e2e8f0', color:'#64748b', fontSize:'15px', padding:'7px 11px', borderRadius:'9px', cursor:'pointer' }}>🔄</button>
      </div>
      ...
      <div style={{ maxWidth:'440px', margin:'0 auto', padding:'0 16px' }}>
        <div style={{ background:C.card, border:'1px solid '+C.border, borderRadius:'16px', padding:'14px', marginBottom:'14px' }}>
          <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:'10px' }}>
            ...
            <span style={{ fontSize:'13px', fontWeight:'900', color:'#E31937' }}>스코어 {Math.round(myScore)}</span>
          </div>
          <div style={{ display:'flex', gap:'8px' }}>
            {[['U · 신규·MNP',myU,C.u],['I · 인터넷',myI,C.i],['T · TV',myT,C.t]].map(([lb,v,col])=>(
              <div key={lb} style={{ flex:1, background:'#ffffff', borderRadius:'10px', padding:'10px', textAlign:'center' }}>
        ...
        <div style={{ display:'flex', gap:'6px', marginBottom:'14px' }}>
          {[['strategies','🎯 내 전략활동'],['myperf','📋 내 활동내역']].map(([id,lb])=>(
            <button key={id} onClick={()=>setTab(id)} style={{ flex:1, padding:'11px', background:tab===id?'rgba(227,25,55,0.15)':'#ffffff', border:'1.5px solid '+(tab===id?'rgba(227,25,55,0.4)':C.border), color:tab===id?'#ff5a6e':C.sub, borderRadius:'11px', fontSize:'14px', fontWeight:tab===id?'800':'500', cursor:'pointer', fontFamily:'inherit' }}>{lb}</button>
          ))}
        </div>
            ...
            : activities.map(a => {
              ...
              return (
                <div key={a.id} style={{ background:C.card, border:'1px solid '+C.border, borderLeft:'4px solid '+c.text, borderRadius:'13px', padding:'13px', marginBottom:'10px' }}>
                  ...
                  {strat && <div style={{ fontSize:'11px', color:'#E31937', marginBottom:'4px' }}>🎯 {strat.title}</div>}
      ...
      {openStrategy && (() => {
        ...
        return (
          <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.6)', zIndex:1000, display:'flex', alignItems:'flex-end', justifyContent:'center', backdropFilter:'blur(4px)' }}>
            <div style={{ background:'#ffffff', borderRadius:'20px 20px 0 0', padding:'20px 18px 32px', width:'100%', maxWidth:'480px', maxHeight:'90vh', overflowY:'auto', position:'relative', border:'1px solid #e2e8f0' }}>
              <button onClick={()=>setOpenStrategy(null)} style={{ position:'absolute', top:'14px', right:'14px', background:'#e2e8f0', border:'none', color:C.text, width:'28px', height:'28px', borderRadius:'50%', fontSize:'16px', cursor:'pointer' }}>✕</button>
              <div style={{ fontSize:'11px', color:'#E31937', fontWeight:'700', marginBottom:'3px' }}>🎯 전략활동</div>
              ...
              {myActs.length===0
                ...
                : myActs.map(a=>{ const c=SC[a.activity_status]||SC['미처리']; return (
                  <div key={a.id} onClick={()=>{ setOpenStrategy(null); setActModal({strategy:s,editAct:a}) }} style={{ background:'#ffffff', border:'1px solid '+C.border, borderRadius:'11px', padding:'11px', marginBottom:'8px', cursor:'pointer' }}>
